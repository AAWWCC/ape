 ...
 * to wire globally.
 */
import { readFileSync, readdirSync, existsSync, renameSync, rmSync, statSync, writeFileSync } from 'node:fs';
...
const readActive = (dir) => {
  try { return JSON.parse(readFileSync(join(dir, '.ape', 'runtime', 'active.json'), 'utf8')); } catch { return null; }
};
...
function main() {
  ...
  const base = dir.split(/[\\/]/).filter(Boolean).slice(-2).join('/') || dir;
  ...
  segs.push({ text: `${G.folder ? `${G.folder} ` : ''}${base}`, fg: 'cyan', bg: 'panel' });
  if (branch) {
  ...
  if (run || existsSync(join(dir, '.ape'))) segs.push(...apeSegments(run));
  ...
  process.stdout.write(renderLine(segs, trailing));
}
