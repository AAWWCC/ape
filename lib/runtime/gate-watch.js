...
/** @returns {Promise<{ pid: number | null, launched: boolean, reason?: string }>} */
async function launchGateRunner(projectDir, files, jobFields, suiteDir = projectDir) {
  ...
  return new Promise((resolve) => {
    const spawned = () => {
      ...
      resolve({ pid: child.pid, launched: true });
    };
    const failed = (error) => {
      ...
      resolve({
        pid: null,
        launched: false,
        // prose-bound-exempt: bounded native spawn error code, never agent-authored content.
        reason: `gate runner process could not start (${String(error?.code ?? 'unknown error').slice(0, 64)})`,
      });
    };
...
async function pollGateSuiteMulti(projectDir, paths, state, config, ctx, watch) {
  ...
  const artifact = await readBoundedJson(watch.artifact_file, RUNTIME_STATE_MAX_BYTES, null).catch(() => null);
