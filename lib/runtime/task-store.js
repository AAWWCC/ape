...
import { atomicReplaceText, atomicWriteJson, syncDirectory } from './storage.js';
...
const MAX_TASK_GENERATIONS = TASK_MAX_GENERATION + 1;
...
const CREATION_HOST = sha256(hostname()).slice(0, 16);
const CREATION_STAGING_PATTERN = /^\.task-create\.v1\.([0-9a-f]{16})\.([1-9]\d{0,9})\.(task-[A-Za-z0-9_-]{43})$/;
...
async function parseGenerationFile(file) {
  ...
  let raw;
  try {
    raw = JSON.parse((await readBoundedFile(file, MAX_GENERATION_FILE_BYTES)).toString('utf8'));
  } catch (error) {
...
function validateGenerationSequence(records, rootBinding, expectedTaskId) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if (index === 0) {
      ...
      if (record.generation > TASK_MAX_UPDATE_GENERATION && !TERMINAL.has(record.status) &&
          (previous.cancellation !== null || record.cancellation === null || record.status !== previous.status)) {
        throw new TaskStoreError('corrupt_task', 'APE reserved task generation is not a first cancellation request');
      }
...
async function readTaskLocked(prepared, taskId, { includeCollected = false } = {}) {
  ...
  try {
    ...
    entries = await recoverAtomicWriteDirectory(directory,
      (name) => GENERATION_FILE_PATTERN.test(name) && Number(name.slice(0, 6)) <= TASK_MAX_GENERATION,
  ...
  if (!includeCollected) {
    const audit = await readGcRecord(path.join(prepared.paths.gc, `${taskId}.json`), prepared);
    if (audit) {
      if (audit.taskId !== taskId || audit.operationId !== current.operationId ||
          audit.generations !== current.generation + 1 || audit.terminalHash !== current.hash) {
        throw new TaskStoreError('corrupt_store', 'APE task GC audit conflicts with the journal; retain both for explicit recovery');
      }
      // The fsynced audit commits expiration even if the process died before
      // its directory rename. A worker must not append after this tombstone.
      return null;
    }
  }
  return current;
...
async function writeNewGeneration(directory, generation) {
  ...
  // Field budgets measure compact JSON. Whitespace amplification must not
  // reject an otherwise supported result or produce an unreadable journal.
  const serialized = `${JSON.stringify(generation)}\n`;
  if (Buffer.byteLength(serialized, 'utf8') > MAX_GENERATION_FILE_BYTES) {
...
function creationProcessIsDead(pid) {
  try { process.kill(pid, 0); return false; }
  catch (error) { return error?.code === 'ESRCH'; }
}

async function recoverTaskCreationStagingLocked(prepared) {
  const result = { removed: 0, retained: [], omitted_retained: 0 };
  const retain = (name, reason) => {
    if (result.retained.length < 64) result.retained.push({ name, reason });
    else result.omitted_retained += 1;
  };
  for (const entry of await readdir(prepared.paths.runtime, { withFileTypes: true })) {
    if (!entry.name.startsWith('.task-create.')) continue;
    const match = CREATION_STAGING_PATTERN.exec(entry.name);
    if (!match) {
      retain(entry.name, 'No creator identity; stop all APE hosts, inspect this unpublished directory, and remove it manually if abandoned.');
      continue;
    }
    if (match[1] !== CREATION_HOST || !creationProcessIsDead(Number(match[2]))) {
      retain(entry.name, 'Creator is on another host or is not proven dead; preserve its unpublished task.');
      continue;
    }
    const directory = path.join(prepared.paths.runtime, entry.name);
    try {
      const metadata = await lstat(directory);
      if (!entry.isDirectory() || entry.isSymbolicLink() || !metadata.isDirectory() ||
          metadata.isSymbolicLink() || hasUnsafePosixMode(metadata)) throw new Error('unsafe staging directory');
      const entries = await recoverAtomicWriteDirectory(directory, (name) => name === '000000.json', MAX_GENERATION_FILE_BYTES);
      if (entries.length > 1 || entries.some((file) => file.name !== '000000.json')) throw new Error('unexpected staging contents');
      if (entries.length) {
        const generation = await parseGenerationFile(path.join(directory, '000000.json'));
        validateGenerationSequence([generation], prepared.rootBinding, match[3]);
      }
      const current = await lstat(directory);
      if (metadata.dev !== current.dev || metadata.ino !== current.ino) throw new Error('staging directory changed');
      await rm(directory, { recursive: true, force: true });
      result.removed += 1;
    } catch {
      retain(entry.name, 'Unpublished staging contains unexpected or unsafe data; inspect it before manual recovery.');
    }
  }
  return result;
}

// Explicit maintenance also reports legacy entries that cannot safely be
// deleted automatically. Task creation invokes the same dead-creator sweep.
export async function recoverTaskCreationStaging(root) {
  return withTaskLock(root, recoverTaskCreationStagingLocked);
}
...
export async function createTask(root, {
  ...
  for (let attempt = 0; attempt < 16; attempt += 1) {
    ...
    const directory = taskDirectory(prepared.paths, taskId);
    const stagingDirectory = path.join(prepared.paths.runtime,
      `.task-create.v1.${CREATION_HOST}.${process.pid}.${taskId}`);
    ...
    try {
      ...
      const committed = await withPreparedTaskLock(prepared, async () => {
        await recoverTaskCreationStagingLocked(prepared);
        ...
        installed = true;
        await syncDirectory(prepared.paths.tasks);
        await syncDirectory(prepared.paths.runtime);
        return true;
      });
...
function nextTaskTimestamp(current) {
  // Readers require a monotonic journal even when the wall clock is corrected
  // backwards between requests. Keep the previous instant until it catches up.
  return new Date(Math.max(Date.now(), Date.parse(current.lastUpdatedAt))).toISOString();
}
...
async function appendTaskGenerationLocked(prepared, taskId, rawPatch) {
  ...
  if (current.generation >= TASK_MAX_UPDATE_GENERATION && !TERMINAL.has(nextStatus)) {
    throw new TaskStoreError('task_update_capacity', 'APE task reached its update capacity; the remaining journal slots are reserved for cancellation and the terminal result');
  }
  ...
  const nextInputRequests = patch.inputRequests ?? (nextStatus === 'input_required' ? current.inputRequests : []);
  if (nextStatus !== 'input_required' && nextInputRequests.length > 0) {
    throw new TaskStoreError('invalid_input_requests', 'APE task input requests require input_required status');
  }
  const next = finalizeGeneration({
    ...
    status: nextStatus,
    lastUpdatedAt: nextTaskTimestamp(current),
    result: nextResult,
    error: nextError,
    inputRequests: nextInputRequests,
...
export async function requestTaskCancellation(root, taskId, request) {
  ...
  return withTaskLock(root, async (prepared) => {
    ...
    const cancellation = {
      requestedAt: nextTaskTimestamp(current),
...
async function readGcRecord(file, prepared) {
  ...
  try {
    ...
    raw = JSON.parse((await readBoundedFile(file, 64 * 1_024)).toString('utf8'));
  } catch (error) {
    if (error?.code === 'ENOENT') return null;
...
async function pruneGcAudits(prepared, timestamp) {
  const entries = await recoverAtomicWriteDirectory(prepared.paths.gc,
    (name) => /^task-[A-Za-z0-9_-]{43}\.json$/.test(name), MAX_GENERATION_FILE_BYTES);
  // The audit already committed expiration. A crash can leave any subset of
  // its quarantined journal behind without corrupting the public inventory.
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.isSymbolicLink() && isTaskId(entry.name)) {
      await removeCollectedTaskDirectory(prepared, entry.name);
    }
  }
  const records = [];
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.isSymbolicLink() && isTaskId(entry.name)) continue;
    ...
    if (record.taskId !== entry.name.slice(0, -5)) throw new TaskStoreError('corrupt_store', 'APE task GC audit filename conflicts with its task');
    let pending = false;
    try { await lstat(taskDirectory(prepared.paths, record.taskId)); pending = true; }
    catch (error) { if (error?.code !== 'ENOENT') throw error; }
    records.push({ file, record, pending });
  }
  const retained = [];
  for (const entry of records) {
    if (!entry.pending && Date.parse(entry.record.collectedAt) + GC_AUDIT_RETENTION_MS <= timestamp) {
  ...
  for (const entry of retained.filter((entry) => !entry.pending).slice(0, Math.max(0, retained.length - MAX_GC_AUDIT_RECORDS))) {
    await rm(entry.file, { force: true });
  }
}

async function removeCollectedTaskDirectory(prepared, taskId) {
  const directory = path.join(prepared.paths.gc, taskId);
  const audit = await readGcRecord(path.join(prepared.paths.gc, `${taskId}.json`), prepared);
  if (!audit || audit.taskId !== taskId) throw new TaskStoreError('corrupt_store', 'APE collected task has no matching GC audit');
  const entries = await readdir(directory, { withFileTypes: true });
  if (entries.length > MAX_TASK_GENERATIONS) throw new TaskStoreError('corrupt_store', 'APE collected task has excessive journal entries');
  for (const entry of entries) {
    if (!entry.isFile() || entry.isSymbolicLink() || !GENERATION_FILE_PATTERN.test(entry.name)) {
      throw new TaskStoreError('corrupt_store', 'APE collected task contains an unexpected journal entry');
    }
    const record = await parseGenerationFile(path.join(directory, entry.name));
    if (record.rootBinding !== prepared.rootBinding || record.taskId !== taskId ||
        record.operationId !== audit.operationId || record.generation !== Number(entry.name.slice(0, 6)) ||
        record.generation >= audit.generations ||
        (record.generation === audit.generations - 1 && record.hash !== audit.terminalHash)) {
      throw new TaskStoreError('corrupt_store', 'APE collected task journal conflicts with its GC audit');
    }
  }
  await rm(directory, { recursive: true, force: true });
  await syncDirectory(prepared.paths.gc);
}
...
export async function collectExpiredTasks(root, { now = Date.now(), limit = 100 } = {}) {
  ...
  return withTaskLock(root, async (prepared) => {
    ...
    for (const taskId of await taskDirectoryNames(prepared.paths)) {
      ...
      const current = await readTaskLocked(prepared, taskId, { includeCollected: true });
      ...
      // Move the complete expired journal out of the readable inventory before
      // recursive deletion. The audit binds recovery of a partially removed
      // quarantine; public readers see the task atomically disappear.
      await rename(taskDirectory(prepared.paths, taskId), path.join(prepared.paths.gc, taskId));
      await syncDirectory(prepared.paths.gc);
      await syncDirectory(prepared.paths.tasks);
      await removeCollectedTaskDirectory(prepared, taskId);
