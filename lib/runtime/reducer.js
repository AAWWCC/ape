...
import { pipelineLimits, stageRecoveryLimits } from './pipeline-limits.js';
import {
  approvedPlan,
  CandidatePlanSchema,
...
// layer independently enforces the ticket's immutable physical-worker policy.
...
function workerProtocolRecovery(state, ticket, receipt, { reviewGroup = false } = {}) {
  ...
  const used = Number.isInteger(redispatches[ticket.stage_id])
    ...
    : 0;
  const maximum = stageRecoveryLimits(ticket.stage_id, state).max_protocol_redispatches;
  if (used < maximum) {
  ...
  return terminalRecoveryBlock(
    ticket.stage_id,
    `stage ${ticket.stage_id} worker protocol failed after its configured ${maximum} same-contract redispatches`,
    {
...
function expirePendingTicket(state, ticket, blockReason) {
  ...
  const maximum = stageRecoveryLimits(ticket.stage_id, state).max_stage_attempts;
  if (attempts < maximum) {
...
function retryContradictionSource(state, resolution = {}) {
  ...
  if (attempts >= stageRecoveryLimits(source.stage_id, state).max_stage_attempts) {
    return terminalRecoveryBlock(
      source.stage_id,
...
export function reduceRun(state, event) {
  ...
  switch (event.type) {
    ...
    case 'RECEIPT_RECORDED': {
      ...
      const reconciliationStage = ['test-reconcile', 'test-recheck'].includes(ticket.stage_id);
      const reconciliationLimits = stageRecoveryLimits(ticket.stage_id, state);
      if (reconciliationStage && receipt.status !== 'passed' &&
          reconciliationLimits.max_protocol_redispatches > 0) {
        const recovery = workerProtocolRecovery(state, ticket, receipt);
        if (recovery) return recovery;
      }
      const reconciliationCanRetry = receipt.status !== 'passed' &&
        receipt.evidence?.failure_kind !== 'capability' &&
        (state.attempts[ticket.stage_id] ?? 1) < reconciliationLimits.max_stage_attempts;
      ...
      if (ticket.stage_id === 'test-reconcile' && !reconciliationCanRetry) {
        if (receipt.status !== 'passed') {
          ...
          return terminalRecoveryBlock(
            'test-reconcile',
            'the independent test-contradiction reconciliation exhausted its configured attempt budget',
            { terminal_reason_code: 'test_contradiction' },
          );
        }
      ...
      if (ticket.stage_id === 'test-recheck' && !reconciliationCanRetry) {
        if (receipt.status !== 'passed') {
          if (receipt.evidence?.failure_kind === 'capability') {
            return capabilityRecoveryBlock(nextState, ticket, receipt);
          }
          return terminalRecoveryBlock(
            'test-recheck',
            'the exact-scope test correction exhausted its configured attempt budget; refusing recursive reconciliation',
            { terminal_reason_code: 'test_contradiction' },
          );
        }
        return retryContradictionSource(nextState, {
      ...
      if (receipt.status !== 'passed' && !reviewVote) {
        ...
        const attempts = state.attempts[ticket.stage_id] ?? 1;
        if (!noRetryFault && attempts < stageRecoveryLimits(ticket.stage_id, state).max_stage_attempts) {
            ...
            patch: {
              ...
              block_reason: testContradiction
                ...
                : capabilityBlocked
                  ...
                  : `stage ${ticket.stage_id} failed after ${stageRecoveryLimits(ticket.stage_id, state).max_stage_attempts} configured attempts${reviewFindings.attemptSummaries(state, ticket.stage_id, ticket.ticket_id, receipt)}`,
              ...(capabilityBlocked
                  ? {
                    terminal_reason_code: 'capability_blocked',
                    failure_domain: 'configuration',
      ...
      // assurance identity set is a strict proper subset. The operator budget
      // is frozen into each new run's execution policy.
      if (ticket.stage_id === 'plan-judge' && groupOutcome([receipt]) === 'disagreed') {
        ...
        const maximum = pipelineLimits(state).max_directed_replans;
        ...
        if (cycles < maximum && strictProgress) {
        ...
        const ceilingReached = cycles >= maximum;
        return terminalRecoveryBlock(
          'plan-judge',
          ceilingReached
            ? `plan judged unsound after the configured ${maximum} directed replan budget was exhausted`
            : 'plan judged unsound because assurance progress was not a strict proper subset of the prior structured set',
          {
            ...
            blocked_recovery: {
              reason_code: ceilingReached
                ? PLANNING_RECOVERY_REASON.ceilingExhausted
                : PLANNING_RECOVERY_REASON.noStrictProgress,
    ...
    case 'REGATE': {
      ...
      if (regateAttempts >= maximum) {
        return [action('reject', {
          // prose-bound-exempt: fixed diagnostic template; ${maximum} is the
          // validated numeric run policy, never agent-authored prose.
          reason: `re-gate attempt limit reached (${maximum}); the gate block is exhausted`,
        })];
      }
      return [
        action('acquire_lock'),
    ...
    case 'SHIP': {
      ...
      // the configured re-gate budget, so the hold↔ship↔regate
    ...
    case 'OVERRIDE': {
      ...
      return [
        audit,
        // The effect bounds this reason before state, history, or response
        // persistence, just as audit_override bounds its independent copy.
        // prose-bound-exempt: reducer action only; apply_override owns the sink.
        action('apply_override', { operation: 'abort', reason: event.reason }),
        action('archive_history', { if_absent: true }),
        action('release_lock'),
        action('persist_state'),
      ];
