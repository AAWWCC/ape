  ...
  switch (event.type) {
    ...
    case 'RECEIPT_RECORDED': {
      ...
      if (receipt.status !== 'passed' && !reviewVote) {
        ...
        // review receipt is handled after group convergence below, and the
      ...
      if (event.stage.parallel_group) {
        ...
        if (synthetic === 'review-disagreed') {
          ...
          const previousFingerprints = new Set(state.remediation_finding_fingerprints ?? []);
          const repeatedFinding = state.remediation_cycles > 0 && (
            fingerprints.length === 0 || fingerprints.some((fingerprint) => previousFingerprints.has(fingerprint))
          );
          if (repeatedFinding) {
            return [
              action('transition', {
                patch: {
                  status: 'blocked',
                  stage: 'remediation',
                  // prose-bound-exempt: both branches are fixed runtime diagnostics; no reviewer text is interpolated.
                  block_reason: fingerprints.length === 0
                    ? 'review disagreement persists after the single remediation cycle'
      ...
      // not an immediate dead end. Exactly one scheduler-owned replan cycle is
      // allowed. A second negative judgment remains terminal and fail-closed.
      if (ticket.stage_id === 'plan-judge' && groupOutcome([receipt]) === 'disagreed') {
        ...
        if ((state.plan_replan_cycles ?? 0) < 1) {
          return [
            action('transition', {
              patch: {
            ...
            ...nextStages(nextState, 'plan-replan-required', receipt).map((stage) =>
              action('issue_ticket', { stage, plan_recovery: recovery })),
            action('persist_state'),
          ];
        }
        return terminalRecoveryBlock(
          'plan-judge',
          'plan judged unsound after the single directed replan cycle',
          {
