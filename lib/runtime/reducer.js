...
import {
  remediationFindingHistoryIsConsistent,
  structuredRemediationRoute,
} from './schemas.js';
import {
...
const CODE_REVIEW_STAGES = new Set([
  'review',
  'security-review',
  'remediation-review',
...
function priorCodeReviewReceiptGroups(state, currentReceipts) {
  const currentTicketIds = new Set(currentReceipts.map((receipt) => receipt.ticket_id));
  const receiptByTicket = new Map(
    (state.receipts ?? []).map((receipt) => [receipt.ticket_id, receipt]),
  );
  const groups = [];
  let tickets = [];
  const flush = () => {
    if (tickets.length === 0) return true;
    if (tickets.some((ticket) => currentTicketIds.has(ticket.ticket_id))) {
      tickets = [];
      return true;
    }
    // Reconstruct the same effective group completedForGroup used when the
    // cycle was first recorded. Same-stage protocol retries leave their
    // superseded receipts durable; counting every historical ticket here
    // would make a valid history fail its provenance check after recovery.
    const latestByStage = new Map();
    for (const ticket of tickets) latestByStage.set(ticket.stage_id, ticket);
    ...
    const receipts = [...latestByStage.values()]
      .map((ticket) => receiptByTicket.get(ticket.ticket_id));
    tickets = [];
    if (receipts.some((receipt) => !receipt)) return false;
    groups.push(receipts);
    return true;
  };
  for (const ticket of state.tickets ?? []) {
    if (CODE_REVIEW_STAGES.has(ticket.stage_id)) {
      tickets.push(ticket);
    } else if (!flush()) {
      return null;
    }
  }
  return flush() ? groups : null;
}

function remediationHistoryIdentitySet(history) {
  return new Set(history.cycles.flat());
}
...
function replaySemanticHistory(
  history,
  receiptGroups,
  startIndex = 0,
  startingAliases = [],
) {
  const priorReceipts = receiptGroups.slice(0, startIndex).flat();
  const globallySeen = new Set(history.cycles.slice(0, startIndex).flat());
  let aliases = startingAliases;
  for (let index = startIndex; index < receiptGroups.length; index += 1) {
    const identityResult = reviewFindings.analyzeIdentities(
      receiptGroups[index],
      priorReceipts,
      [...globallySeen],
      aliases,
    ...
    if (
      ...
      !sameIdentitySet(identityResult.fingerprints, history.cycles[index]) ||
      JSON.stringify(identityResult.provenance) !== JSON.stringify(history.provenance[index])
    ) {
      return null;
    }
    aliases = identityResult.aliases;
    priorReceipts.push(...receiptGroups[index]);
    for (const identity of history.cycles[index]) globallySeen.add(identity);
  }
  return aliases;
}

function remediationIdentityEpoch(history, migration) {
  const legacyCycles = history.cycles.slice(0, migration.legacyCycleCount);
  const legacyProvenance = history.provenance.slice(0, migration.legacyCycleCount);
  const legacyDescriptors = new Set(
    legacyProvenance.flatMap((entry) => entry.descriptor_hashes),
  );
  const legacyAliases = history.aliases.filter((alias) => legacyDescriptors.has(alias.descriptor));
  return {
    version: 1,
    legacy_cycle_count: migration.legacyCycleCount,
...
function remediationHistoryReplay(state, receiptGroups) {
  if (!receiptGroups || !remediationFindingHistoryIsConsistent(state)) {
    return { valid: false, reason: 'history' };
  }
  const history = state.remediation_finding_history;
  if (receiptGroups.length !== history.cycles.length) {
    return { valid: false, reason: 'history' };
  }
  if (!history.identity_epoch) {
    const aliases = replaySemanticHistory(history, receiptGroups);
    if (aliases && JSON.stringify(aliases) === JSON.stringify(history.aliases)) {
    ...
    if (!migration.valid) return {
      valid: false,
      // prose-bound-exempt: migration returns only fixed internal reason codes.
      reason: migration.reason ?? 'history',
    };
    return {
      valid: true,
      history: {
        ...history,
        aliases: migration.aliases,
        identity_epoch: remediationIdentityEpoch(history, migration),
      },
    };
  }

  const legacyCount = history.identity_epoch.legacy_cycle_count;
  const legacyProvenance = history.provenance.slice(0, legacyCount);
  const legacyDescriptors = new Set(
    legacyProvenance.flatMap((entry) => entry.descriptor_hashes),
  );
  const retainedLegacyAliases = history.aliases.filter(
    (alias) => legacyDescriptors.has(alias.descriptor),
  );
  const migration = reviewFindings.migrateLegacyHistory(
    receiptGroups.slice(0, legacyCount),
    history.cycles.slice(0, legacyCount),
    retainedLegacyAliases,
    legacyProvenance,
  );
  if (!migration.valid) return {
    valid: false,
  ...
  const expectedEpoch = remediationIdentityEpoch(history, migration);
  if (JSON.stringify(expectedEpoch) !== JSON.stringify(history.identity_epoch)) {
    return { valid: false, reason: 'history' };
  }
  const aliases = replaySemanticHistory(
    history,
    receiptGroups,
    legacyCount,
    migration.aliases,
...
function expirePendingTicket(state, ticket, blockReason) {
  ...
  const singleAttemptRecovery = ['test-reconcile', 'test-recheck'].includes(ticket.stage_id);
...
function automaticCapabilityRecovery(state, event, ticket, receipt) {
  if (
    receipt.status !== 'failed' ||
    receipt.evidence?.failure_kind !== 'capability' ||
    ['code-review', 'plan-review'].includes(event.stage.parallel_group) ||
    ['test-reconcile', 'test-recheck'].includes(ticket.stage_id) ||
    !['implementer', 'test_writer'].includes(ticket.role)
  ) return null;
  if ((state.capability_recoveries?.[ticket.stage_id] ?? 0) >= 1) {
    return capabilityRecoveryBlock(state, ticket, receipt);
  }

  const required = receipt.evidence?.required_claims ?? {};
  const addedClaimedPaths = event.capability_recovery?.added_claimed_paths ??
    (Array.isArray(required.claimed_paths) ? required.claimed_paths : []);
  const addedTestPaths = event.capability_recovery?.added_test_paths ??
    (Array.isArray(required.test_paths) ? required.test_paths : []);
  const roleCompatible =
    required.required_role === undefined || required.required_role === ticket.role;
  if (
    !roleCompatible ||
    (addedClaimedPaths.length === 0 && addedTestPaths.length === 0)
  ) return capabilityRecoveryBlock(state, ticket, receipt);

  const capabilityPatch = event.capability_recovery?.patch ?? {};
  const { entries: priorAttempts, informative } = reviewFindings.attemptSummaryList(
    state,
    ticket.stage_id,
    ticket.ticket_id,
  ...
  const disclosedClaims = [...addedClaimedPaths, ...addedTestPaths].slice(0, 8);
  const recoveryPriorAttempts = [...priorAttempts];
  if (
    event.capability_recovery &&
    recoveryPriorAttempts.length > 0 && disclosedClaims.length > 0
  ) {
    const last = recoveryPriorAttempts.length - 1;
    recoveryPriorAttempts[last] = `${recoveryPriorAttempts[last]}; required claim(s): ${
      disclosedClaims.join(', ')
    }`.slice(0, 2_000);
  }
  return [
    action('transition', {
      patch: {
        claimed_paths: event.capability_recovery?.claimed_paths ?? [
          ...(state.claimed_paths ?? []),
          ...addedClaimedPaths,
        ],
        test_paths: event.capability_recovery?.test_paths ?? [
          ...(state.test_paths ?? []),
          ...addedTestPaths,
        ],
    ...
    action('issue_ticket', {
      ...
      ...(informative && recoveryPriorAttempts.length
        ? { prior_attempts: recoveryPriorAttempts }
        : {}),
    }),
    action('audit_override', {
      operation: 'capability-recovery',
      source_ticket_id: ticket.ticket_id,
      added_claimed_paths: addedClaimedPaths,
      added_test_paths: addedTestPaths,
      // prose-bound-exempt: the audit_override persistence sink applies
      // boundedGateSummary before this receipt-derived summary is written.
      reason: receipt.evidence?.summary ?? 'bounded additive capability recovery',
    }),
    action('persist_state'),
  ];
}
...
export function reduceRun(state, event) {
  ...
  switch (event.type) {
    ...
    case 'RECEIPT_RECORDED': {
      ...
      const capabilityRecovery = automaticCapabilityRecovery(
        nextState,
        event,
        ticket,
        receipt,
      );
      if (capabilityRecovery) return capabilityRecovery;
      ...
      if (event.stage.parallel_group) {
        ...
        if (synthetic === 'review-disagreed') {
          ...
          const priorReceiptGroups = remediationInProgress
            ? priorCodeReviewReceiptGroups(nextState, receipts)
            : [];
          const identityReceipts = receipts.length === 0 &&
            receipt?.status === 'failed' && receipt?.findings === undefined
            ? [receipt]
            : receipts;
          // Authorize the whole retained-history plus current-review domain in
          // one aggregate pass before replay performs any cycle-local matching.
          // Without this outer guard, each cycle could independently spend the
          // full descriptor/token/byte/work budget and split an oversized
          // comparison across a bounded-looking sequence of calls.
          const historyDomain = !remediationInProgress
            ? { valid: true }
            : priorReceiptGroups
              ? reviewFindings.preflightHistory([...priorReceiptGroups, identityReceipts])
              : { valid: false, reason: 'history' };
          const historyReplay = !remediationInProgress
            ? { valid: true, history: null }
            : historyDomain.valid
              ? remediationHistoryReplay(state, priorReceiptGroups)
              : {
                  valid: false,
                  // prose-bound-exempt: history preflight returns only fixed internal reason codes.
                  reason: historyDomain.reason,
                };
          const historyValid = historyReplay.valid;
          const history = historyReplay.history;
          const historyFailureReason = historyReplay.reason ?? historyDomain.reason;
          const globallySeen = history ? remediationHistoryIdentitySet(history) : new Set();
          const identityResult = state.mode === 'land'
            ? { valid: true, fingerprints: [] }
            : historyValid
              ? reviewFindings.analyzeIdentities(
                identityReceipts,
                remediationInProgress ? priorReceiptGroups.flat() : [],
                [...globallySeen],
                remediationInProgress ? history.aliases : [],
              )
              : {
                  valid: false,
                  // prose-bound-exempt: the comparison selects one of two fixed internal reason codes.
                  reason: historyFailureReason === 'limit'
                    ? 'limit'
                    : historyFailureReason === 'collision'
                      ? 'ambiguous'
                      : 'history',
                  fingerprints: [],
                };
          const fingerprints = identityResult.fingerprints;
            ...
            return [
              action('transition', {
                patch: {
                  status: 'blocked',
                  stage: 'remediation',
                  // prose-bound-exempt: all branches are fixed runtime diagnostics.
                  block_reason: identityResult.reason === 'history'
                    ? 'remediation cycle finding history is missing, malformed, or inconsistent and cannot support comparable progress'
                    : identityResult.reason === 'limit'
                      ? 'review remediation finding identity history exceeds its aggregate resource limits'
                    : identityResult.reason === 'ambiguous'
                      ? 'review remediation finding identity evidence is ambiguous or incomparable'
                      : 'review disagreement lacks a complete comparable structured finding identity set',
                },
              }),
              action('archive_history', { if_absent: true }),
              action('release_lock'),
          ...
          if (remediationInProgress) {
            ...
            const resolvedPrior = previousFingerprints.some(
              (fingerprint) => !currentSet.has(fingerprint),
            );
            const additions = fingerprints.filter(
              (fingerprint) => !previousFingerprints.includes(fingerprint),
            );
            const repeatedOrReintroduced = additions.some(
              (fingerprint) => globallySeen.has(fingerprint),
            );
            if (!resolvedPrior || repeatedOrReintroduced) {
              const unchanged = additions.length === 0 &&
                fingerprints.length === previousFingerprints.length;
              return [
                action('transition', {
                  patch: {
                    status: 'blocked',
                    stage: 'remediation',
                    // prose-bound-exempt: all branches are fixed runtime diagnostics.
                    block_reason: unchanged
                      ? identityResult.legacy
                        ? 'review disagreement persists after a remediation cycle without a comparable structured finding set'
                        : 'a repeated review finding set made no remediation progress'
                      : repeatedOrReintroduced
                        ? 'review remediation reintroduced a globally seen finding identity'
                        : 'review remediation retained every prior finding and made no progress',
                  },
                }),
                action('archive_history', { if_absent: true }),
                action('release_lock'),
          ...
          if (state.remediation_cycles < effectiveRemediationLimit) {
                  ...
                  remediation_finding_history: {
                    ...
                    provenance: [
                      ...(remediationInProgress
                        ? history.provenance
                        : []),
                      identityResult.provenance,
                    ],
                    ...(history?.identity_epoch
                      ? { identity_epoch: history.identity_epoch }
                      : {}),
                  },
