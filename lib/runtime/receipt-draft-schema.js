...
import { canonicalRecoveryClaimPath, normalizeClaimPath, withinClaims } from './path-scope.js';
import {
  planLimitsForTicket,
  planDeviationSchemaForTicket,
  preflightSchemaForTicket,
...
import {
  ...
  RoadmapFollowupSchema,
  LegacyRoadmapFollowupSchema,
  structuredReviewFindingSchemaForTicket,
...
function missingAssuranceSchemaForTicket(ticket) {
  const limits = receiptLimitsForTicket(ticket);
  return z.union([
  z.string().min(1).max(limits.assurance_text),
  z.object({
    summary: z.string().min(1).max(limits.assurance_text).optional(),
    assurance: z.string().min(1).max(limits.assurance_text).optional(),
    reason: z.string().min(1).max(limits.assurance_text).optional(),
    detail: z.string().min(1).max(limits.assurance_text).optional(),
    title: z.string().min(1).max(limits.assurance_text).optional(),
    evidence_anchor: z.string().min(1).max(limits.assurance_anchor).optional(),
...
function testContradictionSchemaForTicket(ticket) {
  ...
  return z.object({
    test_paths: z.array(authorizedTestPath).max(limits.paths).optional(),
    summary: z.string().min(1).max(limits.contradiction_text).optional(),
    incompatible_expectations: z.string().min(1).max(limits.contradiction_text).optional(),
  }).catchall(z.unknown());
}

function scopeExpansionSchemaForTicket(ticket) {
  const limits = receiptLimitsForTicket(ticket);
  return z.object({
    claimed_paths: z.array(boundedClaim).min(1).max(limits.paths),
    reason: z.string().min(1).max(limits.scope_reason),
  }).strict();
}

function newContractStructuredReviewFindingSchema(ticket) {
  return structuredReviewFindingSchemaForTicket(ticket).safeExtend({
...
function evidenceSchemaForTicket(ticket) {
  ...
  const shape = {
    ...
    missing_assurances: z.array(missingAssuranceSchemaForTicket(ticket))
      .min(1).max(receiptLimitsForTicket(ticket).assurances).optional(),
    test_contradiction: testContradictionSchemaForTicket(ticket).optional(),
    scope_expansion: scopeExpansionSchemaForTicket(ticket).optional(),
    plan_deviation: planDeviationSchemaForTicket(ticket).optional(),
    roadmap_followups: z.array(limits.findings === RECEIPT_DRAFT_FINDING_LIMIT
      ? LegacyRoadmapFollowupSchema : RoadmapFollowupSchema)
      .max(limits.findings).optional(),
  };
  if (planningTicket(ticket)) {
    shape.candidate_plan = planContractSchemaForVersion(ticket?.plan_contract_version, planLimitsForTicket(ticket)).optional();
  }
  if (ticket?.role === 'preflight_analyst') {
    shape.preflight_artifact = preflightSchemaForTicket(ticket).optional();
  }
...
export function receiptDraftSchemaForTicket(ticket) {
  ...
  const findings = ticket?.review_contract_version === REVIEW_CONTRACT_VERSION
    ? z.array(newContractStructuredReviewFindingSchema(ticket)).max(limits.findings)
    : z.array(z.record(z.string(), z.unknown())).max(limits.findings);
  return z.object({
    ticket_id: z.string().min(1).max(2_048),
    status: z.enum(RECEIPT_STATUSES),
    tests: z.array(testSchemaForTicket(ticket)).max(limits.tests),
    findings,
    evidence,
    timing: z.object({
...
export function receiptDraftJsonSchemaForTicket(ticket) {
  ...
  if (planningTicket(ticket)) {
    ...
    if (candidate) {
      ...
      const maxBytes = planLimitsForTicket(ticket).candidate_plan_utf8_bytes;
      candidate.description = `Complete candidate plan. Canonical JSON is limited to ${maxBytes} UTF-8 bytes; commands must be exact members of ticket.capability_manifest.${commandField}, and profile IDs must be exact members of ticket.capability_manifest.verification_profiles.`;
      candidate['x-ape-utf8-maxBytes'] = maxBytes;
  ...
  schema['x-ape-receipt-contract'] = {
    ...
    validation_attempts: receiptLimits(ticket).max_validation_submissions_per_worker,
    candidate_plan_utf8_max_bytes: planLimitsForTicket(ticket).candidate_plan_utf8_bytes,
