...
export const RECEIPT_DRAFT_TOOL_EFFECT_LIMIT = 256;
...
export const ReceiptDraftRequiredClaimsSchema = z.object({
  ...
  tool_claims: z.array(boundedClaim).max(64).optional(),
...
function additiveToolClaimSchema(ticket) {
  return boundedClaim.superRefine((value, context) => {
    ...
    if (canonical !== value) {
      context.addIssue({
        code: 'custom',
        message: 'must be a canonical provider:resource:read|write|execute claim',
      });
      return;
    }
    if (toolClaimAllows(ticket?.tool_claims ?? [], {
      provider: parsed.provider,
      effect: parsed.access,
      resources: [parsed.resource],
    })) {
      context.addIssue({
        code: 'custom',
        message: 'is already on the immutable ticket and is not additive',
      });
    }
  });
}
...
function uniqueAdditions(values, context, field) {
  ...
  for (const [index, value] of (values ?? []).entries()) {
    const identity = field === 'tool_claims'
      ? value
...
function requiredClaimsSchemaForTicket(ticket) {
  ...
  return z.object({
    ...
    test_paths: z.array(additivePathSchema(ticket?.test_paths ?? [], 'test path'))
      .max(64).optional(),
    tool_claims: z.array(additiveToolClaimSchema(ticket)).max(64).optional(),
    ...
    uniqueAdditions(claims.tool_claims, context, 'tool_claims');
    if (
      (claims.claimed_paths?.length ?? 0) === 0 &&
      (claims.test_paths?.length ?? 0) === 0 &&
      (claims.tool_claims?.length ?? 0) === 0 &&
...
function planValidationContext(ticket) {
  ...
  return {
    ...
    allowed_evidence_commands: ticket?.receipt_contract_version === 1 &&
...
export function receiptDraftSchemaForTicket(ticket) {
  ...
  return z.object({
    ...
    findings,
    tool_effects: z.array(z.record(z.string(), z.unknown()))
      .max(RECEIPT_DRAFT_TOOL_EFFECT_LIMIT).optional(),
    evidence,
    timing: z.object({
...
function specializeRequiredClaimsSchema(requiredClaims, ticket) {
  ...
  if (properties.tool_claims) {
    properties.tool_claims.uniqueItems = true;
    properties.tool_claims.items['x-ape-canonical-tool-claim'] = true;
    properties.tool_claims.items['x-ape-additive-against-ticket-tool-claims'] = true;
    if ((ticket?.tool_claims ?? []).length > 0) {
      properties.tool_claims.items.not = {
        enum: [...new Set(ticket.tool_claims.filter((entry) => typeof entry === 'string'))],
      };
    }
  }
  ...
  requiredClaims.anyOf = [
    ...
    { properties: { tool_claims: { minItems: 1 } }, required: ['tool_claims'] },
...
export function receiptDraftJsonSchemaForTicket(ticket) {
  ...
  if (planningTicket(ticket)) {
    ...
    if (candidate) {
      candidate.description = `Complete candidate plan. Canonical JSON is limited to ${PLAN_CONTRACT_MAX_BYTES} UTF-8 bytes; commands and profile IDs must be exact members of ticket.capability_manifest.`;
      candidate['x-ape-utf8-maxBytes'] = PLAN_CONTRACT_MAX_BYTES;
      candidate['x-ape-command-enum-ref'] = 'ticket.capability_manifest.allowed_evidence_commands';
