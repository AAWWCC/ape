...
import { additiveCanonicalPathErrors, normalizeClaimPath } from './path-scope.js';
import { renderCommand, templateInvocation } from './runner.js';
...
import { commandProfileAvailableToRole, scopedEvidenceCommands } from './capability-selection.js';
import {
...
export function capabilityManifestGrowthEnabled(state) {
  return [1, CAPABILITY_MANIFEST_GROWTH_CONTRACT_VERSION].includes(
    state?.capability_snapshot?.manifest_growth_contract_version,
  );
}
...
export function ticketCapabilityManifest(state, stage, testPaths) {
  ...
  const scope = { ...state, test_paths: testPaths ?? state.test_paths };
  const commandProfiles = (snapshot.command_profiles ?? []).filter((profile) =>
    commandProfileAvailableToRole(profile, stage.role, scope));
  const reachableRoles = Array.isArray(snapshot.manifest_roles)
    ? new Set(snapshot.manifest_roles)
    : null;
  const planningCommandProfiles = PLANNING_ROLES.has(stage.role) && reachableRoles
    ? (snapshot.command_profiles ?? []).filter((profile) =>
        Array.isArray(profile?.roles) && profile.roles.some((role) =>
          reachableRoles.has(role) && commandProfileAvailableToRole(profile, role, scope)))
    : null;
  assertCapabilitySourceCollectionsFit({
    command_profiles: commandProfiles,
    planning_command_profiles: planningCommandProfiles,
    verification_profiles: snapshot.verification_profiles ?? [],
  });
  ...
  const addCommand = (command, commandTestPaths = testPaths, target = commands) => {
    if (typeof command !== 'string' || command.trim().length === 0) return;
    if (!command.includes('{paths}')) {
      addCapabilityEvidenceCommand(target, command);
      return;
    }
    ...
    addCapabilityEvidenceCommand(target, renderCommand([invocation.command, ...invocation.args]));
  };
  for (const profile of commandProfiles) addCommand(profile.command);
  for (const profile of planningCommandProfiles ?? []) {
    addCommand(profile.command, testPaths, plannableCommands);
  }
  for (const profile of verificationProfiles) {
    addCommand(profile.command);
    if (plannableCommands) addCommand(profile.command, testPaths, plannableCommands);
  }
  for (const command of Object.values(snapshot.test_commands ?? {})) {
    addCommand(command);
    if (plannableCommands) addCommand(command, testPaths, plannableCommands);
  }
  for (const runner of snapshot.runners ?? []) {
    ...
    for (const command of Object.values(runner?.profile ?? {})) {
      ...
      if (plannableCommands) addCommand(command, relativeTestPaths, plannableCommands);
    }
  }
  ...
  return {
    ...
    ...(snapshot.manifest_growth_contract_version === 2
      ? { field_bounds: runContractFieldBounds({ manifest_growth_contract_version: 2, run: state }) }
      : {}),
    required_capabilities: structuredClone(requiredCapabilities),
    allowed_evidence_commands: scopedEvidenceCommands(commands, snapshot.command_profiles ?? [], [stage.role], scope),
    ...
    ...(planningCommandProfiles
      ? {
          ...
          plannable_evidence_commands: scopedEvidenceCommands(plannableCommands, snapshot.command_profiles ?? [], [...reachableRoles], scope),
...
export function validateCapabilityManifestGrowth(state, patch = {}) {
  ...
  const testPaths = [...(candidate.test_paths ?? [])];
  const growthVersion = candidate.capability_snapshot.manifest_growth_contract_version;
  const pathBounds = capabilityTestPathBoundErrors(testPaths, { version: growthVersion });
  const errors = [...pathBounds.errors];
  const pathSetChanged = JSON.stringify(testPaths) !== JSON.stringify(state.test_paths ?? []);
  let concreteCapacityValid = true;
  ...
  for (const role of roles) {
    ...
    const manifest = {
      ...
      field_bounds: runContractFieldBounds({ run: candidate, manifest_growth_contract_version: growthVersion }),
      byte_budgets: runContractByteBudgets(candidate),
  ...
  return {
    ...
    test_path_bounds_valid: pathBounds.valid &&
      !(growthVersion === 2 && pathSetChanged && !concreteCapacityValid),
...
export function prospectiveReceiptCapabilityGrowth(state, ticket, draft, changedFiles) {
  ...
  const requestedClaims =
    draft?.status === 'failed' && evidence.failure_kind === 'capability'
      ? evidence.required_claims ?? {}
      : {};
  const futureTestPaths = [...new Set([
    ...(state.test_paths ?? []),
    ...(ticket.role === 'test_writer' ? changedFiles ?? [] : []),
    ...(Array.isArray(requestedClaims.test_paths) ? requestedClaims.test_paths : []),
  ])];
  const scopeExpansion = extractScopeExpansion(ticket, draft);
  const additiveClaimedPaths = [
    ...scopeExpansion.claimed_paths,
    ...(Array.isArray(requestedClaims.claimed_paths) ? requestedClaims.claimed_paths : []),
  ];
  const claimedPathErrors = scopeExpansion.errors.length === 0
    ? additiveCanonicalPathErrors(state.claimed_paths ?? [], additiveClaimedPaths, {
        label: 'additive claimed_paths',
      })
    : [];
  const futureClaimedPaths = scopeExpansion.errors.length === 0 && claimedPathErrors.length === 0
    ? [
        ...(state.claimed_paths ?? []),
        ...additiveClaimedPaths,
      ]
    : [...(state.claimed_paths ?? [])];
  ...
  const growth = validateCapabilityManifestGrowth(state, {
    claimed_paths: futureClaimedPaths,
    test_paths: futureTestPaths,
    risk_triggers: futureRiskTriggers,
  ...
  return {
    ...growth,
    valid: growth.valid && claimedPathErrors.length === 0,
    errors: [...claimedPathErrors, ...growth.errors],
    claimed_path_admission_valid: claimedPathErrors.length === 0,
  };
}

export function mergeReceiptCapabilityGrowthResult(draftResult, growth) {
  if (growth?.valid !== false) return draftResult;
  const testPathGrowthFailure = growth.test_path_bounds_valid === false;
  const claimedPathGrowthFailure = growth.claimed_path_admission_valid === false;
  const pathGrowthFailure = testPathGrowthFailure || claimedPathGrowthFailure;
  const growthCorrections = growth.errors.slice(0, 20).map((issue) => ({
    field: claimedPathGrowthFailure
      ? 'evidence.required_claims.claimed_paths'
      : testPathGrowthFailure
        ? 'runtime.test_paths'
        : 'capability_manifest',
    issue,
    correction: pathGrowthFailure
      ? claimedPathGrowthFailure
        ? 'use distinct canonical project-relative production claims whose portable case-folded identities do not collide, then validate this exact final draft again'
        : 'correct the canonical test path set or reduce authored test scope until its concrete commands and future manifests fit the ticket-published resource bounds, then validate this exact final draft again'
  ...
  return {
    ...
    valid: false,
    corrections: [
      ...(Array.isArray(draftResult?.corrections) ? draftResult.corrections : []),
      ...growthCorrections,
    ].slice(0, 20),
    capability_growth: {
