...
import { sameCommandArgv } from './capability-selection.js';
...
const WRITE_CONTENT_ROUTES = Object.freeze([
  ...
  Object.freeze({
    tool: 'write_to_file',
    extract: (input) => [input?.CodeContent ?? input?.code_content],
  }),
  Object.freeze({
    tool: 'replace_file_content',
    extract: (input) => [input?.ReplacementContent ?? input?.replacement_content],
  }),
  Object.freeze({
    tool: 'multi_replace_file_content',
    extract: (input) => (
      Array.isArray(input?.ReplacementChunks)
        ? input.ReplacementChunks.map((chunk) => chunk?.ReplacementContent)
        : Array.isArray(input?.replacement_chunks)
          ? input.replacement_chunks.map((chunk) => chunk?.replacement_content)
          : []
    ),
  }),
]);
...
// receipt-validator.js's scope-expansion admission set / pipeline.js's
...
export function normalizePath(value, projectDir) {
  ...
  const lexicalNativeRelative = path.relative(projectDir, absolute);
  const lexicalRelative = lexicalNativeRelative.replaceAll('\\', '/');
  if (
    !path.isAbsolute(lexicalNativeRelative) &&
  ...
  // Absolute paths supplied by a host may use a different spelling of the
  // governed root. macOS exposes the common case directly: /tmp is a symlink
  // to /private/tmp, so a session rooted at one spelling can emit an
  // apply_patch target through the other. A not-yet-created target cannot be
  // realpathed directly; resolve its nearest existing ancestor and reattach
  // the missing suffix, matching the async claim gate below. This fallback is
  // consulted only after lexical containment fails, and it still requires the
  // canonical target to remain beneath the canonical project root.
  const canonicalRoot = nearestExistingPathSync(path.resolve(projectDir));
  const canonicalTarget = nearestExistingPathSync(absolute);
  if (!canonicalRoot || !canonicalTarget) return null;
  const canonicalNativeRelative = path.relative(canonicalRoot, canonicalTarget);
  const canonicalRelative = canonicalNativeRelative.replaceAll('\\', '/');
  if (
    path.isAbsolute(canonicalNativeRelative) ||
    canonicalRelative === '..' ||
...
function nearestExistingPathSync(absolute) {
  let candidate = absolute;
  const suffix = [];
  for (;;) {
    try {
      lstatSync(candidate);
      return path.join(realpathSync(candidate), ...suffix.reverse());
    } catch (error) {
...
function validCommandProfiles(profiles) {
...
function declaredCommandProfiles(projectDir) {
  ...
  try {
    ...
    return validCommandProfiles(stored?.policy?.command_profiles);
  } catch {
    return [];
  }
}

// New native/receipt-contract runs seal command profiles at admission and
// publish the role-filtered subset on each immutable ticket. Hook decisions
// for those runs must never re-read mutable config.json: an operator edit made
// after START may affect the next run, but it cannot silently add or revoke a
// capability from one already in flight. A malformed new-contract snapshot
// therefore degrades to no profiles rather than falling back to live config.
// Legacy states/tickets have neither marker and retain the historical live-
// config lookup byte-for-byte.
function immutableCommandProfiles(context) {
  const ticket = context?.ticket;
  if (ticket?.receipt_contract_version === 1) {
    const snapshot = context?.state?.capability_snapshot;
    if (
      snapshot?.version !== 1 ||
      typeof snapshot.config_hash !== 'string' ||
      ticket.capability_manifest?.config_hash !== snapshot.config_hash
    ) return [];
    const snapshotProfiles = validCommandProfiles(snapshot.command_profiles);
    return validCommandProfiles(ticket.capability_manifest.command_profiles).filter((profile) =>
      snapshotProfiles.some((candidate) =>
        candidate.id === profile.id &&
        candidate.command === profile.command &&
        candidate.effect === profile.effect &&
        candidate.roles.length === profile.roles.length &&
        candidate.roles.every((role, index) => role === profile.roles[index])));
  }
  ...
  if (
    ...
    return validCommandProfiles(state.capability_snapshot.command_profiles);
  }
  return null;
}

export function matchingCommandProfile(projectDir, command, context = null) {
  const immutable = immutableCommandProfiles(context);
  const profiles = immutable ?? declaredCommandProfiles(projectDir);
  const exact = profiles.find((profile) => profile.command === command);
  if (exact) return exact;
  const manifestProfiles = context?.ticket?.capability_manifest?.command_profiles;
  if (immutable === null || !Array.isArray(manifestProfiles)) return null;
  // Lifecycle authorization and hook reconciliation must identify the same
  // immutable profile. Only normalize a candidate already carried by this
  // ticket and validated against the frozen snapshot above.
  return profiles.find((profile) =>
    manifestProfiles.some((entry) => entry?.command === profile.command) &&
    sameCommandArgv(command, profile.command)) ?? null;
}
...
// unresolved change is DENIED at the post event. The hook retains separate
// run-scoped attribution evidence before returning that refusal, and both
// result hooks and receipt admission consult it. Matched parent pre/post
// observations distinguish newly observed drift from existing worker work;
// unmatched mutations use a conservative baseline. Known inspection posts
// remain available without reclassifying existing worker changes. A refusal
// never restores bytes automatically: an authorized repair must restore the
// affected reference entries before new output can be accepted. Pre events
// stay pattern-bound so the operator's read-only
...
export function driftGuardApplies(event, context = null) {
  ...
  if (event.tool_name !== 'Bash' && event.tool_name !== 'run_command') return false;
  const profile = matchingCommandProfile(event.project_dir, event.command ?? '', context);
...
export function evaluateTreePolicy(event, context, changedFiles) {
  ...
  for (const file of changedFiles) {
    ...
    if (ticket.role === 'test_writer' && !withinTestScope(file, ticket.test_paths, ticket.test_scope === 'exact')) {
      // prose-bound-exempt: file is a runtime-derived changed path.
