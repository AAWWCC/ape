...
// Shared by other authority-expansion boundaries. Keep the enumeration owned
// here so receipt-time claim admission cannot drift from the write-time policy
// that guards the same execution-redirecting files.
export function isExecutionRedirectingConfigPath(value) {
  ...
  return EXECUTION_CONFIG_TAIL.has(segments.at(-1)) ||
    EXECUTION_CONFIG_TAIL_PAIR.has(segments.slice(-2).join('/'));
}
...
export function matchingCommandProfile(projectDir, command, context = null) {
  ...
  return profiles.find((profile) => profile.command === command) ?? null;
}
