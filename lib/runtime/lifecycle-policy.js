...
// Codex's functions.exec wrapper may render an argv vector as adjacent JSON
// string literals (for example `"git" "diff" "--stat"`) before the command
// reaches the lifecycle hook. That spelling is shell-equivalent to the plain
// evidence command but the policy's positive alphabet correctly refuses the
// quote characters. Canonicalize only the complete, unambiguous shape: every
// token must be one escape-free JSON string, separators must be ASCII spaces,
// and a token may not contain whitespace because joining it would lose argv
// boundaries. JSON escapes are deliberately refused: a shell does not decode
// `\uNNNN`, so doing that here could turn a denied executable spelling into an
// allowed one. Mixed or malformed quoting stays untouched and fails closed in
// parseEvidenceCommand; metacharacters do too.
function normalizeLifecycleCommand(raw) {
  if (typeof raw !== 'string' || !raw.startsWith('"')) return raw;
  const tokens = [];
  let cursor = 0;
  while (cursor < raw.length) {
    JSON_ARGV_TOKEN.lastIndex = cursor;
    const match = JSON_ARGV_TOKEN.exec(raw);
    if (!match) return raw;
    const token = match[0].slice(1, -1);
    if (token.length === 0 || /\s/u.test(token)) return raw;
    tokens.push(token);
    cursor = JSON_ARGV_TOKEN.lastIndex;
    if (cursor === raw.length) break;
    const separatorStart = cursor;
    while (raw[cursor] === ' ') cursor += 1;
    if (cursor === separatorStart || raw[cursor] !== '"') return raw;
  }
  return tokens.join(' ');
}
