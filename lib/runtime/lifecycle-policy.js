import { readBoundedJsonSync } from './bounded-file.js';
...
import { commandProfileAvailableToRole, invokesDeclaredWriteProfile } from './capability-selection.js';
import { looksLikeTest, withinClaim, withinTestScope } from './path-scope.js';
import { SHELL_WRITE, WRITE_TOOLS, executionConfigTail, extractApplyPatchPaths, matchingCommandProfile, normalizePath, parseDeletionCommand } from './write-policy.js';
import { BARE_TEST_RUNNER, EVIDENCE_COMMAND_FAMILIES, GIT_EVIDENCE_VERB_TOKEN, INSPECTION_BUILTIN, LINT_EXEC_PREFIX, LINT_TOOL, NODE_EVIDENCE_FLAG, PACKAGE_MANAGER_HEAD, PACKAGE_MANAGER_RUNNER, PYTHON_HEAD, PYTHON_MANAGER_HEAD, PYTHON_MANAGER_TEST_HEAD, PYTHON_TEST_MODULE, RUFF_EVIDENCE_VERB, evidencePathOperands, evidenceOperandEscapes, evidenceOperandIsGitNoIndexDevNull, evidenceTokenUsesPositiveAlphabet, gitEvidenceArgsSafe, inspectionEvidenceArgsSafe, leadingEvidenceCd, parseEvidenceCommand } from './evidence-policy.js';
...
// Receipt draft validation is the one APE-owned in-protocol tool a bound
// worker may call. It is pure and non-mutating; all control-plane tools remain
// orchestrator-only below.
export const SUBAGENT_PROTOCOL_TOOLS = /^(?:mcp__(?:ape|plugin_ape_ape)__)?ape_validate_receipt$/;
const BOOTSTRAP_PROTOCOL_TOOL = /^(?:mcp__(?:ape|plugin_ape_ape)__)?ape_bind$/;

// The runtime's own control-plane tools, matched bare or under APE's two
// shipped server namespaces. Shared with bin/ape-hook.mjs so the main-
// session stage-guard exemption and the subagent one-owner deny key off one
// pattern (friction #13).
export const CONTROL_PLANE_TOOLS = /^(?:mcp__(?:ape|plugin_ape_ape)__)?ape_(run|status|config|history)$/;

// APE does not authorize, classify, or audit tools owned by other MCP servers.
// Their names and schemas are dynamic host capabilities, and attempting to
// forecast them in an immutable APE ticket made unrelated integrations fail as
// soon as a server renamed or added an operation. The host and operator own
// those permissions. Keep only APE's own control plane and receipt-validation
// protocol inside the lifecycle boundary.
export function isExternalMcpTool(toolName) {
  return (
    typeof toolName === 'string' &&
    toolName.startsWith('mcp__') &&
    !CONTROL_PLANE_TOOLS.test(toolName) &&
    !SUBAGENT_PROTOCOL_TOOLS.test(toolName) &&
    !BOOTSTRAP_PROTOCOL_TOOL.test(toolName)
  );
}
...
function stripLeadingCd(command) {
  return leadingEvidenceCd(command)?.remainder ?? command;
}

const CORRECTABLE_READ_HEAD = new Set(['cat', 'ls', 'pwd', 'which', 'sha256sum', 'shasum', 'rg', 'grep', 'head', 'tail']);
const CORRECTABLE_GIT_READ_VERB = new Set([
  'status', 'diff', 'log', 'show', 'rev-parse', 'describe', 'ls-files', 'ls-tree',
]);

// Classify only an obvious inspection intent as correctable command shape.
// Arbitrary or potentially mutating heads (`cp`, `make`, `git push`, `env`)
// must keep the edit-channel remedy instead of spending planner/worker retries
// as if a syntax tweak could authorize them. This is diagnostic routing only;
// every command still faces the same fail-closed parser and allowlist below.
function looksLikeCorrectableReadShape(command) {
  if (typeof command !== 'string') return false;
  const [head, second] = stripLeadingCd(command).trimStart().split(/ +/u);
  if (CORRECTABLE_READ_HEAD.has(head)) return true;
  return head === 'git' && CORRECTABLE_GIT_READ_VERB.has(second);
}
...
function evidenceContainment(parsed, projectDir, sessionCwd = projectDir) {
  const { cdTarget, tokens } = parsed;
  const resolveOperand = (operand, cwd) =>
    typeof cwd === 'string' && cwd.length > 0 ? path.resolve(cwd, operand) : operand;
  // Keep the governed root as the boundary while resolving operands where
  // the shell executes them. Legacy callers with no root retain their existing
  // lexical checks rather than inheriting this process's working directory.
  const executionCwd = typeof sessionCwd === 'string' && sessionCwd.length > 0
    ? cdTarget === null ? sessionCwd : path.resolve(sessionCwd, cdTarget)
    : null;
  if (cdTarget !== null) {
    ...
    if (
      cdTarget.startsWith('-') ||
      cdTarget.startsWith('+') ||
      evidenceOperandEscapes(resolveOperand(cdTarget, sessionCwd), projectDir)
    ) {
      return { safe: false, operand: cdTarget };
    }
  }
  for (const { tokenIndex, candidate } of evidencePathOperands(tokens)) {
    if (evidenceOperandIsGitNoIndexDevNull(tokens, tokenIndex, candidate)) continue;
    if (evidenceOperandEscapes(resolveOperand(candidate, executionCwd), projectDir)) {
...
// New native runs derive this tier from the admission-time capability snapshot,
// never config.json. Keep the derivation identical to the legacy config shape:
// test command slots, runner-local slots, and the explicit evidence-script
// lever are the only inputs that can widen a read-only package-script name.
function collectSnapshotRunScripts(snapshot, names) {
  collectRunScriptNames(snapshot.test_commands, names);
  if (Array.isArray(snapshot.runners)) {
  ...
  if (Array.isArray(snapshot.evidence_scripts)) {
    for (const entry of snapshot.evidence_scripts) {
      if (typeof entry === 'string' && RUN_SCRIPT_NAME.test(entry)) names.add(entry);
    }
  }
}

function immutableRunScripts(context) {
  const ticket = context?.ticket;
  const state = context?.state;
  if (ticket?.receipt_contract_version === 1) {
    const names = new Set(EVIDENCE_SCRIPT_FLOOR);
    const snapshot = state?.capability_snapshot;
    // The ticket binds the state snapshot by its admission-time config hash.
    // Do not derive names from allowed_evidence_commands: that list also holds
    // exact command profiles, including profiles whose effect is `write`, and
    // reducing one of those to a package-script name would accidentally widen
    // an exact profile into the generic read-only run-script family.
    if (
      snapshot?.version !== 1 ||
      typeof snapshot.config_hash !== 'string' ||
      ticket.capability_manifest?.config_hash !== snapshot.config_hash
    ) return names;
    collectSnapshotRunScripts(snapshot, names);
    return names;
  }
  if (
  ...
  const names = new Set(EVIDENCE_SCRIPT_FLOOR);
  collectSnapshotRunScripts(state.capability_snapshot, names);
  return names;
}

function declaredRunScripts(projectDir, context = null) {
  const immutable = immutableRunScripts(context);
  if (immutable) return immutable;
  // Legacy runs synchronously read the governed project's OWN raw
  // `.ape/runtime/config.json` overlay rather than merged config. Every shipped
  // default here is null/empty, so the overlay is the complete declaration set
  // and this stays free of the async config loader. TOTAL try/catch degrades
  // restrictively to the floor on missing, unreadable, malformed, or wrong-
  // typed config and must never throw (see the SYNCHRONY note above).
  const names = new Set(EVIDENCE_SCRIPT_FLOOR);
  if (typeof projectDir !== 'string' || projectDir.length === 0) return names;
  try {
    const stored = readBoundedJsonSync(runtimePaths(projectDir).config, RUNTIME_STATE_MAX_BYTES);
...
const JSON_ARGV_TOKEN = /"[^"\\\u0000-\u0020]*"/uy;
const SHELL_SINGLE_ARGV_TOKEN = /'[^'\\\u0000-\u0020]*'/uy;

// Codex command wrappers may render an argv vector as adjacent, uniformly
// quoted tokens (for example `"git" "diff" "--stat"` or `'cat' 'package.json'`)
// before the command reaches the lifecycle hook. Those complete spellings are
// shell-equivalent to the plain evidence command, but the policy's positive
// alphabet correctly refuses quote characters. Canonicalize only the complete,
// unambiguous shape: every token uses the same quote style, is escape-free and
// non-empty, separators are ASCII spaces, and no token contains whitespace.
// JSON/shell escapes and any token outside the evidence policy's positive
// alphabet are deliberately refused: decoding shell syntax such as a quoted
// `&&` would turn argv data into a command operator. Mixed, partial, malformed,
// or concatenated quoting likewise stays untouched and fails closed later.
//
// A complete single-quoted Next.js route operand keeps its outer quotes while
// ordinary tokens are dequoted. The bracket-aware evidence parser then proves
// the route segments and containment against the literal argv value instead of
// accidentally reintroducing shell glob semantics during canonicalization.
function normalizeLifecycleCommand(raw) {
  if (typeof raw !== 'string' || (raw[0] !== '"' && raw[0] !== "'")) return raw;
  // Preserve literal argv spelling: removing quotes from '^pattern' or
  // '~filename' invents shell expansion that the original call never used.
  if (parseEvidenceCommand(raw)?.literal_inspection === true) return raw;
  const quote = raw[0];
  const tokenPattern = quote === '"' ? JSON_ARGV_TOKEN : SHELL_SINGLE_ARGV_TOKEN;
  const tokens = [];
  let cursor = 0;
  while (cursor < raw.length) {
    ...
    if (token.length === 0 || /\s/u.test(token)) return raw;
    const bracketedRoute = quote === "'" && (token.includes('[') || token.includes(']'));
    if (!evidenceTokenUsesPositiveAlphabet(token, { allowBrackets: bracketedRoute })) return raw;
    tokens.push(token);
    cursor = tokenPattern.lastIndex;
    if (cursor === raw.length) break;
    const separatorStart = cursor;
    while (raw[cursor] === ' ') cursor += 1;
    if (cursor === separatorStart || raw[cursor] !== quote) return raw;
  }
  return tokens
    .map((token) => (
      quote === "'" && (token.includes('[') || token.includes(']'))
        ? `'${token}'`
        : token
    ))
    .join(' ');
...
export function evaluateLifecyclePolicy(event, context) {
  ...
  if (isToolChannelEvent(event) && isExternalMcpTool(event.tool_name)) {
    return {
      decision: 'allow',
      reason: 'external MCP tool is governed by the host and operator, not APE',
    };
  }
  ...
  if (SEALED_STATUSES.has(context.state.status)) {
    if (
      (context.ticket || event.sealed_binding_evidence_unreadable === true) &&
      isToolChannelEvent(event) &&
      event.out_of_project !== true &&
      (WRITE_TOOLS.has(event.tool_name) ||
  ...
  if (isToolChannelEvent(event) && event.host === 'codex' &&
      event.is_subagent && event.ape_managed === true && !context?.ticket &&
      !BOOTSTRAP_PROTOCOL_TOOL.test(event.tool_name)) {
    return {
      decision: 'deny',
      reason: 'APE managed child has no live ticket binding; complete the authorized bootstrap or return control to the orchestrator',
    };
  }
  if (
    event.host === 'claude' &&
  ...
  if (event.tool_name === 'Bash' || event.tool_name === 'run_command') {
    ...
    if (context?.ticket) {
      ...
      const commandProfile = matchingCommandProfile(event.project_dir, command, context);
      const snapshottedWriters = context.ticket.capability_manifest?.version === 1 &&
        Array.isArray(context.state.capability_snapshot?.command_profiles)
        ? context.state.capability_snapshot.command_profiles.filter((profile) =>
            invokesDeclaredWriteProfile(command, profile))
        : [];
      const ticketScope = context.ticket.role === 'test_writer'
        ? { test_paths: context.ticket.claimed_paths ?? [] }
        : { claimed_paths: context.ticket.claimed_paths ?? [] };
      if (snapshottedWriters.some((profile) => !commandProfile ||
          !commandProfileAvailableToRole(profile, context.ticket.role, ticketScope))) {
        return {
          decision: 'deny',
          reason: 'APE command profile denied: this declared writer is unavailable in the ticket or its outputs exceed the ticket scope; request the needed capability and output scope before running it',
        };
      }
      if (commandProfile) {
      ...
      const unrecognized = looksLikeCorrectableReadShape(command)
        ? {
          decision: 'deny',
          reason: 'APE command-shape denied: a bound subagent may run only recognized non-mutating evidence commands; keep the command head unquoted, use only the ticket\'s published token alphabet, and single-quote only complete Next.js bracketed path operands',
        }
        : {
          decision: 'deny',
          reason: 'APE shell denied: a bound subagent may run only recognized non-mutating evidence commands; production edits must use the host edit tool',
        };
      ...
      // Executable identity is meaningful only after the command has passed
      // the evidence-family grammar. The async hook precomputes a conservative
      // verdict for every shell head, including `cp` and other forbidden write
      // commands; consulting it earlier misdiagnosed the policy refusal as a
      // missing trusted-start executable and sent operators toward the wrong
      // recovery. Recognized commands still fail closed on any identity drift.
      ...
      if (
        ...
        const admitted = declaredRunScripts(event.project_dir, context);
      ...
      const containment = evidenceContainment(parsed, event.project_dir, verdict?.session_cwd);
      if (!containment.safe) {
        return {
          decision: 'deny',
      ...
      // Complete literal words have already separated shell syntax from argv
      // data. A quoted regex pipe or space is not a shell operator. Apply the
      // same command effects and path checks without reinterpreting that data
      // through the raw shell metacharacter expressions below.
      if (parsed.literal_inspection === true && evidenceHead > 0 &&
          gitEvidenceArgsSafe(parsed.tokens) && inspectionEvidenceArgsSafe(parsed.tokens)) {
        return { decision: 'allow', reason: 'literal read-only inspection within the governed project' };
      }
      const evidenceCommand = stripLeadingCd(command);
      if (
        ((evidenceHead > 0 && EVIDENCE_COMMAND.test(evidenceCommand) && gitEvidenceArgsSafe(parsed.tokens) && inspectionEvidenceArgsSafe(parsed.tokens)) ||
          (lintHead > 0 &&
    ...
    if (!event.is_subagent) {
      const literalRead = parseEvidenceCommand(command);
      if (literalRead?.literal_inspection === true &&
          recognizeEvidenceHead(literalRead.tokens) > 0 &&
          gitEvidenceArgsSafe(literalRead.tokens) && inspectionEvidenceArgsSafe(literalRead.tokens)) {
        return { decision: 'allow', reason: 'main-session literal inspection has no production write effect' };
      }
  ...
  if (!WRITE_TOOLS.has(event.tool_name)) {
    if (
      ...
      !SUBAGENT_PROTOCOL_TOOLS.test(event.tool_name)
    ) {
...
export function formatHookResponse(event, result) {
  ...
  if (event.event === 'PreToolUse') {
    ...
    if (event.host === 'codex' && result.decision === 'allow') {
