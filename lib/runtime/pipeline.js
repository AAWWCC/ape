...
function primaryReviewStages(run) {
  const stages = [stage('review', 'reviewer', {
    parallel_group: 'code-review',
    required_checks: phaseBuildChecks(run),
  })];
...
export function nextStages(run, completedStageId, receipt) {
  ...
  if (completedStageId === 'build') {
    return primaryReviewStages(run).map((entry) =>
      entry.id === 'review'
        ? stage('review', 'reviewer', { parallel_group: 'code-review' })
        : entry);
  }
  return [];
}
...
export function projectedPipeline(run = {}) {
  ...
  if (run.mode === 'debug') {
    ...
    if (projectedLane === 'fast') {
      ...
      conditionalBranches.push({ id: 'plan-replan', label: 'One directed replan (on judge rejection)' });
  ...
  const byModelTier = {};
  let total = 0;
  const directedPlanRounds = stages.some((entry) => entry.id === 'plan-replan') ? 2 : 1;
  const remediationDispatches = remediationCycleLimit(run) + (MAX_STAGE_ATTEMPTS - 1);
