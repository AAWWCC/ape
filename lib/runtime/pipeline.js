import { MAX_REMEDIATION_CYCLES, ROLE_POLICIES } from './constants.js';
...
function stage(id, role, options = {}) {
  ...
  return Object.freeze({
    ...
    output_schema: options.output_schema ?? RECEIPT_INPUT_SCHEMA,
  });
}
...
// updated together by hand. lib/runtime/hooks.js's WRITE_CONTENT_HAZARD_CHARS
...
// hooks.js's own comment). Never re-sync hooks.js's copy down to match this
// pair's narrower set.
//
...
export function nextStages(run, completedStageId, receipt) {
  ...
  if (completedStageId === 'review-disagreed' && run.remediation_cycles < MAX_REMEDIATION_CYCLES) {
    ...
    if (declaredTestRemediationPaths(run).length > 0) {
      ...
      return [stage('remediation-test', 'test_writer', { required_checks: [] })];
    }
