...
function testRecheckStage() {
  // Only an implementer's contradiction can enter independent reconciliation.
  // Observe the exact correction against the current tree: a repaired fixture
  // may pass already, or expose production work for the resumed implementer.
  // Neither verdict replaces initial red-first evidence. This separate check
  // requires stable runtime execution without demanding one of those outcomes.
  return stage('test-recheck', 'test_writer', { required_checks: ['test-correction'] });
}
...
// Legacy path declarations come from the first code-review group. Later
// remediation cycles use their structured route and convergence evidence;
// increasing the cycle policy must not make an old declaration accumulate.
...
export function roleReceiptRequirements(role, _stageId) {
  return ROLE_RECEIPT_REQUIREMENTS[role] ?? { required_fields: COMMON_REQUIRED_FIELDS };
}

export function nextStages(run, completedStageId, _receipt) {
...
function projectedArtifacts(run, stages) {
  ...
  for (const producer of stages) {
    for (const check of producer.required_checks.filter((value) => ['red-test', 'green-test', 'test-correction'].includes(value))) {
      const id = `runtime:observe-${check}:${producer.id}`;
      const observation = check === 'red-test' ? 'red' : check === 'green-test' ? 'green' : 'correction';
      const action = check === 'test-correction' ? 'observe_test_correction' : `observe_${observation}_test`;
      node(id, action, ['runtime-execution', 'exact-authored-test-paths', 'tree-bound-observation']);
      edge(`authored-tests:${producer.id}`, 'runtime-observed-test-tree', [producer.id], [id]);
      edge(`${observation}-evidence:${producer.id}`, 'runtime-sealed-test-observation', [id], present(['build', 'review', 'remediation-build']));
    }
  }
...
export function projectedPipeline(run = {}) {
  ...
  if (run.mode === 'debug') {
    ...
    if (projectedLane === 'fast') {
      ...
      conditionalBranches.push({ id: 'plan-replan', label: `Up to ${limits.max_directed_replans} converging directed replans (on judge rejection)` });
  ...
  // A zero optional budget makes the branch unreachable. Remove it before
  // deriving capabilities, artifact edges and dispatch bounds: retry budgets
  // cannot create a first ticket the scheduler will never issue.
  if (limits.max_remediation_cycles === 0) {
    stages = stages.filter((entry) => !entry.id.startsWith('remediation-'));
    conditionalBranches = conditionalBranches.filter((entry) => entry.id !== 'remediation');
  }
  if (limits.max_directed_replans === 0) {
    stages = stages.filter((entry) => entry.id !== 'plan-replan');
    conditionalBranches = conditionalBranches.filter((entry) => entry.id !== 'plan-replan');
  }
  ...
  // Initial plan plus the configured strict-subset directed replans admitted
  // by the reducer. Global stage retries are added once across all rounds.
  ...
  for (const s of stages) {
    const stageLimits = stageRecoveryLimits(s.id, run);
    const attempts = ['test-reconcile', 'test-recheck'].includes(s.id)
      ? stageLimits.max_stage_attempts
      ...
      // global, so its retry allowance is added across those rounds once.
      : ['plan-check', 'plan-critic', 'plan-judge'].includes(s.id)
        ? directedPlanRounds + (limits.max_stage_attempts - 1)
  ...
  const physicalUpperBound = (logicalUpperBound + protocolReplacementUpperBound)
    * limits.max_physical_workers_per_ticket;
  if (![total, logicalUpperBound, protocolReplacementUpperBound, physicalUpperBound,
    physicalUpperBound * limits.max_validation_submissions_per_worker,
    ...Object.values(byStage), ...Object.values(physicalByStage)].every(Number.isSafeInteger)) {
    throw new Error('execution policy pipeline forecast exceeds safe integer arithmetic');
  }
