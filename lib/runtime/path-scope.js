...
// Shared field grammar for configuration, tickets, and plan/preflight references.
export const VERIFICATION_PROFILE_ID_PATTERN = /^[A-Za-z][A-Za-z0-9._-]{0,63}$/;

export function isCanonicalVerificationRoot(value) {
  if (typeof value !== 'string' || value.length === 0 || value.length > 512) return false;
  if (value === '.') return true;
  if (/[\\\u0000-\u001f\u007f]/u.test(value) || value.startsWith('/') || /^[A-Za-z]:/.test(value)) return false;
...
// Recovery additions use the same grammar at schema and receipt validation.
export function canonicalRecoveryClaimPath(value) {
  if (
