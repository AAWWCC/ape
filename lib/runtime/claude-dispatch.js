import { isCanonicalTimestamp as boundedDispatchTimestamp, timestampAtLeast } from './timestamps.js';
...
import { atomicReplaceText, atomicWriteJson as atomicWriteStoredJson, ensureDir } from './storage.js';
...
import { receiptLimits } from './pipeline-limits.js';
...
async function atomicWriteJson(file, value) {
  const maximum = value?.ticket_id ? MAX_DISPATCH_INTENT_FILE_BYTES : MAX_SUBAGENT_START_DIAGNOSTIC_FILE_BYTES;
  if (Buffer.byteLength(`${JSON.stringify(value, null, 2)}\n`, 'utf8') > maximum) {
    throw new Error('dispatch metadata exceeds its readable artifact byte budget; the prior authority was preserved');
  }
  return atomicWriteStoredJson(file, value);
}
...
async function readDispatchIntentArtifact(file, { allowMissing = false } = {}) {
  ...
  try {
    ...
    const openedAfterRead = await statFileHandle(handle);
...
// FUNCTION (resolveClaudeBindingOutcome, exactly like
...
// only exception is a configured physical receipt-repair worker authorized after
// an observed predecessor stop caused by validation exhaustion. Pre-upgrade
...
// the next physical worker carrying the original deadline alongside a new bounded
// host-dispatch horizon.
function activeReceiptProtocolRecovery(state, ticket, record) {
  const workers = record?.physical_worker_dispatches;
  const priorWorkers = workers - 1;
  const maximum = receiptLimits(ticket).max_physical_workers_per_ticket;
  const source = record?.receipt_protocol_recovery_source;
  const expectedExhaustions = source?.physical_worker_dispatches === priorWorkers
    ? source.validation_exhaustions : priorWorkers;
  const validationExhaustionRecovery = Boolean(
    Number.isSafeInteger(expectedExhaustions) && expectedExhaustions > 0 &&
    record?.receipt_validation_exhaustions === expectedExhaustions &&
    (state?.receipt_contract_exhaustions?.[ticket?.ticket_id] ?? 0) === expectedExhaustions
  );
  ...
  return Boolean(
    ...
    Number.isSafeInteger(workers) && workers > 1 && workers <= maximum &&
...
async function prepareDispatchIntent(paths, ticket, agentType, host, options = {}) {
  ...
  await withDispatchLock(paths, async () => {
    ...
    if (
      ticket.receipt_contract_version === 1 &&
      (!Number.isSafeInteger(physicalWorkerDispatches) ||
        physicalWorkerDispatches > receiptLimits(ticket).max_physical_workers_per_ticket)
    ) {
    ...
    const generation = reprepareUnlaunched
      ...
      : priorGeneration + 1;
    if (!Number.isSafeInteger(generation)) throw new Error('dispatch generation exceeds safe integer arithmetic');
    launchKey = await dispatchLaunchKey(paths);
    ...
    const record = {
      ...
      ...(priorValidationExhaustions > 0
        ...
        : {}),
      ...(hasRecoveryDeadline && physicalWorkerDispatches > 1 &&
          physicalWorkerDispatches <= receiptLimits(ticket).max_physical_workers_per_ticket &&
          (current?.receipt_validation?.exhausted === true || priorBudgetInterruptedStop)
        ? {
            receipt_protocol_recovery: true,
            immutable_ticket_deadline_at: ticket.deadline_at,
            receipt_protocol_recovery_source: {
              physical_worker_dispatches: priorPhysicalWorkers,
              validation_exhaustions: priorValidationExhaustions,
            },
...
export async function expireClaudeIntent(paths, ticketId, { preserveActiveBound = false } = {}) {
  return withDispatchLock(paths, async () => {
    const file = intentPath(paths, ticketId);
    const current = await readCanonicalIntent(paths, ticketId);
    if (!current) return current;
    if (current.status === 'expired') return current;
    // Automatic recovery observes liveness before taking this dispatch lock.
    // SubagentStart can bind the authorized launch in that interval, so a
    // stale observation cannot revoke a worker that is now physically live.
    // Audited expiry/abort retain their explicit revocation behavior.
    if (preserveActiveBound && current.status === 'bound' && !current.agent_stopped_at) return current;
    const expiredAt = iso(timestampAtLeast(Date.now(), current.prepared_at, current.launched_at, current.bound_at));
    const voided = {
...
export async function expireClaudeIntentsForRun(paths, runId) {
  return withDispatchLock(paths, async () => {
    ...
    for (const name of names.sort()) {
      ...
      const expiredAt = iso(timestampAtLeast(Date.now(), record.prepared_at, record.launched_at, record.bound_at));
      const next = {
...
async function launchDispatchIntent(paths, state, input, host) {
  ...
  return withDispatchLock(paths, async () => {
    ...
    if (['authorized', 'launched'].includes(record.status) && expired(record.launch_expires_at)) {
      const orphanedAt = iso(timestampAtLeast(Date.now(), record.launched_at));
      await atomicWriteJson(file, {
    ...
    const launchedAt = timestampAtLeast(Date.now(), record.prepared_at);
...
async function bindDispatchSubagent(paths, state, input, host) {
  ...
  return withDispatchLock(paths, async () => {
    ...
    // a subagent resume-after-crash could take — resolveClaudeBindingOutcome (the read
    ...
    //
    // Mirrors resolveClaudeBindingOutcome's own shape: filter() plus a length !== 1
    ...
    for (const candidate of launchedForIdentity) {
      if (expired(candidate.record.launch_expires_at ?? candidate.record.expires_at)) {
        const orphanedAt = iso(timestampAtLeast(Date.now(), candidate.record.launched_at));
    ...
    const boundAt = iso(timestampAtLeast(Date.now(), record.launched_at));
...
export async function bootstrapCodexSubagent(paths, state, input) {
  ...
  return withDispatchLock(paths, async () => {
    ...
    const now = iso(timestampAtLeast(Date.now(), record.launched_at, record.bound_at));
...
// Reports WHICH class of ground denied the binding, alongside resolveClaudeBindingOutcome's
// own record-or-null outcome (F1, plan review: ONE admission predicate, never two
// independently maintained copies of the same filter — resolveClaudeBindingOutcome below
...
async function resolveDispatchBindingOutcome(paths, state, input, host) {
  ...
  const intents = await readIntents(paths);
  // Exactly resolveClaudeBindingOutcome's original filter, unwidened.
...
// resolveClaudeBindingOutcome can no longer name the orphan; this resolver exists
...
export async function validateClaudeReceiptBinding(
  ...
  // (see resolveClaudeBindingOutcome); a receipt's self-reported identity carries no
...
export async function completeClaudeReceiptBinding(paths, ticket, binding, inputHash, receipt) {
  ...
  await withDispatchLock(paths, async () => {
    ...
    const completedAt = iso(timestampAtLeast(Date.now(), current.bound_at));
...
function validationRecovery(attempts, exhausted, exhaustionCount = 0, validationState = null, intent = {}) {
  const limits = receiptLimits(intent);
  const permanentlyBlocked = exhausted && (intent.physical_worker_dispatches ?? exhaustionCount)
  ...
  return {
    attempt: attempts,
    max_attempts: limits.max_validation_submissions_per_worker,
    invalid_attempts: invalidAttempts,
    first_validation_valid: firstValidationValid,
    corrections_remaining: Math.max(0, limits.max_validation_submissions_per_worker - attempts),
    exhausted,
...
function resolveReceiptContractBinding(current, request) {
  ...
  if (
    ...
    (request?.receipt_limits !== undefined &&
      JSON.stringify(request.receipt_limits) !== JSON.stringify(receiptLimits(current)))
  ) return null;
  return {
...
async function validateAndAttestCurrent(file, current, request) {
  const maximum = receiptLimits(current).max_validation_submissions_per_worker;
  if (typeof request?.input_hash !== 'string' || !/^[0-9a-f]{64}$/i.test(request.input_hash)) {
    ...
    if (prior?.exhausted === true || attempts >= maximum) {
      return {
        observed: true,
        validation_performed: false,
        idempotent: false,
    ...
    const exhausted = result.valid !== true && nextAttempts >= maximum;
    ...
    return {
      ...
      validation: validationRecovery(nextAttempts, exhausted, exhaustionCount, receiptValidation, current),
      result,
    };
}
