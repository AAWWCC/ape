...
// already contains the same immutable ticket, its dispatch reservation and
// worker-budget charge crossed the state persistence boundary together. A
// later NEXT must therefore re-emit that exact physical-worker opportunity
// without charging it again. Keep this read-only classifier next to the intent
...
export async function isPreparedUnlaunchedDispatchReplay(paths, state, ticket) {
  ...
  const current = await readJson(intentPath(paths, ticket.ticket_id), null);
  return current?.status === 'prepared' &&
    (current.launch_attempts ?? 0) === 0 &&
    current.run_id === state.run_id &&
    current.ticket_id === ticket.ticket_id &&
    current.ticket_hash === ticket.ticket_hash &&
    intentMatchesHost(current, state.host);
}
...
// observed first-worker stop caused by either validation exhaustion or an
// active-budget interruption. Its proof is split across durable run state and
...
async function prepareDispatchIntent(paths, ticket, agentType, host, options = {}) {
  ...
  await withDispatchLock(paths, async () => {
    ...
    const reprepareUnlaunched =
      current?.status === 'prepared' && (current.launch_attempts ?? 0) === 0;
...
function validationRecovery(attempts, exhausted, exhaustionCount = 0) {
 ...
 * @param {{ input_hash: string, receipt_capability: string, continuation_budget?: { allowed: boolean, code?: string }, validate: () => { valid: boolean, corrections: object[], budgets: object } }} request
 */
async function validateAndAttestCurrent(file, current, request) {
  if (typeof request?.input_hash !== 'string' || !/^[0-9a-f]{64}$/i.test(request.input_hash)) {
    ...
    if (
      ...
      // work, so no continuation-budget checkpoint is needed on this replay.
      return {
        observed: true,
    ...
    const continuationBlocked =
      result.valid !== true && !exhausted && request?.continuation_budget?.allowed === false
        ? {
            code: request.continuation_budget.code ?? 'active-time-exhausted',
            observed_at: validatedAt,
          }
        : null;
    const receiptValidation = {
      ...
      last_result: result,
      ...(continuationBlocked ? { continuation_blocked: continuationBlocked } : {}),
      ...(result.valid === true
        ? {
    ...
    return {
      ...
      validation: validationRecovery(nextAttempts, exhausted, exhaustionCount),
      result,
    };
}
...
export async function readDispatchReceiptAttestation(
  ...
  const current = await readJson(intentPath(paths, ticketId), null);
