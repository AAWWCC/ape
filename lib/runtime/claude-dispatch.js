...
import { encodeGeminiProjectDir } from './gemini-host.js';
...
function agentTypeInputValid(agentType, host) {
  return (host === 'codex' || host === 'gemini') && agentType == null
    ? true
...
function intentAgentTypeMatches(record, agentType, host) {
  if (host === 'gemini') return agentType == null || record.agent_type === agentType;
...
// alias family ('haiku' | 'sonnet' | 'opus' | 'fable') or Gemini alias
// ('flash' | 'pro' | 'flash_lite'); the Agent / subagent tool accepts either
// that alias verbatim or a fully qualified model id of the same family.
const CLAUDE_MODEL_FAMILIES = Object.freeze(['haiku', 'sonnet', 'opus', 'fable']);
const GEMINI_MODEL_FAMILIES = Object.freeze(['flash', 'pro', 'flash_lite']);
...
function requestedModelSatisfiesTicket(requested, ticketModel) {
  ...
  if (GEMINI_MODEL_FAMILIES.includes(ticketModel)) {
    return new RegExp(`^gemini-[\\w.-]*${ticketModel}[\\w.-]*$`, 'i').test(requested) || requested.includes(ticketModel);
  }
  return false;
}
...
export async function dispatchIntentStatuses(paths, state) {
  if (!state || !['claude', 'codex', 'gemini'].includes(state.host)) return [];
...
async function observeDispatchSubagentStop(paths, state, input, host) {
  ...
  return withDispatchLock(paths, async () => {
    ...
    if (record.agent_stopped_at) return { observed: true, record, reason: 'stop already observed' };
...
export async function observeGeminiSubagentStop(paths, state, input) {
  return observeDispatchSubagentStop(paths, state, input, 'gemini');
}
...
async function prepareDispatchIntent(paths, ticket, agentType, host) {
  ...
  const nonce = (host === 'claude' || host === 'gemini') ? randomBytes(32).toString('base64url') : null;
  const agentName = (host === 'codex' || host === 'gemini')
  ...
  const prompt = (host === 'claude' || host === 'gemini')
    ? [
        `Execute the immutable APE StageTicket ${ticket.ticket_id}.`,
        ...(host === 'gemini'
          ? [`APE_PROJECT_DIR_B64=${encodeGeminiProjectDir(paths.root)}`]
          : []),
  ...
  return host === 'claude'
    ...
    : host === 'gemini'
      ? { nonce, agent_name: agentName, expires_at: record.expires_at, prompt }
...
export async function prepareGeminiIntent(paths, ticket, agentType) {
  return prepareDispatchIntent(paths, ticket, agentType, 'gemini');
}
...
async function launchDispatchIntent(paths, state, input, host) {
  ...
  const nonce = (host === 'claude' || host === 'gemini') ? extractNonce(rawPrompt) : null;
  ...
  return withDispatchLock(paths, async () => {
    ...
    const subagentModel = Array.isArray(toolInput.Subagents) ? toolInput.Subagents[0]?.Model : null;
    const requestedModel =
      typeof toolInput.model === 'string' && boundedIdentity(toolInput.model, 256)
        ? toolInput.model
        : typeof subagentModel === 'string' && boundedIdentity(subagentModel, 256)
          ? subagentModel
          : null;
...
export async function launchGeminiIntent(paths, state, input) {
  return launchDispatchIntent(paths, state, input, 'gemini');
}
...
async function bindDispatchSubagent(paths, state, input, host) {
  ...
  return withDispatchLock(paths, async () => {
    ...
    if (resumeMatches.length === 1) {
      ...
      // re-admission gets the same treatment, leaving status, capability_hash
      // and bound_agent_id untouched, so a bound intent can be resumed an
      // unbounded number of times and each is still individually attributable.
      await atomicWriteJson(file, {
        ...record,
        resumed_at: iso(),
...
export async function bindGeminiSubagent(paths, state, input) {
  return bindDispatchSubagent(paths, state, input, 'gemini');
}

// Antigravity exposes no SubagentStart hook. Its supported PreInvocation hook
// does expose the child conversation and transcript, whose first user message
// contains the one-time dispatch nonce. Bind that child before its first model
// invocation and inject the receipt capability as an ephemeral system step.
export async function bindGeminiInvocation(paths, state, input) {
  const conversationId = input.conversationId ?? input.session_id ?? input.sessionId;
  const nonce = input.gemini_dispatch_nonce;
  if (!boundedIdentity(conversationId) || !NONCE_PATTERN.test(nonce ?? '')) {
    return { matched: false, valid: false, reason: 'APE Gemini invocation carries no dispatch identity' };
  }
  return withDispatchLock(paths, async () => {
    const matches = (await readIntents(paths)).filter(({ record }) =>
      record.run_id === state?.run_id &&
      intentMatchesHost(record, 'gemini') &&
      record.status === 'launched' &&
      record.parent_session_id !== conversationId &&
      record.nonce_hash === digest(nonce));
    if (matches.length === 0) {
      return { matched: false, valid: false, reason: 'APE Gemini invocation is not a launched child' };
    }
    if (matches.length !== 1) {
      return { matched: true, valid: false, reason: 'APE Gemini binding denied: dispatch identity is ambiguous' };
    }
    const { file, record } = matches[0];
    const ticket = state?.tickets?.find((candidate) => candidate.ticket_id === record.ticket_id);
    if (!pendingTicket(state, record.ticket_id)) {
      return { matched: true, valid: false, reason: 'APE Gemini binding denied: ticket is not active and pending' };
    }
    if (expired(record.launch_expires_at ?? record.expires_at) || expired(ticket?.deadline_at)) {
      await atomicWriteJson(file, { ...record, status: 'expired', expired_at: iso() });
      return { matched: true, valid: false, reason: 'APE Gemini binding denied: ticket deadline elapsed' };
    }
    const collision = (await readIntents(paths)).some(({ record: other }) =>
      other.run_id === state.run_id &&
      intentMatchesHost(other, 'gemini') &&
      other.status === 'bound' &&
      other.bound_agent_id === conversationId &&
      other.ticket_id !== record.ticket_id);
    if (collision) {
      return { matched: true, valid: false, reason: 'APE Gemini binding denied: conversation is already bound' };
    }
    const capability = randomBytes(32).toString('base64url');
    await atomicWriteJson(file, {
      ...record,
      status: 'bound',
      bound_session_id: conversationId,
      bound_agent_id: conversationId,
      capability_hash: digest(capability),
      bound_at: iso(),
    });
    return {
      matched: true,
      valid: true,
      reason: `APE Gemini native conversation bound to ${record.ticket_id}`,
      ticket_id: record.ticket_id,
      additional_context: [
        `APE_BOUND_CAPABILITY=${capability}`,
        `APE_RECEIPT_CAPABILITY=${capability}`,
...
export async function resolveGeminiBindingOutcome(paths, state, input) {
  return resolveDispatchBindingOutcome(paths, state, input, 'gemini');
}
...
export async function resolveGeminiBinding(paths, state, input) {
  return (await resolveGeminiBindingOutcome(paths, state, input)).record;
}
...
export async function resolveSealedGeminiBinding(paths, state, input) {
  return resolveSealedDispatchBinding(paths, state, input, 'gemini');
}
