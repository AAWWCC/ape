...
import { atomicReplaceText, atomicWriteJson, ensureDir, readJson } from './storage.js';
...
async function dispatchLaunchKey(paths) {
  ...
  try {
    ...
    const handle = await open(paths.dispatchLaunchKey, 'r');
    let encoded;
    try {
      ...
      if (!privateLaunchKeyFile(after) || !sameFileIdentity(after, opened)) {
...
function derivedLaunchCapability(key, host, ticket, seed) {
  ...
  return host === 'codex'
    ? `ape_${ticket.role}_${hmac.digest('hex').slice(0, 32)}`
...
async function prepareDispatchIntent(paths, ticket, agentType, host, options = {}) {
  ...
  await withDispatchLock(paths, async () => {
    ...
    const launchCapability = derivedLaunchCapability(launchKey, host, ticket, launchSeed);
    const baseRecord = {
      version: 2,
      host,
  ...
  const preparedLaunchCapability = durablePreparedRecord.prepared_launch_capability ??
    derivedLaunchCapability(durableLaunchKey, host, ticket, durablePreparedRecord.launch_seed);
  ...
  return withDispatchLock(paths, async () => {
    ...
    if (resumeMatches.length === 1) {
      ...
      try {
        ...
        return { valid: false, reason: `APE ${label} binding denied: authoritative context unavailable` };
      }
      if (host === 'codex') {
    ...
    await atomicWriteJson(file, { ...bound, status: 'bound' });
...
export async function completeClaudeReceiptBinding(paths, ticket, binding, inputHash, receipt) {
  ...
  await withDispatchLock(paths, async () => {
    ...
    await atomicWriteJson(binding.file, { ...completed, status: 'completed' });
  });
}
