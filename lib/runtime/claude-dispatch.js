...
// Claude's Agent lifecycle carries agent_type on every identity-bearing event.
// Codex Multi-Agent V2 does not: collaboration.spawn_agent has no agent_type
// input and the corresponding SubagentStart/tool/stop events may omit it too.
// For Codex the randomized task name has already selected one unique launched
// intent before this boundary, so session_id + agent_id are the host-attested
// identity and the persisted intent remains the authority for the expected
// role. An explicitly supplied type is still validated exactly; only omission
// is tolerated. This keeps Claude's stricter native contract unchanged.
...
function intentAgentTypeMatches(record, agentType, host) {
  return host === 'codex' && agentType == null ? true : record.agent_type === agentType;
}

// Documented minimal model equivalence (F11): a ticket resolves a Claude model
// alias family ('haiku' | 'sonnet' | 'opus' | 'fable'); the Agent tool accepts
// either that alias verbatim or a fully qualified model id of the same family
// (e.g. 'claude-fable-5' satisfies 'fable', 'claude-sonnet-4-5' satisfies
// 'sonnet'). Anything else — an absent parameter, a different family, or a
// mismatched explicit id — violates the ticket's model policy and the launch
// is denied.
...
async function launchDispatchIntent(paths, state, input, host) {
  ...
  const nonce = host === 'claude' ? extractNonce(toolInput.prompt ?? toolInput.message) : null;
  ...
  // prepared intent's type only after that capability uniquely matches. Claude
  // still requires its native type, and an explicitly supplied Codex type must
  // remain bounded and match the intent exactly.
