...
async function persist(paths, state, tree) {
...
export async function startRun(projectDir, rawInput) {
  ...
  let branch = await currentBranch(paths.root);
  ...
  // Land computes its admission baseline from HEAD (above), before any branch
  // switch. From a leftover ape/* checkout the leftover-ape arm below would
  // re-base that validated diff onto the resolved base tip, so the run would
  // attest a baseline the gates never reviewed. Refuse it here, among the pure
  // pre-lock validations, so no lock is taken and no branch is created.
  if (input.mode === 'land' && branch.startsWith('ape/')) {
    throw new Error(`mode land cannot start from a leftover ape/* branch (${branch}): a fresh ape/* run re-bases onto the resolved base branch, so the validated working-tree diff would no longer match its attested baseline. Switch to the base branch (or the feature branch that holds the work) before landing`);
  }
  ...
  let emitted;
  try {
    // A leftover ape/* branch is a dead run's workspace — abort never switches
    // back (only post-merge cleanup does). Branching a new ape/* run off its
    // tip inherits the dead run's committed-but-unmerged work as this run's
    // attested base_commit_sha/base_tree_sha, so clean_tree, tree_binding, and
    // receipts all pass yet the eventual PR diff carries commits no receipt or
    // gate ever attributed. Branch off the RESOLVED base tip instead. main and
    // master keep their exact prior behavior (branch off implicit HEAD); a
    // user's own non-ape feature branch is still adopted verbatim (no switch).
    // The explicit start-point plus --no-track is required: `git switch -c
    // <new> main` resolves to LOCAL main and would silently defeat
    // resolveBaseBranch's remote-first ordering.
    if (branch === 'main' || branch === 'master') {
      branch = `ape/${input.mode}-${runId.slice(-8)}`;
      await runGit(paths.root, ['switch', '-c', branch]);
    } else if (branch.startsWith('ape/')) {
