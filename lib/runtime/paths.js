...
export function runtimeHost(env = process.env) {
  if (
    ...
    env.GEMINICODE === '1' ||
    env.GEMINI_CODE === '1' ||
    Boolean(env.GEMINI_PROJECT_DIR) ||
    Boolean(env.GEMINI_CONFIG_DIR) ||
...
// Codex uses only its stable CODEX_CWD pin, and Gemini uses GEMINI_PROJECT_DIR.
...
export function resolveGovernedRoot({
  ...
  const selectedHost = host === 'claude' || host === 'codex' || host === 'gemini' ? host : runtimeHost(env);
  const stablePin = selectedHost === 'claude'
    ? nonEmpty(env.CLAUDE_PROJECT_DIR)
    : selectedHost === 'gemini'
      ? nonEmpty(env.GEMINI_PROJECT_DIR)
