...
import { ancestorManifests, entryScripts, packageScript, packageShellInvocation, selectedPackageScript } from './admission-baseline.js';
...
export async function inspectAdmissionCommandPrerequisites(root, commands, executableFacts) {
  ...
  const inspectCommand = async (command, commandRoot, env, depth = 0, resolvedOuter = undefined, shellScript = false) => {
    ...
    if (shellScript) {
      const shell = packageShellInvocation(argv, env);
      argv = shell.argv;
      env = shell.env;
    ...
    const selected = selectedPackageScript(manifest, script);
