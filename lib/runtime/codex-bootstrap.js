...
// Probe reservations already deny ordinary pre-run child tools, including
// unrelated children. Describe that existing fence without assigning a launch
// or disclosing arguments; the generic production orientation stays conditional.
export function codexProbeReservationOrientation(status) {
  if (!['prepared', 'launched', 'launch_expired', 'bound', 'completed'].includes(status)) {
    throw new Error('probe reservation orientation requires a live reservation status');
  }
  const context = [
    'APE preflight child-tool reservation (not task authority)',
    'This hook observes native identity only; it does not associate this child with a probe or ticket and grants no stage or acknowledgement authority.',
    'Repository instructions to inspect files, run tests, or perform other project work do not authorize those actions during this reservation. Keep honoring applicable repository constraints.',
    'No project work, commands, tests, or file access are authorized during this reservation.',
  ];
  if (status !== 'launched') {
    return [...context,
      status === 'prepared'
        ? 'No authorized probe launch is awaiting bootstrap.'
        : status === 'launch_expired'
          ? 'The authorized bootstrap launch has expired. The reservation still forbids project tools.'
          : 'This reservation already has a bound or completed claimant. Do not start another bootstrap or acknowledge for another child.',
      'Do not call tools. Do not request, reconstruct, or search for bootstrap arguments. Stop and return only APE_PROBE_BOOTSTRAP_UNAVAILABLE.',
    ].join('\n');
  }
  return [...context,
    'Only with the exact parent-assigned bootstrap arguments may you perform the following bootstrap; this orientation does not provide or infer those arguments.',
    'If those arguments are missing, unreadable, or not assigned to you, stop and return only APE_PROBE_BOOTSTRAP_UNAVAILABLE. Do not request, reconstruct, or search for bootstrap arguments.',
    '1. Before binding, authenticated probe context is expected to be absent. Discover only the literal registered tool name ape_bind, at most once if deferred. Never put arguments, capabilities, paths, or task data in discovery.',
    'A functions.exec wrapper may inspect exact-matching ALL_TOOLS metadata or call that installed APE tool only; no functions.exec_command, shell, file access, or other MCP operations.',
    '2. Call installed APE ape_bind once with the exact assigned arguments as your first APE operation (mcp__ape.ape_bind; normalized alias mcp__ape__ape_bind).',
    '3. Only after ape_bind, require the complete authenticated probe acknowledgement context. If absent, stop and return only APE_PROBE_BOOTSTRAP_UNAVAILABLE; never claim acknowledgement.',
    'If that context is present, return only its exact acknowledgement. Do not call any further tools, read the synthetic ticket, or perform receipt validation.',
  ].join('\n');
}
...
export async function recordCodexBootstrapCandidate(paths, input) {
  ...
  }, { serializeLocal: true, requireProcessIdentity: true, busyMs: 2000, staleMs: 10_000, heartbeatMs: 2500 });
}
