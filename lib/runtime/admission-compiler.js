...
function plannerTemplate({ input, config, classification, planning_commands }) {
  ...
  const command = distinct(planning_commands)
    .filter((entry) => typeof entry === 'string' && entry.trim() && entry.length <= PLAN_CONTRACT_LIMITS.command_chars)
  ...
  if (templateBytes > PLAN_CONTRACT_LIMITS.candidate_plan_utf8_bytes) {
    // Descriptive placeholders and all-to-all requirement references are not
    // a lower bound on a complete plan. Keep scope intact and let the planner
    // choose its own schema-valid representation instead of rejecting it.
    return {
      blocking,
      planner: {
        ...base,
        ...
        advisory: {
          code: 'planner-template-exceeds-byte-budget',
          provided: templateBytes,
          limit: PLAN_CONTRACT_LIMITS.candidate_plan_utf8_bytes,
          reason: 'The illustrative template does not fit; a smaller complete plan may fit the same authorized scope.',
        },
      },
    };
  } else {
