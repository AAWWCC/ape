...
import { compileRunAdmissionContract } from './admission-compiler.js';
import { commandProfileAvailableToRole, requiresTestRunnerAdmission, scopedEvidenceCommands } from './capability-selection.js';
...
// Advisory workload heuristics, not measured model-capacity ceilings. Actual
// artifact schemas, command authority, and transport envelopes still admit or
// reject the request independently of this recommendation.
...
function complexityAdmission(input, classification) {
  ...
  // A large, otherwise simple test fanout is governed by the stricter exact
  // test-path item/byte contract. Preserve the general 48-point decomposition
  // recommendation while allowing that independent contract to reach its documented
  // 64-item boundary.
  const boundedTestFanout =
    testPaths.length > COMPLEXITY_SCORE_THRESHOLD &&
    testPaths.length <= capabilityDynamicTestPathBounds().max_items &&
    productionClaims.length <= 1 &&
    requirements.length === 0 &&
    riskTriggers.length === 0 &&
    verificationProfileIds.length === 0;
  return {
    advisory: true,
    admitted: true,
    decomposition_recommended:
      !(totalScore <= COMPLEXITY_SCORE_THRESHOLD || boundedTestFanout) ||
...
function capabilitySnapshot(
  ...
  return {
    ...
    // Capability recovery can request a different role after configuration
    // has changed. Persist the complete derivation inputs at START so the
    // successor never consults mutable config or today's role table.
    frozen_recovery_authority: {
      models: structuredClone(config.models ?? {}),
      role_models: structuredClone(config.role_models ?? {}),
      deadlines_ms: structuredClone(config.deadlines_ms ?? {}),
      role_policies: structuredClone(ROLE_POLICIES),
      stages: structuredClone((projection?.stages ?? []).map((stage) => ({
        id: stage.id,
        role: stage.role,
      ...
      run_scope: {
        host: input.host,
        ...(input.binding_protocol
          ? { binding_protocol: input.binding_protocol }
          : {}),
        claimed_paths: structuredClone(input.claimed_paths ?? []),
        test_paths: structuredClone(input.test_paths ?? []),
        risk_triggers: structuredClone(classification?.risk_triggers ?? []),
        lane: classification?.lane,
        mode: input.mode,
        behavioral: input.behavioral ?? input.mode !== 'spike',
        design_assurance_required: config.policy?.design_assurance_required === true,
        ...(input.test_intent ? { test_intent: input.test_intent } : {}),
      },
    },
  };
}
...
function derivedCapabilityRequirements(input, classification, projection) {
  ...
  const behavioralLane =
    ...
    requiresTestRunnerAdmission(projection);
  return {
...
function addCommand(commands, derivationErrors, command, testPaths) {
  if (derivationErrors.length > 0) return;
  try {
  if (typeof command !== 'string' || command.trim().length === 0) return;
  if (!command.includes('{paths}')) {
    addCapabilityEvidenceCommand(commands, command);
    return;
  }
  if (!Array.isArray(testPaths) || testPaths.length === 0) return;
    const invocation = templateInvocation(command, testPaths);
    if (invocation) addCapabilityEvidenceCommand(commands, [invocation.command, ...invocation.args].join(' '));
  } catch (error) {
...
function prospectiveRoleCapabilityManifest(
  ...
  const scope = { ...input, test_paths };
  const commandProfiles = allCommandProfiles.filter((profile) =>
    commandProfileAvailableToRole(profile, role, scope));
  ...
  const planningCommandProfiles = PLANNING_ROLES.has(role)
    ? allCommandProfiles.filter((profile) =>
        Array.isArray(profile?.roles) && profile.roles.some((entry) =>
          reachableRoles.has(entry) && commandProfileAvailableToRole(profile, entry, scope)))
    : null;
  try {
    assertCapabilitySourceCollectionsFit({
      command_profiles: commandProfiles,
      planning_command_profiles: planningCommandProfiles,
      verification_profiles: config.verification?.profiles ?? [],
    });
  } catch (error) {
    return { capacity_error: error.message, derivationErrors: [], manifest: { plannable_evidence_commands: [] } };
  }
  ...
  return {
    ...
    manifest: {
      ...
      allowed_evidence_commands: scopedEvidenceCommands(commands, allCommandProfiles, [role], scope),
      ...
      ...(planningCommandProfiles
        ? {
            ...
            plannable_evidence_commands: scopedEvidenceCommands(plannableCommands, allCommandProfiles, [...reachableRoles], scope),
      ...
      field_bounds: runContractFieldBounds({ run: { policy: config.policy } }),
...
function capabilityManifestReadiness(input, config, classification, projection, configHash) {
  ...
  if (!initialPathBounds.valid) {
    ...
    if (initialPathBounds.usage.max_bytes !== null &&
        initialPathBounds.usage.used_bytes > initialPathBounds.usage.max_bytes) {
      blocking.push({
  ...
  // Version-two growth checks the concrete current inputs here and every
  // actual proposed mutation before persistence. A hypothetical maximum-sized
  // path that no worker has authored is not evidence of impossible admission.
  for (const role of roles) {
    for (const scenario of scenarios) {
      ...
      if (prospective.capacity_error) {
        blocking.push({ code: 'capability-manifest-resource-over-limit', role,
          source: scenario.source, message: prospective.capacity_error });
        continue;
      }
        ...
        blocking.push({
          ...
          ...(evidenceCommandsField
            ? {
                ...
                limit: CAPABILITY_MANIFEST_MAX_ALLOWED_EVIDENCE_COMMANDS,
                configured_script_count: (config.policy?.evidence_scripts ?? []).length,
                derivation: 'Exact command aliases, profiles, and runner commands share this derived catalog limit.',
              }
            : {}),
        });
      }
  ...
  return blocking.filter((entry) => {
    ...
    const { source: _source, ...stable } = entry;
 ...
 */
export function evaluateRunReadiness({ input, config, classification, projection, discovered = { targeted: false, full: false }, usable_verification_profile_ids = null }) {
  const blocking = [];
  const warnings = [];
  const availableCatalog = availableCapabilityCatalog(input, config);
  if (usable_verification_profile_ids !== null) {
    const usable = new Set(usable_verification_profile_ids);
    config = { ...config, verification: { ...config.verification,
      profiles: (config.verification?.profiles ?? []).filter((profile) => usable.has(profile.id)) } };
  }
  ...
  if (complexity.decomposition_recommended) {
    ...
    delete warnings.at(-1).complexity.admitted;
  }
  ...
  // Reachable workers need representable contracts whether or not the caller
  // explicitly requested optional capabilities. Empty requirements are not an
  // opt-out from the immutable ticket consumer's field and byte ceilings.
  const manifestBlocking = capabilityManifestReadiness(
      input,
      config,
      classification,
      projection,
  ...
  const planningManifest = prospectiveRoleCapabilityManifest(
    input, config, classification, 'planner', availableCatalog.config_hash,
    { reachable_roles: derivedRequirements.stage_roles },
  ).manifest;
  const compiledContract = compileRunAdmissionContract({
    input, config, classification, projection,
    planning_commands: planningManifest.plannable_evidence_commands ?? [],
  });
  const admissionContract = {
    ...compiledContract,
    valid: compiledContract.valid && manifestBlocking.length === 0,
    blocking: [...manifestBlocking, ...compiledContract.blocking],
    capability_manifest_checks: {
      roles: derivedRequirements.stage_roles,
      concrete_initial_and_mutation_checks: true,
      dynamic_test_path_bounds: derivedRequirements.dynamic_test_paths,
    },
  };
  blocking.push(...admissionContract.blocking);
  ...
  for (const requirement of input.required_capabilities ?? []) {
    if (requirement.kind === 'command_profile') {
      ...
      if (!verificationProfiles.some((entry) => entry.id === requirement.id)) {
        blocking.push({ code: availableCatalog.verification_profiles.some((entry) => entry.id === requirement.id)
          ? 'verification-profile-root-unavailable' : 'missing-verification-profile', capability: requirement });
      }
