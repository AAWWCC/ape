...
function complexityAdmission(input, classification) {
  ...
  // policy while allowing that independent contract to reach its documented
  // 64-item boundary.
...
function capabilityManifestReadiness(input, config, classification, projection, configHash) {
  ...
  const sourceCollectionsWithinBounds = blocking.length === 0;
  ...
  // An oversized source catalog cannot produce a ticket. Keep the initial
  // and future-field diagnostics, but do not multiply invalid manifests by
  // every possible runner root. Valid catalogs still get full dynamic sizing.
  if (sourceCollectionsWithinBounds && roles.includes('test_writer')) {
...
export function evaluateRunReadiness({ input, config, classification, projection, discovered = { targeted: false, full: false }, usable_verification_profile_ids = null }) {
  ...
  const admissionContract = {
    ...
    capability_manifest_checks: {
      ...
      initial_and_future_monotone_fields: true,
