...
export async function doctor(projectDir, context = {}) {
  ...
  if (parsedRun.success) {
    try {
      ...
      const enforced = input.execution_budget_required === true || input.required_capabilities.length > 0;
      checks.push({
        name: 'run-readiness',
        ...
        detail: runReadiness.ready
          ? 'declared run capabilities, test runners, and execution budget are ready'
