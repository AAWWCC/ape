...
import { detectAmbiguousConfigOverrides, loadRuntimeConfig, readStoredRuntimeConfig } from './config.js';
import { classifyLane } from './lane-policy.js';
import { pipelineRunSpec, projectedPipeline } from './pipeline.js';
import { RunStartInputSchema } from './schemas.js';
import { discoverRunReadiness, evaluateRunReadiness, uniqueReadinessCodes } from './readiness.js';
import { inspectRunLock } from './lock.js';
import { readBoundedRegularFileUtf8 } from './spawn.js';
import { activeState } from './active-state.js';
import { readBoundedFile, readBoundedFileSync } from './bounded-file.js';

// Diagnostics may inspect symlinked plugin deployments, but the resolved
// target must be a bounded regular file. Resolve first, then use the same
// nonblocking descriptor reader that protects runtime metadata from races.
const DIAGNOSTIC_BUNDLE_MAX_BYTES = 16 * 1024 * 1024;
async function diagnosticBundleBytes(file) {
  return readBoundedFile(await realpath(file), DIAGNOSTIC_BUNDLE_MAX_BYTES);
}
...
function canaryGroupsFor(events, eventName) {
  return Array.isArray(events?.[eventName])
    ? events[eventName].filter((group) =>
        Array.isArray(group?.hooks) && group.hooks.some((hook) =>
          typeof hook?.command === 'string' &&
...
export function validateCodexHookWiring(manifest) {
  ...
  const canary = canaryGroupsFor(events, 'PreToolUse');
  const missing = [];
  ...
  for (const toolName of ['ape_bind', 'mcp__ape__ape_bind', 'mcp__plugin_ape_ape__ape_bind']) {
    if (!pre.some((group) => matcherCovers(group.matcher, toolName))) {
      missing.push(`PreToolUse ${toolName} matcher`);
    }
    if (!post.some((group) => matcherCovers(group.matcher, toolName))) {
      missing.push(`PostToolUse ${toolName} matcher`);
    }
  }
  if (start.length === 0) missing.push('SubagentStart binding hook');
  if (!canary.some((group) => group.matcher === '*')) {
    missing.push('PreToolUse exact-canary wildcard hook');
  }
  return missing.length === 0
    ? {
        passed: true,
        detail: 'shipped Codex hook manifest separately covers legacy spawn_agent, Multi-Agent V2 collaborationspawn_agent, exact ape_bind aliases on pre/post hooks, SubagentStart, and the exact-canary wildcard fence; operational delivery must still pass the mandatory live binding preflight',
      }
    : {
        passed: false,
...
async function inspectCodexHookWiring(moduleUrl = import.meta.url) {
  for (const candidate of codexHookManifestCandidates(moduleUrl)) {
    try {
      const manifest = JSON.parse((await readBoundedFile(await realpath(candidate), 1024 * 1024)).toString('utf8'));
...
function captureLoadedBundleStamp(candidates) {
  for (const candidate of candidates) {
    try {
      ...
      // this statSync and this bounded read can at worst make the HASH
      ...
      const bytes = readBoundedFileSync(resolved, DIAGNOSTIC_BUNDLE_MAX_BYTES);
      return {
        url: candidate,
...
// loaded? Package builds pin mtime to SOURCE_DATE_EPOCH, so size + mtime alone
// cannot identify an unchanged bundle. Include file identity and change time
// before skipping the hash. A changed hint still compares bytes, so an
// idempotent rebuild stays quiet even when publication replaces the inode.
async function loadedBundleDrift(stamp) {
  ...
  if (!info?.isFile() || info.size > DIAGNOSTIC_BUNDLE_MAX_BYTES) return null;
  if (info.size === stamp.size && info.mtimeMs === stamp.mtime_ms &&
      info.ctimeMs === stamp.ctime_ms && info.dev === stamp.dev && info.ino === stamp.ino) return null;
  const bytes = await diagnosticBundleBytes(stamp.url).catch(() => null);
...
export async function doctor(projectDir, context = {}) {
  ...
  if (context.behavioral !== false && !['debug', 'spike', 'land'].includes(context.mode)) {
    precondition(
      'test-path-claims',
  ...
  let runtimeAncestorUnsafe = false;
  try {
    let configured;
    try {
      configured = await validateGovernedRuntimeAncestor(paths);
    } catch (error) {
      runtimeAncestorUnsafe = true;
      throw error;
    }
    if (!configured) add('state-dir', true, 'no runtime state directory yet (created on first run)');
    else {
      ...
      try {
        await activeState(paths);
        add('state-dir', true, 'runtime state directory is usable');
      } catch (error) {
        // Schema diagnosis stays advisory; unsafe, oversized and unparseable
        // artifacts retain the unreadable-state failure below. The shared
        // reader also validates ticket/receipt collections and file identity.
        if (error?.code !== 'APE_CORRUPT_ACTIVE_STATE' || error.variant !== 'schema-invalid') throw error;
        checks.push({
          name: 'state-dir',
          passed: null,
          informational: true,
          warning: true,
          detail:
            'runtime state directory holds a schema-invalid active.json (valid JSON but an invalid run state or ticket/receipt collection); recover with ape_run override reset plus an audit reason before starting',
        });
      }
    }
  } catch (error) {
  ...
  try {
    const storedConfig = await readStoredRuntimeConfig(paths.config);
  ...
  if (blenderProject) {
    checks.push({
      ...
      passed: true,
      informational: true,
      detail: 'Blender project detected from a root .blend file',
    });
  }
  const packageJson = await readBoundedRegularFileUtf8(join(paths.root, 'package.json')).catch(() => '');
  ...
  if (playwrightProject) {
    checks.push({
      ...
      passed: true,
      informational: true,
      detail: 'Playwright project detected from package.json or a playwright.config file',
    });
  }
  try {
    if (runtimeAncestorUnsafe) throw new Error('runtime ancestor is unsafe; lock ownership cannot be inspected');
    ...
    if (!lock.present) add('lock-health', true, 'no active-run lock held');
      ...
      // (activeState's null fallback) keeps the orphan classification below.
      let active = null;
      let activeCorrupt = false;
      try {
        active = await activeState(paths);
      } catch {
        activeCorrupt = true;
      }
      if (activeCorrupt) {
        add(
          'lock-health',
          false,
          `active-run lock held by ${lock.run_id ?? 'unknown'} beside a corrupt active.json (state exists but cannot be safely read or validated); ` +
  ...
  if (executingBundle) {
    try {
      ...
      for (const name of trackedNames) {
        const projectBytes = await diagnosticBundleBytes(join(projectDir, 'dist', name)).catch(() => null);
        const executingBytes = await diagnosticBundleBytes(new URL(name, executingDist)).catch(() => null);
  ...
  const parsedRun = RunStartInputSchema.safeParse(context);
  if (parsedRun.success) {
    try {
      const input = parsedRun.data;
      const config = await loadRuntimeConfig(runtimePaths(projectDir).config);
      ...
      runReadiness = evaluateRunReadiness(readinessInput);
      if (runReadiness.blocking.some(
        (entry) => entry.code === 'missing-targeted-test-runner' || entry.code === 'missing-full-test-runner',
      )) {
        runReadiness = evaluateRunReadiness({
          ...readinessInput,
          discovered: await discoverRunReadiness(projectDir),
        });
      }
      const enforced = input.capability_contract_required === true || input.required_capabilities.length > 0;
      checks.push({
        name: 'run-readiness',
        passed: enforced ? runReadiness.ready : null,
        informational: !enforced,
        ...(runReadiness.ready ? {} : { warning: true }),
        detail: runReadiness.ready
          ? 'declared run capabilities and test runners are ready'
          : `run readiness is blocked by: ${uniqueReadinessCodes(runReadiness).join(', ')}`,
      });
    } catch (error) {
      checks.push({
        name: 'run-readiness',
        passed: false,
        detail: `run readiness could not be evaluated: ${error.message}`,
      });
    }
  }
  return {
    healthy: checks.every((check) => check.informational === true || check.passed === true),
    checks,
