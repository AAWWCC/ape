...
// Static proof of the shipped Codex hook seam. This cannot attest that a host
// actually fired a hook in a particular session (hooks_trusted remains the
// operator/host fact), but it prevents doctor from blessing a package whose
// own matcher cannot observe the native tool the adapter tells Codex to call.
export function validateCodexHookWiring(manifest) {
  ...
  return missing.length === 0
    ? {
        passed: true,
        detail: 'shipped Codex hooks observe legacy spawn_agent and Multi-Agent V2 collaborationspawn_agent before/after launch, then bind SubagentStart receipt capabilities',
      }
    : {
        passed: false,
