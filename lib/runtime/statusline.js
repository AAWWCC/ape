...
const SUPPORTED_HOSTS = new Set(['claude', 'codex', 'gemini']);
...
const GEMINI_RENDERER_LIMITATION =
  'Gemini / Antigravity displays agent and run progress natively in the session interface; external status-line injection is not required.';

function assertHost(host) {
  if (!SUPPORTED_HOSTS.has(host)) {
    throw new Error(`statusline wiring supports hosts 'claude', 'codex', and 'gemini'; got '${host}'`);
  }
}
...
export async function statuslineState({ host = 'claude' } = {}) {
  ...
  if (host === 'gemini') return geminiStatuslineState();
...
async function geminiStatuslineState() {
  return {
    wired: false,
    mode: 'native',
    renderer: 'gemini-native',
    custom_renderer: false,
    limitation: GEMINI_RENDERER_LIMITATION,
  };
}
