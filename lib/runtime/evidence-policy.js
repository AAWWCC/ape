...
function decodeEvidenceToken(rawToken, { allowStaticQuotedOperand = false } = {}) {
  if (
    ...
    if (EVIDENCE_TOKEN_CHAR_REFUSED.test(rawToken) || NON_ASCII_HAZARD.test(rawToken)) return null;
  ...
  if (allowStaticQuotedOperand && isCompleteStaticQuote) {
    ...
    if (
      !EVIDENCE_TOKEN_CHAR_REFUSED.test(token) &&
      !NON_ASCII_HAZARD.test(token)
    ) {
  ...
  const token = rawToken.slice(1, -1);
  if (
    EVIDENCE_BRACKET_PATH_CHAR_REFUSED.test(token) ||
    NON_ASCII_HAZARD.test(token)
  ) {
    return null;
  }
