...
//     token is in a POSITIVE ALPHABET, or the token is the one narrowly parsed
//     single-quoted dynamic-route path form. Rounds 1-5 all asserted a NEGATIVE over an adversarial
...
//     word splitting: the only quoted form cannot contain a space, and its
//     exact outer pair is removed to reproduce the argv token.
...
// three things need treatment outside the alphabet, and all are handled here:
...
// outer single quotes of a dynamic-route path token. Round 5's site 1 was
...
// quotes and metacharacters there. The sole quoted dynamic-route form is marked
// by decodeEvidenceToken and skips this unquoted-word expansion rule.
//
...
function decodeEvidenceToken(rawToken) {
