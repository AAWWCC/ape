...
//
// (1) Refuse the whole command outright unless EVERY character of every plain
//     token is in a POSITIVE ALPHABET, or the token is one of the narrowly
//     parsed quoted operand forms below. Rounds 1-5 all asserted a NEGATIVE over an adversarial
...
// (2) Split on plain spaces. After (1) that split PROVABLY equals the shell's
//     word splitting: neither quoted form can contain a space, and its exact
//     outer pair is removed to reproduce the argv token.
...
// four things need treatment outside the alphabet, and all are handled here:
...
// facing the SAME ordinary-token predicate as the remainder — plus the exact
// outer single quotes of a dynamic-route path token, plus the outer single or
// double quotes of a static `cat`/`ls` operand. Round 5's site 1 was
...
// the same string — at which point the remainder carries no ambiguous shell
// syntax and no whitespace but U+0020; the narrow quoted dynamic-route form is
// dequoted by that parser, so parsed.tokens remains token-for-token identical
// to argv — and because BOTH its rules are MONOTONE in the safe
