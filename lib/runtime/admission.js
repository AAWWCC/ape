...
import { requiredCommandProfileRoles, requiresTestRunnerAdmission } from './capability-selection.js';
import { MAX_ADMISSION_CHANGED_PATHS } from './resource-limits.js';
...
export async function inspectAdmissionRepository(root, _input) {
  const canonicalRoot = await realpath(root);
...
export async function evaluateAdmission({ root, input, config, classification, projection, readiness, health }) {
  ...
  const commandProfiles = [...(config.policy?.command_profiles ?? []), ...(input.run_command_profiles ?? [])]
    .filter((profile) => requiredCommandProfileRoles(profile, input, roles).length > 0);
  const outputPaths = sorted(commandProfiles.flatMap((profile) => profile.output_paths ?? []));
  for (const profile of commandProfiles) {
    for (const role of requiredCommandProfileRoles(profile, input, roles)) {
  ...
  const shippingReachable = (projection.runtime_stages ?? []).some((stage) => stage.action === 'ship_after_gates');
  const shipping = shippingReachable &&
    (config.shipping?.target != null || (config.shipping?.auto_merge === true && input.auto_merge_authorized === true))
  ...
  const testRunnerCommands = requiresTestRunnerAdmission(projection) ? [
  ...
  const requiredVerification = new Set((input.required_capabilities ?? [])
    .filter((entry) => entry.kind === 'verification_profile').map((entry) => entry.id));
  const commands = [
    ...testRunnerCommands,
    ...(config.verification?.profiles ?? []).filter((profile) => requiredVerification.has(profile.id))
      .map((profile) => ({ id: `verification:${profile.id}`, command: profile.command, root: profile.root ?? '.' })),
  ...
  const manifest = {
    ...
    verification_profile_roots: readiness.verification_profile_roots ?? [],
    unavailable_verification_profiles: readiness.unavailable_verification_profiles ?? [],
