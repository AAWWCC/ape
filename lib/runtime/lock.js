...
import { link, lstat, mkdir, open, readdir, rename, rm, stat, utimes, writeFile } from 'node:fs/promises';
import path from 'node:path';
import { ensureDir } from './storage.js';
import { readBoundedFile } from './bounded-file.js';

const LOCK_METADATA_MAX_BYTES = 64 * 1024;
async function readLockText(file, _encoding) {
  return (await readBoundedFile(file, LOCK_METADATA_MAX_BYTES)).toString('utf8');
}
...
// generation to dead/stale ones). Automatic release refuses read faults;
// only the audited operator reset (overrideRun in service.js) passes no
// `expected` and gets the plain, unverified rename.
//
...
export async function stealLockFileByRename(lockPath, expected) {
  ...
  if (expected !== undefined) {
    let renamed;
    try {
      renamed = await readLockText(tombstone, 'utf8');
    } catch {
      ...
      for (;;) {
        try {
          ...
          if (error?.code === 'EEXIST') {
            ...
            let occupant;
            try {
              occupant = await readLockText(lockPath, 'utf8');
            } catch {
...
async function releaseDirLock(lockPath, token, busyMs = 1_000) {
  ...
  for (;;) {
    try {
      const owner = await readLockText(path.join(lockPath, 'owner'), 'utf8');
      if (owner !== token) return;
      await rename(lockPath, tombstone);
      break;
    } catch (error) {
      // Windows readers/scanners can transiently deny a directory rename.
      // Treating every failure as a successful steal strands our retiring
      // generation after its heartbeat stops. Retry under a deadline, reading
      // ownership again each time so a successor is never deliberately moved.
      // Missing/unverifiable generations and permanent faults fail closed.
      if (!['EPERM', 'EACCES', 'EBUSY'].includes(error?.code)
        || Date.now() - releaseStarted >= busyMs) return;
      await sleep(10);
    }
  }
  let renamedOwner = null;
  try {
    renamedOwner = await readLockText(path.join(tombstone, 'owner'), 'utf8');
  } catch {
  ...
  await rm(tombstone, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 }).catch(() => {});
}
...
// refreshes with a heartbeat. Governed writers additionally require exact
// process identity and proven same-host death before stealing a stale owner;
// generic legacy callers may retain age-only recovery. Stealing uses a rename
// tombstone with post-rename verification; release verifies an owner token.
//
...
async function withDirLockFilesystem(lockPath, callback, options) {
  ...
  const requiresProcessIdentity = options.requireProcessIdentity === true ||
    path.basename(lockPath) === 'receipt-effects.lock';
  ...
  const barrier = async () => {
    ...
    let owner = null;
    try {
      owner = await readLockText(path.join(lockPath, 'owner'), 'utf8');
    } catch {
  ...
  for (;;) {
    ...
    if (Date.now() - metadata.mtimeMs > staleMs) {
      ...
      try {
        observedOwner = await readLockText(path.join(lockPath, 'owner'), 'utf8');
      } catch {
      ...
      try {
        observedProcessBytes = await readLockText(path.join(lockPath, 'process'), 'utf8');
      ...
      if (renamed) {
        ...
        if (stillStale) {
          if (requiresProcessIdentity && observedOwner !== null && observedProcessBytes !== null) {
            ...
            try {
              tombstoneOwner = await readLockText(path.join(tombstone, 'owner'), 'utf8');
              tombstoneProcess = await readLockText(path.join(tombstone, 'process'), 'utf8');
            } catch {
    ...
    let freshOwner = null;
    try {
      freshOwner = await readLockText(path.join(lockPath, 'owner'), 'utf8');
    } catch {
  ...
  const heartbeat = setInterval(() => {
    ...
    // Best-effort: opted-in stores additionally prove process death before
    // treating missed heartbeats as permission to steal the generation.
    (async () => {
...
export async function acquireRunLock(lockPath, runId, options = {}) {
  ...
  const sweepOwnDebris = async (keepBytes) => {
    ...
    for (const tombstone of await listTombstones()) {
      let bytes;
      try {
        bytes = await readLockText(tombstone, 'utf8');
      } catch {
        continue;
      }
  ...
  const barrier = async (payloadBytes) => {
    for (;;) {
      let current;
      try {
        current = await readLockText(lockPath, 'utf8');
      } catch {
  ...
  for (let attempts = 0; attempts < MAX_ACQUIRE_ATTEMPTS; attempts += 1) {
    ...
    let existing;
    try {
      raw = await readLockText(lockPath, 'utf8');
      ...
      // A read fault has no bytes to compare and cannot authorize a steal.
      if (recoverStale && typeof raw === 'string') {
...
export async function inspectRunLock(lockPath) {
  ...
  try {
    ...
    if (error?.code === 'APE_UNSAFE_FILE' || error?.code === 'ELOOP') return { present: true, readable: false };
    throw error;
  }
  let payload;
  try {
    payload = JSON.parse(raw);
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return { present: true, readable: false };
  } catch {
...
export async function releaseRunLock(lockPath, runId) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      ...
      let reinstalled;
      try {
        reinstalled = await readLockText(lockPath, 'utf8');
      } catch {
    ...
    // A failed/unsafe read supplies no owner bytes. Preserve the path for an
    // explicit audited reset rather than risk deleting another live owner.
    throw new Error('refusing to release an unreadable active-run lock; use override reset with an audit reason');
  }
  let existing;
  try {
    existing = JSON.parse(raw);
