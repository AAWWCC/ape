...
import { projectReceiptRejectionRecovery } from './recovery-guidance.js';
...
import { SESSION_GUIDANCE_MAX_BYTES } from './constants.js';
...
export const OUTPUT_SCHEMA_REFERENCE = Object.freeze({
  see: 'full output_schema: read .ape/runtime/tickets/<ticket_id with ":" replaced by "_">.json',
});

// The historical 48KB value limit also applies to escaped MCP text framing.
// Offline pinned-host reproduction showed that a complete stored inner result
// can be truncated when the host presents its larger escaped wrapper to the
// model. Measure both UTF-8 representations; this is not a token estimate or
// proof of every host's context limit. Existing lossless/reference fallbacks
// preserve authority instead of clipping JSON or native launch arguments.
...
function admittedManifestReference(state) {
  const admission = state.admission;
  // Only an already-admitted, hash-consistent native Codex record has a durable
  // run-file source. Preview remains complete and has no reference substitute.
  // Malformed/legacy records keep their existing representation; this is not
  // admission validation, repair, or a source of new execution authority.
  if (state.host !== 'codex' || state.binding_protocol !== 'native-v1'
      || typeof state.run_id !== 'string' || !/^run-[A-Za-z0-9_-]{1,128}$/.test(state.run_id)
      || !admission || typeof admission !== 'object' || Array.isArray(admission)
      || admission.version !== 1 || Object.keys(admission).length !== 3
      || typeof admission.digest !== 'string' || !/^[a-f0-9]{64}$/.test(admission.digest)
      || !admission.manifest || typeof admission.manifest !== 'object' || Array.isArray(admission.manifest)
      || admission.manifest.version !== 1 || admission.manifest.authorization !== 'reviewed-inputs-only'
      || admission.manifest.ready !== true) return admission;
  try {
    if (sha256(admission.manifest) !== admission.digest) return admission;
  } catch { return admission; }
  return {
    version: 1, digest: admission.digest, projection: 'bounded-reference-v1',
    manifest_ref: { path: `.ape/runtime/runs/${state.run_id}.json`, json_pointer: '/admission/manifest' },
  };
}

export function projectRunState(state) {
  // Apart from explicit metadata projections below, run-level fields pass
  // through as-is, and run.objective is the SINGLE
  ...
  // Ticket/receipt transformations apply only when they are arrays; minimal
  // fixture states without these metadata fields round-trip untouched.
  const projected = { ...state };
  if (state.admission !== undefined) projected.admission = admittedManifestReference(state);
...
function projectAction(action, runObjective) {
  if (action?.type === 'dispatch_agent') {
    // Bootstrap-v1 receipt-contract workers get complete authority from their
    // immutable ticket and authenticated bind context, not the parent's copy.
    // Reuse the established references even below the global byte limit while
    // retaining exact native launch arguments and ticket control fields. Legacy
    // and Claude under-budget dispatch tickets remain complete. The second copy
    // embedded in the dispatch metadata is
    ...
    return {
      ...action,
      ...(action.dispatch?.host === 'codex' && action.dispatch.bootstrap_protocol === 1
          && action.ticket?.receipt_contract_version === 1
        ? { ticket: compactPendingTicket(action.ticket, runObjective) }
        : {}),
      ...(action.dispatch
        ? {
            dispatch: {
...
function ticketReference(ticket) {
  ...
  return {
    ...
    ticket_hash: ticket.ticket_hash,
    ...([1, 2].includes(ticket.plan_contract_version) ? { plan_contract_version: ticket.plan_contract_version } : {}),
...
function minimalTicketReference(ticket) {
  ...
  if ([1, 2].includes(ticket.plan_contract_version)) minimal.plan_contract_version = ticket.plan_contract_version;
...
function runReference(state) {
  ...
  return {
    ...
    mode: state.mode,
    lane: state.lane,
    ...([1, 2].includes(state.plan_contract_version) ? { plan_contract_version: state.plan_contract_version } : {}),
  ...
  if (dispatch.host === 'codex') {
    ...
    return {
      ...
      ...(dispatch.ticket_projection !== undefined
        ...
        : {}),
      ...(dispatch.bootstrap_protocol === 1 ? { bootstrap_protocol: 1 } : {}),
      ...(model && typeof model === 'object' && !Array.isArray(model)
        ? {
            model: {
...
export function projectHistoryResponse(response) {
  ...
  const projected = dedupeWirePlans({
    ...response,
    ...(roadmapChanged ? { roadmap } : {}),
    records: response.records.map((record) =>
      record?.active === true ? record : summarizeHistoryRecord(record)),
  });
  if (!response.pagination || !Array.isArray(response.history_record_cursors)) {
    return hardBoundHistoryResponse(projected);
  }
  // A byte-limited page must advance only over delivered rows. Retaining the
  // service's original cursor after dropping rows would make those rows
  // permanently invisible to a caller following the advertised continuation.
  const cursors = [...response.history_record_cursors];
  const originalBytes = responseUtf8Bytes(projected);
  let shortened = false;
  while (overBudget(projected) && projected.records.length > 1) {
    if (projected.records.length === 2 && cursors[0] === null) {
      projected.records.shift();
      cursors.shift();
    } else {
      projected.records.pop();
      cursors.pop();
      shortened = true;
      projected.pagination = { ...response.pagination, returned: projected.records.length,
        has_more: true, next_cursor: cursors.at(-1) };
    }
  }
  if (overBudget(projected) && projected.records.length === 1) {
    const record = projected.records[0];
    projected.records = [{
      run_id: boundedWireText(record.run_id, 128),
      status: boundedWireText(record.status, 40),
      record_hash: boundedWireText(record.record_hash, 64),
      records_ref: '.ape/runtime/history/',
      projection: projectionMarker(originalBytes, '.ape/runtime/history/'),
    }];
  }
  projected.pagination = {
    ...response.pagination,
    returned: projected.records.length,
    ...(shortened ? { has_more: true, next_cursor: cursors.at(-1) } : {}),
  };
  return projected;
...
function overBudget(response) {
  try {
    ...
    if (Buffer.byteLength(text, 'utf8') > RESPONSE_BUDGET_BYTES) return true;
    // Include both optional result metadata fields conservatively, even when
    // a successful response omits isError. Pretty formatting matches the host
    // tool-result representation, not the compact JSON-RPC storage line.
    const framed = JSON.stringify({ resultType: 'complete', isError: true, content: [{ type: 'text', text }] }, null, 2);
    return Buffer.byteLength(framed, 'utf8') > RESPONSE_BUDGET_BYTES;
  } catch {
    return false;
  }
}
...
function deriveNextAction(response, continuationNextAction = null) {
  if (response.probe && typeof response.probe === 'object' &&
      !Array.isArray(response.probe) && response.probe.infrastructure_status === 'failed') {
    // A successful status read does not mean the native binding worked. Keep
    // this pre-run orchestration failure explicit without granting a retry or
    // consuming a product attempt, including through the reference fallback.
    return {
      kind: 'blocked',
  ...
  // Explicit lifecycle results and returned dispatches above take precedence:
  // in particular, waiting for physical-worker retirement stays a real wait.
  // Generic ticket/watch waits below must not hide retained work that requires
  // the parent to call NEXT before any progress can happen.
  if (continuationNextAction) return continuationNextAction;
...
function legacyContinuationNextAction(response) {
  ...
  if (run.status === 'input_required' && !paused) return null;
  ...
  return paused || continuation
    ? { kind: 'wait', state: 'continuation_pending', required_control_action: 'ape_run_next' }
    : null;
}
...
function hardBoundRunResponse(response) {
  ...
  if (response.advisory === true && response.blueprint && !response.run) {
    ...
    if (response.admission !== undefined || response.admission_digest !== undefined) {
      const complete = {
        ...preview,
        admission: response.admission,
        admission_digest: response.admission_digest,
      ...
      if (!overBudget(complete)) return complete;
      // The reviewed manifest outranks the redundant human-readable blueprint.
      const { blueprint: _blueprint, ...manifestOnly } = complete;
      if (!overBudget(manifestOnly)) return manifestOnly;
      return {
        ok: false, advisory: true, blocked: true, code: 'admission-response-too-large',
        attempts_consumed: 0,
        admission: { version: 1, ready: false },
        reason: 'The complete admission manifest exceeds the response budget; decompose the request and review a fresh complete preview before starting. No usable admission digest was issued.',
        projection: boundedInlineProjectionMarker(originalBytes),
      };
    }
    return preview;
  ...
  const roadmap = response.roadmap && typeof response.roadmap === 'object'
    ...
    : response.roadmap;
  const runtimeGuidance = typeof response.runtime_guidance === 'string' &&
    Buffer.byteLength(response.runtime_guidance, 'utf8') <= SESSION_GUIDANCE_MAX_BYTES
    ? response.runtime_guidance
    : null;
  const recovery = projectReceiptRejectionRecovery(response.recovery);
  const projected = {
    ok: response.ok,
    ...(response.active !== undefined ? { active: response.active } : {}),
    ...(response.blocked !== undefined ? { blocked: response.blocked } : {}),
    ...
    ...(response.reason ? { reason: boundedWireText(response.reason) } : {}),
    ...(runtimeGuidance ? { runtime_guidance: runtimeGuidance } : {}),
    ...(recovery ? { recovery } : {}),
...
export function projectRunResponse(response) {
  ...
  if (projected.recovery && Object.hasOwn(projected.recovery, 'cause')) {
    const recovery = projectReceiptRejectionRecovery(projected.recovery);
    if (recovery) projected.recovery = recovery;
    else delete projected.recovery;
  }
  ...
  if (Array.isArray(projected.actions)) {
    projected.actions = projected.actions.map((action) => projectAction(action, projected.run?.objective));
