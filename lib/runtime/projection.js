...
const NEXT_ACTION_KIND_SET = new Set(NEXT_ACTION_KINDS);
  ...
  if (ticket.receipt_contract_version === 1 && ticket.capability_manifest) {
    compact.capability_manifest = {
      ...
      ...(ticket.capability_manifest.run_contract
        ...
        : {}),
      see: 'full role-filtered capability_manifest: read the immutable ticket file referenced by ticket_id',
    };
  }
  return compact;
}
    ...
    ...(orchestration
      ? {
          orchestration: {
            ...
            budget_pauses: orchestration.budget_pauses,
...
export function projectRunState(state) {
  ...
  if (state.budget_continuation) {
    projected.budget_continuation = {
      version: state.budget_continuation.version,
      source: state.budget_continuation.source,
      queued_at: state.budget_continuation.queued_at,
      actions_hash: state.budget_continuation.actions_hash,
      config_hash: state.budget_continuation.config_hash,
      pending_action_count: Array.isArray(state.budget_continuation.actions)
        ? state.budget_continuation.actions.length
        : 0,
    };
  }
...
function boundedCapabilityCatalog(catalog) {
  ...
  return {
    ...
    declared_tool_claims: boundedCatalogNames(catalog.declared_tool_claims, (value) => value),
  };
}
...
function boundedPreviewBlueprint(blueprint) {
  ...
  return {
    ...
    readiness: {
      ...
      execution_budget: structuredClone(readiness.execution_budget ?? null),
    },
  };
}
...
function deriveNextAction(response) {
  ...
  if (dispatches.length > 0) {
    ...
    return {
      ...
      ticket_ids: dispatches
        ...
        .filter((ticketId) => typeof ticketId === 'string')
        .slice(0, 16),
      ...(recoveryKind === 'stage_retry'
        ? { consumes_product_attempt: !kinds.includes('reissue_same_contract') }
        : {}),
  ...
  if (run.status === 'input_required') {
    ...
    return {
      kind: run.input_required?.kind === 'execution_budget' ? 'extend_budget' : 'answer_preflight',
    };
  }
...
function hardBoundRunResponse(response) {
  ...
  const minimalActions = (projected.actions ?? [])
    ...
    .map((entry) => ({
      type: entry.type,
      ...(entry.recovery_kind ? { recovery_kind: entry.recovery_kind } : {}),
      ticket: ticketReference(entry.ticket),
...
export function projectRunResponse(response) {
  ...
  const nextAction = deriveNextAction(projected);
  if (nextAction) {
