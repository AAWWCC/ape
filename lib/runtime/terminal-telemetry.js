...
import { PLANNING_RECOVERY_REASON } from './constants.js';
import { INPUT_LIMITS, RECEIPT_INPUT_MAX_BYTES } from './input-guard.js';
import { RUNTIME_STATE_MAX_BYTES } from './resource-limits.js';

export const TERMINAL_REASON_TAXONOMY_VERSION = 2;
...
// Retain the former terminal code for immutable older records alongside the
// two outcomes emitted by the current bounded directed-replan policy.
const PLANNING_RECOVERY_REASONS = new Set(Object.values(PLANNING_RECOVERY_REASON));
...
function boundedRecoveryList(value, maxItems = INPUT_LIMITS.maxArrayLength, maxLength = 512) {
...
function capabilityRecovery(value) {
  ...
  const legacyToolClaims = claims.tool_claims === undefined
    ? null
    : boundedRecoveryList(claims.tool_claims);
  if (!claimedPaths || !testPaths || (claims.tool_claims !== undefined && !legacyToolClaims)) return null;
  ...
  return {
    ...
    additive_claims: {
      ...
      ...(legacyToolClaims ? { tool_claims: legacyToolClaims } : {}),
...
function planningRecovery(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  if (
    !PLANNING_RECOVERY_REASONS.has(value.reason_code)
    || !Number.isSafeInteger(value.directed_replan_attempts)
    || value.directed_replan_attempts < 0
    || !Array.isArray(value.missing_assurances)
    || value.missing_assurances.length > INPUT_LIMITS.maxArrayLength
  ) return null;
  const missingAssurances = [];
  let remainingBytes = RUNTIME_STATE_MAX_BYTES;
  for (const entry of value.missing_assurances) {
    if (
      ...
      || !boundedRecoveryString(entry.summary, RECEIPT_INPUT_MAX_BYTES)
      || !boundedRecoveryString(entry.evidence_anchor, RECEIPT_INPUT_MAX_BYTES)
    ...
    remainingBytes -= Buffer.byteLength(JSON.stringify(projected), 'utf8') + 1;
    if (remainingBytes < 0) return null;
    missingAssurances.push(projected);
  }
  return {
    reason_code: value.reason_code,
            ...
            ...(recovery.reason_code === 'capability_denied'
              ? {
                  additive_claim_counts: {
                    ...
                    ...(recovery.additive_claims.tool_claims
...
function isLandReviewDisagreement(record) {
  const reason = typeof record?.block_reason === 'string'
    ? record.block_reason.slice(0, 1_024).toLowerCase()
    : '';
  return record?.mode === 'land'
    && REVIEW_STAGES.has(terminalStage(record, reason))
    && record.remediation_cycles === 0
    && reason.includes('no writing stage');
}
...
export function terminalReasonCode(record) {
  ...
  if (isLandReviewDisagreement(record)) return 'land_review_disagreement';
  if (
