...
// This is the exact-integer representation boundary, not a workflow budget.
...
export function receiptFailureDomain(receipt, { reviewGroup: _reviewGroup = false } = {}) {
  if (String(receipt?.status).toLowerCase() === 'passed') return null;
...
export function validatedOrchestrationTelemetry(value) {
  ...
  if (COUNTER_FIELDS.some((field) => !boundedInteger(value[field]))) return null;
...
export function recordAcceptedReceipt(value, input = {}) {
  const suppliedAttempts = input.validation_attempts ?? input.validation_attempt;
  const attempts = boundedInteger(suppliedAttempts) && suppliedAttempts >= 1
    ? suppliedAttempts
    : null;
  if (attempts === null) return normalizedTelemetry(value);
  const invalidAttempts = boundedInteger(input.invalid_attempts) && input.invalid_attempts <= attempts
    ? input.invalid_attempts
    : Math.max(0, attempts - 1);
  const firstValidationValid = typeof input.first_validation_valid === 'boolean'
    ? input.first_validation_valid
    : attempts === 1;
  const validationSummaryRecorded = input.validation_summary_recorded === true;
  let telemetry = validationSummaryRecorded
    ? normalizedTelemetry(value)
    : incrementOrchestration(value, 'receipt_record_attempts', attempts);
  telemetry = incrementOrchestration(telemetry, 'receipt_accepts');
  // First-pass receipt acceptance means the exact draft that reached record
  // was the physical worker's first validation submission. A prior valid but
  // different draft is not a rejection, but it is still a resubmission and
  // must not let a live-certification run appear first-pass-perfect.
  if (
    !validationSummaryRecorded &&
    attempts === 1 &&
    firstValidationValid &&
    telemetry.repair_started_at === null
  ) {
    telemetry = incrementOrchestration(telemetry, 'receipt_first_pass_accepts');
  }
  if (!validationSummaryRecorded && invalidAttempts > 0) {
    telemetry = incrementOrchestration(telemetry, 'receipt_rejections', invalidAttempts);
    telemetry = {
      ...telemetry,
...
export function recordReceiptContractExhaustion(value, input = {}) {
  ...
  telemetry = {
    ...
    receipt_rejections_by_class: {
      ...telemetry.receipt_rejections_by_class,
      contract: addBounded(telemetry.receipt_rejections_by_class.contract, invalidAttempts),
    },
  };
...
export function recordRepairCompleted(value, completedAt) {
  ...
  if (!boundedInteger(elapsed)) return telemetry;
  return {
...
export function recordFirstWriterLatency(value, runCreatedAt, writerBoundAt) {
  ...
  if (!Number.isFinite(start) || !Number.isFinite(end) || !boundedInteger(end - start)) return telemetry;
  return {
    ...telemetry,
    time_to_first_writer_ms: end - start,
  };
}
