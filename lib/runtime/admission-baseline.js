...
export function packageScript(argv, commandRoot) {
  ...
  // The separator forwards everything after it to the selected script. Those
  // arguments cannot change the package root or request package-manager help.
  ...
  if (executable === 'npm' && args.some((arg) => ['--help', '-h', '--version', '-v'].includes(arg))) return null;
  ...
  return name && /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(name)
    ? { root, name, ...(executable === 'npm' && name === 'start' ? { default_command: 'node server.js' } : {}) }
    : null;
}

/** The same literal shell prefixes accepted by current prerequisite inspection. */
export function packageShellInvocation(input, inherited) {
  ...
  const builtin = !argv.length || [':', 'true', 'false', 'echo', 'printf', 'test', '[', 'pwd', 'cd', 'exit', 'export', 'unset', 'set', 'shift', 'return', 'readonly', 'umask', 'read', 'type'].includes(argv[0]);
...
export async function inspectAdmissionBaseline(root, input, repository, commands, executableFacts) {
  ...
  try {
    ...
    const exactExecutables = async (rawArgv, commandRoot, target = direct, env = process.env, resolveSelected = false) => {
      ...
      return { argv: invocation.argv, env: /** @type {NodeJS.ProcessEnv} */ (invocation.env) };
    };
    ...
    for (const script of scripts) {
      const key = `${script.root}\0${script.name}\0${script.default_command ?? ''}\0${script.env.PATH ?? ''}\0${script.env.PATHEXT ?? ''}`;
      ...
      for (const value of [before, after]) for (const name of [`pre${script.name}`, script.name, `post${script.name}`]) {
        const command = name === script.name ? selectedPackageScript(value, script) : value.scripts?.[name];
        if (typeof command !== 'string') continue;
        const commandRoot = path.resolve(root, path.dirname(headPath));
        const packageEnv = { ...script.env, PATH: [...new Set([path.join(commandRoot, 'node_modules', '.bin'), ...(script.env.PATH ?? '').split(path.delimiter)])].join(path.delimiter) };
        const shell = packageShellInvocation(splitCommand(command), packageEnv);
        if (shell.builtin) continue;
        const invocation = await exactExecutables(shell.argv, commandRoot, scriptEntries, shell.env, true);
        const argv = invocation.argv;
        for (const operand of entryScripts(argv)) {
          add(contained(root, path.resolve(commandRoot, operand)), scriptEntries);
        }
        const nested = packageScript(argv, commandRoot);
        if (nested) {
