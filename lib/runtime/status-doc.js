...
import { AUTO_MERGE_HOLD_REASON } from './constants.js';
import { pipelineLimits } from './pipeline-limits.js';
import { projectRunDiagnostic, safeDiagnosticText, validatedArchiveSnapshot } from './diagnostics.js';
...
function legacyDiagnosticState(state, dispatchState) {
  ...
  for (const key of ['mode', 'lane', 'status', 'stage', 'tickets', 'receipts', 'expired_tickets', 'block_reason']) {
  ...
  return {
    ...
    ...(state.block_reason === AUTO_MERGE_HOLD_REASON ? { block_reason: AUTO_MERGE_HOLD_REASON } : {}),
  };
}
...
// while budget remains: at the admitted max_regate_attempts the scheduler's REGATE arm
...
export function blockRecoveryHint(state) {
  if (state?.stage === 'gates') {
    ...
    const limit = pipelineLimits(state).max_regate_attempts;
    if (used < limit) {
      return `re-gate with REGATE after fixing the environment — the full gate suite re-runs with no bypass or waiver (${used} of ${limit} bounded attempts used)`;
    }
    return `the bounded re-gate budget is exhausted (${used} of ${limit} attempts used) and REGATE will be rejected — recover through the audited path: ABORT the run or OVERRIDE reset with a reason`;
  }
...
export function renderStatusDoc(state = {}, options = {}) {
  const bounded = validatedArchiveSnapshot(state);
  if (!bounded) return corruptDocument(projectRunDiagnostic(null, { corrupt: true }));
  ...
  const cappedTickets = Array.isArray(state?.tickets);
  const cappedReceipts = Array.isArray(state?.receipts);
  const cappedExpired = state?.expired_tickets === undefined || (
    Array.isArray(state.expired_tickets)
  );
