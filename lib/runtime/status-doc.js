...
export function renderStatusDoc(state = {}, options = {}) {
  ...
  if (state?.execution_budget) {
    ...
    const elapsedSeconds = Number.isFinite(budget.active_elapsed_ms)
      ? Math.floor(budget.active_elapsed_ms / 1_000)
      : 0;
    const activeRemaining = Math.max(0, safeCount(budget.max_active_seconds) - elapsedSeconds);
    lines.push(
      `**Execution budget:** ${used}/${safeCount(budget.max_worker_dispatches)} worker dispatches; ` +
        `${activeRemaining}s active time remaining; ` +
        (budget.active_deadline_at
          ? `active deadline ${safeDiagnosticText(budget.active_deadline_at, 64) ?? 'invalid'}`
          : 'clock paused for operator input'),
    );
  }
