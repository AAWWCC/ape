import {
  ...
  CHECKS_REGISTRATION_RETRY_DELAY_MS,
...
import { EXECUTION_POLICY_DEFAULTS, EXECUTION_POSITIVE_COUNTS, assertExecutionPolicy } from './pipeline-limits.js';
import { detectTestRunner, splitCommand } from './runner.js';
import { readBoundedRegularFileUtf8 } from './spawn.js';
import { canonicalProjectRelativePathError, isCanonicalVerificationRoot, VERIFICATION_PROFILE_ID_PATTERN } from './path-scope.js';
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  test_commands: Object.freeze({
    targeted: null,
    // Per-path command template for runners without proven file selection:
    // npm/script-test may ignore arguments, while cargo/maven/gradle/rake
    // select tests by name. The
...
// Allow ordinary pretty-printing above the compact 64 KiB control-input
// budget, while bounding bytes before allocation/parsing of persisted input.
const CONFIG_FILE_MAX_BYTES = 1024 * 1024;

export async function readStoredRuntimeConfig(configPath) {
  return assertSafeInput(await readBoundedJson(configPath, CONFIG_FILE_MAX_BYTES, {}));
}

export async function loadRuntimeConfig(configPath) {
  const configured = await readStoredRuntimeConfig(configPath);
  ...
  // configuration value.
  const config = { ...merge(DEFAULT_CONFIG, storedOverrides(configured)), version: RUNTIME_VERSION };
  validateExecutionNumbers(config);
  // Reject legacy noncanonical roots before a new run can create its branch.
  // Leave catalog size/shape checks to readiness so it can report all of the
  // unrepresentable collections together rather than throwing on the first.
  if (Array.isArray(config.verification?.profiles)) {
    for (const [index, profile] of config.verification.profiles.entries()) {
      if (profile?.root !== undefined && !isCanonicalVerificationRoot(profile.root)) {
        throw new Error(`config key verification.profiles[${index}].root must be a contained project-relative POSIX root`);
      }
    }
  }
  return config;
...
for (const name of Object.keys(EXECUTION_POLICY_DEFAULTS)) {
  PATH_VALUE_ASSERTIONS.set(`policy.${name}`, (key, value) =>
    assertIntegerRange(key, value, EXECUTION_POSITIVE_COUNTS.has(name) ? 1 : 0, Number.MAX_SAFE_INTEGER));
}

function assertIntegerRange(key, value, minimum, maximum) {
  if (!Number.isSafeInteger(value) || value < minimum || value > maximum) {
    throw new Error(`config key ${key} expects a finite number expressed as an integer from ${minimum} through ${maximum}`);
  }
}
...
function assertDeadlineValue(key, value) {
  // Explicit zero/negative horizons remain immediately/already expired. Keep
  // both ends representable by ticket Date arithmetic and Node timer delays.
...
function assertShippingTargetValue(key, value) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value) ||
      Object.keys(value).some((field) => !['origin', 'repository', 'base'].includes(field)) ||
      typeof value.origin !== 'string' || value.origin.length > 512 ||
      !/^(?:https:\/\/github\.com\/|git@github\.com:|ssh:\/\/git@github\.com\/)[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(?:\.git)?$/.test(value.origin) ||
      typeof value.repository !== 'string' || !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(value.repository) ||
      typeof value.base !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._/-]{0,199}$/.test(value.base) ||
      value.base.includes('..') || value.base.includes('//') || value.base.includes('/.') || value.base.endsWith('/') ||
      value.base.endsWith('.') || value.base.split('/').some((part) => part.endsWith('.lock'))) {
    throw new Error(`config key ${key} expects null or an explicit {origin, repository, base} GitHub shipping target without credentials`);
  }
  const repository = value.origin.replace(/^(?:https:\/\/github\.com\/|git@github\.com:|ssh:\/\/git@github\.com\/)/, '').replace(/\.git$/, '');
  if (repository.toLowerCase() !== value.repository.toLowerCase()) throw new Error(`config key ${key} origin and repository must identify the same repository`);
}
...
function assertVerificationProfilesValue(key, value) {
  ...
  value.forEach((profile, index) => {
    ...
    if (typeof profile.id !== 'string' || !VERIFICATION_PROFILE_ID_PATTERN.test(profile.id)) {
    ...
    if (typeof profile.description !== 'string' || !profile.description.trim()) {
      throw new Error(`config key ${itemKey}.description expects non-blank text within the configuration input envelope`);
    }
    ...
    if (!Number.isInteger(profile.timeout_ms) || profile.timeout_ms < 1 || profile.timeout_ms > MAX_TIMER_DELAY_MS) {
      throw new Error(`config key ${itemKey}.timeout_ms must be an integer from 1 through ${MAX_TIMER_DELAY_MS}`);
    }
  });
}
...
function assertCommandProfilesValue(key, value) {
  ...
  value.forEach((profile, index) => {
    ...
    const allowed = new Set(['id', 'command', 'roles', 'effect', 'output_paths']);
    const extras = Object.keys(profile).filter((field) => !allowed.has(field));
    ...
    if (profile.output_paths !== undefined) {
      if (!Array.isArray(profile.output_paths) || profile.output_paths.length > 64 ||
          profile.output_paths.some((file) => typeof file !== 'string' || file.length > 512 ||
            canonicalProjectRelativePathError(file) || file === '.' || /[*?\[\]{}]/.test(file) || ['.ape', '.git'].includes(file) || file.startsWith('.ape/') || file.startsWith('.git/'))) {
        throw new Error(`config key ${itemKey}.output_paths expects at most 64 exact contained project-relative output files; no broad directories or runtime paths`);
      }
      if (profile.output_paths.length && (profile.effect !== 'write' || profile.roles.some((role) => ROLE_POLICIES[role]?.writable !== true))) {
        throw new Error(`config key ${itemKey} tracked outputs require effect write and writable worker roles; read-only workers remain read-only`);
      }
    }
  });
}

export function validateAdmissionCommandConfiguration(config) {
  assertCommandProfilesValue('policy.command_profiles', config.policy?.command_profiles ?? []);
  assertVerificationProfilesValue('verification.profiles', config.verification?.profiles ?? []);
  for (const profile of config.policy?.command_profiles ?? []) {
    if (profile.effect === 'write' && !(profile.output_paths?.length > 0)) {
      throw new Error(`command profile ${profile.id} modifies tracked files and must declare exact output_paths before admission`);
    }
    if ((profile.output_paths?.length ?? 0) > 0 && (config.verification?.profiles ?? []).some((verification) => verification.command === profile.command)) {
      throw new Error(`generator ${profile.id} cannot also be a read-only verification profile`);
    }
    if ((profile.output_paths?.length ?? 0) > 0 && Object.values(config.test_commands ?? {}).includes(profile.command)) {
      throw new Error(`generator ${profile.id} cannot also be a runtime test verification command`);
    }
    if ((profile.output_paths?.length ?? 0) > 0 && (config.runners ?? []).some((runner) => Object.values(runner.profile ?? {}).includes(profile.command))) {
      throw new Error(`generator ${profile.id} cannot also be a monorepo runner verification command`);
    }
  }
}
...
export async function setRuntimeConfig(configPath, key, value) {
  return setRuntimeConfigBatch(configPath, [[key, value]]);
}

// Init is one operator action: validate the complete resulting overlay before
// publishing any of its slots. Its additive evidence names must also merge
// with the base read under this lock, so concurrent approved additions survive.
export async function setRuntimeConfigBatch(configPath, updates, { appendEvidenceScripts = [] } = {}) {
  ...
  assertSafeInput(appendEvidenceScripts);
  assertEvidenceScriptsValue('policy.evidence_scripts', appendEvidenceScripts);
  ...
  // the store — `ape_config set` and the complete `ape_config init --apply` —
  // rides the same serialization. Validation above stays outside the lock: an
  // invalid set never contends for the store.
  return withDirLock(
    `${configPath}.lock`,
    async () => {
      const storedRaw = await readStoredRuntimeConfig(configPath);
      let stored = storedOverrides(storedRaw);
      ...
      for (const [key, value] of updates) {
        stored = setNested(stored, key, value);
        explicit.add(key);
      }
      if (appendEvidenceScripts.length > 0) {
        const existing = merge(DEFAULT_CONFIG, stored).policy?.evidence_scripts ?? [];
        assertEvidenceScriptsValue('policy.evidence_scripts', existing);
        const merged = [...new Set([...existing, ...appendEvidenceScripts])];
        assertEvidenceScriptsValue('policy.evidence_scripts', merged);
        stored = setNested(stored, 'policy.evidence_scripts', merged);
        explicit.add('policy.evidence_scripts');
      }
      const overrides = pruneDefaults(stored, DEFAULT_CONFIG, explicit);
      ...
      const resolved = { ...merge(DEFAULT_CONFIG, storedOverrides(overrides)), version: RUNTIME_VERSION };
      validateExecutionNumbers(resolved);
      // Both the aggregate parsed value and the actual serialized bytes must
      // fit the next reader. Separate valid values can exceed the total budget.
      assertSafeInput(overrides);
      if (Buffer.byteLength(`${JSON.stringify(overrides, null, 2)}\n`, 'utf8') > CONFIG_FILE_MAX_BYTES) {
        throw new Error(`config file exceeds ${CONFIG_FILE_MAX_BYTES} UTF-8 bytes`);
      }
...
// Ticket authorization is mode-aware for read-only investigations. Debug and
// spike runs intentionally carry no production claims, which makes auto lane
// classification choose `full`; using only deadlines_ms[lane] would therefore
// turn every small investigation into a one-hour ticket. Change-producing modes
// retain their lane deadline, while sparse/legacy direct-test configs fall back
// to the lane and then the historical 45-minute safety value.
export function resolveTicketDeadline(config, mode, lane) {
  const modeKey = mode === 'debug' || mode === 'spike' ? mode : null;
  const modeValue = modeKey ? config?.deadlines_ms?.[modeKey] : undefined;
  // Preserve explicit zero/negative legacy values as immediate/already-expired
  // authorization. Falling back to 45 minutes would silently broaden a
  // fail-closed project setting merely because it is non-positive. A present
  // non-number remains malformed and throws instead of being mistaken for an
  // absent sparse-config key and silently widened by a fallback.
  if (modeValue !== undefined) {
    try { assertDeadlineValue(`deadlines_ms.${modeKey}`, modeValue); }
    catch (error) { throw new RangeError(`invalid ticket deadline for mode:${modeKey}; ${error.message}`); }
    return { deadline_ms: modeValue, source: `mode:${modeKey}` };
  }
  const laneValue = config?.deadlines_ms?.[lane];
  if (laneValue !== undefined) {
    try { assertDeadlineValue(`deadlines_ms.${lane}`, laneValue); }
    catch (error) { throw new RangeError(`invalid ticket deadline for lane:${lane}; ${error.message}`); }
    return { deadline_ms: laneValue, source: `lane:${lane}` };
  }
  return { deadline_ms: 45 * 60_000, source: 'fallback' };
}
...
export function resolveModel(config, host, tier, role = null) {
  ...
  if (
    modelMax !== undefined &&
    (typeof selected.model !== 'string' || selected.model.length > modelMax)
  ) {
    throw new Error(`invalid ${host} model mapping configured for tier ${tier}`);
  }
  if (
    host === 'codex' &&
    selected.reasoning_effort !== undefined &&
    (
      typeof selected.reasoning_effort !== 'string' ||
      selected.reasoning_effort.length === 0 ||
      selected.reasoning_effort.length > CODEX_REASONING_EFFORT_MAX_CHARS
    )
  ) {
    throw new Error(`invalid codex reasoning effort configured for tier ${tier}`);
  }
  return selected;
}
...
// A genuinely blank repository has no manifest from which ordinary onboarding
// can derive a runner. The prospective test paths are still an exact,
// user-authorized fact of the pending run, so two standard-library runners can
// be grounded without guessing a dependency or executing project code. These
// commands work before a manifest exists: the test writer can prove red against
// the missing implementation, and the same frozen commands prove green after
// the implementer creates the project.
const BLANK_REPOSITORY_RUNNER_TABLE = Object.freeze({
  'node-test-bootstrap': {
    targeted_template: 'node --test {paths}',
    full: 'node --test',
  },
  'node-test-typescript-bootstrap': {
    targeted_template: 'node --experimental-strip-types --test {paths}',
    full: 'node --experimental-strip-types --test',
  },
...
const BLANK_REPOSITORY_IGNORED_ROOT_NAMES = Object.freeze(new Set([
  '.ape',
  '.git',
  '.gitignore',
  '.gitattributes',
...
function blankRepositoryMetadataName(name) {
  return /^(?:README|LICENSE|LICENCE|COPYING|NOTICE|CHANGELOG|CONTRIBUTING|CODE_OF_CONDUCT|SECURITY)(?:\..*)?$/iu
    .test(name);
}

async function blankRepositoryForBootstrap(projectDir) {
  const entries = await readdir(projectDir, { withFileTypes: true }).catch(() => null);
  if (entries === null) return false;
  return entries.every((entry) =>
    BLANK_REPOSITORY_IGNORED_ROOT_NAMES.has(entry.name) ||
    (entry.isFile() && blankRepositoryMetadataName(entry.name)));
}

function blankRepositoryRunnerFromTestPaths(testPaths) {
  if (!Array.isArray(testPaths) || testPaths.length === 0 || testPaths.length > 64) return null;
  if (Buffer.byteLength(JSON.stringify(testPaths), 'utf8') > 4_096) return null;
  ...
  for (const value of testPaths) {
    ...
    if (['.js', '.mjs', '.cjs'].includes(extension)) {
      families.add('node-test-bootstrap');
    } else if (['.ts', '.mts', '.cts'].includes(extension)) {
      families.add('node-test-typescript-bootstrap');
    } else if (extension === '.py') {
      families.add('python-unittest-bootstrap');
    } else {
      return null;
    }
  }
  // Native type stripping is opt-in on the minimum supported Node 22.12.
  // One flagged runner can execute both JS and TS in a mixed Node project.
  if (families.has('node-test-typescript-bootstrap')) families.delete('node-test-bootstrap');
...
function runnerProfileTable(family) {
  ...
  if (Object.prototype.hasOwnProperty.call(BLANK_REPOSITORY_RUNNER_TABLE, family)) {
    return BLANK_REPOSITORY_RUNNER_TABLE[family];
  }
...
async function detectRunnerAt(dir) {
  let pkg = null;
  try {
    pkg = JSON.parse(await readBoundedRegularFileUtf8(path.join(dir, 'package.json')));
  } catch {
...
// Deterministic foreign-repo onboarding proposal. Normally inspects only the project's
...
// A metadata-only blank repository may additionally use the prospective run's
// exact test paths to select one dependency-free Node or Python stdlib runner;
// mixed and unsupported extensions produce no commands. This performs NO
// writes; status-service.js persists it only on apply. Fully deterministic: no
// network, env, timestamps, or process.platform influence, and stable key
// order, so the same tree and prospective paths always yield the same proposal.
export async function proposeTestCommands(projectDir, options = {}) {
  const proposed = {};

  const detected = await detectRunnerAt(projectDir);
  const blankRepository = detected === null && await blankRepositoryForBootstrap(projectDir);
  const bootstrapFamily = blankRepository && options.behavioral !== false
    ? blankRepositoryRunnerFromTestPaths(options.test_paths)
    : null;
  const selected = detected ?? (bootstrapFamily
    ? { family: bootstrapFamily, source: 'prospective-blank-repository-test-paths' }
    : null);
  if (selected) {
    const table = runnerProfileTable(selected.family);
    for (const slot of PROFILE_SLOTS) {
      if (table[slot]) proposed[slot] = { value: table[slot], rationale: selected.source };
    }
  }
  ...
  if (selected) {
    proposal.detected_runner = { family: selected.family, rationale: selected.source };
  }
  if (bootstrapFamily) {
    proposal.blank_repository_bootstrap = {
      version: 1,
      family: bootstrapFamily,
      evidence: 'prospective-test-path-extensions',
    };
  } else if (blankRepository && Array.isArray(options.test_paths) && options.test_paths.length > 0) {
    proposal.blank_repository_bootstrap = {
      version: 1,
      family: null,
  ...
  const pkg = await readBoundedRegularFileUtf8(path.join(projectDir, 'package.json'))
