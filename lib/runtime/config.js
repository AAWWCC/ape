...
function assertVerificationProfilesValue(key, value) {
  if (!Array.isArray(value) || value.length > 64) {
    throw new Error(`config key ${key} expects at most 64 verification profile objects`);
  }
  const ids = new Set();
...
function assertEvidenceScriptsValue(key, value) {
  if (!Array.isArray(value)) {
    throw new Error(`config key ${key} expects an array of package-script names; got ${JSON.stringify(value)}`);
  }
...
function assertCommandProfilesValue(key, value) {
  if (!Array.isArray(value)) {
    throw new Error(`config key ${key} expects an array of command profile objects; got ${JSON.stringify(value)}`);
  }
  const ids = new Set();
  value.forEach((profile, index) => {
    const itemKey = `${key}[${index}]`;
...
export function assertRunnersValue(key, value) {
  if (!Array.isArray(value)) {
    throw new Error(`config key ${key} expects an array of runner objects; got ${JSON.stringify(value)}`);
  }
