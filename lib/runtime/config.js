...
export function resolveTicketDeadline(config, mode, lane) {
  ...
  if (modeValue !== undefined) {
    if (!Number.isFinite(modeValue)) {
      throw new RangeError(`invalid ticket deadline for mode:${modeKey}; expected a finite number`);
    }
    return { deadline_ms: modeValue, source: `mode:${modeKey}` };
  }
  const laneValue = config?.deadlines_ms?.[lane];
  if (laneValue !== undefined) {
    if (!Number.isFinite(laneValue)) {
      throw new RangeError(`invalid ticket deadline for lane:${lane}; expected a finite number`);
    }
