...
import { CHECKS_REGISTRATION_WINDOW_MS, MERGE_PROVENANCE } from './constants.js';
...
async function assertMergedTreeAttested(projectDir, state, base, target, mergeOid) {
  ...
  if (!/^[0-9a-f]{40}$/i.test(mergeOid ?? '')) {
    throw new Error('shipping cannot verify the merged tree without an exact observed merge commit');
  }
  ...
  // The base may advance again between the PR merge and this poll. Verify the
  // immutable commit GitHub reports for this exact PR, while proving that it
  // belongs to the fetched frozen repository/base and descends from admission.
  // Comparing the latest base tree instead would misreport a completed merge
  // as a failure whenever a later independent change landed first.
  try {
    await runGit(projectDir, ['merge-base', '--is-ancestor', state.base_commit_sha, mergeOid]);
    await runGit(projectDir, ['merge-base', '--is-ancestor', mergeOid, `refs/remotes/origin/${base}`]);
  } catch {
    throw new Error('observed merge commit is not on the fetched frozen base history');
  }
  const observed = await runGit(projectDir, ['rev-parse', `${mergeOid}^{tree}`]);
  if (observed !== expected) {
    throw new Error(`merged commit ${mergeOid} tree ${observed} does not equal the attested tree ${expected}`);
  }
}

// Parse the newest `STATE URL MERGED_AT HEAD_OID MERGE_OID BASE` line out of the PR probe.
...
function parsePrProbe(output) {
  const line = output.trim().split(/\r?\n/)
    ...
    .reverse()
    .find((entry) => /^(OPEN|MERGED|CLOSED) https:\/\/\S+ \S+ \S+(?: \S+){0,2}$/.test(entry));
  if (!line) return null;
  const [prState, url, mergedAt, headOid, mergeOid, base] = line.split(' ');
  return { state: prState, url, merged_at: mergedAt === '-' ? null : mergedAt, head_oid: headOid, merge_oid: mergeOid === '-' ? null : mergeOid, base };
}

// Retargeting a PR leaves its URL/head unchanged. Query the base alongside the
// head before submissions that follow creation or a failed immediate merge.
async function assertOpenShippingPr(projectDir, watch, target) {
  ...
  if (!pr || pr.state !== 'OPEN' || pr.url !== watch.pr_url ||
      !shippingPrUrlMatches(pr.url, target) || pr.base !== target.base || pr.head_oid !== watch.head_oid) {
    throw new Error('open pull request does not match the frozen shipping URL, base, and pushed head; no merge submission is authorized');
  }
}

// gh never receives --delete-branch: its local deletion cannot compare the
// branch tip with the attested push. Remote branches are retained explicitly.
...
async function finalizeAfterMerge(projectDir, branch, base, target, expectedHead, state) {
  try {
    await assertLocalShippingBranchCurrent(projectDir, branch, expectedHead);
    ...
    try {
      ...
      // Only the exact local base commit admitted as finished land work may
      // be replaced by its tree-identical squash commit. A later local commit
      // is independent work even when its tree is identical. The admission
      // manifest was hash-validated by resolveFrozenShippingTarget at entry.
      const admitted = state.admission?.manifest?.repository;
      if (state.mode !== 'land' || admitted?.branch !== base) throw pullError;
      await assertLocalShippingBranchCurrent(projectDir, base, admitted.head);
      const dirty = (await workingTreeStatus(projectDir)).filter(line => !line.slice(3).startsWith('.ape/'));
      if (dirty.length) throw new Error('local shipping base retained: working tree or index changed before squash alignment');
      const remoteHead = await runGit(projectDir, ['rev-parse', remoteBase]);
      const [localTree, remoteTree] = await Promise.all([
        runGit(projectDir, ['rev-parse', `${admitted.head}^{tree}`]),
        runGit(projectDir, ['rev-parse', `${remoteHead}^{tree}`]),
      ]);
      if (localTree !== remoteTree) throw pullError;
      // The index and checkout are already clean at this same tree. Change
      // only the ref, atomically comparing its old head; never reset user bytes.
      await runAuthorizedGitMutation(projectDir, ['update-ref', '--no-deref', `refs/heads/${base}`, remoteHead, admitted.head]);
    }
    await deleteLocalShippingBranch(projectDir, branch, expectedHead);
    return { cleaned: true, remote_branch_retained: true };
  } catch (error) {
    ...
    return { cleaned: false, reason: safeDiagnosticText(boundedTail(error?.message ?? String(error)), 400), remote_branch_retained: true };
  }
}
...
async function reconcileMergedAfterCommandFailure(projectDir, selector, watch, state, target) {
  ...
  const pr = probe.passed ? parsePrProbe(probe.output) : null;
  if (pr && pr.base !== target.base) return { failed: 'pull request base does not match the frozen shipping target' };
  if (pr?.state !== 'MERGED') return null;
  if (!shippingPrUrlMatches(pr.url, target) || pr.url !== watch.pr_url) {
    return { failed: 'pull request URL does not match the frozen shipping watch' };
  }
  ...
  await assertMergedTreeAttested(projectDir, state, watch.base, target, pr.merge_oid);
  const cleanup = await finalizeAfterMerge(projectDir, watch.branch, watch.base, target, pr.head_oid, state);
  return {
    merged: {
      provider: 'github',
      head_oid: pr.head_oid, cleanup,
      url: pr.url ?? watch.pr_url,
      branch: watch.branch,
      base: watch.base,
      merged_at: pr.merged_at ?? new Date().toISOString(),
      provenance: MERGE_PROVENANCE.afterCommand,
    },
  };
}
...
async function observeSubmittedMerge(projectDir, watch, state, target, reason = 'awaiting merge') {
  ...
  if (!shippingPrUrlMatches(pr.url, target) || pr.url !== watch.pr_url) return { failed: 'pull request URL does not match the frozen shipping watch' };
  if (pr.base !== target.base) return { failed: 'pull request base does not match the frozen shipping target' };
  ...
  return {
    merged: {
      provider: 'github', head_oid: pr.head_oid, cleanup, url: pr.url, branch: watch.branch, base: watch.base,
      merged_at: pr.merged_at, provenance: MERGE_PROVENANCE.afterCommand,
    },
  };
}
 ...
 *   provider?: string, url?: string, branch?: string, base?: string, merged_at?: string, head_oid?: string, cleanup?: object,
 * }>}
 */
export async function autoMergeGithub(projectDir, state, config) {
  ...
  // Ship the live branch when it is a feature branch. A previous cleanup may
  // have switched to the base before failing, so the run's recorded branch
  // stands in for the merged-PR probe below. Committing or pushing FROM the
  // base stays forbidden either way.
  ...
  if (probe.passed && !existing) throw new Error('pull request observation is malformed; no shipping mutation is authorized');
  if (existing && existing.base !== target.base) throw new Error('pull request base does not match the frozen shipping target');
  ...
  if (existing?.state === 'MERGED') {
    ...
    // run started (re-entry after cleanup switched the checkout to the base
    ...
    if (provenBySelf) {
      // The merge is already proven; a re-entry must not push new local work
      // or recreate a remote branch independently removed after the merge.
      await assertMergedTreeAttested(projectDir, state, base, target, existing.merge_oid);
      const cleanup = await finalizeAfterMerge(projectDir, branch, base, target, existing.head_oid, state);
      return {
        provider: 'github',
        head_oid: existing.head_oid, cleanup,
        url: existing.url,
        branch,
        base,
  ...
  // shape (no watch key).
  await assertShippingBaseCurrent(projectDir, state, target);
  await assertOpenShippingPr(projectDir, watch, target);
  const merge = await runAuthorizedGithub(
    projectDir,
    ['pr', 'merge', url, '--squash', '--match-head-commit', committedHead],
    target,
    120_000,
  );
...
export async function pollRemoteChecksAndMerge(projectDir, state, config) {
  ...
  if (!checks.passed) {
    if (NO_CHECKS_PATTERN.test(checks.output)) {
      ...
      const registrationWindow = state.execution_policy?.shipping?.checks_registration_window_ms
        ?? config.shipping?.checks_registration_window_ms ?? CHECKS_REGISTRATION_WINDOW_MS;
      const withinWindow = Number.isFinite(createdAt)
        && Date.now() < createdAt + registrationWindow;
      if (withinWindow) {
        return { pending: { summary: 'no remote checks registered yet', reason: 'checks not yet registered' } };
      }
      return { failed: `no remote checks registered within ${registrationWindow / 1000}s of PR creation; inspect the admitted CI configuration; required remote checks are frozen for this run and cannot be waived by changing configuration` };
    }
  ...
  const pr = probe.passed ? parsePrProbe(probe.output) : null;
  if (pr && pr.base !== target.base) return { failed: 'pull request base does not match the frozen shipping target' };
  if (pr && (!shippingPrUrlMatches(pr.url, target) || pr.url !== watch.pr_url)) return { failed: 'pull request URL does not match the frozen shipping watch' };
  if (!pr) {
    ...
    if (pr.head_oid === watch.head_oid) {
      ...
      return {
        merged: {
          ...
          base: watch.base,
          merged_at: pr.merged_at ?? new Date().toISOString(),
          provenance: MERGE_PROVENANCE.external,
        },
      };
    }
    return { failed: `pull request ${watch.pr_url} was merged at a drifted head (${pr.head_oid}), not the pushed commit ${watch.head_oid}; refusing to complete this run on an external merge (invariant 8)` };
  ...
  const merge = await runAuthorizedGithub(
    projectDir,
    ['pr', 'merge', selector, '--squash', '--match-head-commit', watch.head_oid],
    target,
    120_000,
  );
  if (!merge.passed) {
    ...
    const auto = await runAuthorizedGithub(
      projectDir,
      ['pr', 'merge', selector, '--squash', '--auto', '--match-head-commit', watch.head_oid],
      target,
      120_000,
    );
    if (!auto.passed) {
