...
const PUBLIC_ORIGIN = 'https://github.com/AAWWCC/ape.git';
const PUBLIC_REPOSITORY = 'AAWWCC/ape';
...
async function resolveAuthorizedShippingTarget(projectDir, enforce = true) {
  if (!enforce) {
  ...
  if (normalizePublicOrigin(remote) !== PUBLIC_ORIGIN) {
    throw new Error(`shipping requires the authorized public origin ${PUBLIC_ORIGIN}`);
  }
  return Object.freeze({
    enforce: true,
    git_remote: String(remote).trim(),
    github_repository: PUBLIC_REPOSITORY,
  });
}

function assertFrozenAutoMergeAuthority(state, config) {
  if (
    Object.hasOwn(state, 'ship_requested') &&
    (state.auto_merge_authorized !== true || state.ship_requested !== true)
  ) {
    throw new Error('explicit auto-merge authorization and the audited SHIP consent marker are required');
  }
  if (
    !Object.hasOwn(state, 'ship_requested') &&
    ...
    throw new Error('explicit auto-merge authorization (auto_merge_authorized) is required');
  }
}

function requiresStrictShippingAuthority(state) {
  return Object.hasOwn(state, 'ship_requested');
}

async function runAuthorizedGitMutation(projectDir, args, options = undefined) {
...
async function assertMergedTreeAttested(projectDir, state, base, target) {
  // States predating the frozen SHIP marker retain their historical cleanup
  // behavior; marker-bearing runs use the strict public-release attestation.
  if (!target.enforce) return;
  const expected = state.gates?.tree_sha ?? null;
  if (!/^[0-9a-f]{40}$/i.test(expected ?? '')) {
    throw new Error('shipping cannot verify origin/main without the attested gate tree');
  }
...
async function finalizeAfterMerge(projectDir, branch, base, target) {
  try {
    await runAuthorizedGitMutation(projectDir, fetchBaseArgs(target, base), { timeout_ms: 120_000 });
    ...
    try {
      await runAuthorizedGitMutation(projectDir, pullBaseArgs(target, base), { timeout_ms: 120_000 });
    } catch (pullError) {
...
export async function autoMergeGithub(projectDir, state, config) {
  ...
  const enforce = requiresStrictShippingAuthority(state);
  // Resolve the mutable remote name exactly once. Every network mutation below
  // receives this immutable value directly, so a later `git remote set-url`
  // cannot retarget either Git or GitHub after authorization.
  const target = await resolveAuthorizedShippingTarget(projectDir, enforce);
  ...
  if (
    ...
    const advertisedBaseTip = await remoteBranchTip(projectDir, target.git_remote, base);
  ...
  const pushArgs = target.enforce
    ? ['push', target.git_remote, `HEAD:refs/heads/${branch}`]
    : ['push', '--set-upstream', 'origin', branch];
  await runAuthorizedGitMutation(projectDir, pushArgs, { timeout_ms: 120_000 });
  ...
  const merge = await runAuthorizedGithub(
    projectDir,
    ['pr', 'merge', ...(target.enforce ? [url] : []), '--squash', '--delete-branch'],
    target,
    120_000,
  );
...
export async function pollRemoteChecksAndMerge(projectDir, state, config) {
  ...
  const checks = await runAuthorizedGithub(
    projectDir,
    ['pr', 'checks', selector],
    target,
    120_000,
