...
export async function autoMergeGithub(projectDir, state, config) {
  ...
  // Recheck run-specific consent at the external sink. Admission-time config
  // validation cannot authorize a recovered/legacy run, nor can it cover a
  // configuration change after start. SHIP's audited one-shot marker is the
  // only alternative to the durable start authorization.
  if (
    config.shipping?.auto_merge === true &&
    state.auto_merge_authorized !== true &&
    state.ship_requested !== true
  ) {
    throw new Error('auto-merge requires explicit run authorization (auto_merge_authorized or audited SHIP)');
  }
