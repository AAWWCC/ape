...
// Parse the newest `STATE URL MERGED_AT HEAD_OID MERGE_OID` line out of the PR probe.
...
function parsePrProbe(output) {
  const line = output.trim().split(/\r?\n/)
    ...
    .reverse()
    .find((entry) => /^(OPEN|MERGED|CLOSED) https:\/\/\S+ \S+ \S+(?: \S+)?$/.test(entry));
  if (!line) return null;
  const [prState, url, mergedAt, headOid, mergeOid] = line.split(' ');
  return { state: prState, url, merged_at: mergedAt === '-' ? null : mergedAt, head_oid: headOid, merge_oid: mergeOid === '-' ? null : mergeOid };
}
