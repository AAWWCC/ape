...
export async function deriveRoadmap(paths, { historyMetrics = null } = {}) {
  ...
  // archived 'completed' AND either explicitly declared the id complete
  // (advances vs completes) OR an operator attestation binds the id to that
  // run. Legacy records with no completes array still require an attestation.
  ...
  for (const entry of entries) {
    for (const runId of requirements[entry.id] ?? []) {
      ...
      if (
        ...
        (
          (Array.isArray(record.completes) && record.completes.includes(entry.id)) ||
          attestedPairs.has(`${entry.id}\0${runId}`)
        )
      ) {
...
// completed run. The run's immutable record stays untouched; attestations are
// stored in a separate overlay the derivation reads at status time. The
// requirement-index is updated so queryHistory can find the relationship.
export async function attestRequirements(paths, { requirement_ids = [], run_id = '', reason = '' } = {}) {
  ...
  if (record.status !== 'completed') {
    throw new Error(`roadmap-attest requires a completed run; ${run_id} is ${record.status}`);
  }
