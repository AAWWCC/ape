...
import { AUTO_MERGE_HOLD_REASON, SCHEMA_VERSION, TERMINAL_STATUSES } from './constants.js';
...
// Each mutation shares the ordinary 64 KiB input and structural budgets.
// The durable ledger has no lifetime entry cap: graph validation reads all
// retained entries. IDs retain their cross-runtime identity bound.
const LIMITS = Object.freeze({ id: 128, discoveredBy: 128 });
...
export function normalizeRoadmapDeclaration(entry, { strict = true } = {}) {
  ...
  const title = requireString(entry.title, 'title');
  const description = requireString(entry.description, 'description');
  const acceptance = requireString(entry.acceptance, 'acceptance');
...
async function auditHasMutation(paths, mutationId, legacyAttestation = null) {
  ...
  for (const line of text.split('\n')) {
    ...
    try {
      ...
      if (mutationId && entry?.mutation_id === mutationId) return true;
      // Older attestation audit lines covered a batch without its per-pair
      // mutation ids. Recognize the exact persisted original observation so a
      // retry can repair the index without duplicating that audit evidence.
      if (legacyAttestation && entry?.mutation_id === undefined &&
          entry?.operation === 'roadmap-attest' &&
          entry.run_id === legacyAttestation.run_id &&
          entry.at === legacyAttestation.attested_at &&
          entry.reason === legacyAttestation.reason &&
          Array.isArray(entry.requirement_ids) &&
          entry.requirement_ids.includes(legacyAttestation.requirement_id)) return true;
    } catch {
...
export async function registerEntries(paths, { entries = [], reason = '' } = {}) {
  assertSafeInput({ entries, reason });
...
export async function supersedeEntries(paths, { ids = [], reason = '', replaced_by = [] } = {}) {
  assertSafeInput({ ids, reason, replaced_by });
  if (typeof reason !== 'string' || reason.trim() === '') {
...
// A disabled auto-merge policy is a post-gate operator hold, not a failed
// implementation. Keep the exception structural and fail closed: only the
// exact archived shape emitted by the merge hold is eligible for an explicit
// roadmap attestation. Other blocked records remain non-attestable.
function isVerifiedShippingHold(record) {
  return record?.status === 'blocked' &&
    record?.stage === 'merge' &&
    record?.gates?.passed === true &&
    record?.block_reason === AUTO_MERGE_HOLD_REASON;
}

function acceptsRoadmapAttestation(record) {
  return record?.status === 'completed' || isVerifiedShippingHold(record);
}
...
export async function deriveRoadmap(paths, { historyMetrics = null } = {}) {
  ...
  // Attestations (RM8): roadmap-attest can close requirements against an
  // archived run without modifying the run's immutable completes array.
  // Read attestations early so their run IDs join the single bulk history
  // query — handles the crash window where the attestation store landed but
  // the requirement-index update did not.
  const attestationStore = await readJson(paths.roadmapAttestations, null);
  const attestedPairs = new Set();
  const attestedRunsByRequirement = new Map();
  if (attestationStore?.attestations) {
    for (const a of attestationStore.attestations) {
      runIds.add(a.run_id);
      attestedPairs.add(`${a.requirement_id}\0${a.run_id}`);
      const runs = attestedRunsByRequirement.get(a.requirement_id) ?? new Set();
      runs.add(a.run_id);
      attestedRunsByRequirement.set(a.requirement_id, runs);
    }
  }
  ...
  // A requirement is satisfied iff SOME serving run's effective record either
  // reached archived 'completed' and declared the id complete, or an operator
  // attestation binds the id to an attestable record. The sole blocked-record
  // exception is the exact verified shipping hold; it never satisfies from
  // `completes` alone because the reason-audited attestation is the authority.
  // Legacy completed records with no completes array also require attestation.
  const completed = new Set();
  for (const entry of entries) {
    const servingRuns = new Set([
      ...(requirements[entry.id] ?? []),
      ...(attestedRunsByRequirement.get(entry.id) ?? []),
    ]);
    for (const runId of servingRuns) {
      const record = effectiveByRun.get(runId);
      const attested = attestedPairs.has(`${entry.id}\0${runId}`);
      if (
        record &&
        (
          (
          ...
          (attested && acceptsRoadmapAttestation(record))
        )
      ) {
...
// attestRequirements (RM8): close roadmap requirements against an archived
// completed run or the exact verified shipping-hold terminal shape. The run's
// immutable record stays untouched; attestations (including their required
// audit reason) are stored in a separate overlay the derivation reads at status
// time. The requirement-index is updated so queryHistory can find the
// relationship.
export async function attestRequirements(paths, { requirement_ids = [], run_id = '', reason = '' } = {}) {
  assertSafeInput({ requirement_ids, run_id, reason });
  if (typeof reason !== 'string' || reason.trim() === '') {
    throw new Error('roadmap-attest requires a non-empty reason');
  }
  if (!Array.isArray(requirement_ids) || requirement_ids.length === 0) {
    throw new Error('roadmap-attest requires a non-empty requirement_ids array');
  }
  const normalizedIds = requirement_ids.map((value) => requireString(value, 'requirement_id', LIMITS.id));
  const duplicate = normalizedIds.find((id, index) => normalizedIds.indexOf(id) !== index);
  if (duplicate) throw new Error(`roadmap-attest rejects duplicate requirement: ${duplicate}`);
  if (typeof run_id !== 'string' || run_id.trim() === '') {
    throw new Error('roadmap-attest requires a non-empty run_id');
  }
  requireString(run_id, 'run_id', LIMITS.discoveredBy);

  const store = await readStore(paths);
  const byId = new Map(store.entries.map((entry) => [entry.id, entry]));
  for (const id of normalizedIds) {
    const entry = byId.get(id);
    if (!entry) throw new Error(`roadmap-attest rejects unknown requirement: ${id}`);
    if (entry.superseded) throw new Error(`roadmap-attest rejects stale requirement: ${id}`);
  }

  const effectiveRecords = await queryEffectiveHistory(paths, new Set([run_id]));
  const record = effectiveRecords.get(run_id);
  if (!record) throw new Error(`roadmap-attest rejects unknown run: ${run_id}`);
  if (!acceptsRoadmapAttestation(record)) {
    throw new Error(
      `roadmap-attest requires a completed run or verified shipping hold; ${run_id} is ${record.status}`,
    );
  }

  const attestationStore = await readJson(
    paths.roadmapAttestations,
    { schema_version: SCHEMA_VERSION, attestations: [] },
  );
  const existing = new Set(
    attestationStore.attestations.map((a) => `${a.requirement_id}\0${a.run_id}`),
  );
  const ts = now();
  const added = [];
  for (const id of normalizedIds) {
    const key = `${id}\0${run_id}`;
    if (!existing.has(key)) {
      attestationStore.attestations.push({
        requirement_id: id,
        run_id,
        reason,
        attested_at: ts,
  ...
  if (added.length > 0) {
    await atomicWriteJson(paths.roadmapAttestations, attestationStore);
  }
  // The attestation is the durable intent. Replay every requested persisted
  // pair, including pairs written before an interrupted audit/index update.
  // Use its original reason/time/id, never replacement prose from the retry.
  for (const id of normalizedIds) {
    const attestation = attestationStore.attestations.find((entry) =>
      entry.requirement_id === id && entry.run_id === run_id);
    if (!(await auditHasMutation(paths, attestation.mutation_id, attestation))) {
      await appendJsonLine(paths.overrideLog, {
        operation: 'roadmap-attest',
        at: attestation.attested_at,
        requirement_ids: [id],
        run_id,
        reason: attestation.reason,
        ...(attestation.mutation_id ? { mutation_id: attestation.mutation_id } : {}),
      });
    }
  }
  const index = await readRequirementIndex(paths);
  ...
  for (const id of normalizedIds) {
    const runs = new Set(index.requirements[id] ?? []);
    if (runs.has(run_id)) continue;
    runs.add(run_id);
    index.requirements[id] = [...runs].sort();
    indexChanged = true;
  }
  if (indexChanged) {
