...
// Generated package/release artifacts live below stable repository-owned
// prefixes rather than at the project root. Keep these exceptions explicit:
// treating every nested `dist` or `build` segment as generated would demote
// ordinary monorepo source such as packages/build/src/index.ts.
const NESTED_GENERATED_ROOTS = [
  /^plugins\/[^/]+\/dist\//,
  /^release\/generated\//,
];
...
function isMechanicalPath(path) {
  ...
  if (NESTED_GENERATED_ROOTS.some((pattern) => pattern.test(normalized))) return true;
  return CONFIG_NAMES.has(normalized.split('/').at(-1));
}

// Classification reads only named paths; it never walks a directory to guess
// which descendants the writer intends to touch. A directory claim authorizes
// future descendants too, so it cannot truthfully count as one production file.
// Missing exact paths remain valid prospective file claims.
export async function resolveLaneScope(projectDir, input) {
  const rawClaims = input.test_intent === 'green-maintenance' && (input.claimed_paths?.length ?? 0) === 0
    ? input.test_paths ?? []
    : input.claimed_paths ?? [];
  let unbounded = input.unbounded_scope === true;
  for (const raw of new Set(rawClaims)) {
    const claim = normalizeClaimPath(raw);
    if (claim === '.' || claim === '..' || claim.startsWith('../') || path.posix.isAbsolute(claim) || /[\\/]$/.test(raw)) {
      unbounded = true;
      continue;
    }
    try {
      if (!(await stat(path.resolve(projectDir, claim))).isFile()) unbounded = true;
    } catch (error) {
      if (error.code !== 'ENOENT') unbounded = true;
    }
  }
  return { ...input, unbounded_scope: unbounded };
}

export function classifyLane(input, policy = {}) {
  // A test-only green-maintenance run is bounded by its immutable authored
  // test allowlist even though it deliberately has no production claims. The
  // ordinary/red-first classifier continues to count production claims only,
  // so this exception cannot make a broad behavioral implementation look
  // fast merely because it also names a small test set.
  const laneClaims =
    input.test_intent === 'green-maintenance' && (input.claimed_paths?.length ?? 0) === 0
      ? input.test_paths
      : input.claimed_paths;
  const claimSet = new Set((laneClaims ?? []).map(normalizeClaimPath));
  const claimed = [...claimSet];
  const unbounded = input.unbounded_scope === true || claimed.length === 0 ||
    claimed.some((claim) => {
      if (claim === '.' || claim === '..' || claim.startsWith('../')) return true;
      for (let slash = claim.indexOf('/'); slash >= 0; slash = claim.indexOf('/', slash + 1)) {
        if (claimSet.has(claim.slice(0, slash))) return true;
      }
      return false;
    });
  const configuredLimit = policy.fast_max_files ?? 6;
  if (!Number.isSafeInteger(configuredLimit) || configuredLimit < 1) {
    throw new Error('policy.fast_max_files must be a positive safe integer');
  }
  ...
  if (requested === 'mechanical' || requested === 'fast') {
    ...
    } else if (unbounded || claimed.length > configuredLimit) {
      violations.push(unbounded ? 'unbounded-scope' : `scope-over-${configuredLimit}-files`);
    }
    ...
    const withinFastBounds =
      knownRisk.length === 0 && !unbounded && claimed.length <= configuredLimit;
    return {
  ...
  if (unbounded || claimed.length > configuredLimit) {
    return {
      lane: 'full',
      reasons: [unbounded ? 'unbounded-scope' : `scope-over-${configuredLimit}-files`],
      risk_triggers: [],
    };
  }
