...
import { isCanonicalRunId, strictIsoMs, validatedArchiveSnapshot } from './diagnostics.js';
...
// an explicit resource ceiling for inspecting retained runtime state while
// keeping status/SessionStart memory use finite under a damaged or hostile
// artifact.
export const ACTIVE_STATE_MAX_BYTES = RUNTIME_STATE_MAX_BYTES;
const MISSING_ACTIVE_STATE = Symbol('missing-active-state');
const ACTIVE_STATE_READ_ATTEMPTS = 4;
...
function sameActiveStateFileIdentity(left, right) {
  return String(left.dev) === String(right.dev) && String(left.ino) === String(right.ino);
}

function activeStateBusyError(file) {
  return Object.assign(new Error(`active run state at ${file} was replaced while being read; retry the operation`), {
    code: 'APE_ACTIVE_STATE_BUSY',
  });
}
...
// immutable-sized, single-link regular file that was opened. APE's atomic
// replacement can change that identity during a read; retry from the pathname
// without consuming the superseded descriptor or diagnosing it as corruption.
async function readActiveStateArtifactOnce(file) {
  let before;
  try {
  ...
  try {
    ...
    if (opened.isFile() && opened.nlink === 0) throw activeStateBusyError(file);
    ...
    if (!sameActiveStateFileIdentity(before, opened)) throw activeStateBusyError(file);
    ...
    if (!ordinaryActiveStateFile(after)) {
      throw unsafeStateError(file, before, true);
    }
    if (
    ...
    if (!sameActiveStateFileIdentity(opened, after)) throw activeStateBusyError(file);
    if (!ordinaryActiveStateFile(openedAfter)) throw unsafeStateError(file, before, true);
    if (
...
async function readActiveStateArtifact(file) {
  for (let attempt = 1; ; attempt += 1) {
    try {
      return await readActiveStateArtifactOnce(file);
    } catch (cause) {
      const error = cause?.code === 'APE_FILE_IDENTITY_CHANGED' ? activeStateBusyError(file) : cause;
      if (error?.code !== 'APE_ACTIVE_STATE_BUSY' || attempt >= ACTIVE_STATE_READ_ATTEMPTS) throw error;
    }
  }
}
...
// Count limits must not contradict an admitted execution policy. The shared
// structural/depth budget and the exact on-disk byte ceiling bound retained
// state instead. Check before either durable state copy can be replaced.
export function assertActiveStateWritable(state) {
  if (!validatedArchiveSnapshot(state) || !isRunStateShape(state)) {
    throw Object.assign(new Error('run state exceeds its structural budget or has an invalid shape'), {
      code: 'APE_STATE_WRITE_INVALID',
    });
  }
  if (Buffer.byteLength(`${JSON.stringify(state, null, 2)}\n`, 'utf8') > ACTIVE_STATE_MAX_BYTES) {
    throw Object.assign(new Error(`run state exceeds the ${ACTIVE_STATE_MAX_BYTES}-byte persistence budget`), {
      code: 'APE_STATE_WRITE_TOO_LARGE',
    });
  }
}
...
function isActionableRunStateShape(value) {
  ...
  return isRunStateShape(value) &&
    ...
    // Older holds legitimately omit their not-yet-created first audit entry.
...
export async function activeState(paths, { requireActionable = false } = {}) {
  ...
  if (
    state !== null &&
    (!validatedArchiveSnapshot(state) || !isRunStateShape(state) || (
      // Answer-specific fields only govern a run eligible for that action.
      // Healthy runs at another stage must reach the handler's run-ID and
      // lifecycle checks, including a retry after the hold was resolved.
      requireActionable && state.status === 'input_required' && state.stage === 'preflight' &&
      !isActionableRunStateShape(state)
    ))
