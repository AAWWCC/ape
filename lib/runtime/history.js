...
import { readRequirementIndex } from './requirement-index.js';
...
import {
  validatedSuccessorAttestation,
  validateSuccessorLineageBinding,
...
const MAX_HISTORY_FILE_BYTES = 2 * RUNTIME_STATE_MAX_BYTES;

function readHistoryRecord(file) {
  return readBoundedJson(file, MAX_HISTORY_FILE_BYTES, null);
}

function readHistoryActive(paths) {
  return readBoundedJson(paths.active, RUNTIME_STATE_MAX_BYTES, null);
}
...
const PRETERMINAL_LINEAGE_STATUSES = new Set([
  'planning',
  'starting',
  'input_required',
  'running',
...
// the merge, not what merged. Cleanup can succeed on a retry after an earlier
// local failure. The stable merge identity (provider, url, branch, base,
// head_oid) stays inside the hash, so a retry carrying a DIFFERENT PR or head
...
function stableMergeFields(merge) {
  ...
  const { merged_at, provenance, cleanup, ...stable } = merge;
  return stable;
}
...
function immutableRunRecord(state, supersedes = null) {
  ...
  const successorAttestation = validatedSuccessorAttestation(
    state.successor_attestation,
    state.supersedes_run,
    state.run_id,
    state.successor_request_hash,
    state.start_config_hash,
  );
  const admittedStartIdentity = validatedAdmittedStartIdentity(state);
  const record = {
    ...
    ...(state.plan_contract_version
      ...
      : {}),
    ...(typeof state.start_config_hash === 'string'
      ? { start_config_hash: state.start_config_hash }
      : {}),
    ...(admittedStartIdentity && state.admission ? { admission: structuredClone(state.admission) } : {}),
    ...(admittedStartIdentity && state.shipping_target ? { shipping_target: structuredClone(state.shipping_target) } : {}),
    ...(admittedStartIdentity
      ? {
          admitted_start_identity_version: admittedStartIdentity.version,
          start_request_hash: admittedStartIdentity.start_request_hash,
          admitted_run_contract: admittedStartIdentity.admitted_run_contract,
          admitted_start_identity_hash: admittedStartIdentity.hash,
        }
      : {}),
    ...
    // Cross-run supersession (friction #10) remains readable legacy metadata.
    // A structured successor additionally carries the runtime-validated,
    // request-bound attestation below; both fields participate in record_hash.
    // Old records omit the additive attestation and keep their historical hash.
    ...
    ...(successorAttestation
      ? {
          ...
          successor_attestation: successorAttestation,
        }
      : {}),
...
async function indexRequirements(paths, state) {
  const index = await readRequirementIndex(paths);
...
async function resolveSupersedes(paths, state, options) {
  ...
  if (options.superseding === true) {
    const existing = await readHistoryRecord(path.join(paths.history, `${state.run_id}.json`));
    return existing?.record_hash ?? null;
  }
  return null;
}

// An upgrade can resume the old crash window after its head-less merge record
// was archived. Compare the new evidence under exactly that historical hash
// contract, retaining the first immutable record. New archives always bind the
// pushed head; this compatibility path never weakens a head-bearing record.
function legacyMergeRecordHash(record) {
  if (!isValidMergeEvidence(record.merge) || !record.merge.head_oid) return null;
  const merge = { ...stableMergeFields(record.merge) };
  delete merge.head_oid;
  return hashRecord({ ...record, merge }, ['record_hash', 'completed_at', 'timing']);
}

function isLegacyMergeRetry(existing, legacyHash) {
  return legacyHash !== null && existing?.record_hash === legacyHash &&
    isValidMergeEvidence(existing.merge) && !Object.hasOwn(existing.merge, 'head_oid') &&
    validatedArchiveSnapshot(existing)?.hashVerified === true;
}
...
export async function archiveRun(paths, state, options = {}) {
  ...
  if (Buffer.byteLength(`${JSON.stringify(record, null, 2)}\n`) > MAX_HISTORY_FILE_BYTES) {
    throw new Error('immutable history record exceeds its durable file bound');
  }
  const legacyHash = legacyMergeRecordHash(record);
  ...
  if (supersedes) {
    ...
    if (legacyHash) {
      const legacy = await readHistoryRecord(path.join(paths.history, `${state.run_id}.${legacyHash}.json`));
      if (isLegacyMergeRetry(legacy, legacyHash)) {
        await indexRequirements(paths, state);
        return legacy;
      }
    }
    if (!(await readHistoryRecord(supersedingPath))) await atomicWriteJson(supersedingPath, record);
    await indexRequirements(paths, state);
    return record;
  }
  const existing = await readHistoryRecord(destination);
  ...
  if (existing && existing.record_hash !== record.record_hash && options.ifAbsent !== true &&
      !isLegacyMergeRetry(existing, legacyHash)) {
...
async function readHistoryJson(paths, file, metrics = null) {
  const record = await readHistoryRecord(path.join(paths.history, file));
  if (metrics && typeof metrics === 'object') {
...
function compareHistoryKeys(left, right) {
  const [leftRun, leftFile = ''] = left.split('\0');
  const [rightRun, rightFile = ''] = right.split('\0');
  return leftRun === rightRun
    ? (leftFile < rightFile ? -1 : leftFile > rightFile ? 1 : 0)
    : (leftRun > rightRun ? -1 : 1);
}

function historyPageRequest(query, scope) {
  const limit = query.limit ?? MAX_HISTORY_RECORDS;
  if (!Number.isInteger(limit) || limit < 1 || limit > MAX_HISTORY_RECORDS) {
    throw new Error(`history limit must be between 1 and ${MAX_HISTORY_RECORDS}`);
  }
  const scopeHash = sha256(scope);
  let after = null;
  if (query.cursor !== undefined) {
    try {
      if (typeof query.cursor !== 'string' || !/^[A-Za-z0-9_-]+$/.test(query.cursor)) throw new Error();
      const value = JSON.parse(Buffer.from(query.cursor, 'base64url').toString('utf8'));
      if (!value || Object.keys(value).sort().join(',') !== 'after,scope,version' ||
          value.version !== 1 || value.scope !== scopeHash || typeof value.after !== 'string' ||
          !safeRunId(value.after.split('\0')[0])) throw new Error();
      after = value.after;
    } catch {
      throw new Error('history cursor is invalid or belongs to different filters');
    }
  }
  const cursor = (key) => Buffer.from(JSON.stringify({ version: 1, scope: scopeHash, after: key })).toString('base64url');
  return { limit, after, cursor };
}

// Page selection does not delete history or turn a presentation budget into a
// lifetime project limit. The cursor names the last consumed immutable key,
// so a newly archived run cannot shift later pages as an offset would.
export async function queryHistoryPage(paths, query = {}) {
  const page = historyPageRequest(query, { action: 'query', run_id: query.run_id ?? null, requirement: query.requirement ?? null });
  const { primaries, supersedingByRun } = partitionHistoryFiles(await listHistoryFiles(paths));
  const filtered = Boolean(query.run_id || query.requirement);
  let descriptors;
  if (filtered) {
    const index = await readRequirementIndex(paths);
    const ids = query.run_id ? [query.run_id] : index.requirements[query.requirement] ?? [];
    descriptors = [...new Set(ids)].filter(safeRunId).flatMap((id) => [
      { id, file: `${id}.json`, key: `${id}\0` },
      ...(supersedingByRun.get(id) ?? []).map((file) => ({ id, file, key: `${id}\0${file}` })),
    ]);
  } else descriptors = primaries.map((file) => ({ id: file.slice(0, -5), file, key: `${file.slice(0, -5)}\0` }));
  descriptors.sort((left, right) => compareHistoryKeys(left.key, right.key));
  const remaining = descriptors.filter((entry) => page.after === null || compareHistoryKeys(entry.key, page.after) > 0);
  const records = [];
  const recordCursors = [];
  if (!filtered && page.after === null) {
    try {
      const active = await readHistoryActive(paths);
      if (active !== null) {
        records.push(isValidActiveState(active)
          ? { run_id: active.run_id, status: active.status, active: true }
          : { run_id: 'unknown', status: 'corrupt_state', active: true });
        recordCursors.push(null);
      }
    } catch (error) {
      if (!(error instanceof SyntaxError) && error?.code !== 'APE_UNSAFE_FILE' && error?.code !== 'ELOOP') throw error;
      records.push({ run_id: 'unknown', status: 'corrupt_state', active: true });
      recordCursors.push(null);
    }
  }
  // Always consume at least one immutable key when history exists; an active
  // stub alone must not create a page with no usable continuation cursor.
  if (records.length === page.limit && remaining.length) {
    records.length = 0;
    recordCursors.length = 0;
  }
  const selected = remaining.slice(0, page.limit - records.length);
  for (const entry of selected) {
    let record = await readHistoryJson(paths, entry.file);
    if (!record) continue;
    if (!filtered) {
      const superseding = [];
      for (const file of supersedingByRun.get(entry.id) ?? []) {
        const candidate = await readHistoryJson(paths, file);
        if (candidate) superseding.push(candidate);
      }
      record = selectEffectiveRecord(record, superseding);
    }
    records.push(record);
    recordCursors.push(page.cursor(entry.key));
  }
  const hasMore = remaining.length > selected.length;
  return {
    records,
    pagination: { limit: page.limit, returned: records.length, has_more: hasMore,
      next_cursor: hasMore ? page.cursor(selected.at(-1).key) : null },
    record_cursors: recordCursors,
  };
}
...
export async function queryHistory(paths, query = {}) {
  ...
  if (!query.requirement && !query.run_id) {
    ...
    // `null`/absent file (the reader's null fallback) means no active run.
    let activeStub = null;
    try {
      const active = await readHistoryActive(paths);
      if (active !== null) {
...
function logicalLineageGraph(records, trustedRunIds = new Set()) {
  const byId = new Map(
    records
      .filter((record) => isCanonicalRunId(record?.run_id))
      .map((record) => [record.run_id, record]),
  ...
  const children = new Map([...byId.keys()].map((runId) => [runId, []]));
  const adjacency = new Map([...byId.keys()].map((runId) => [runId, new Set()]));
  const invalidReasons = new Map();
  const markInvalid = (runId, reason) => {
    invalidReasons.set(runId, new Set([...(invalidReasons.get(runId) ?? []), reason]));
  };
  for (const record of byId.values()) {
    // Logical lineage is allowed to promote an archived leaf to the primary
    // project outcome only when the immutable archive envelope verifies. A
    // missing hash is legacy-readable audit evidence, but it is not proof of a
    // successful logical result, so treat absence and mismatch identically.
    if (
      !trustedRunIds.has(record.run_id) &&
      validatedArchiveSnapshot(record)?.hashVerified !== true
    ) {
      markInvalid(record.run_id, 'corrupt-record');
    }
  }
  for (const record of byId.values()) {
    if (record.supersedes_run === undefined || record.supersedes_run === null) continue;
    const predecessor = record.supersedes_run;
    if (!isCanonicalRunId(predecessor)) {
      markInvalid(record.run_id, 'invalid-predecessor');
    } else if (predecessor === record.run_id) {
      markInvalid(record.run_id, 'self-link');
    } else if (!byId.has(predecessor)) {
      markInvalid(record.run_id, 'missing-predecessor');
    } else {
      const activeStartIdentity = trustedRunIds.has(record.run_id)
        ? validatedAdmittedStartIdentity(record)
        : null;
      const archivedStartIdentity = !trustedRunIds.has(record.run_id)
        && record.admitted_start_identity_version === 1
        && typeof record.start_request_hash === 'string'
        && /^[0-9a-f]{64}$/u.test(record.start_request_hash)
        && typeof record.admitted_start_identity_hash === 'string'
        && /^[0-9a-f]{64}$/u.test(record.admitted_start_identity_hash)
        && Object.hasOwn(record, 'admitted_run_contract')
        ? { start_request_hash: record.start_request_hash }
        : null;
      const binding = validateSuccessorLineageBinding(record.successor_attestation, {
        predecessor: byId.get(predecessor),
        supersedesRun: predecessor,
        successorRunId: record.run_id,
        successorRequestHash: record.successor_request_hash,
        successorStartConfigHash: record.start_config_hash,
        successorStartRequestHash:
          (activeStartIdentity ?? archivedStartIdentity)?.start_request_hash,
      });
      if (!binding.ok) {
        // Legacy or internally inconsistent links remain visible in raw/audit
        // history, but cannot promote an unrelated completion into recovery.
        markInvalid(record.run_id, binding.reason);
      } else {
        parent.set(record.run_id, predecessor);
        children.get(predecessor).push(record.run_id);
        adjacency.get(record.run_id).add(predecessor);
        adjacency.get(predecessor).add(record.run_id);
      }
    }
  ...
  const color = new Map();
  const visit = (runId, stack = []) => {
    if (color.get(runId) === 2) return;
    if (color.get(runId) === 1) {
      const start = stack.indexOf(runId);
      for (const member of stack.slice(start < 0 ? 0 : start)) markInvalid(member, 'cycle');
      markInvalid(runId, 'cycle');
      return;
    }
    color.set(runId, 1);
    const predecessor = parent.get(runId);
    if (predecessor) visit(predecessor, [...stack, runId]);
    color.set(runId, 2);
  };
  for (const runId of byId.keys()) visit(runId);
  return { byId, parent, children, adjacency, invalidReasons };
}

function lineageComponents(graph) {
  const components = [];
  const visited = new Set();
  for (const start of graph.byId.keys()) {
    if (visited.has(start)) continue;
    const stack = [start];
    const members = [];
    while (stack.length > 0) {
      ...
      visited.add(runId);
      members.push(runId);
      for (const neighbor of graph.adjacency.get(runId) ?? []) stack.push(neighbor);
    }
...
function primaryLogicalOutcome(records, includeLeaf = (_record) => true) {
  const graph = logicalLineageGraph(records);
  const outcomes = { recovered: 0, unresolved_blocked: 0, aborted: 0, unknown: 0 };
  const runLifecycle = {
  ...
  for (const members of lineageComponents(graph)) {
    const leaves = members.filter((runId) => graph.children.get(runId).length === 0);
    const includedLeaves = leaves.filter((runId) => includeLeaf(graph.byId.get(runId)));
    // Metrics filters select logical leaves. A wholly excluded component must
    // contribute nothing—not even corrupt/incomplete coverage or lifecycle
    // counters—or a filtered result mixes unrelated project history into the
    // requested cohort.
    if (includedLeaves.length === 0) continue;
    const reasons = new Set(
      members.flatMap((runId) => [...(graph.invalidReasons.get(runId) ?? [])]),
    );
    if (reasons.size > 0) {
      for (const reason of reasons) incompleteReasons.add(reason);
      uncountedRuns += members.length;
      continue;
    }
    if (leaves.length !== 1) {
      incompleteReasons.add('ambiguous-leaf');
      uncountedRuns += members.length;
      continue;
    }
    for (const runId of members) {
      const record = graph.byId.get(runId);
      if (graph.children.get(runId).length > 0) runLifecycle.superseded += 1;
      else if (record.status === 'blocked') runLifecycle.unresolved_blocked += 1;
      else if (record.status === 'running' && graph.parent.has(runId)) runLifecycle.recovering += 1;
      else if (record.status === 'completed') runLifecycle.recovered += 1;
    }
    for (const leafId of includedLeaves) {
      const leaf = graph.byId.get(leafId);
      total += 1;
      if (leaf.status === 'completed') outcomes.recovered += 1;
      else if (leaf.status === 'blocked') outcomes.unresolved_blocked += 1;
      else if (leaf.status === 'aborted') outcomes.aborted += 1;
      else outcomes.unknown += 1;
    }
  }
  return {
    ...
    total,
    outcomes,
    success_rate: total > 0 ? outcomes.recovered / total : 0,
    blocked_rate: total > 0 ? outcomes.unresolved_blocked / total : 0,
    run_lifecycle: runLifecycle,
    incomplete: incompleteReasons.size > 0,
    ...(uncountedRuns > 0 ? { uncounted_runs: uncountedRuns } : {}),
...
async function effectiveHistoryRecords(paths, requestedRunId) {
  const { primaries, supersedingByRun } = partitionHistoryFiles(await listHistoryFiles(paths));
  const records = [];
  const selected = primaries.slice(0, MAX_HISTORY_RECORDS);
  const requestedFile = `${requestedRunId}.json`;
  if (isCanonicalRunId(requestedRunId) && primaries.includes(requestedFile) && !selected.includes(requestedFile)) {
    selected.push(requestedFile);
  }
  for (const file of selected) {
    const primary = await readHistoryJson(paths, file);
    if (!primary) continue;
    const superseding = [];
    for (const candidate of supersedingByRun.get(file.slice(0, -5)) ?? []) {
      const record = await readHistoryJson(paths, candidate);
      if (record) superseding.push(record);
    }
    const effective = selectEffectiveRecord(primary, superseding);
    if (effective) records.push(effective);
  }
  return { records, coverage: { available_runs: primaries.length, processed_runs: selected.length,
    limit: MAX_HISTORY_RECORDS, requested_run_included: selected.includes(requestedFile),
    truncated: selected.length < primaries.length } };
}

function activeArchiveIdentityMatches(active, archived) {
  const admissionIdentityFields = [
    'admitted_start_identity_version',
    'start_request_hash',
    'admitted_run_contract',
    'admitted_start_identity_hash',
  ];
  // Only a complete admitted-start commitment is strong enough to identify
  // the immutable terminal archive as the same authority as mutable active
  // state. Legacy commitment-less archives remain available for audit and
  // metrics, but cannot override active state during crash reconciliation.
  if (!admissionIdentityFields.every((field) => Object.hasOwn(archived, field))) {
    return false;
  }
  const activeIdentity = validatedAdmittedStartIdentity(active);
  return activeIdentity !== null
    && archived.admitted_start_identity_version === activeIdentity.version
    && archived.start_request_hash === activeIdentity.start_request_hash
    && canonicalJson(archived.admitted_run_contract)
      === canonicalJson(activeIdentity.admitted_run_contract)
    && archived.admitted_start_identity_hash === activeIdentity.hash
    // The effective lane may escalate after admission, but it cannot change
    // during the terminal archive/persist crash window itself.
    && active.lane === archived.lane;
}

function terminalActiveMatchesArchive(active, archived) {
  return active.status === archived.status
    && active.tree_sha === archived.final_tree_sha;
}

export async function logicalLineageForRun(paths, runId, currentRecord = null) {
  const { records, coverage } = await effectiveHistoryRecords(paths, runId);
  const trustedRunIds = new Set();
  let reconciliation = null;
  if (currentRecord && isCanonicalRunId(currentRecord.run_id)) {
    const index = records.findIndex((record) => record.run_id === currentRecord.run_id);
    if (index < 0) {
      records.push(currentRecord);
      trustedRunIds.add(currentRecord.run_id);
      // Mutable active state is sufficient to describe live progress, but a
      // terminal outcome is never verified without its immutable archive. Keep
      // the bounded terminal projection visible while marking it incomplete.
      if (METRICS_STATUSES.has(currentRecord.status)) {
        reconciliation = {
          run_id: currentRecord.run_id,
          reason: 'missing-terminal-archive',
        };
      }
    } else {
      const archived = validatedArchiveSnapshot(records[index]);
      if (archived?.hashVerified === true) {
        const archiveIsTerminal = METRICS_STATUSES.has(archived.snapshot.status);
        const identitiesMatch = activeArchiveIdentityMatches(currentRecord, archived.snapshot);
        const activeIsRecognizedPreterminal = PRETERMINAL_LINEAGE_STATUSES.has(currentRecord.status);
        const terminalSnapshotsMatch = METRICS_STATUSES.has(currentRecord.status)
          && terminalActiveMatchesArchive(currentRecord, archived.snapshot);
        // archive_history is deliberately durable before persist_state. If the
        // process dies between those effects, active.json is a compatible
        // preterminal copy while history already contains the terminal truth.
        // Prefer that verified archive. Any other same-run disagreement is an
        // integrity conflict and must not let mutable active state mask history.
        if (
          !archiveIsTerminal
          || !identitiesMatch
          || (!activeIsRecognizedPreterminal && !terminalSnapshotsMatch)
        ) {
          reconciliation = {
            run_id: currentRecord.run_id,
            reason: 'active-archive-conflict',
          };
        }
      }
      // A missing or invalid archive hash remains in `records`; the graph will
      // classify it as corrupt-record. Never replace it with trusted active
      // state, because that would hide the durable integrity failure.
    }
  }
  const graph = logicalLineageGraph(records, trustedRunIds);
  if (reconciliation) {
    graph.invalidReasons.set(
      reconciliation.run_id,
      new Set([
        ...(graph.invalidReasons.get(reconciliation.run_id) ?? []),
        reconciliation.reason,
      ]),
    );
  }
  if (!graph.byId.has(runId)) return null;
  const members = lineageComponents(graph).find((component) => component.includes(runId)) ?? [runId];
  const reasons = new Set(
    members.flatMap((member) => [...(graph.invalidReasons.get(member) ?? [])]),
  );
  if (coverage.truncated) reasons.add('history-window-truncated');
  const roots = members.filter((member) => !graph.parent.has(member));
  const leaves = members.filter((member) => graph.children.get(member).length === 0);
  if (reasons.size === 0 && (roots.length !== 1 || leaves.length !== 1)) {
    reasons.add('ambiguous-leaf');
  }
  const complete = reasons.size === 0 && roots.length === 1 && leaves.length === 1;
  const leaf = leaves.length === 1 ? graph.byId.get(leaves[0]) : null;
  const state = !complete || !leaf
    ? 'unknown'
    : leaf.status === 'completed'
      ? (members.length > 1 ? 'recovered' : 'completed')
      : leaf.status === 'running' && members.length > 1
        ? 'recovering'
        : leaf.status === 'blocked'
          ? 'unresolved_blocked'
          : leaf.status === 'aborted'
            ? 'aborted'
            : 'unknown';
  const auditOutcomes = { completed: 0, blocked: 0, aborted: 0 };
  for (const member of members) {
    const status = graph.byId.get(member)?.status;
    if (Object.hasOwn(auditOutcomes, status)) auditOutcomes[status] += 1;
  }
  return {
    version: 1,
    state,
    root_run_id: roots[0] ?? runId,
    leaf_run_id: leaves[0] ?? runId,
    complete,
    immutable_run_count: members.length,
    coverage,
    audit_outcomes: auditOutcomes,
    ...(reasons.size > 0 ? { incomplete_reasons: [...reasons].sort() } : {}),
  };
}
...
export async function calculateProjectMetrics(paths, query = {}) {
  ...
  const page = historyPageRequest(query, { action: 'metrics', filters });
  const remainingPrimaries = primaries.filter((file) => page.after === null || compareHistoryKeys(`${file.slice(0, -5)}\0`, page.after) > 0);
  const selectedPrimaries = remainingPrimaries.slice(0, page.limit);
  ...
  return {
    ...
    pagination: { limit: page.limit, returned: selectedPrimaries.length,
      has_more: remainingPrimaries.length > selectedPrimaries.length,
      next_cursor: remainingPrimaries.length > selectedPrimaries.length
        ? page.cursor(`${selectedPrimaries.at(-1).slice(0, -5)}\0`) : null },
    total_runs: totalRuns,
    outcomes,
    primary_outcome: primaryLogicalOutcome(effectiveRecords, matchesFilters),
    audit_outcomes: {
      total_runs: totalRuns,
      outcomes: { ...outcomes },
    },
