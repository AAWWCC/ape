...
export function explainRun(record) {
  ...
  const lines = [
    ...
    `Status: ${record.status}; lane: ${record.lane ?? 'none'}; mode: ${record.mode ?? 'none'}.`,
    `Agents: ${passed} passed receipts, ${failed} non-passing receipts.`,
    record.merge?.url ? `Merged: ${record.merge.url}` : 'Merge: not recorded.',
  ];

  // Tiers
  const tiers = [...new Set((record.tickets ?? []).map((t) => t.model_tier).filter(Boolean))];
  if (record.model_tier && !tiers.includes(record.model_tier)) {
    tiers.push(record.model_tier);
  }
  if (tiers.length > 0) {
    lines.push(`Tiers: ${tiers.join(', ')}`);
  }
  ...
  const durationMs = (Number.isFinite(startMs) && Number.isFinite(endMs) && endMs >= startMs)
    ? endMs - startMs
    : (typeof record.timing?.raw_ms === 'number' && Number.isFinite(record.timing.raw_ms) ? record.timing.raw_ms : null);
  ...
  if (durationMs !== null) {
    ...
    lines.push(`Timing: ${durationFormatted}`);
  }
  ...
  if (record.gates && typeof record.gates === 'object') {
    ...
    const checksStr = record.gates.checks_count ? ` (${record.gates.checks_count} checks)` : '';
  ...
  if (record.recovery && typeof record.recovery === 'object') {
    const recType = record.recovery.type ?? 'unknown';
    const detail = record.recovery.original_block ? ` (resolved: ${record.recovery.original_block})` : '';
    lines.push(`Recovery: ${recType}${detail}`);
  }

  if (record.block_reason) {
    lines.push(`Block reason: ${record.block_reason}`);
  }
  ...
  // Remediation route
  if (record.remediation_route && typeof record.remediation_route === 'object') {
    const route = record.remediation_route.route ?? 'unknown';
    const cycle = record.remediation_route.cycle ? ` (cycle ${record.remediation_route.cycle})` : '';
    const testPaths = Array.isArray(record.remediation_route.test_paths) && record.remediation_route.test_paths.length > 0
      ? ` - test paths: ${record.remediation_route.test_paths.join(', ')}`
      : '';
    lines.push(`Remediation route: ${route}${cycle}${testPaths}`);
  }

  // Retries count
  if (typeof record.retries_count === 'number' || typeof record.retry_count === 'number') {
    const count = record.retries_count ?? record.retry_count;
    lines.push(`Retries: ${count}`);
  }

  // Input hold
  if (record.input_hold && typeof record.input_hold === 'object') {
    const count = record.input_hold.questions_answered ?? record.input_hold.question_count ?? 0;
    lines.push(`Input-hold: answered ${count} question${count === 1 ? '' : 's'}`);
  }

  // Imported legacy
  if (record.imported) {
    lines.push(`Imported from legacy planning: ${record.source_path ?? 'legacy plan'}`);
  }
...
export async function calculateProjectMetrics(paths, query = {}) {
  ...
  const sinceMs = query.since ? Date.parse(query.since) : -Infinity;
  const untilMs = query.until ? Date.parse(query.until) : Infinity;
  ...
  const matching = effectiveRecords.filter((record) => {
    ...
    if (query.host !== undefined && record.host !== query.host) return false;
