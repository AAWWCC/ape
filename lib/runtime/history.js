...
const METRICS_HOSTS = new Set(['claude', 'codex', 'gemini']);
...
export function deriveArchivedRunFacts(record) {
  ...
  const retryCount = typeof record.retries_count === 'number'
  ...
  const remediationCycle = record.remediation_route?.cycle ?? record.remediation_cycles ?? 0;
  ...
  return {
    ...
    input_hold: {
      ...
      question_count:
        explicitInputHold?.questions_answered
...
export function explainRun(record) {
  ...
  const facts = deriveArchivedRunFacts(record);
  ...
  if (facts.recovery.active) {
    const attempts = facts.recovery.regate_attempts > 0
      ...
      : '';
    const detail = facts.recovery.original_block ? ` (resolved: ${facts.recovery.original_block})` : '';
    lines.push(`Recovery: ${facts.recovery.type}${attempts}${detail}`);
  }
  ...
  if (facts.remediation.active) {
    const route = facts.remediation.route ?? 'unknown';
    const cycle = facts.remediation.cycle ? ` (cycle ${facts.remediation.cycle})` : '';
    const testPaths = facts.remediation.test_paths.length > 0
      ? ` - test paths: ${facts.remediation.test_paths.join(', ')}`
      : '';
