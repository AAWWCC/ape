...
export const SESSION_GUIDANCE_MAX_BYTES = 4 * 1024;
...
export const TEST_INTENTS = Object.freeze(['red-first', 'green-maintenance']);
export const MODEL_TIERS = Object.freeze(['fast', 'balanced', 'deep']);
export const RECEIPT_STATUSES = Object.freeze(['passed', 'failed']);

// These are native launch limits, not configuration-size budgets. Keep model
// selection inside the exact host-observed envelope so every issued ticket can
// be bound by its launch hook.
export const NATIVE_MODEL_MAX_CHARS = Object.freeze({
  claude: 256,
  codex: 512,
});
export const CODEX_REASONING_EFFORT_MAX_CHARS = 64;
...
// Shared producer/consumer vocabulary for immutable terminal evidence. Older
// merge records may omit provenance and older planning records retain their
// original single directed-replan rejection code.
export const MERGE_PROVENANCE = Object.freeze({
  external: 'observed-external',
  afterCommand: 'observed-after-merge-command',
});
export const PLANNING_RECOVERY_REASON = Object.freeze({
...
// so valid receipts are not rejected purely for lateness. Read-only modes have
// their own horizons because their empty write claims deliberately classify as
// `full`; a spike/debug ticket must not silently inherit a one-hour change-lane
// horizon merely because there is no production scope to classify.
...
// Shipped defaults and legacy fallback values. New runs freeze configured
// values through pipeline-limits.js rather than treating these as ceilings.
export const MAX_STAGE_ATTEMPTS = 2;
export const MAX_REMEDIATION_CYCLES = 3;
// The default receipt contract allows one replacement physical worker and
// three draft submissions per worker. Readiness forecasts configured physical
// work, while already-issued tickets retain their own bound contract.
...
export const CAPABILITY_MANIFEST_MAX_REQUIRED_CAPABILITIES = INPUT_LIMITS.maxArrayLength;
export const CAPABILITY_MANIFEST_MAX_ALLOWED_EVIDENCE_COMMANDS = INPUT_LIMITS.maxArrayLength;
export const CAPABILITY_MANIFEST_MAX_COMMAND_PROFILES = INPUT_LIMITS.maxArrayLength;
export const CAPABILITY_MANIFEST_MAX_VERIFICATION_PROFILES = INPUT_LIMITS.maxArrayLength;
export const CAPABILITY_CATALOG_MAX_EVIDENCE_SCRIPTS = INPUT_LIMITS.maxArrayLength;
export const CAPABILITY_CATALOG_MAX_RUNNERS = INPUT_LIMITS.maxArrayLength;
