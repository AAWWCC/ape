...
export const RECEIPT_INPUT_SCHEMA = deepFreeze({
  ...
  properties: {
    ...
    status: {
      ...
      description:
        'Stage outcome. Receipt-contract v1 requires the exact value passed or failed; synonyms are returned as field-specific corrections and are never coerced.',
    },
    tests: {
      type: 'array',
      description:
        'One entry per test command executed. Receipt-contract v1 requires an array and never wraps or reorders entries.',
      items: {
        type: 'object',
        ...
        properties: {
          ...
          output_hash: {
            ...
            description:
              'sha256 hex of the test output; omit when unknown. Null is invalid and is never removed automatically.',
          },
        },
      },
    },
    findings: {
      ...
      items: { type: 'object' },
      description:
        'Structured finding objects. Strings and single objects are invalid and are never wrapped or reinterpreted.',
    },
    evidence: {
      type: 'object',
      properties: {
...
export function normalizeReceiptInput(raw, options = {}) {
  // Contract v1 deliberately permits no semantic coercion. JSON object-key
  // ordering and negative-zero normalization are handled by canonicalJson at
  // the hashing boundary; the parsed evidence object itself remains exactly
  // as the worker supplied it. Legacy callers retain the historical edge
  // coercions below so persisted pre-contract transactions remain replayable.
  if (options.receipt_contract_version === 1) {
    return {
      input: raw,
      normalized_fields: [],
      correction_deltas: [],
...
// Locate every candidate in one pass, retaining opening order so an outer
// receipt still takes precedence over objects in its evidence. Nested objects
// and prose-wrapped receipts remain extractable without rescanning each suffix.
// Candidate count/depth bound the subsequent JSON parsing work as well: each
// character can belong to at most maxDepth + 1 overlapping candidates.
function receiptObjectCandidates(text) {
  const candidates = [];
  const stack = [];
  let inString = false;
  let escaped = false;
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (inString) {
      // Raw line breaks invalidate a JSON string, including after a backslash.
      // Discard that malformed example so a later receipt can start afresh.
    ...
    if (character === '"' && stack.length > 0) {
      ...
      if (stack.length > INPUT_LIMITS.maxDepth || candidates.length >= INPUT_LIMITS.maxNodes) return [];
      const candidate = { start: index, end: -1 };
...
export function extractReceiptDraftFromText(text) {
  ...
  // Bound transport and aggregate parse work separately. Pretty-printing and
  // prose wrappers can exceed the receipt object's own serialized allowance.
  const textLimit = 8 * 1024 * 1024;
  if (Buffer.byteLength(text, 'utf8') > textLimit) return null;
  let remainingParseChars = 2 * textLimit;
  const parseCandidate = (candidate) => {
    if (candidate.length > remainingParseChars) return null;
    remainingParseChars -= candidate.length;
    try {
      const parsed = JSON.parse(candidate);
      return looksLikeReceipt(parsed) ? parsed : null;
    } catch { return null; }
  };
  ...
  for (const { start, end } of receiptObjectCandidates(text)) {
    if (end < 0 || end - start + 1 > remainingParseChars) continue;
...
// One continuation must carry every correction the validator can return.
// Derive its size from that shared contract instead of slicing the combined
// message halfway through an actionable correction.
export const RECEIPT_CORRECTIONS_MAX = 20;
export const RECEIPT_CORRECTION_FIELD_MAX_BYTES = 240;
export const RECEIPT_CORRECTION_TEXT_MAX_BYTES = 600;
const RECEIPT_BEARER_REDACTION = '[receipt-capability-redacted]';

export function boundedReceiptCorrectionText(value, maxBytes = RECEIPT_CORRECTION_TEXT_MAX_BYTES) {
...
export function receiptBearerValue(raw) {
  const descriptor = raw && typeof raw === 'object' && !Array.isArray(raw)
    ? Object.getOwnPropertyDescriptor(raw, 'receipt_capability')
    : null;
  return typeof descriptor?.value === 'string' ? descriptor.value : null;
}

export function receiptTicketIdValue(raw) {
  const descriptor = raw && typeof raw === 'object' && !Array.isArray(raw)
    ? Object.getOwnPropertyDescriptor(raw, 'ticket_id')
    : null;
  return typeof descriptor?.value === 'string' ? descriptor.value : null;
}

// A draft is attacker-controlled until its canonical capability field proves
// authority against the dispatch intent's one-way hash. Callers must not use a
// copied token discovered in an arbitrary draft field as authority: doing so
// would turn redaction itself into a capability oracle. When this returns null,
// reflective validation/correction channels must emit fixed prose only.
export function authenticatedReceiptBearer(raw, capabilityHash) {
  const bearer = receiptBearerValue(raw);
  return (
    typeof capabilityHash === 'string' &&
    /^[0-9a-f]{64}$/iu.test(capabilityHash) &&
    bearer !== null &&
    sha256(bearer) === capabilityHash.toLowerCase()
  ) ? bearer : null;
}

export function redactReceiptBearer(value, bearer) {
  if (typeof bearer !== 'string' || bearer.length === 0) return value;
  if (typeof value === 'string') {
    return value.includes(bearer)
      ? value.split(bearer).join(RECEIPT_BEARER_REDACTION)
      : value;
  }
  if (Array.isArray(value)) {
    return value.map((entry) => redactReceiptBearer(entry, bearer));
  }
  if (value === null || typeof value !== 'object') return value;
  return Object.fromEntries(Object.entries(value).map(([key, nested]) => [
    key.includes(bearer)
      ? key.split(bearer).join(RECEIPT_BEARER_REDACTION)
      : key,
    redactReceiptBearer(nested, bearer),
  ]));
}
 ...
 * @returns {string}
 */
export function formatDraftCorrections(corrections, ticket, bearer = null) {
  if (!Array.isArray(corrections) || corrections.length === 0) {
    return '';
  }

  const lines = [
    `Receipt draft corrections for ${boundedReceiptCorrectionText(ticket?.ticket_id ?? 'unknown', 512)}:`,
    'Reuse the hook-injected receipt envelope and role-specific output_schema excerpt; change only the fields named below.',
  ];
  for (const entry of redactReceiptBearer(corrections.slice(0, RECEIPT_CORRECTIONS_MAX), bearer)) {
    const field = boundedReceiptCorrectionText(entry.field, RECEIPT_CORRECTION_FIELD_MAX_BYTES);
    const issue = boundedReceiptCorrectionText(entry.issue);
    const correction = boundedReceiptCorrectionText(entry.correction);
    const line = `- ${field}: ${issue} -> ${correction}`;
    lines.push(line);
  }
  if (corrections.length > RECEIPT_CORRECTIONS_MAX) {
    lines.push(`[APE runtime: ${corrections.length - RECEIPT_CORRECTIONS_MAX} corrections exceed the validation response contract.]`);
  }
  return lines.join('\n');
}
