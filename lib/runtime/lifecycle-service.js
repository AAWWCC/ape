...
export async function startRun(projectDir, rawInput) {
  ...
  if (input.mode !== 'land') {
    ...
    if (dirty.length > 0) {
      ...
      const predecessorBaseTree = successorContext
        ? await runGit(paths.root, ['rev-parse', `${existing.base_commit_sha}^{tree}`])
        : null;
      const carriedFiles = predecessorBaseTree
        ? await diffFiles(paths.root, predecessorBaseTree, currentTree)
        : [];
      const carryIsExact =
        successorContext !== null &&
