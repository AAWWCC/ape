...
import { access, link, lstat, mkdir, readFile, realpath, rename, rm, stat } from 'node:fs/promises';
...
import { atomicReplaceText, atomicWriteJson, appendJsonLine, readJson } from './storage.js';
...
import { declaredTestRemediationPaths, declaredTestRemediations, extractTestRemediation, pipelineRunSpec, projectedPipeline } from './pipeline.js';
import { REVIEW_FINDINGS_BLOCK_LIMIT, REVIEW_FINDINGS_MAX, reduceRun } from './scheduler.js';
import {
  finalizeReceipt,
  finalizeTicket,
  RunStartInputSchema,
  AnswerPreflightInputSchema,
} from './schemas.js';
import { currentBranch, currentCommitSha, currentTreeSha, diffFiles, remoteBranchTip, resolveBaseBranch, runGit, treeHasPath, treeShaSession, workingTreeStatus } from './git.js';
...
import { activeState, activeStateDiagnosisMatchesEntry, boundedGateSummary, checkoutCleanupIncomplete, dispatchLiveness, now, statusRun } from './status-service.js';
import { PLAN_ARTIFACT_MARKER_BRAND, PLAN_ARTIFACT_MAX_CHARS, PLAN_ARTIFACT_MAX_ENTRIES, SCOPE_EXPANSION_PATHS_MAX, SCOPE_EXPANSION_PATH_MAX_CHARS, SCOPE_EXPANSION_REASON_MAX_CHARS, applyActions, boundedSerialize, cleanupGateSuite, executeTaskOperationTransaction, lockRecoveryAuditLine, persist, prepareGovernedRuntimeAncestor, reconcileTerminalCheckout, recordReceipt, recordReceiptLocked, recoverReceiptLocked, resumeLegacyBudgetContinuation, retireLegacyExecutionBudget, settleReceiptValidationSubagentStop, statusDocPath, withReceiptLock } from './receipt-service.js';
...
async function redTestNotice(paths, config) {
  let notice =
    ' Red-test admission: do not execute the authored tests from the worker and return tests: []; the runtime exclusively executes the exact authored test paths after receipt submission and must observe them fail. A failure of unrelated tests is not accepted, and side-effect writes to the tree are refused. The authored tests execute twice at admission and BOTH invocations must fail: a fail-then-pass pair is refused as nondeterministic, so the authored red must fail deterministically.';
  const hasTemplate =
...
// cannot be produced, and repeated returns exhaust the configured stage budget
// with a remediation cycle spent. That is the very wedge this
...
export async function resumeRun(projectDir) {
  ...
  await settleReceiptValidationSubagentStop(projectDir);
  const state = await activeState(paths);
  if (!state) return { ok: false, reason: 'no active run' };
