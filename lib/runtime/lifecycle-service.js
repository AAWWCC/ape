...
import { PLAN_ARTIFACT_MARKER_BRAND, PLAN_ARTIFACT_MAX_CHARS, PLAN_ARTIFACT_MAX_ENTRIES, SCOPE_EXPANSION_PATHS_MAX, SCOPE_EXPANSION_PATH_MAX_CHARS, SCOPE_EXPANSION_REASON_MAX_CHARS, applyActions, boundedSerialize, cleanupGateSuite, executeTaskOperationTransaction, lockRecoveryAuditLine, persist, reconcileTerminalCheckout, recordReceipt, recordReceiptLocked, resumeBudgetContinuation, settleReceiptValidationSubagentStop, statusDocPath, withReceiptLock } from './receipt-service.js';
...
function unresolvedReviewFindings(state) {
  const tickets = state?.tickets ?? [];
  let lastWriter = -1;
  for (let index = tickets.length - 1; index >= 0; index -= 1) {
    if (['build', 'remediation-build', 'remediation-test'].includes(tickets[index]?.stage_id)) {
      lastWriter = index;
      break;
    }
  }
  const reviewTicketIds = new Set(
    tickets.slice(lastWriter + 1)
      .filter((ticket) => ticket?.parallel_group === 'code-review')
      .map((ticket) => ticket.ticket_id),
  );
  const receipts = (state?.receipts ?? []).filter((receipt) => reviewTicketIds.has(receipt.ticket_id));
  return reviewFindings.select(state, receipts);
}
...
export async function startRun(projectDir, rawInput) {
  ...
  const existing = await activeState(paths);
  const succeedsBlockedRun = existing?.status === 'blocked' && input.supersedes_run === existing.run_id;
  if (existing && !['completed', 'aborted'].includes(existing.status) && !succeedsBlockedRun) {
    throw new Error(`run ${existing.run_id} is already ${existing.status}`);
  }
  const successorContext = succeedsBlockedRun
    ? {
        run_id: existing.run_id,
        tree_sha: existing.tree_sha,
        review_findings: unresolvedReviewFindings(existing),
      }
    : null;
  ...
  if (
    !readiness.ready &&
    (input.execution_budget_required === true || input.required_capabilities.length > 0)
  ) {
    return {
      ok: false,
      blocked: true,
  ...
  // A persistent repository preference is not consent to publish this run.
  // Public/native starts must re-attest that authority on every invocation,
  // before doctor, locks, branch creation, or any other effect can occur.
  if (
    input.binding_protocol === 'native-v1' &&
    config.shipping?.auto_merge === true &&
    input.auto_merge_authorized !== true
  ) {
    throw new Error('shipping.auto_merge is enabled; a public start requires auto_merge_authorized: true after explicit operator authorization');
  }
  ...
  let successorAdmission = null;
  let successorStartPoint = null;
  if (input.mode !== 'land') {
    const dirty = (await workingTreeStatus(paths.root))
      .map((line) => line.slice(3))
      .filter((file) => !file.startsWith('.ape/'));
    if (successorContext) {
      const currentBranchName = await currentBranch(paths.root);
      const currentCommit = await currentCommitSha(paths.root);
      const currentTree = await currentTreeSha(paths.root);
      const predecessorBaseTree = await runGit(paths.root, ['rev-parse', `${existing.base_commit_sha}^{tree}`]);
      const carriedFiles = await diffFiles(paths.root, predecessorBaseTree, currentTree);
      const commonCarryIsExact =
        currentBranchName === existing.branch &&
        currentTree === existing.tree_sha &&
        carriedFiles.length > 0 &&
        carriedFiles.every((file) => withinClaims(file, [...input.claimed_paths, ...input.test_paths]));
      const dirtyCarryIsExact =
        dirty.length > 0 &&
        currentCommit === existing.base_commit_sha &&
        commonCarryIsExact;
      const committedCarryIsExact =
        dirty.length === 0 &&
        currentCommit !== existing.base_commit_sha &&
        commonCarryIsExact &&
        await runGit(paths.root, ['merge-base', '--is-ancestor', existing.base_commit_sha, currentCommit])
          .then(() => true, () => false);
      const carryIsExact = dirtyCarryIsExact || committedCarryIsExact;
      if (!carryIsExact) {
        throw new Error(`successor carry-forward refused: check out blocked run ${existing.run_id}'s exact retained tree on ${existing.branch}; it must be either the original dirty tree or its clean runtime-owned descendant commit, and every changed file must remain claimed`);
      }
      successorAdmission = {
        base_tree_sha: predecessorBaseTree,
        head_tree_sha: currentTree,
        changed_files: carriedFiles,
      };
      if (committedCarryIsExact) successorStartPoint = currentCommit;
    } else if (dirty.length > 0) {
      throw new Error(`mode ${input.mode} requires a clean working tree at start; dirty paths: ${dirty.join(', ')}. Commit, stash, or revert them before starting, use mode land to gate an already-finished diff, or explicitly supersede the blocked run whose exact retained tree is checked out`);
    }
  }
  ...
  if (successorContext && existing.base_commit_sha !== baseCommitSha) {
    throw new Error(`successor carry-forward requires the resolved default tip to remain ${existing.base_commit_sha}; it is now ${baseCommitSha}. Rebase the retained tree before retrying`);
  }
  ...
  try {
    ...
    await runGit(paths.root, ['switch', '-c', branch, successorStartPoint ?? landStartPoint ?? base.start_point, '--no-track']);
    ...
    state = {
      ...
      ...(input.required_capabilities.length > 0 || input.execution_budget_required === true || input.execution_budget
        ? { capability_snapshot: snapshotRunCapabilities(readiness) }
        : {}),
      ...(input.execution_budget
        ? { execution_budget: initializeExecutionBudget(input.execution_budget, createdAt) }
        : {}),
      host: input.host,
      ...
      ...(successorContext ? { carry_forward: successorContext } : {}),
    ...
    const initialAdmission = landAdmission ?? successorAdmission;
    if (initialAdmission) {
      ...
      const admission = finalizeReceipt({
        ...
        run_id: runId,
        ticket_id: `${runId}:${landAdmission ? 'land-admission' : 'carry-forward-admission'}`,
        ticket_hash: sha256({
          run_id: runId,
          stage_id: landAdmission ? 'land-admission' : 'carry-forward-admission',
        }),
        agent: {
          host: input.host,
          role: 'runtime',
        ...
        evidence: {
          ...(landAdmission
            ...
            : {
                carry_forward_admission: {
                  supersedes_run: existing.run_id,
                  base_commit_sha: state.base_commit_sha,
                  validated_at: createdAt,
                },
              }),
...
async function nextRunLocked(paths, options = {}) {
    ...
    const at = now();
    // Terminal rejection must win before budget admission. Otherwise an
    // elapsed clock could rewrite a sealed run into execution-budget input and
    // make a later extension appear to resurrect it.
    ...
    // Input holds authorize no execution, so an elapsed active-time clock must
    // not overwrite their payload. In particular, preflight questions remain
    // answerable, and an extended red-test hold continues to require the exact
    // attested receipt instead of allowing NEXT to redispatch the ticket.
    ...
    const config = await loadRuntimeConfig(paths.config);
    const budgetDecision = executionBudgetGuard(state, { at, dispatches: 0 });
    if (!budgetDecision.allowed) {
      const pause = pauseForExecutionBudget(state, budgetDecision, at);
      await persist(paths, state, treeShaSession(paths.root));
      return { ok: true, run: state, actions: pause ? [pause] : [] };
    }
    if (state.budget_continuation) {
        ...
        if (stillRunning) {
          return {
            ...
            actions: [{
              ...
              reason: 'receipt correction stopped at the execution-budget boundary; wait for the original physical worker to stop before APE redispatches the same immutable ticket',
            }],
            next_action: {
              kind: 'wait',
              ticket_id: stillRunning.ticket_id,
      ...
      const actions = await resumeBudgetContinuation(
        paths,
        state,
        config,
    ...
    // before budget admission so an already-live worker is represented as
    // dispatch_pending and is neither counted nor prepared as a new physical
    // dispatch. Without this normalization a one-slot remaining budget could
    // be rejected as though the two-member group needed two new workers.
...
export async function extendBudget(projectDir, rawInput = {}) {
  assertSafeInput(rawInput);
  const allowed = new Set(['max_worker_dispatches', 'max_active_seconds', 'reason']);
  const extras = Object.keys(rawInput).filter((key) => !allowed.has(key));
  if (extras.length > 0) throw new Error(`extend-budget has unknown fields: ${extras.join(', ')}`);
  if (typeof rawInput.reason !== 'string' || !rawInput.reason.trim() || Buffer.byteLength(rawInput.reason, 'utf8') > 4_096) {
    throw new Error('extend-budget requires a non-blank audit reason of at most 4096 UTF-8 bytes');
  }
  for (const key of ['max_worker_dispatches', 'max_active_seconds']) {
    const value = rawInput[key];
    if (value === undefined) continue;
    const maximum = key === 'max_worker_dispatches' ? 10_000 : 2_592_000;
    if (!Number.isInteger(value) || value < 1 || value > maximum) {
      throw new Error(`extend-budget ${key} must be an integer from 1 through ${maximum}`);
    }
  }
  if (rawInput.max_worker_dispatches === undefined && rawInput.max_active_seconds === undefined) {
    throw new Error('extend-budget requires max_worker_dispatches or max_active_seconds');
  }
  const paths = runtimePaths(projectDir);
  return withReceiptLock(paths, async () => {
    const state = await activeState(paths);
    ...
    if (TERMINAL_STATUSES.has(state.status)) {
      return {
        ok: false,
        reason: boundedGateSummary(`run ${state.run_id} is already ${state.status}`, 512),
      };
    }
    const before = structuredClone(state.execution_budget ?? null);
    const pausedInput = structuredClone(state.input_required ?? null);
    const at = now();
    const budget = extendExecutionBudgetState(state, rawInput, at);
    const receiptRetryRequired = pausedInput?.resume_action === 'retry_receipt';
    if (receiptRetryRequired) {
      const resumeStatus = state.status;
      const resumeStage = state.stage;
      state.status = 'input_required';
      state.stage = 'receipt-retry';
      state.input_required = {
        kind: 'receipt_retry',
        ticket_id: pausedInput.ticket_id,
        input_hash: pausedInput.input_hash,
        resume_status: resumeStatus,
        resume_stage: resumeStage,
        extended_at: at,
      };
      // Waiting for the parent to replay an already-attested receipt is a
      // control hold, not active execution time.
      syncExecutionBudgetClock(state, at);
    }
    await appendJsonLine(paths.overrideLog, {
      at,
      run_id: state.run_id,
      operation: 'extend-budget',
      reason: boundedGateSummary(rawInput.reason, 4_000),
      before,
      after: structuredClone(budget),
    });
    await persist(paths, state, treeShaSession(paths.root));
    return {
      ok: true,
      run: state,
      action: { type: 'budget_extended', before, after: structuredClone(budget) },
      ...(receiptRetryRequired
        ...
        : {}),
      next_safe_action: receiptRetryRequired
        ? `retry ape_run record with the identical attested receipt for ticket ${pausedInput.ticket_id}`
        : 'call ape_run next to continue the run',
    };
  }, { busyMessage: 'receipt effects are busy; retry ape_run extend-budget' });
}
