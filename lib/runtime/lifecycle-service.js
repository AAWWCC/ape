...
import { AUTO_MERGE_HOLD_REASON, CHECKS_REGISTRATION_RETRY_DELAY_MS, GATE_INLINE_GRACE_MS, GATE_NEXT_MAX_WAIT_MS, GATE_NEXT_POLL_FLOOR_MS, GATE_POLL_RETRY_DELAY_MS, MAX_REMEDIATION_CYCLES, RISK_TRIGGERS, RUNTIME_VERSION, SCHEMA_VERSION, SCOPE_EXPANSION_REASONS_MAX, SEALED_STATUSES, TERMINAL_STATUSES } from './constants.js';
  ...
  try {
    ...
    state = {
      ...
      policy: {
        ...
        max_remediation_cycles: config.policy?.max_remediation_cycles ?? MAX_REMEDIATION_CYCLES,
  ...
  return executeTaskOperationTransaction(projectDir, operation, async (paths, receiptLease) => {
    try {
      ...
      if (['record', 'recover-receipt'].includes(operation.action)) {
        try {
          assertSafeInput(operation.action === 'record'
            ? input.receipt ?? {}
            : {
