import { lstatFile } from './file-stats.js';
import { randomUUID } from 'node:crypto';
import path from 'node:path';
import { link, mkdir, realpath, rename, rm, stat } from 'node:fs/promises';
import { GATE_NEXT_MAX_WAIT_MS, GATE_NEXT_POLL_FLOOR_MS, RISK_TRIGGERS, RUNTIME_VERSION, SCHEMA_VERSION, SEALED_STATUSES, TERMINAL_STATUSES } from './constants.js';
import { runtimePaths, validateGovernedRuntimeAncestor } from './paths.js';
import { evaluateAdmission, inspectAdmissionRepository, reviewedAdmissionInput } from './admission.js';
import { projectRunResponse } from './projection.js';
...
import { finalizeReceipt, RunStartInputSchema, AnswerPreflightInputSchema } from './schemas.js';
import { currentBranch, currentCommitSha, currentTreeSha, diffFiles, remoteBranchTip, resolveBaseBranch, runGit, treeShaSession, workingTreeStatus } from './git.js';
import { snapshotEvidenceExecutables } from './hooks.js';
import { additiveCanonicalPathErrors, canonicalProjectRelativePathError, looksLikeTest, portableCaseFoldedPathIdentity, withinClaim, withinClaims } from './path-scope.js';
import { doctor } from './doctor.js';
import { inspectShippingAdmission, resolveFrozenShippingTarget } from './shipping-target.js';
...
import {
  ...
  recoverStaleDispatchIntentEvidence,
} from './claude-dispatch.js';
import { GENERAL_INPUT_MAX_BYTES, assertSafeInput, assertSafeReceiptInput, assertSafeReceiptRecoveryInput } from './input-guard.js';
import { executionConfigForRun, executionPolicySnapshot, pipelineLimits } from './pipeline-limits.js';
...
import {
  structuredSuccessorRefusal,
  successorGuidanceForState,
} from './successor-guidance.js';
import {
  ADMITTED_START_IDENTITY_VERSION,
  admittedStartIdentityHash,
} from './admitted-start-identity.js';
import { discoverRunReadiness, evaluateRunReadiness, snapshotRunCapabilities } from './readiness.js';
import { emptyOrchestrationTelemetry } from './orchestration-telemetry.js';
import { initializeRunContractManifest } from './run-contract.js';
import { validateCapabilityManifestGrowth } from './capability-manifest.js';
...
import { settleReceiptValidationSubagentStopLocked } from './receipt-service.js';
import { activeState, activeStateDiagnosisMatchesEntry, boundedGateSummary, checkoutCleanupIncomplete, dispatchLiveness, now } from './status-service.js';
import { applyActions, boundedSerialize, cleanupGateSuite, executeTaskOperationTransaction, lockRecoveryAuditLine, persist, prepareGovernedRuntimeAncestor, reconcileTerminalCheckout, recordReceiptLocked, recoverReceiptLocked, resumeLegacyBudgetContinuation, retireLegacyExecutionBudget, statusDocPath, withReceiptLock } from './receipt-service.js';
...
export async function prepareNativeBindingProbe(projectDir, input = {}) {
  ...
  if (existing && !SEALED_STATUSES.has(existing.status)) {
    return {
      ok: false,
      blocked: true,
  ...
  // `ape_run probe` is the explicit, pre-run repair boundary for stale
  // malformed dispatch evidence. Preserve every byte under a non-authority
  // `.corrupt-*` name before the production-path probe prepares its intent.
  await recoverStaleDispatchIntentEvidence(paths);
...
const WIN32_ACTIVE_QUARANTINE_RETRY_CODES = new Set(['EPERM', 'EACCES', 'EBUSY']);

// A forensic move must operate on the active.json directory entry itself.
// copyFile/readFile would follow a symlink and opening a FIFO could block, so
// Windows transient failures are retried as renames only; a still-busy entry is
// left untouched for an identical audited retry.
async function assertDiagnosedActiveStateEntry(activeFile, corruptError) {
  let currentEntry;
  try {
    currentEntry = await lstatFile(activeFile);
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
    throw new Error(
      'active run state changed after it was diagnosed; override reset did not move any state — retry against the current runtime state',
    );
  }
  if (!activeStateDiagnosisMatchesEntry(currentEntry, corruptError)) {
    throw new Error(
      'active run state changed after it was diagnosed; override reset did not move the replacement — retry against the current runtime state',
    );
  }
}

async function quarantineActiveStateEntry(activeFile, quarantineFile, corruptError) {
  for (let attempt = 0; ; attempt += 1) {
    // Audit I/O and Windows backoff both yield to external filesystem writers.
    // Recheck the diagnosed entry at each actual rename attempt.
    await assertDiagnosedActiveStateEntry(activeFile, corruptError);
    try {
      await rename(activeFile, quarantineFile);
      break;
    } catch (error) {
      if (
        process.platform !== 'win32' ||
        !WIN32_ACTIVE_QUARANTINE_RETRY_CODES.has(error?.code) ||
        attempt >= 10
      ) throw error;
      await new Promise((resolve) => setTimeout(resolve, 10 * (attempt + 1)));
    }
  }
  const captured = await lstatFile(quarantineFile);
  if (activeStateDiagnosisMatchesEntry(captured, corruptError, { afterRename: true })) return;

  // Even the final lstat/rename pair has a scheduling window. Restore a raced
  // replacement without overwriting any newer active entry. link() operates
  // on the captured entry itself, including symlinks, and never reads it.
  let restored = false;
  try {
    await link(quarantineFile, activeFile);
    restored = true;
  } catch {
    // Unsupported hard links, directories, or an occupied active path keep
    // the captured entry intact at the reported forensic path. Do not steal
    // a run lock or discard the current status projection in this case.
  }
  if (restored) await rm(quarantineFile, { force: true });
  throw new Error(
    restored
      ? 'active run state changed during quarantine; override reset restored the replacement and left its run lock untouched — retry against the current runtime state'
      : `active run state changed during quarantine; override reset preserved the replacement at ${quarantineFile} and left the run lock untouched — inspect the current runtime state before retrying`,
  );
}

// Corrupt-state reset (invariants 7/8): an unreadable active.json is an
// ownership artifact no reducer can transition, so a reset QUARANTINES it —
// moving corrupt bytes or the unsafe directory entry itself aside to a
// forensic active.json.corrupt-<ts> name (never deleting/following evidence),
// auditing the move like every override, stealing any associated run lock, and
// dropping the stale status.md so a subsequent start succeeds. Ordering is
// crash-conscious: the audit lands BEFORE the destructive move, and the lock
// steal precedes the status.md drop so a crash-interrupted retry re-enters the
// orphaned-lock arm (which now clears status.md too) while the lock survives.
async function quarantineCorruptState(paths, corruptError, reason) {
  ...
  await assertDiagnosedActiveStateEntry(paths.active, corruptError);
  ...
  // Rename the exact entry without opening it. This preserves a regular file's
  // bytes, a symlink as a symlink (without touching its target), and a FIFO or
  // other special-file node as inert forensic evidence.
  await quarantineActiveStateEntry(paths.active, quarantineFile, corruptError);
  // The state is unreadable, so its run_id is unknown: steal the lock by rename
...
export async function shouldTaskWrapApeRun(projectDir, input = {}) {
  if (['record', 'recover-receipt', 'regate', 'ship'].includes(input.action)) return true;
...
// Both snapshots belong to this task's charged effect while receipt-effects.lock
// is held. Unlocked status changes cannot prove which task created a watch.
async function withTaskGateAttribution(paths, operation, effect) {
  if (typeof operation.onGateCreated !== 'function') return effect();
  const before = await activeState(paths);
  try {
    return await effect();
  } finally {
    // An effect can persist a watch before a later step fails. Keep ownership
    // evidence for cancellation even then, without replacing the original fault
    // if the final state cannot be read safely.
    const after = await activeState(paths).catch(() => null);
    if (before?.run_id === operation.expectedRunId &&
        after?.run_id === operation.expectedRunId && after?.gates_watch &&
        !sameGateWatch(before.gates_watch, after.gates_watch)) {
      operation.onGateCreated({ runId: after.run_id, watch: { ...after.gates_watch } });
    }
  }
}
...
async function snapshotVerificationProfileRoots(projectDir, profiles) {
  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      unavailable.push({ id: profile.id, root, reason: 'root is missing or unreadable' });
      continue;
    }
    if (
      ...
      unavailable.push({ id: profile.id, root, reason: 'root must be a real contained directory without symlinks' });
      continue;
    }
    snapshots.push({
      id: profile.id,
...
function assertPlanContractPreflightEligible(input, classification) {
  if (input.plan_contract_version !== 2) return;
  if (
    input.mode !== 'phase' ||
    input.behavioral !== true ||
    !['fast', 'full'].includes(classification.lane)
  ) {
    throw new Error('plan_contract_version 2 requires a behavioral phase run in the fast or full lane so APE can schedule its required preflight; omit plan_contract_version (or use version 1) for non-behavioral and non-phase work');
  }
}

async function inspectRunReadiness(projectDir, input, config, classification, projection) {
  const roots = await snapshotVerificationProfileRoots(projectDir, config.verification?.profiles ?? []);
  const facts = { input, config, classification, projection,
    usable_verification_profile_ids: roots.snapshots.map((entry) => entry.id) };
  const initial = evaluateRunReadiness(facts);
  const readiness = initial.blocking.some((entry) => entry.code === 'missing-targeted-test-runner' || entry.code === 'missing-full-test-runner')
    ? evaluateRunReadiness({ ...facts, discovered: await discoverRunReadiness(projectDir) })
    : initial;
  return { ...readiness, verification_profile_roots: roots.snapshots, unavailable_verification_profiles: roots.unavailable };
}

function withConfiguredRunAutomation(input, config) {
  // The explicit APE invocation is already the human-intent boundary for the
  // complete run. When the repository has opted into automatic shipping, do
  // not ask for a second spelling of the same consent on this one start.
  if (
    ['phase', 'land'].includes(input.mode) &&
    config.shipping?.auto_merge === true &&
    input.explicit_invocation === true &&
    input.auto_merge_authorized !== true
  ) {
    return { ...input, auto_merge_authorized: true };
  }
  return input;
}

export async function previewRun(projectDir, rawInput) {
  const successorRefusal = structuredSuccessorRefusal(rawInput);
  if (successorRefusal) return successorRefusal;
  assertSafeInput(rawInput);
  let input = RunStartInputSchema.parse(rawInput);
  ...
  const config = await loadRuntimeConfig(paths.config);
  input = withConfiguredRunAutomation(input, config);
  const classification = classifyLane(await resolveLaneScope(paths.root, {
  ...
  assertPlanContractPreflightEligible(input, classification);

  const spec = pipelineRunSpec(input, classification, config);
  const projection = projectedPipeline(spec);
  const readiness = await inspectRunReadiness(paths.root, input, config, classification, projection);
  const health = await doctor(paths.root, input.mode === 'land' ? { ...input, behavioral: false } : input);
  const ticketDeadline = resolveTicketDeadline(config, input.mode, classification.lane);
  ...
  const blueprint = {
    readiness: { ...health, ...readiness, doctor: health },
  ...
  const admission = await evaluateAdmission({ root: paths.root, input, config, classification, projection, readiness, health });
  ...
  // Start reuses this read-only evaluator. A complete manifest that cannot be
  // delivered must be refused here, before a caller-supplied digest can reach
  // branch/ticket mutation, not only at the outer MCP response boundary.
  if (input.admission_contract_version === 1 || input.expected_admission_digest !== undefined) {
    const wire = projectRunResponse(result);
    if (wire.code === 'admission-response-too-large') return wire;
  }
  return result;
}
...
export async function startRun(projectDir, rawInput) {
  const successorRefusal = structuredSuccessorRefusal(rawInput);
  if (successorRefusal) return successorRefusal;
  // A legacy supersedes_run remains valid as historical metadata, but it may
  // name the active blocked run. Serialize that case with receipt effects too
  // so the decision cannot race a reset, receipt, or another start.
  if (!rawInput?.supersedes_run) {
    return startRunLocked(projectDir, rawInput);
  }
  // A rejected supersession must not create a receipt lock or follow a
  // redirected runtime ancestor before its ordinary admission checks. This
  // early check is read-only; the same checks run again inside the lock so
  // reset/receipt/start races remain serialized at the authoritative boundary.
  const prevalidated = await prevalidateStartAdmission(projectDir, rawInput);
  if (prevalidated.refusal) return prevalidated.refusal;
  const paths = runtimePaths(projectDir);
  return withReceiptLock(
    paths,
    () => startRunLocked(projectDir, rawInput),
    { busyMessage: 'run start is busy; retry the identical request' },
  );
}

async function prevalidateStartAdmission(projectDir, rawInput) {
  assertSafeInput(rawInput);
  const input = RunStartInputSchema.parse(rawInput);
  const refuse = (refusal) => ({ input, reviewed: null, refusal });
  const preflightPaths = runtimePaths(projectDir);
  // The supersession wrapper also admits legacy inputs, whose absence of a
  // versioned admission digest never permits locking through unsafe ancestors.
  await validateGovernedRuntimeAncestor(preflightPaths);
  const requiresAdmission = input.admission_contract_version === 1 || input.expected_admission_digest !== undefined;
  if (requiresAdmission) {
    const current = await activeState(preflightPaths);
    if (current?.status === 'blocked' && input.supersedes_run === current.run_id) {
      return refuse({ ok: false, blocked: true, attempts_consumed: 0,
        errors: ['an active blocked run cannot be superseded by a new start; after explicit operator direction use audited override reset, then start an ordinary fresh run'] });
    }
  }
  if (requiresAdmission && !input.expected_admission_digest) {
    return refuse({ ok: false, blocked: true, code: 'admission-preview-required', attempts_consumed: 0,
      reason: 'Call preview with these complete run inputs, review its admission manifest, then supply expected_admission_digest to start. The digest binds inputs, not independent human authorization.' });
  }
  const prospective = requiresAdmission ? await previewRun(projectDir, input) : null;
  if (prospective?.code === 'admission-response-too-large') return refuse(prospective);
  if (prospective && !('admission' in prospective)) return refuse(prospective);
  const reviewed = prospective && 'admission' in prospective ? prospective : null;
  if (reviewed && reviewed.admission_digest !== input.expected_admission_digest) {
    return refuse({ ...reviewed, ok: false, blocked: true, code: 'admission-drift', attempts_consumed: 0,
      reason: 'Admission inputs changed after preview; review a fresh preview before starting. No branch, ticket, or worker was created.' });
  }
  if (reviewed && !reviewed.admission.ready) {
    return refuse({ ...reviewed, ok: false, blocked: true, code: 'admission-not-ready', attempts_consumed: 0,
      reason: 'Resolve the exact admission prerequisites before starting; no worker has been dispatched.' });
  }
  return { input, reviewed, refusal: null };
}

async function startRunLocked(projectDir, rawInput) {
  const prevalidated = await prevalidateStartAdmission(projectDir, rawInput);
  if (prevalidated.refusal) return prevalidated.refusal;
  let input = prevalidated.input;
  const reviewed = prevalidated.reviewed;
  const testPathAdmission = capabilityTestPathBoundErrors(input.test_paths);
  if (!testPathAdmission.valid) {
    throw new Error(
      `test_paths failed the canonical bounded admission contract: ${testPathAdmission.errors.join('; ')}`,
    );
  }
  const supersedesRun = input.supersedes_run;
  ...
  const paths = runtimePaths(projectDir);
  // Validate/create the governed state ancestors before any roadmap read,
  // run-lock write, or dispatch recovery can follow a repository-planted
  // `.ape`/`runtime` symlink outside the project.
  await prepareGovernedRuntimeAncestor(paths);
  await assertRoadmapRequirementsReady(paths, input.requirements, { phase: 'start' });
  const existing = await activeState(paths);
  if (
    existing?.status === 'blocked' &&
    input.supersedes_run === existing.run_id
  ) {
    return {
      ok: false,
      blocked: true,
  ...
  const config = await loadRuntimeConfig(paths.config);
  input = withConfiguredRunAutomation(input, config);
  const startRequestHash = sha256(reviewedAdmissionInput(input));
  const classification = classifyLane(await resolveLaneScope(paths.root, {
    requested_lane: input.lane,
    claimed_paths: input.claimed_paths,
    test_paths: input.test_paths,
  ...
  // would spend stage retries on an identical ticket. Debug/spike
  ...
  // A clean unborn repository is a legitimate bootstrap target. There is no
  // ref for resolveBaseBranch to discover yet, so preserve the checked-out
  // branch name and create the empty root commit only after the run lock and
  // (for Codex) live binding proof are secured below. Existing repositories
  // retain the remote-first base resolution path byte-for-byte.
  const resolvedHead = await runGit(paths.root, ['rev-parse', '--verify', 'HEAD'])
    .catch(() => null);
  const symbolicHead = resolvedHead === null
    ? await runGit(paths.root, ['symbolic-ref', '-q', 'HEAD']).catch(() => null)
    : null;
  const unbornRepository = resolvedHead === null && symbolicHead === `refs/heads/${startingBranch}`;
  if (resolvedHead === null && !unbornRepository) {
    throw new Error('startRun cannot resolve HEAD and cannot prove a clean unborn branch');
  }
  if (unbornRepository && input.mode === 'land') {
    throw new Error('mode land requires an existing commit; use phase mode to bootstrap an unborn repository');
  }
  const base = unbornRepository
    ? { branch: startingBranch, start_point: `refs/heads/${startingBranch}` }
    : await resolveBaseBranch(paths.root);
  let baseCommitSha = unbornRepository
    ? null
    : await runGit(paths.root, ['rev-parse', base.start_point]);
  if (
    ['phase', 'land'].includes(input.mode) &&
    config.shipping?.auto_merge === true &&
    input.auto_merge_authorized === true &&
    base.start_point === `refs/remotes/origin/${base.branch}`
  ) {
    const advertisedTip = await remoteBranchTip(paths.root, 'origin', base.branch);
    if (advertisedTip !== baseCommitSha.toLowerCase()) {
      throw new Error(`auto-merge start refused because origin/${base.branch} is stale locally at ${baseCommitSha} while the remote advertises ${advertisedTip}; run git fetch origin ${base.branch} and retry`);
    }
  }
  const verificationProfileRoots = input.plan_contract_version === 2 ? readiness.verification_profile_roots : [];
  if (reviewed && sha256(readiness.verification_profile_roots) !== sha256(reviewed.admission.verification_profile_roots ?? [])) {
    return { ok: false, code: 'admission-drift', attempts_consumed: 0,
      reason: 'Verification profile root identities changed after the reviewed preview; preview the current available capabilities again.' };
  }
  if (input.mode === 'land') {
    const currentCommit = await currentCommitSha(paths.root);
    const descendsFromBase = currentCommit === baseCommitSha || await runGit(
      paths.root,
      ['merge-base', '--is-ancestor', baseCommitSha, currentCommit],
    ).then(() => true, () => false);
    if (!descendsFromBase) {
      throw new Error(`mode land requires the finished diff to descend from the resolved default branch tip (${base.branch} at ${baseCommitSha}); current HEAD is ${currentCommit}. Rebase the feature branch onto ${base.branch}, then retry`);
    }
    // Friction #32 (gate-and-land): validate the whole finished feature diff,
    // including already-committed work and any dirty finishing edits, against
    // the resolved default tip. Requiring HEAD itself to equal that tip forced
    // callers to reconstruct a normal feature-branch change as an uncommitted
    // worktree patch before APE would review it.
    const baseTreeSha = await runGit(paths.root, ['rev-parse', `${baseCommitSha}^{tree}`]);
    ...
    if (changedFiles.length === 0) {
      throw new Error(`mode land requires a non-empty finished diff against ${base.branch}: land gates and lands finished work; implement the change first, or use the building mode (phase)`);
    }
  ...
  try {
    if (reviewed) {
      const currentConfig = await loadRuntimeConfig(paths.config);
      const currentAdmission = await evaluateAdmission({ root: paths.root, input, config: currentConfig, classification, projection, readiness, health });
      if (currentAdmission.admission_digest !== reviewed.admission_digest) {
        await releaseRunLock(paths.lock, runId);
        return { ok: false, blocked: true, code: 'admission-drift', attempts_consumed: 0,
          reason: 'Admission changed while start acquired its lock; review a fresh preview.', ...currentAdmission };
      }
    }
    // Receipt publication is a first-class part of every run generation. Make
    // the empty store durable at admission so later rejection/recovery checks
    // can compare it without treating an absent directory as a special state.
    await mkdir(paths.receipts, { recursive: true });
    // START is also an explicit no-live-run maintenance boundary. Repair stale
    // malformed dispatch storage before any intent write so Claude and Codex
    // cannot be permanently wedged by a prior torn file or redirected
    // container. Valid historical evidence is left byte-for-byte intact.
    await recoverStaleDispatchIntentEvidence(paths);
    if (requiresBindingProbe) {
    ...
    if (unbornRepository) {
      if ((await runGit(paths.root, ['ls-files', '--stage'], { env: { GIT_OPTIONAL_LOCKS: '0' } })).length > 0) {
        throw new Error('unborn bootstrap requires an empty real Git index, including APE metadata; preserve pre-existing staged content and obtain an operator decision before retrying');
      }
      const bootstrapDirt = (await workingTreeStatus(paths.root))
        .map((line) => line.slice(3))
        .filter((file) => !file.startsWith('.ape/'));
      if (bootstrapDirt.length > 0) {
        throw new Error(`unborn repository bootstrap requires a clean working tree; dirty paths: ${bootstrapDirt.join(', ')}`);
      }
      await runGit(paths.root, [
        'commit',
        '--allow-empty',
        '-m',
        'Initialize repository for APE',
      ]);
      baseCommitSha = await currentCommitSha(paths.root);
      const postCommitDirt = (await workingTreeStatus(paths.root))
        .map((line) => line.slice(3))
        .filter((file) => !file.startsWith('.ape/'));
      if (postCommitDirt.length > 0) {
        throw new Error(`repository bootstrap hook changed the working tree; inspect before retrying: ${postCommitDirt.join(', ')}`);
      }
    }
    // Every host and mode receives an isolated APE-owned branch. Ordinary runs
    // start at the resolved default tip; land starts at the admitted feature
    // commit so already-committed work remains present, while Git preserves any
    // admitted dirty finishing edits across the switch.
    if (reviewed && !unbornRepository) {
      const finalAdmission = await evaluateAdmission({ root: paths.root, input, config: await loadRuntimeConfig(paths.config), classification, projection, readiness, health });
      if (finalAdmission.admission_digest !== reviewed.admission_digest) {
        throw new Error('admission drifted during probe consumption; no branch or worker was created. Preserve the tree and review a fresh preview; a new probe may be required');
      }
    }
    const branch = `ape/${input.mode}-${runId.slice(-8)}`;
    await runGit(paths.root, ['switch', '-c', branch, landStartPoint ?? baseCommitSha, '--no-track']);
    if (reviewed) {
      const afterSwitch = await inspectAdmissionRepository(paths.root, input);
      const original = reviewed.admission.repository;
      const drift = afterSwitch.head !== (landStartPoint ?? baseCommitSha) ||
        afterSwitch.branch !== branch || sha256(await loadRuntimeConfig(paths.config)) !== sha256(config) ||
        (input.mode !== 'land' ? afterSwitch.dirty_paths.length > 0 :
          afterSwitch.index_digest !== original.index_digest || sha256(afterSwitch.changed_content) !== sha256(original.changed_content));
      if (drift) throw new Error(`repository changed during the admitted branch switch; no ticket or worker was created. The branch ${branch} and all current files are preserved for operator inspection`);
    }
    ...
    state = {
      ...
      // Durable operational provenance is stamped once at admission. Never
      // infer it later from whichever APE binary happens to archive an old
      // active state: an upgraded runtime may legitimately finish a run that
      // began under an earlier contract.
      ...runVersionProvenance(input.host),
      run_id: runId,
      status: 'starting',
      stage: 'start',
      ...
      behavioral: input.behavioral,
      // Frozen independently from behavioral: green-maintenance is an
      // explicit test-authoring contract, never inferred from the objective
      // or changed by preflight answers and later scope growth.
      test_intent: input.test_intent,
      ...
      policy: {
        ...pipelineLimits({ policy: config.policy }),
        high_risk_security_review: config.policy?.high_risk_security_review !== false,
      ...
      start_config_hash: sha256(config),
      capability_snapshot: snapshotRunCapabilities(readiness),
      ...(reviewed ? { admission: { version: 1, digest: reviewed.admission_digest, manifest: reviewed.admission },
        ...(reviewed.admission.shipping_target ? { shipping_target: structuredClone(reviewed.admission.shipping_target) } : {}) } : {}),
      host: input.host,
      ...(input.binding_protocol ? { binding_protocol: input.binding_protocol } : {}),
      ...(input.auto_merge_authorized === true ? { auto_merge_authorized: true } : {}),
       ...(input.plan_contract_version
        ? { plan_contract_version: input.plan_contract_version }
        : {}),
      ...(input.plan_contract_version === 2
        ? {
            verification_profiles: structuredClone(readiness.capabilities.verification_profiles),
      ...
      // historical run this start relates to. Legacy supersedes_run remains
      // audit metadata and cannot bypass an active blocked run.
      ...(supersedesRun ? { supersedes_run: supersedesRun } : {}),
    ...
    // New native runs seal their complete capability catalog before the first
    // ticket is issued. Only the compact pointer enters state; legacy runs do
    // not acquire a field, artifact, or ticket-hash change.
    await initializeRunContractManifest(paths, state, createdAt);
    const admittedRunContract = Reflect.get(state, 'run_contract');
    Object.assign(state, {
      ...(admittedRunContract
        ? { admitted_run_contract: structuredClone(admittedRunContract) }
        : {}),
      admitted_start_identity_version: ADMITTED_START_IDENTITY_VERSION,
    });
    Object.assign(state, {
      admitted_start_identity_hash: admittedStartIdentityHash(state),
    });
    const initialAdmission = landAdmission;
    if (initialAdmission) {
      ...
      const admission = finalizeReceipt({
        ...
        status: 'passed',
        base_tree_sha: initialAdmission.base_tree_sha,
        head_tree_sha: initialAdmission.head_tree_sha,
        changed_files: initialAdmission.changed_files,
        tests: [],
        findings: [],
        evidence: {
          land_admission: {
...
async function nextRunLocked(paths, options = {}) {
    await settleReceiptValidationSubagentStopLocked(paths);
    const state = await activeState(paths);
    if (!state) return { ok: false, reason: 'no active run' };
    const at = now();
    if (TERMINAL_STATUSES.has(state.status)) {
      const rejection = reduceRun(state, { type: 'NEXT', at })
        .find((action) => action.type === 'reject');
      return {
        ok: false,
        reason: rejection?.reason
          ?? boundedGateSummary(`run ${state.run_id} is already ${state.status}`, 512),
      };
    }
    const retiredLegacyBudget = retireLegacyExecutionBudget(state);
    if (retiredLegacyBudget) {
      await persist(paths, state, treeShaSession(paths.root));
    }
    if (state.status === 'input_required') {
      if (state.input_required?.kind === 'receipt_retry') {
        return {
          ok: false,
          run: state,
          // prose-bound-exempt: ticket_id is a runtime-issued immutable id
          // constrained by StageTicketSchema, never agent-authored prose.
          reason: `retry ape_run record with the identical attested receipt for ticket ${state.input_required.ticket_id}`,
      ...
      return {
        ok: true,
        run: state,
        actions: reduceRun(state, { type: 'NEXT', at }),
      };
    }
    const config = await loadRuntimeConfig(paths.config);
    if (state.budget_continuation) {
      if (state.budget_continuation.source === 'receipt-protocol-budget-recovery') {
        const recoveryTicketIds = new Set(
          state.budget_continuation.actions
            ?.filter((action) => action?.type === 'dispatch_agent')
            .map((action) => action.ticket_id),
        );
        const stillRunning = (await dispatchIntentStatuses(paths, state)).find((entry) =>
          recoveryTicketIds.has(entry.ticket_id) &&
          !['missing', 'expired'].includes(entry.status) &&
        ...
        if (stillRunning) {
          return {
            ...
            actions: [{
              ...
              ticket_id: stillRunning.ticket_id,
              agent_state: stillRunning.agent_state ?? 'not-bound',
              reason: 'a legacy receipt correction is waiting for the original physical worker to stop before APE redispatches the same immutable ticket',
            }],
            next_action: {
              kind: 'wait',
              ticket_id: stillRunning.ticket_id,
      ...
      const actions = await resumeLegacyBudgetContinuation(
        paths,
        state,
        config,
        treeShaSession(paths.root),
      );
      return { ok: true, run: state, actions: actions ?? [] };
    }
    ...
    // NEXT, not only RESUME, can observe a partial parallel group: one ticket
    // is already bound while a sibling never launched (for example the parent
    // process died between two native spawn calls). Resolve current liveness
    // before replay so an already-live worker is represented as
    // dispatch_pending and is not prepared as a duplicate physical dispatch.
    const liveTicketIds = options.live_ticket_ids instanceof Set
      ? options.live_ticket_ids
      : (await dispatchLiveness(paths, state)).live_ticket_ids;
    if (liveTicketIds.size > 0) {
      actions = actions.map((entry) =>
        entry.type === 'dispatch_agent' && liveTicketIds.has(entry.ticket_id)
          ? {
    ...
    const emitted = await applyActions(paths, state, actions, config);
...
export async function nextRun(projectDir, options = {}) {
  const paths = runtimePaths(projectDir);
  // SubagentStop stays dependency-light: the hook only validates/attests the
  // final draft and stamps the exact native worker stopped. The parent-owned
  // NEXT boundary settles any resulting protocol exhaustion under the normal
  // receipt-effects lock before it advances or redispatches the run.
  // nextRunLocked performs that settlement inside the same critical section
  // used by ordinary NEXT and task-backed NEXT.
...
// Task-only entry point for the gate-crossing ape_run actions. The
...
export async function executeApeRunTaskOperation(projectDir, operation) {
  ...
  const paths = runtimePaths(projectDir);
  const receiptTicketId = receiptTicketIdValue(input.receipt);
  const capabilityHash = await readDispatchReceiptCapabilityHash(paths, receiptTicketId);
  const authoritativeBearer = authenticatedReceiptBearer(input.receipt, capabilityHash);
  const redactionBearer = authoritativeBearer;
  if (operation?.action === 'next') {
    return executeNextTaskOperation(projectDir, operation, {
      taskOperationRunBindingRefusal,
      nextRunLocked: (heldPaths) => withTaskGateAttribution(heldPaths, operation, () => nextRunLocked(heldPaths)),
    });
  }
  return executeTaskOperationTransaction(projectDir, operation, async (paths, receiptLease) => {
    try {
      ...
      return await withTaskGateAttribution(paths, operation, async () => {
      if (
        ['record', 'recover-receipt'].includes(operation.action) &&
        capabilityHash !== null &&
        authoritativeBearer === null
      ) {
        return {
          ok: false,
          rejected: true,
          errors: ['receipt capability does not match the exact bound physical dispatch'],
          ...(receiptTicketId ? { ticket_id: receiptTicketId } : {}),
        };
      }
      if (['record', 'recover-receipt'].includes(operation.action)) {
        try {
          if (operation.action === 'record') {
            assertSafeReceiptInput(input.receipt ?? {});
          } else {
            assertSafeReceiptRecoveryInput(input.receipt ?? {}, {
              ...(input.receipt_input_hash !== undefined
                ? { receipt_input_hash: input.receipt_input_hash }
                : {}),
              // prose-bound-exempt: input-guard inspection only; this
              // object is not persisted or rendered, and the recovery
              // service bounds the reason independently at its sink.
              ...(input.reason !== undefined ? { reason: input.reason } : {}),
            });
          }
        } catch (cause) {
          if (redactionBearer === null) {
            return taskToolError(new Error(
              'receipt input failed safety validation before its exact capability was authenticated',
            ));
          }
          throw cause;
        }
      ...
      if (operation.action === 'record') {
        ...
        const ticket = state?.tickets?.find(
          (entry) => entry.ticket_id === input.receipt?.ticket_id,
        );
        const { input: receipt, normalized_fields } = normalizeReceiptInput(
          input.receipt ?? {},
          { receipt_contract_version: ticket?.receipt_contract_version },
        );
        return redactReceiptBearer(
          await recordReceiptLocked(projectDir, receipt, normalized_fields, { receipt_lease: receiptLease }),
          redactionBearer,
        );
      }
      if (operation.action === 'recover-receipt') {
        return redactReceiptBearer(
          await recoverReceiptLocked(projectDir, input.receipt ?? {}, {
            ...(input.receipt_input_hash !== undefined
              ? { receipt_input_hash: input.receipt_input_hash }
              : {}),
            // prose-bound-exempt: this is an internal propagation into
            // recoverReceiptLocked, which validates the string and routes it
            // through boundedGateSummary before constructing any persisted sink.
            ...(input.reason !== undefined ? { reason: input.reason } : {}),
          }, receiptLease),
          redactionBearer,
        );
      ...
      if (redactionBearer === null) return taskToolError(cause);
      const message = cause instanceof Error ? cause.message : String(cause);
      return taskToolError(new Error(redactReceiptBearer(message, redactionBearer)));
    }
  });
}
...
export async function resumeRun(projectDir) {
  const paths = runtimePaths(projectDir);
  // Resume can recover the run lock and revoke dispatch intents before NEXT.
  // Keep its state/liveness snapshot and every recovery effect in the same
  // transaction as abort, reset and receipt recording. Otherwise an abort can
  // release the lock between this snapshot and recovery, then resume recreates
  // a live lock for the sealed run before NEXT notices that it was aborted.
  return withReceiptLock(paths, () => resumeRunLocked(paths), {
    busyMessage: 'receipt effects are busy; retry ape_run resume',
  });
}

async function resumeRunLocked(paths) {
  // A host may be restarted after SubagentStop persisted receipt-validation
  // exhaustion but before the parent called NEXT. Reconcile that durable
  // observation first so RESUME has the same bounded same-ticket recovery and
  // second-worker terminal block semantics as NEXT.
  await settleReceiptValidationSubagentStopLocked(paths);
  const state = await activeState(paths);
  if (!state) return { ok: false, reason: 'no active run' };
  if (checkoutCleanupIncomplete(state)) {
    ...
    try {
      ...
      return {
        ...
        run: state,
        dispatch_state: 'none',
        resume_state: state.checkout_cleanup.status === 'returned'
          ? 'checkout-returned'
          : 'checkout-retained',
        actions: [{ type: 'checkout_cleanup', result: state.checkout_cleanup }],
      };
    } finally {
      await releaseRunLock(paths.lock, state.run_id);
    }
  }
  let before = await dispatchLiveness(paths, state);
  ...
  if (resumable && !before.live_lock) {
    ...
    // A dead parent owner does not attest that its native children stopped.
    // Restoring the lock restores the second half of their live binding;
    // recompute before deciding which pending intents need replacement.
    before = await dispatchLiveness(paths, state);
  }
  ...
  if (state.status === 'shipping' && state.shipping_watch) {
    ...
    return {
      ...
      actions: [{
        ...
        reason: `remote checks in progress for ${state.shipping_watch.pr_url}${summary ? ` (${summary})` : ''}; call ape_run next with wait_ms: 300000 for bounded server-side polling`,
      }],
    };
  }
  ...
  if (state.status === 'gating' && state.gates_watch) {
    ...
    return {
      ...
      actions: [{
        ...
        reason: `the merge-gate suite is still running${summary ? ` (${summary})` : ''}; call ape_run next with wait_ms: 300000 for bounded server-side polling`,
      }],
    };
  }
  if (state.status === 'running') {
    ...
    for (const ticketId of before.pending_ticket_ids) {
      if (!before.live_ticket_ids.has(ticketId) && !awaitingRetirement.has(ticketId)) {
        // Binding runs under the dispatch lock independently of receipt
        // effects. A launch that bound after the liveness snapshot must retain
        // its worker; only the explicit expire-dispatch lever forces revocation.
        await expireClaudeIntent(paths, ticketId, { preserveActiveBound: true });
      }
    }
    // The guarded expiry may have preserved a newly bound worker. Refresh the
    // dispatch set before NEXT so it observes that same physical identity.
    before = await dispatchLiveness(paths, state);
    const result = await nextRunLocked(paths, { live_ticket_ids: before.live_ticket_ids });
    if (!result.ok) return result;
    const allLive = before.dispatch_state === 'live';
    return {
      ...result,
      dispatch_state: allLive ? 'live' : 'needs-redispatch',
      resume_state: allLive ? resumeState : 'recovered-orphan',
    };
  }
  return nextRunLocked(paths);
}
...
function corruptActiveStateCause(corruptError) {
  if (corruptError.variant === 'schema-invalid') return 'schema-invalid';
  if (corruptError.variant === 'unsafe') return 'unsafe';
  if (corruptError.variant === 'oversized') return 'oversized';
  return 'corrupt and unparseable';
}

function unconfirmableCorruptStateRefusal(run_id, corruptError) {
  const cause = corruptActiveStateCause(corruptError);
  return {
    ok: false,
    // prose-bound-exempt: echoRunId wraps boundedSerialize before returning
    ...
    // already-bounded value or a fixed schema/unsafe/oversized diagnostic,
...
async function shipRunLocked(paths, reason) {
  if (!reason?.trim()) throw new Error('ship requires an audit reason');
    ...
    // attacker-controlled text.
    if (rejection) return { ok: false, reason: rejection.reason };
    // Lifecycle eligibility is decided first. SHIP may prove prerequisites
    // for an already-admitted target, never certify or replace a legacy one.
    try {
      await resolveFrozenShippingTarget(paths.root, state, config);
    } catch {
      return { ok: false, code: 'shipping-admission-invalid', attempts_consumed: 0,
        reason: 'SHIP could not validate the frozen admission. Preserve this run and restore its approved configuration, or obtain a fresh reviewed admission with operator approval. No legacy target is upgraded and no gates or shipping effects were started.' };
    }
    const shippingAdmission = await inspectShippingAdmission(paths.root, { ship_requested: true }, config);
    if (!shippingAdmission.ready || sha256(shippingAdmission.shipping_target ?? null) !== sha256(state.shipping_target)) {
      return { ok: false, code: 'shipping-prerequisites-unverified', attempts_consumed: 0,
        reason: 'SHIP prerequisites are not proven for the existing admitted target; preserve this run and resolve the listed prerequisite before retrying. No gates or shipping effects were started.',
        prerequisites: shippingAdmission.prerequisites ?? null,
        blocking: shippingAdmission.blocking };
    }
    const emitted = await applyActions(paths, state, actions, config);
    const successorGuidance = successorGuidanceForState(state, config);
    return {
          ...
          reason:
            error.variant === 'schema-invalid'
              ...
              : error.variant === 'unsafe'
                ? `active run state path is unsafe (${error.parse_error}); only override reset can clear it — it quarantines the unsafe entry without following it, so ${operation} cannot operate on unsafe state`
                : error.variant === 'oversized'
                  ? `active run state is oversized (${error.parse_error}); only override reset can clear it — it quarantines the oversized bytes with your audit reason, so ${operation} cannot operate on oversized state`
...
function answerPreflightPath(value, label) {
  if (
    ...
    canonicalProjectRelativePathError(value)
...
export async function answerPreflight(projectDir, input) {
  ...
  if (typeof input.reason !== 'string' || !input.reason.trim() || input.reason.length > GENERAL_INPUT_MAX_BYTES) {
    throw new Error(`answer-preflight requires a non-empty audit reason within the ${GENERAL_INPUT_MAX_BYTES}-byte input envelope`);
  }
  input = AnswerPreflightInputSchema.parse(input);
  if (Buffer.byteLength(JSON.stringify(input.answers), 'utf8') > 65_536) throw new Error('answer-preflight answers exceed the 65536-byte bound');
  for (const [label, values] of [['claimed_paths', input.claimed_paths ?? []], ['test_paths', input.test_paths ?? []]]) {
    for (const value of values) answerPreflightPath(value, label);
  }
  const paths = runtimePaths(projectDir);
  return withReceiptLock(paths, async () => {
    const state = await activeState(paths, { requireActionable: true });
    // run_id is an exact-active-run confirmation, not a selector. Check it
    // from the state read under the receipt lock and before any lifecycle
    // eligibility check, so a stale answer cannot be misdiagnosed as merely
    // being at the wrong stage. Explicit null is an invalid supplied aim, not
    // an omission. Bound both echoes because active.json is read as evidence
    // here and may itself be malformed.
    if (
      ...
      const activeRunId = state ? echoRunId(state.run_id) : '(none)';
      throw new Error(
        `answer-preflight run_id confirmation mismatch: provided ${echoRunId(input.run_id)}; active ${activeRunId}`,
      );
    }
    ...
    for (const answer of input.answers) {
      if (!answer || typeof answer !== 'object' || Array.isArray(answer) || Object.keys(answer).some((key) => !['id', 'answer'].includes(key)) || typeof answer.answer !== 'string' || !answer.answer.trim()) {
    ...
    const pathErrors = [
      ...additiveCanonicalPathErrors(state.claimed_paths ?? [], claimed, { label: 'claimed_paths' }),
      ...additiveCanonicalPathErrors(state.test_paths ?? [], tests, { label: 'test_paths' }),
    ];
    if (pathErrors.length > 0) throw new Error(`answer-preflight scope additions are invalid: ${pathErrors.join('; ')}`);
    ...
    const scopeOverlap = finalClaimed.find((productionPath) => finalTests.some((testPath) => {
      const production = portableCaseFoldedPathIdentity(productionPath);
      const test = portableCaseFoldedPathIdentity(testPath);
      return withinClaim(production, test) || withinClaim(test, production);
    }));
    if (scopeOverlap) {
    ...
    const reclassification = escalateLane(state.lane, await resolveLaneScope(paths.root, {
      claimed_paths: finalClaimed,
      test_paths: finalTests,
      test_intent: state.test_intent,
      behavioral: state.behavioral === true,
      risk_triggers: [...new Set([...(state.risk_triggers ?? []), ...addRisks])],
    }), config.policy);
    const finalRisks = [...new Set([...(state.risk_triggers ?? []), ...addRisks])];
    const capabilityGrowth = validateCapabilityManifestGrowth(state, {
      claimed_paths: finalClaimed,
      test_paths: finalTests,
      risk_triggers: finalRisks,
      high_risk: state.high_risk === true || finalRisks.length > 0,
      lane: reclassification.lane,
    });
    if (!capabilityGrowth.valid) {
      throw new Error(
        `answer-preflight would exceed the immutable capability contract: ${capabilityGrowth.errors.join('; ')}`,
      );
    }
    const actions = reduceRun(state, {
      type: 'PREFLIGHT_ANSWERED',
      preflight_hash: input.preflight_hash,
      reason: boundedGateSummary(input.reason, GENERAL_INPUT_MAX_BYTES),
