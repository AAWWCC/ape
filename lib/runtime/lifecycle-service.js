...
import { PLAN_ARTIFACT_MARKER_BRAND, PLAN_ARTIFACT_MAX_CHARS, PLAN_ARTIFACT_MAX_ENTRIES, SCOPE_EXPANSION_PATHS_MAX, SCOPE_EXPANSION_PATH_MAX_CHARS, SCOPE_EXPANSION_REASON_MAX_CHARS, applyActions, boundedSerialize, cleanupGateSuite, executeTaskOperationTransaction, lockRecoveryAuditLine, persist, reconcileTerminalCheckout, recordReceipt, recordReceiptLocked, recoverReceiptLocked, resumeLegacyBudgetContinuation, retireLegacyExecutionBudget, settleReceiptValidationSubagentStop, statusDocPath, withReceiptLock } from './receipt-service.js';
...
function runProjectionSpec(input, classification, config) {
  return {
    mode: input.mode,
...
export async function previewRun(projectDir, rawInput) {
  ...
  const spec = runProjectionSpec(input, classification, config);
...
async function startRunLocked(projectDir, rawInput) {
  ...
  const config = await loadRuntimeConfig(paths.config);
  input = withConfiguredRunAutomation(input, config);
  const startRequestHash = sha256(input);
  const classification = classifyLane({
    requested_lane: input.lane,
    claimed_paths: input.claimed_paths,
    test_paths: input.test_paths,
  ...
  assertPlanContractPreflightEligible(input, classification);
  const projection = projectedPipeline(runProjectionSpec(input, classification, config));
  const readiness = await inspectRunReadiness(paths.root, input, config, classification, projection);
  if (
    !readiness.ready &&
    (input.capability_contract_required === true || input.required_capabilities.length > 0)
  ) {
    return {
      ok: false,
      blocked: true,
