...
async function inspectRunReadiness(projectDir, input, config, classification, projection) {
  const initial = evaluateRunReadiness({ input, config, classification, projection });
  if (!initial.blocking.some((entry) => entry.code === 'missing-targeted-test-runner' || entry.code === 'missing-full-test-runner')) {
    return initial;
  }
  return evaluateRunReadiness({
    ...
    classification,
    projection,
    discovered: await discoverRunReadiness(projectDir),
  });
}
