...
// Shared by the producer template and the authoritative consumer schema.
// Exporting the existing ceilings does not enlarge any admitted plan.
...
const boundedText = z.string().min(1).max(PLAN_CONTRACT_LIMITS.text_chars).refine((value) => value.trim().length > 0, {
...
const PlanRequirementSchema = z.object({
  ...
  workstreams: z.array(contractId).min(1).max(PLAN_CONTRACT_LIMITS.workstreams),
}).strict();
...
const PlanWorkstreamV1Schema = z.object({
  id: contractId,
  outcome: boundedText,
  paths: z.array(PlanPathSchema).min(1).max(PLAN_CONTRACT_LIMITS.paths_per_workstream),
  steps: z.array(boundedText).min(1).max(PLAN_CONTRACT_LIMITS.list_items),
  acceptance: z.array(boundedText).min(1).max(PLAN_CONTRACT_LIMITS.list_items),
  evidence_commands: z.array(boundedText).min(1).max(PLAN_CONTRACT_LIMITS.list_items),
}).strict();

const PlanWorkstreamV2Schema = PlanWorkstreamV1Schema.extend({
  verification_profiles: z.array(z.string().regex(VERIFICATION_PROFILE_ID_PATTERN)).max(PLAN_CONTRACT_LIMITS.verification_profiles),
}).strict();
...
const PlanAssuranceSchema = z.object({
  ...
  failure_modes: z.array(boundedText).min(1).max(PLAN_CONTRACT_LIMITS.list_items),
  crash_recovery: boundedText,
  migration: boundedText,
  determinism: boundedText,
  executable_tests: z.array(boundedText).min(1).max(PLAN_CONTRACT_LIMITS.list_items),
}).strict();
...
export const PreflightArtifactSchema = z.object({
  version: z.literal(1),
...
const PlanContractV1ShapeSchema = z.object({
  version: z.literal(PLAN_CONTRACT_VERSION),
  requirements: z.array(PlanRequirementSchema).min(1).max(PLAN_CONTRACT_LIMITS.requirements),
  workstreams: z.array(PlanWorkstreamV1Schema).min(1).max(PLAN_CONTRACT_LIMITS.workstreams),
  risks: z.array(PlanRiskSchema).max(PLAN_CONTRACT_LIMITS.list_items),
  non_goals: z.array(boundedText).max(PLAN_CONTRACT_LIMITS.list_items),
}).strict();

const PlanContractV2ShapeSchema = z.object({
  version: z.literal(CURRENT_PLAN_CONTRACT_VERSION),
  preflight_hash: digest,
  requirements: z.array(PlanRequirementSchema).min(1).max(PLAN_CONTRACT_LIMITS.requirements),
  workstreams: z.array(PlanWorkstreamV2Schema).min(1).max(PLAN_CONTRACT_LIMITS.workstreams),
  risks: z.array(PlanRiskSchema).max(PLAN_CONTRACT_LIMITS.list_items),
  assurances: z.array(PlanAssuranceSchema).max(PLAN_CONTRACT_LIMITS.list_items).optional(),
  non_goals: z.array(boundedText).max(PLAN_CONTRACT_LIMITS.list_items),
}).strict();
...
export const PlanContractV1Schema = PlanContractV1ShapeSchema.superRefine(refinePlan);
export const PlanContractV2Schema = PlanContractV2ShapeSchema.superRefine(refinePlan);
...
export function planContractSchemaForVersion(version) {
