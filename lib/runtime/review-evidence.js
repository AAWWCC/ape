...
export const REVIEW_FINDING_EVIDENCE_MAX = 16;
...
function findingAnchor(finding) {
  ...
  const normalizedFile = file ? flattenReviewText(file) : null;
  return {
    file: normalizedFile,
    line,
...
// Stable identity is evidence-anchor first. The versioned output contract asks
// reviewers to preserve an explicit id across remediation, but a newly allocated
// id must not buy another cycle for the same file + line + remediation owner.
// Explicit ids remain the fallback for findings that have no usable file/line
// anchor. Title/detail wording is deliberately excluded whenever either stable
// source exists.
function stableFindingMaterial(finding) {
  const anchor = findingAnchor(finding);
  if (anchor.file && anchor.line) {
    return {
      file: anchor.file.toLowerCase(),
      line: anchor.line.toLowerCase(),
      owner: finding?.remediation?.owner ?? null,
      blocking: finding?.blocking !== false,
    };
  }
  const explicit = explicitFindingId(finding);
  if (explicit) return { explicit_id: explicit.toLowerCase() };
  const title = findingValue(finding, FINDING_TITLE_KEYS) ?? findingValue(finding, FINDING_BODY_KEYS);
  return {
    file: anchor.file?.toLowerCase() ?? '(no file)',
    title: title ? flattenReviewText(title).toLowerCase() : '(no title)',
  };
}

function stableFindingId(finding) {
  return `rf-${sha256(stableFindingMaterial(finding)).slice(0, 16)}`;
}
...
function boundReviewFindingsBlock(rendered, recordedTotal = rendered.length, unrenderableStage = null, structuredEvidenceOmitted = 0) {
  const budget = REVIEW_FINDINGS_BLOCK_LIMIT - REVIEW_FINDINGS_DISCLOSURE_RESERVE;
  const disclosureNeeded = structuredEvidenceOmitted > 0 || recordedTotal > REVIEW_FINDINGS_MAX ||
    recordedTotal > rendered.length || rendered.reduce((total, entry) => total + reviewFindingCost(entry.text), 0) > budget;
  const retainedLimit = REVIEW_FINDINGS_MAX - (disclosureNeeded ? 1 : 0);
  ...
  for (let index = 0; index < rendered.length; index += 1) {
    ...
    if (included.size >= retainedLimit) continue;
  ...
  for (let index = 0; index < rendered.length; index += 1) {
    ...
    if (included.size >= retainedLimit) {
  ...
  if (dropped > 0 || structuredEvidenceOmitted > 0) {
    ...
    entries.push(
      `${label}: ${dropped} of ${recordedTotal} review findings were dropped by this ticket's bound ` +
        ...
        (structuredEvidenceOmitted > 0
          ? `The separate review_finding_evidence list omits ${structuredEvidenceOmitted} additional identities beyond its ${REVIEW_FINDING_EVIDENCE_MAX}-entry limit. `
          : '') +
...
function selectReviewFindings(state, receipts) {
  ...
  const structuredEvidenceOmitted = Math.max(0,
    selectReviewFindingEvidence(state, receipts, { limit: Infinity }).length - REVIEW_FINDING_EVIDENCE_MAX);
  return boundReviewFindingsBlock(rendered, recordedTotal, unrenderableStage, structuredEvidenceOmitted);
}

function selectReviewFindingEvidence(state, receipts, { limit = REVIEW_FINDING_EVIDENCE_MAX } = {}) {
  const positive = new Set(['agree', 'pass', 'passed']);
  const stageByTicket = new Map(
    (state.tickets ?? []).map((ticket) => [ticket.ticket_id, ticket.stage_id]),
  );
...
function recoveryAssuranceEntry(sourceStage, raw, fallbackAnchor, limits = receiptLimitsForTicket()) {
  ...
  const summary = boundedRecoveryText(summarySource, limits.assurance_text);
  const explicitAnchor = object
    ? firstNonEmptyString(object.evidence_anchor, object.anchor, object.file)
    : null;
  const evidenceAnchor = boundedRecoveryText(explicitAnchor ?? fallbackAnchor, limits.assurance_anchor);
  ...
  return {
    id: `pa-${sha256({
      ...
      // ticket-bounded result of boundedRecoveryText above; this copy
      // only contributes to the deterministic assurance id.
      summary,
    }).slice(0, 16)}`,
    source_stage: sourceStage,
    // prose-bound-exempt: summary is the already control/bidi-neutralized,
    // ticket-bounded result of boundedRecoveryText above.
    summary,
    evidence_anchor: evidenceAnchor,
...
// One bounded, scheduler-authored feedback envelope per directed replan.
// Explicit `evidence.missing_assurances` wins; legacy judge findings,
...
function planRecovery(_state, ticket, receipt) {
  ...
  const missingAssurances = candidates
    .slice(0, limits.assurances)
    .map(({ value, anchor }) => recoveryAssuranceEntry(sourceStage, value, anchor, limits))
    .filter(Boolean);
  return {
    version: 1,
    attempt: 1,
...
function testReconciliation(ticket, receipt) {
  ...
  const selectedPaths = (testPaths.length > 0 ? testPaths : [...authorized]).slice(0, limits.paths);
  ...
  return {
    ...
    // Reserve the notice after shortening the report so an overlong summary
    // cannot erase the disclosure about omitted authority-bearing paths.
    report: `${boundedRecoveryText(rawReport, limits.contradiction_text - suffix.length)}${suffix}`,
    test_paths: selectedPaths,
  };
}
...
function capabilityResolution(state, ticket, receipt) {
  ...
  const list = (key, maxLength = 512) => Array.isArray(claims[key])
    ? [...new Set(claims[key]
        ...
        .map((entry) => boundedRecoveryText(entry, maxLength)))].slice(0, limits.paths)
    : [];
...
function selectReviewFindingFingerprints(receipts) {
  ...
  for (const receipt of receipts) {
    ...
    for (const finding of receipt.findings ?? []) {
      if (finding?.blocking === false) continue;
      const material = stableFindingMaterial(finding);
      if (material.file === '(no file)' && material.title === '(no title)') continue;
      fingerprints.add(sha256(material));
      recorded = true;
    }
    if (!recorded) {
      const summary = firstNonEmptyString(receipt.evidence?.summary);
      if (summary) fingerprints.add(sha256({ summary: flattenReviewText(summary).toLowerCase() }));
    }
  }
