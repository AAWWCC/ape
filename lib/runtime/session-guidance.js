...
const EXISTING_PROBE_GUIDANCE = Object.freeze({
  prepared: 'refresh the existing ape_run probe-status; recover a lost prepared envelope only through ape_run probe and its checked response; do not prepare a replacement or reconstruct launch authority',
  launched: 'refresh the existing ape_run probe-status; wait for the original child binding result; do not prepare another probe, launch another child, or reconstruct bootstrap authority',
  bound: "refresh the existing ape_run probe-status; acknowledge only the original child's exact returned probe acknowledgement; do not launch another child or reconstruct acknowledgement authority",
  completed: 'refresh the existing ape_run probe-status; original parent: review ape_run preview, then ape_run start with expected_admission_digest from that ready preview; do not prepare another probe, launch another child, or reconstruct authority',
});
...
function contractSummary(state) {
  ...
  // Active state already has a bounded file reader. Operational policy can
  // legitimately issue more than 256 tickets, so a display shortcut must not
  // silently describe those versioned receipts as unversioned.
...
function responseNextSafeAction(response) {
  ...
  if (actions.some((action) => action?.type === 'dispatch_pending')) {
    // RESUME also uses dispatch_pending for detached gate and shipping
    // watches. Those watches advance only when NEXT polls them; no worker
    // dispatch exists to expire.
    if ((response.run?.status === 'gating' && response.run?.gates_watch) ||
        (response.run?.status === 'shipping' && response.run?.shipping_watch)) {
      return 'ape_run next';
    }
...
export function runtimeGuidanceForState(state, options = {}) {
  ...
  const nextSafeAction = safeDiagnosticText(options.nextSafeAction, 256)
    ?? (busy ? 'retry ape_status to read a stable snapshot' : null)
  ...
  if (busy) {
    lines.push(
      'Run state: active state is being updated; lifecycle status is not yet available.',
      `Next safe action: ${nextSafeAction}.`,
    );
  } else if (diagnostic.reason_code === 'inactive') {
    lines.push(
...
export async function loadSessionGuidance(projectDir, options = {}) {
  ...
  try {
    if (!(await validateGovernedRuntimeAncestor(paths))) return null;
  } catch {
    // A redirected state ancestor is not this project's run, probe, or child
    // identity. Leaf-state reset cannot repair an unsafe ancestor because its
    // own mutation guard refuses that path before quarantine.
    return runtimeGuidanceForState(null, {
      corrupt: true,
      host: options.host,
      source: options.source,
      nextSafeAction: 'have the operator restore ordinary governed .ape and .ape/runtime directories within this project, then retry ape_status before invoking APE recovery',
    });
  }
  ...
  try {
    ...
    return runtimeGuidanceForState(null, {
      corrupt: error?.code !== 'APE_ACTIVE_STATE_BUSY',
      busy: error?.code === 'APE_ACTIVE_STATE_BUSY',
      host: options.host,
      source: options.source,
    });
  }
  ...
  if ((options.host ?? state?.host) === 'codex' &&
    ...
    try {
      ...
      const probe = await bindingProbeStatus(paths, { readOnly: true });
      // A live handshake is not a new-start prerequisite. Refresh its current
      // phase without turning session restart into another prepare or launch.
      nextSafeAction = probe.infrastructure_status === 'failed'
        ? FAILED_PROBE_GUIDANCE
        : EXISTING_PROBE_GUIDANCE[probe.status];
    } catch {
