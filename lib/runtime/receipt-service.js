...
import { isExecutionRedirectingConfigPath } from './write-policy.js';
import {
...
const CAPABILITY_RECOVERY_PATH_LIMIT = 64;
const CAPABILITY_RECOVERY_GLOB = /[*?]/;
const CAPABILITY_RECOVERY_EXTERNAL = /(?:^[A-Za-z][A-Za-z0-9+.-]*:|:\/\/)/;
const CAPABILITY_RECOVERY_PRIVATE = /(?:^|\/)\./;
const GENERATED_TICKET_UUID =
...
function exactCapabilityRecovery(state, ticket, receipt) {
  if (
  ...
  if (!required || typeof required !== 'object' || Array.isArray(required)) {
    return {
      eligible: false,
      errors: ['capability recovery requires an exact required_claims object'],
    };
  }
  const errors = [];
  const automaticRole = ['implementer', 'test_writer'].includes(ticket.role);
  const addedClaimedPaths = [];
  const addedTestPaths = [];
  const inspect = (field, existing, target, expectTest) => {
    const values = required[field] ?? [];
    if (!Array.isArray(values)) {
      errors.push(`capability recovery ${field} must be an array`);
      return;
    }
    const seen = new Set();
    for (const [index, value] of values.entries()) {
      const canonical = typeof value === 'string' ? normalizeClaimPath(value) : null;
      const safe =
        typeof value === 'string' && value.length > 0 && value.length <= 512 &&
        canonical === value && canonical !== '.' && canonical !== '..' &&
        !value.includes('\\') && !value.includes('//') && !value.endsWith('/') &&
        !CAPABILITY_RECOVERY_GLOB.test(value) &&
        !CAPABILITY_RECOVERY_EXTERNAL.test(value) &&
        !CAPABILITY_RECOVERY_PRIVATE.test(value) &&
        !isExecutionRedirectingConfigPath(value);
      if (!safe) {
        errors.push(`capability recovery ${field}.${index} is not a safe canonical repository-local path`);
        continue;
      }
      const testShaped = TEST_PATH_PATTERN.test(canonical) || withinClaims(canonical, state.test_paths ?? []);
      if (expectTest !== testShaped) {
        errors.push(
          `capability recovery ${field}.${index} violates production/test scope separation`,
        );
        continue;
      }
      if (seen.has(canonical)) {
        errors.push(`capability recovery ${field}.${index} is a duplicate claim`);
        continue;
      }
      seen.add(canonical);
      if (withinClaims(canonical, existing)) {
        errors.push(`capability recovery ${field}.${index} is already authorized and not additive`);
        continue;
      }
      target.push(canonical);
    }
  };
  inspect('claimed_paths', state.claimed_paths ?? [], addedClaimedPaths, false);
  inspect('test_paths', state.test_paths ?? [], addedTestPaths, true);

  if (
    automaticRole &&
    required.required_role !== undefined &&
    required.required_role !== ticket.role
  ) {
    errors.push('capability recovery required_role conflicts with the immutable stage role');
  }
  if (
    automaticRole &&
    addedClaimedPaths.length === 0 && addedTestPaths.length === 0
  ) {
    errors.push('capability recovery requires at least one exact additive repository path claim');
  }
  if ((state.claimed_paths?.length ?? 0) + addedClaimedPaths.length > CAPABILITY_RECOVERY_PATH_LIMIT) {
    errors.push(`capability recovery claimed_paths exceeds the aggregate ${CAPABILITY_RECOVERY_PATH_LIMIT}-path limit`);
  }
  if ((state.test_paths?.length ?? 0) + addedTestPaths.length > CAPABILITY_RECOVERY_PATH_LIMIT) {
    errors.push(`capability recovery test_paths exceeds the aggregate ${CAPABILITY_RECOVERY_PATH_LIMIT}-path limit`);
  }
  return {
    eligible: automaticRole && errors.length === 0,
    errors,
    added_claimed_paths: addedClaimedPaths,
    added_test_paths: addedTestPaths,
    claimed_paths: [...(state.claimed_paths ?? []), ...addedClaimedPaths],
    test_paths: [...(state.test_paths ?? []), ...addedTestPaths],
  };
}

function validatedDurableSuccessor(entry, candidate, state, stageId) {
  const prefix = `${state.run_id}:${stageId}:`;
  const uuid = typeof candidate?.ticket_id === 'string' &&
    candidate.ticket_id.startsWith(prefix)
    ? candidate.ticket_id.slice(prefix.length)
    : '';
  const expectedEntry = typeof candidate?.ticket_id === 'string'
    ? `${candidate.ticket_id.replaceAll(':', '_')}.json`
    : null;
  const validation = validateTicket(candidate);
  if (
    !validation.valid ||
    !GENERATED_TICKET_UUID.test(uuid) ||
    entry !== expectedEntry ||
    validation.value.run_id !== state.run_id ||
    validation.value.stage_id !== stageId ||
    validation.value.objective !== state.objective ||
    JSON.stringify(validation.value) !== JSON.stringify(candidate)
  ) {
    throw new Error('durable successor ticket failed schema, identity, or filename binding');
  }
  return validation.value;
}

function stablePreparedEffectState(state) {
...
async function capabilityRecoveryAudited(file, runId, sourceTicketId, recovery) {
  let text;
  try {
  ...
  return text.trim().split('\n').some((line) => {
    ...
    return entry.operation === 'capability-recovery' &&
      entry.run_id === runId &&
      entry.source_ticket_id === sourceTicketId &&
      JSON.stringify(entry.added_claimed_paths ?? []) ===
        JSON.stringify(recovery.added_claimed_paths) &&
      JSON.stringify(entry.added_test_paths ?? []) ===
        JSON.stringify(recovery.added_test_paths);
  });
}

async function replayPreparedReceiptEffect(paths, state, transaction, transactionFile) {
  const effect = transaction?.prepared_effect;
  if (transaction?.status !== 'prepared' || !effect) return null;
  const sourceTicket = state.tickets?.find(
    (entry) => entry.ticket_id === transaction.ticket_id,
  );
  const sourceTicketValidation = validateTicket(sourceTicket);
  const receiptValidation = validateReceipt(transaction.receipt);
  const capabilityEffect = receiptValidation.valid &&
    receiptValidation.value.status === 'failed' &&
    receiptValidation.value.evidence?.failure_kind === 'capability';
  const capabilityRecovery = sourceTicket && receiptValidation.valid
    ? exactCapabilityRecovery(state, sourceTicket, receiptValidation.value)
    : { eligible: false, errors: [] };
  const runFile = path.join(paths.runs, `${state.run_id}.json`);
  const durableRun = await readJson(runFile, null);
  if (
    effect.version !== 1 ||
    !sourceTicketValidation.valid ||
    !receiptValidation.valid ||
    transaction.pre_state_hash !== sha256(state) ||
    effect.active_state_hash !== sha256(effect.active_state) ||
    effect.active_state_hash !== sha256(durableRun) ||
    JSON.stringify(effect.active_state) !== JSON.stringify(durableRun) ||
    effect.active_state?.run_id !== state.run_id ||
    effect.source_ticket_id !== transaction.ticket_id ||
    effect.source_ticket_hash !== sourceTicket.ticket_hash ||
    effect.source_receipt_hash !== receiptValidation.value.receipt_hash ||
    receiptValidation.value.ticket_hash !== sourceTicket.ticket_hash ||
    (
      capabilityEffect &&
      (
        !capabilityRecovery.eligible ||
        JSON.stringify(stablePreparedEffectState(effect.active_state)) !==
          JSON.stringify(stablePreparedEffectState(state)) ||
        JSON.stringify(effect.active_state.claimed_paths) !==
          JSON.stringify(capabilityRecovery.claimed_paths) ||
        JSON.stringify(effect.active_state.test_paths) !==
          JSON.stringify(capabilityRecovery.test_paths) ||
        JSON.stringify(effect.active_state.attempts) !== JSON.stringify(state.attempts) ||
        effect.active_state.auto_merge_authorized !== state.auto_merge_authorized ||
        JSON.stringify(effect.active_state.capability_recoveries) !== JSON.stringify({
          ...(state.capability_recoveries ?? {}),
          [sourceTicket.stage_id]: 1,
        })
      )
    )
  ) {
    throw new Error('prepared receipt effect failed trusted state, authority, or consent binding');
  }
  const durableReceipt = effect.active_state.receipts?.find(
    (entry) => entry.ticket_id === transaction.ticket_id,
  );
  if (
    !durableReceipt ||
    durableReceipt.receipt_id !== transaction.receipt?.receipt_id ||
    durableReceipt.receipt_hash !== transaction.receipt?.receipt_hash
  ) {
    throw new Error('prepared receipt effect does not bind the durable source receipt');
  }
  const receiptFile = path.join(paths.receipts, `${durableReceipt.receipt_id}.json`);
  const receiptRecord = await readJson(receiptFile, null);
  if (
    !receiptRecord ||
    receiptRecord.receipt_hash !== durableReceipt.receipt_hash ||
    hashRecord(receiptRecord, ['receipt_hash']) !== durableReceipt.receipt_hash
  ) {
    throw new Error('prepared receipt effect lost or mismatched its durable receipt');
  }

  const expectedSuccessors = effect.successors ?? [];
  if (capabilityEffect && expectedSuccessors.length !== 1) {
    throw new Error('prepared receipt effect must bind exactly one durable successor');
  }
  if (
    effect.active_state.receipts?.length !== (state.receipts?.length ?? 0) + 1 ||
    JSON.stringify(effect.active_state.receipts?.slice(0, -1)) !==
      JSON.stringify(state.receipts ?? []) ||
    effect.active_state.tickets?.length !==
      (state.tickets?.length ?? 0) + expectedSuccessors.length ||
    JSON.stringify(effect.active_state.tickets?.slice(
      0,
      effect.active_state.tickets.length - expectedSuccessors.length,
    )) !==
      JSON.stringify(state.tickets ?? [])
  ) {
    throw new Error('prepared receipt effect does not represent one additive receipt transition');
  }
  const activeIds = new Set((state.tickets ?? []).map((entry) => entry.ticket_id));
  const candidateTickets = [];
  for (const entry of await readdir(paths.tickets)) {
    ...
    if (
      ...
      const expected = expectedSuccessors.find(
        (successor) => successor.ticket_id === candidate.ticket_id,
      );
      const snapshot = effect.active_state.tickets?.find(
        (ticket) => ticket.ticket_id === candidate.ticket_id,
      );
      if (!expected || !snapshot) {
        throw new Error('prepared receipt effect has competing or ambiguous successor evidence');
      }
      candidateTickets.push(validatedDurableSuccessor(
        entry,
        candidate,
        state,
        snapshot.stage_id,
  ...
  if (candidateTickets.length !== expectedSuccessors.length) {
    throw new Error('prepared receipt effect has missing, competing, or ambiguous successor evidence');
  }
  for (const expected of expectedSuccessors) {
    const candidate = candidateTickets.find((entry) => entry.ticket_id === expected.ticket_id);
    const snapshot = effect.active_state.tickets?.find(
      (entry) => entry.ticket_id === expected.ticket_id,
    );
    if (
      !candidate || !snapshot ||
      expected.ticket_id !== snapshot.ticket_id ||
      expected.ticket_hash !== snapshot.ticket_hash ||
      candidate.ticket_hash !== expected.ticket_hash ||
      candidate.ticket_hash !== hashRecord(candidate, ['ticket_hash']) ||
      JSON.stringify(candidate) !== JSON.stringify(snapshot)
    ) {
      throw new Error('prepared receipt effect successor is forged, rebound, or incompatible');
    }
  }

  await atomicWriteJson(paths.active, effect.active_state);
  ...
  return {
    ...
    run: effect.active_state,
    actions: [],
    idempotent: true,
    recovered: true,
    ...(Array.isArray(effect.warnings) && effect.warnings.length > 0
      ? { warnings: structuredClone(effect.warnings) }
      : {}),
  };
}
...
async function recoverDurableSuccessorTicket(paths, state, expectedTicket) {
  ...
  for (const entry of entries) {
    ...
    if (candidate.ticket_hash !== hashRecord(candidate, ['ticket_hash'])) {
      throw new Error('durable successor evidence has a forged or invalid ticket hash');
    }
    const artifactHash = state.preflight?.artifact_hash;
    if (artifactHash && candidate.preflight?.artifact_hash !== artifactHash) {
      throw new Error('durable successor evidence is rebound to incompatible preflight state');
    }
    candidates.push(validatedDurableSuccessor(
      entry,
      candidate,
      state,
      expectedTicket.stage_id,
    ));
  }
  if (candidates.length === 0) return null;
  if (candidates.length !== 1) {
    throw new Error('multiple durable successor candidates are ambiguous; refusing replay');
  }
  const expectedHash = hashRecord(durableSuccessorMaterial(expectedTicket));
  const matching = candidates[0];
  if (hashRecord(durableSuccessorMaterial(matching)) === expectedHash) return matching;
  throw new Error(
    'durable successor is incompatible with the prepared receipt effect',
  );
}
...
async function issueTicket(paths, state, stage, config, tree, retryOf = null, evidence = {}) {
  ...
  const forwardedPreflight = stage.role !== 'preflight_analyst' && state.preflight?.artifact
    ...
    : null;
  const narrowedTestPaths = evidence.full_scope === true
    ? null
    : narrowedTestClaims(state, stage);
  ...
  const inheritedCapabilityManifest =
    evidence.source_ticket?.receipt_contract_version === RECEIPT_CONTRACT_VERSION
      ? evidence.source_ticket.capability_manifest
      : null;
  const capabilityManifestBase = ticketCapabilityManifest(state, stage, manifestTestPaths) ??
    (inheritedCapabilityManifest
      ? (() => {
          const {
            receipt_schema: _receiptSchema,
            field_bounds: _fieldBounds,
            byte_budgets: _byteBudgets,
  ...
  const expectedTicket = finalizeTicket({
    ...
    ...(forwardsPlanningContractContext
      ? { risk_triggers: [...(state.risk_triggers ?? [])] }
      : {}),
    claimed_paths: evidence.full_scope === true
      ? [...state.claimed_paths]
      : narrowedTestPaths ? [...narrowedTestPaths] : ticketClaims(state, stage),
    ...
    attempt: Number.isInteger(evidence.attempt)
      ? evidence.attempt
      : retryOf ? (state.attempts[stage.id] ?? 2) : 1,
    ...
    switch (action.type) {
      ...
      case 'issue_ticket': {
        const { ticket } = await issueTicket(paths, state, action.stage, config, tree, action.retry_of, {
          ...
          full_scope: action.full_scope,
          source_ticket: action.source_ticket,
        });
      ...
      case 'audit_override': {
        ...
        await appendJsonLine(paths.overrideLog, {
          ...
          // their historical line shape.
           ...
           ...(action.source_ticket_id ? { source_ticket_id: action.source_ticket_id } : {}),
           ...(action.added_claimed_paths
             ? { added_claimed_paths: action.added_claimed_paths }
             : {}),
           ...(action.added_test_paths ? { added_test_paths: action.added_test_paths } : {}),
...
export async function recordReceiptLocked(projectDir, raw, normalizedFields = [], options = {}) {
  ...
  const durableStateHashAtEntry = sha256(state);
  const ticketIdsAtEntry = new Set((state.tickets ?? []).map((entry) => entry.ticket_id));
  ...
  const preparedEffectReplay = await replayPreparedReceiptEffect(
    paths,
    state,
    transaction,
    transactionFile,
  );
  if (preparedEffectReplay) return preparedEffectReplay;
  ...
  // Capability recovery is admitted at the last pre-durable boundary. The
  // worker receipt is untrusted: exact canonical paths, production/test
  // separation, aggregate limits, and role compatibility are all proved
  // before the prepared transaction, receipt file, audit, or active state can
  // change. A second otherwise-valid request is intentionally admitted below
  // so the reducer can record the receipt and make the one bounded terminal
  // decision without growing scope or issuing another ticket.
  const capabilityRecovery = exactCapabilityRecovery(state, ticket, receipt);
  if (capabilityRecovery.errors.length > 0) {
    return {
      ok: false,
      rejected: true,
      errors: capabilityRecovery.errors,
      ...normalizationNote,
    };
  }
  ...
  if (!transaction) {
    await atomicWriteJson(transactionFile, {
      ...
      ticket_id: ticket.ticket_id,
      input_hash: inputHash,
      pre_state_hash: durableStateHashAtEntry,
      status: 'prepared',
      prepared_at: now(),
      receipt,
    });
  ...
  const prospectiveClaimedPaths = capabilityRecovery.eligible
    ? capabilityRecovery.claimed_paths
    : [...new Set([...state.claimed_paths, ...productionChanges, ...proposedClaims])];
  const prospectiveTestPaths = capabilityRecovery.eligible
    ? capabilityRecovery.test_paths
    : state.test_paths;
  const expansion = escalateLane(state.lane, {
    claimed_paths: prospectiveClaimedPaths,
    test_paths: prospectiveTestPaths,
  ...
  if (
    !capabilityRecovery.eligible &&
    (expansion.escalated || newRiskTriggers || proposedClaims.length > 0)
  ) {
              ...
              capability_recovery: {
                ...
                patch: {
                  ...(expansion.escalated
                    ...
                    : {}),
                  ...(knownRisk.length > 0
                    ? { risk_triggers: knownRisk, high_risk: true }
                    : {}),
                },
              },
            }
  ...
  if (transaction && capabilityRecovery.eligible) {
    const audit = actions.find(
      (entry) => entry.type === 'audit_override' && entry.operation === 'capability-recovery',
    );
    if (
      audit &&
      await capabilityRecoveryAudited(
        paths.overrideLog,
        state.run_id,
        ticket.ticket_id,
        capabilityRecovery,
      ...
      actions = actions.filter((entry) => entry !== audit);
    }
  }
  ...
  let emitted;
  try {
    emitted = await applyActions(paths, state, actions, config, tree);
  } catch (error) {
    if (/durable successor|successor evidence/i.test(error?.message ?? '')) {
      return {
        ok: false,
        rejected: true,
  ...
  const successors = state.tickets
    .filter((entry) => !ticketIdsAtEntry.has(entry.ticket_id))
    .map((entry) => ({ ticket_id: entry.ticket_id, ticket_hash: entry.ticket_hash }));
  await atomicWriteJson(transactionFile, {
    ...prepared,
    prepared_effect: {
      version: 1,
      ...
      active_state: structuredClone(state),
      active_state_hash: sha256(state),
      successors,
      warnings: structuredClone(receiptWarnings),
    },
    status: 'committed',
    committed_at: now(),
  });
