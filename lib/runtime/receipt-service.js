...
import { atomicReplaceText, atomicWriteJson, appendJsonLine, publishImmutableDirectory, readJson, replaceFile } from './storage.js';
...
function preparedCapabilityRecoveryError(transaction, state, ticket, inputHash, attestation = null) {
  ...
  const expectedScope = capabilityRecoveryReceipt(transaction.receipt)
    ? additiveRecoveryScope(ticket, transaction.receipt)
    : null;
  ...
  const expectedSuccessorRole = transaction.receipt?.evidence?.required_claims?.required_role ??
    ticket.role;
  const successorRolePolicy = ROLE_POLICIES[expectedSuccessorRole];
  ...
  const expectedPreviousGeneration = state.recovery_generation
    ? {
        generation: state.recovery_generation.generation,
        hash: state.recovery_generation.hash,
      }
    : null;
  ...
  if (
    ...
    effect?.lane !== state.lane ||
    !exactObject(effect?.risk_triggers, state.risk_triggers) ||
    ...
    (!stateAlreadyAdopted && (
      generation.generation !== (state.recovery_generation?.generation ?? 0) + 1 ||
  ...
  if (stateAlreadyAdopted && !exactObject(state.recovery_generation, generation)) {
    return 'prepared capability recovery conflicts with the adopted generation';
  }
  return null;
}
...
async function preparedCapabilityRecoveryRuntimeError(
  ...
  const scope = additiveRecoveryScope(ticket, transaction.receipt);
  ...
  const expectedModel = role === ticket.role
    ? ticket.model
    : resolveModel(config, state.host, modelTier, role);
  const { deadline_ms: deadlineMs } = resolveTicketDeadline(config, state.mode, state.lane);
  const expectedDeadline = new Date(
    Date.parse(successor.issued_at) + deadlineMs,
  ).toISOString();
  const capabilityBase = ticketCapabilityManifest(
    { ...state, claimed_paths: scope.claimed_paths, test_paths: scope.test_paths },
    { role },
    scope.test_paths,
  );
  ...
  const capabilityManifest = capabilityBase && outputSchema
    ...
    : null;
  const alreadyAdopted = (state.receipts ?? []).some((entry) =>
    entry.ticket_id === ticket.ticket_id &&
    entry.receipt_hash === transaction.receipt.receipt_hash);
  if (
    !capabilityManifest ||
...
async function prepareCapabilityRecovery(
  ...
  const previousGeneration = state.recovery_generation
    ? {
        generation: state.recovery_generation.generation,
        hash: state.recovery_generation.hash,
      }
  ...
  const generationBody = {
    version: 1,
    generation: (state.recovery_generation?.generation ?? 0) + 1,
  ...
  const effect = {
    ...
    policy: {
      role: ticket.role,
      writable: ticket.writable,
      required_checks: structuredClone(ticket.required_checks),
      model_tier: ticket.model_tier,
      model: structuredClone(ticket.model),
    },
    lane: state.lane,
    risk_triggers: structuredClone(state.risk_triggers),
...
async function publishCapabilityRecovery(
  ...
  const generationName = [
    String(effect.recovery_generation.generation).padStart(8, '0'),
    effect.recovery_generation.hash,
  ].join('-');
  const generationDirectory = path.join(paths.recoveryGenerations, generationName);
  ...
  if (!existingSuccessor) {
    ...
    nextState.tickets.push(successor);
    nextState.claimed_paths = [...new Set([
      ...(nextState.claimed_paths ?? []),
      ...effect.scope.successor.claimed_paths,
    ])];
    nextState.test_paths = [...effect.scope.successor.test_paths];
    ...
    const transactionName = path.basename(receiptTransactionPath(paths, ticket.ticket_id));
    const entries = {
    ...
    generationManifest = { ...manifestBody, hash: sha256(manifestBody) };
  ...
  const durableManifest = await readJson(path.join(generationDirectory, 'manifest.json'), null);
  const manifestHashValid = durableManifest &&
    durableManifest.hash === hashRecord(durableManifest, ['hash']);
  if (
    !manifestHashValid ||
    durableManifest.version !== 1 ||
    durableManifest.prepared_effect_hash !== transaction.prepared_effect_hash ||
    !exactObject(durableManifest.generation, effect.recovery_generation) ||
    (generationManifest && !exactObject(durableManifest, generationManifest))
  ) {
    throw new Error('capability recovery generation manifest failed exact validation');
  }
  for (const [name, expectedHash] of Object.entries(durableManifest.files ?? {})) {
    const member = await readJson(path.join(generationDirectory, name), null);
    if (member === null || sha256(member) !== expectedHash) {
      throw new Error(`capability recovery generation member failed validation: ${name}`);
    }
  }
  const durableState = await readJson(path.join(generationDirectory, 'active.json'));
  const durableReceipt = await readJson(
    path.join(generationDirectory, 'receipts', `${transaction.receipt.receipt_id}.json`),
  );
  const durableSuccessor = await readJson(
    path.join(generationDirectory, 'tickets', `${successor.ticket_id.replaceAll(':', '_')}.json`),
  );
  if (
    ...
    throw new Error('capability recovery generation does not bind the prepared successor state');
  }
  ...
  const installProjection = async (relative, destination, { replace = false } = {}) => {
    const source = path.join(generationDirectory, relative);
    ...
    await atomicReplaceText(destination, bytes);
  };
  const contractHash = successor.capability_manifest.run_contract.hash;
  const schemaHash = successor.capability_manifest.receipt_schema.hash;
  const receiptName = `${transaction.receipt.receipt_id}.json`;
  ...
  if (!existingSuccessor) {
    await atomicWriteJson(paths.active, durableState);
  ...
  const dispatch = await dispatchForTicket(paths, state, successor);
...
export async function recordReceipt(projectDir, raw) {
  ...
  try {
    const result = await withReceiptLock(paths, async () => {
      const state = await activeState(paths);
      const ticket = state?.tickets?.find((entry) => entry.ticket_id === raw?.ticket_id);
...
export async function recoverReceiptLocked(projectDir, raw, recovery = {}) {
...
export async function recoverReceipt(projectDir, raw, recovery = {}) {
  ...
  try {
    const result = await withReceiptLock(
      paths,
      () => recoverReceiptLocked(projectDir, raw, recovery),
