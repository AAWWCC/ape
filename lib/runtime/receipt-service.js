...
import { access, mkdir, open, opendir, readdir, realpath, rename, rm } from 'node:fs/promises';
import { AUTO_MERGE_HOLD_REASON, CHECKS_REGISTRATION_RETRY_DELAY_MS, GATE_INLINE_GRACE_MS, GATE_POLL_RETRY_DELAY_MS, RISK_TRIGGERS, ROLE_POLICIES, SCHEMA_VERSION, SCOPE_EXPANSION_REASONS_MAX, TERMINAL_STATUSES } from './constants.js';
import { runtimePaths } from './paths.js';
import { atomicReplaceText, atomicWriteJson, appendJsonLine, publishImmutableDirectory, publishImmutableJson, readJson } from './storage.js';
import { recoverAtomicWriteDirectory } from './atomic-write-recovery.js';
import { readBoundedFile, readBoundedJson } from './bounded-file.js';
import { receiptAuditContains } from './receipt-audit.js';
import { acquireRunLock, assertDirLockLease, releaseRunLock, withDirLock, withDirLockLeaseMutation } from './lock.js';
import { loadRuntimeConfig, resolveModel, resolveTicketDeadline } from './config.js';
import { escalateLane, resolveLaneScope } from './lane-policy.js';
import { declaredTestRemediationPaths, extractTestRemediation } from './pipeline.js';
import { reduceRun } from './scheduler.js';
import {
  finalizeReceipt,
  finalizeTicket,
...
import { currentBranch, currentTreeSha, diffFiles, resolveBaseBranch, runGit, treeHasPath, treeShaSession, workingTreeStatus } from './git.js';
import { checkTreeAttribution, treeAttributionRefusal } from './tree-attribution.js';
import { looksLikeTest, canonicalProjectRelativePathError, normalizeClaimPath, withinClaims } from './path-scope.js';
import { assertLocalShippingBranchCurrent, deleteLocalShippingBranch } from './shipping-cleanup.js';
import { assertActiveStateWritable } from './active-state.js';
import {
...
import { archiveRun as archiveRunRecord } from './history.js';
...
import { assertSafeReceiptInput, assertSafeReceiptRecoveryInput } from './input-guard.js';
import { executionConfigForRun, pipelineLimits, receiptLimits } from './pipeline-limits.js';
import {
  normalizeReceiptInput,
  authenticatedReceiptBearer,
  receiptInputHash,
  receiptTicketIdValue,
  redactReceiptBearer,
...
import { receiptRejectionRecovery } from './recovery-guidance.js';
import {
...
import { assertRoadmapRequirementsReady, deriveRoadmap } from './roadmap.js';
import { compactArchivedArtifacts, recordArtifactRetentionStatus } from './retention.js';
...
import {
  ...
  prepareTicketRunContract,
  readRunContractManifest,
...
import { capabilityTestPathBoundErrors } from './capability-contract.js';
...
// The live pipeline's widest parallel group is two. Legacy execution-budget
// continuations may contain a much larger durable tail, but minting more than
// one native group before returning would create nonce-bearing capabilities
// that cannot all fit on the bounded response wire.
const MAX_NATIVE_DISPATCHES_PER_RESPONSE = 2;
...
// A polling transaction can retain both a 2 MiB poll snapshot and a 2 MiB
// final result. Match the task journal's durable ceiling and write compact
// JSON so individually supported fields cannot expand past their read budget.
const TASK_OPERATION_MAX_BYTES = 8 * 1_024 * 1_024;
...
async function readBoundedTaskOperationJson(file, schema, rootBinding, label) {
  ...
  try {
    ...
    raw = await readBoundedJson(file, TASK_OPERATION_MAX_BYTES);
  } catch (error) {
...
export async function writeTaskOperationTransaction(file, store, record) {
  ...
  if (Buffer.byteLength(serialized, 'utf8') > TASK_OPERATION_MAX_BYTES) {
    throw new Error('APE task operation transaction exceeds its durable file bound');
  }
  await atomicReplaceText(file, serialized);
  return finalized;
}

async function pruneTaskOperationGc(store, timestamp) {
  const entries = await recoverAtomicWriteDirectory(store.gc,
    (name) => /^[0-9a-f]{64}\.json$/.test(name), TASK_OPERATION_MAX_BYTES);
  const retained = [];
...
export async function collectExpiredTaskOperationTransactions(_paths, store, timestamp = Date.now()) {
  await pruneTaskOperationGc(store, timestamp);
  const entries = await recoverAtomicWriteDirectory(store.directory,
    (name) => /^[0-9a-f]{64}\.json$/.test(name), TASK_OPERATION_MAX_BYTES);
  let retained = 0;
  for (const entry of entries) {
...
export async function executeTaskOperationTransaction(projectDir, operation, effect) {
  ...
  return withReceiptLock(paths, async (receiptLease) => {
    ...
    const value = operation.isCancelled?.()
      ...
      : await effect(paths, receiptLease);
...
// why rebasing it out was rejected). The reducer marks the dead ticket expired
// before issuing its retry. Receipt-side disclosure prefers the immutable
// exact predecessor reference; old tickets without that reference use their
// most recent earlier expired stage ticket. This remains meaningful with more
// than two configured stage attempts and never treats a pre-existing test file
// alone as evidence of an expired worker. Issuance uses its exact retry_of
// argument directly in expiredPredecessorByRetryOf below.
function expiredPredecessorTicket(state, ticket) {
  const expired = new Set(state.expired_tickets ?? []);
  if (ticket.expired_predecessor?.ticket_id) {
    return state.tickets.find((entry) => entry.ticket_id === ticket.expired_predecessor.ticket_id &&
      expired.has(entry.ticket_id)) ?? null;
  }
  const index = state.tickets.findIndex((entry) => entry.ticket_id === ticket.ticket_id);
  return state.tickets.slice(0, index < 0 ? state.tickets.length : index)
    .findLast((entry) => entry.stage_id === ticket.stage_id && expired.has(entry.ticket_id)) ?? null;
}
...
function structuredCandidatePlan(paths, state, stage) {
  ...
  const sourceCommandContext = sourceManifest
    ? {
        plan_limits: planLimitsForTicket(sourceTicket),
...
async function issueTicket(paths, state, stage, config, tree, retryOf = null, evidence = {}) {
  config = executionConfigForRun(config, state);
  const executionLimits = state.execution_policy ? pipelineLimits(state) : null;
  const issuedAt = now();
  ...
  const { deadline_ms: deadlineMs } = resolveTicketDeadline(config, state.mode, state.lane);
  ...
  const claimedPaths = narrowedTestPaths ? [...narrowedTestPaths] : ticketClaims(state, stage);
  const requiredChecks = stage.id === 'remediation-test'
  ...
  const outputSchema = capabilityManifestBase
    ? receiptOutputSchemaForTicket({
        ...
        test_paths: manifestTestPaths,
        required_checks: requiredChecks,
        receipt_contract_version: RECEIPT_CONTRACT_VERSION,
        ...(executionLimits ? { execution_limits: executionLimits } : {}),
  ...
  const capabilityManifest = capabilityManifestBase
    ? {
        ...
        field_bounds: runContractFieldBounds({
          ...
          manifest_growth_contract_version: state.capability_snapshot?.manifest_growth_contract_version ?? 1,
        }),
  ...
  const expectedTicket = finalizeTicket({
    ...
    ...(state.plan_contract_version
      ? { plan_contract_version: state.plan_contract_version }
      : {}),
    ...(state.test_intent ? { test_intent: state.test_intent } : {}),
    ...(capabilityManifest
      ? {
          receipt_contract_version: RECEIPT_CONTRACT_VERSION,
          capability_manifest: capabilityManifest,
    ...
    ...(forwardsPlanningContractContext
      ...
      : {}),
    ...(forwardsPlanningContractContext || capabilityManifest
    ...
    ...(reviewContractVersion === REVIEW_CONTRACT_VERSION
      ...
      : {}),
    ...((stage.id === 'remediation-test' && state.remediation_route)
      || stage.id === 'test-recheck'
      || (stage.id === 'test' && state.test_intent === 'green-maintenance')
      ? { test_scope: 'exact' }
      : {}),
    required_checks: requiredChecks,
    ...
    // observeAuthoredTest/observeAuthoredTestPerRunner instead make the inherited base
...
export async function persist(paths, state, tree, { refreshTree = true } = {}) {
  ...
  assertActiveStateWritable(state);
...
export async function reconcileTerminalCheckout(paths, state) {
  ...
  const githubShipment = state.status === 'completed' && state.merge?.provider === 'github';
  const pushedHead = state.merge?.head_oid ?? state.shipping_watch?.head_oid;
  let baseBranch;
  try {
    baseBranch = await terminalCheckoutBase(paths, state);
    if (githubShipment) {
      await assertLocalShippingBranchCurrent(paths.root, runBranch, pushedHead);
    }
    ...
    if (state.status === 'completed' && apeOwnedBranch(runBranch)) {
      if (githubShipment) {
        await deleteLocalShippingBranch(paths.root, runBranch, pushedHead);
      } else if (await localBranchExists(paths.root, runBranch)) {
...
async function dispatchForTicket(
  ...
  const intent = state.host === 'codex'
    ? await prepareCodexIntent(paths, ticket, basic.agent_type, { ...options, bootstrap_protocol: 1 })
...
function splitDispatchBatch(actions) {
  let dispatches = 0;
  for (const [index, action] of actions.entries()) {
    if (action.type !== 'issue_ticket' && action.type !== 'dispatch_agent') continue;
    dispatches += 1;
    if (dispatches > MAX_NATIVE_DISPATCHES_PER_RESPONSE) {
      return { batch: actions.slice(0, index), remaining: actions.slice(index) };
    }
  }
  return { batch: actions, remaining: [] };
}

function dispatchBatchContinuation(state, actions, config) {
  const durableActions = structuredClone(actions);
  const configSnapshot = structuredClone(config);
  return {
    version: LEGACY_CONTINUATION_VERSION,
    source: 'response-wire-dispatch-batch',
    run_id: state.run_id,
    actions: durableActions,
    actions_hash: sha256(durableActions),
...
export async function resumeLegacyBudgetContinuation(
  ...
  if (!continuation) return null;
  const { batch, remaining } = splitDispatchBatch(continuation.actions);
  if (remaining.length > 0) {
    state.budget_continuation = {
      ...continuation.record,
      actions: structuredClone(remaining),
      actions_hash: sha256(remaining),
    };
  } else {
    delete state.budget_continuation;
  }
  const emitted = await applyActions(paths, state, batch, continuation.config, tree);
  // Dispatch-only NEXT chains have no persist_state action. Persist either the
  // shortened durable tail or its deletion after a successful replay. A
  // terminal chain owns final persistence.
...
export async function applyActions(paths, state, actions, config, tree = treeShaSession(paths.root)) {
  config = executionConfigForRun(config, state);
  const split = splitDispatchBatch(actions);
  let scheduledActions = actions;
  let createdDispatchContinuation = false;
  if (split.remaining.length > 0) {
    if (state.budget_continuation) {
      throw new Error('cannot nest a response-wire dispatch batch inside a retained continuation');
    }
    state.budget_continuation = dispatchBatchContinuation(state, split.remaining, config);
    scheduledActions = split.batch;
    createdDispatchContinuation = true;
  }
  const emitted = [];
  ...
  await Promise.all(scheduledActions.map(async (action, index) => {
  ...
  const dispatchGroupSize = scheduledActions.filter(
    (action) => action.type === 'issue_ticket' || action.type === 'dispatch_agent',
  ).length;
  let deferredTerminalRelease = null;
  for (const [actionIndex, action] of scheduledActions.entries()) {
    switch (action.type) {
      case 'acquire_lock':
      ...
      case 'issue_ticket': {
        ...
        // A crash may durably publish both the successor ticket and its
        // prepared/unlaunched dispatch intent before active.json records the
        // reducer transition. issueTicket then recovers the exact immutable
        // ticket from disk; recover its matching prepared intent as the same
        // launch authority instead of trying to mint a second capability.
        const preparedReplay = await isPreparedUnlaunchedDispatchReplay(paths, state, ticket);
        const dispatch = await dispatchForTicket(
          paths,
          state,
          ticket,
          dispatchGroupSize,
          preparedReplay ? { allow_prepared_replay: true } : {},
        );
        ...
        try {
          ...
          if (merge.watch) {
            state.shipping_watch = {
              ...
              ...(merge.watch.shipping_target ? { shipping_target: structuredClone(merge.watch.shipping_target) } : {}),
              ...(merge.watch.merge_request_submitted === true ? { merge_request_submitted: true } : {}),
      ...
      case 'poll_shipping': {
        ...
        if (poll.merged) {
          ...
          state.shipping_watch = {
            ...state.shipping_watch,
            ...(poll.pending?.merge_request_submitted === true ? { merge_request_submitted: true } : {}),
            last_poll_at: now(),
          ...
          emitted.push({
            ...
            retry_after_ms: config.shipping?.checks_registration_retry_delay_ms ?? CHECKS_REGISTRATION_RETRY_DELAY_MS,
      ...
      case 'apply_override':
        if (action.operation === 'abort') {
          // Keep the reason with the terminal state before archive_history
          // projects it. Repeating an abort may reconcile cleanup, but cannot
          // replace the cause of an already-recorded terminal transition.
          if (state.status !== 'aborted') state.abort_reason = boundedGateSummary(action.reason);
        ...
        emitted.push(action.operation === 'abort'
          ? { ...action, reason: boundedGateSummary(action.reason) }
          : action);
        break;
      case 'dispatch_agent': {
        const ticket = state.tickets.find((entry) => entry.ticket_id === action.ticket_id);
        ...
        const receiptRecovery =
          ...
          (
            (state.receipt_contract_exhaustions?.[ticket.ticket_id] ?? 0) > 0 &&
            (state.receipt_contract_pending_redispatches ?? []).includes(ticket.ticket_id)
          );
        try {
          const recoveryStartedAt = receiptRecovery ? now() : null;
          const { deadline_ms: recoveryDeadlineMs } = resolveTicketDeadline(config, state.mode, state.lane);
          ...
          const dispatch = await dispatchForTicket(paths, state, ticket, dispatchGroupSize, {
            ...(idempotentReplay ? { allow_prepared_replay: true } : {}),
  ...
  if (createdDispatchContinuation && !TERMINAL_STATUSES.has(state.status)) {
    await persist(paths, state, tree);
  }
  return emitted;
}
...
// (service.js:1412, 1454, 1480 here and observeAuthoredTestPerRunner's own site),
...
function emptyAuthoredTestPathsRefusal(state, ticket, fallbackMessage) {
  const check = ticket.required_checks?.includes('test-correction') ? 'test-correction'
    : ticket.required_checks?.includes('green-test') ? 'green-test' : 'red-test';
  const predecessor = expiredPredecessorTicket(state, ticket);
  if (!predecessor) return fallbackMessage;
  if (expiredPredecessorLeftNoOutput(predecessor, ticket)) {
    return (
      `${check} admission found no runtime-verifiable authored test files: ` +
...
// Runtime-owned test intent admission: a test-writer receipt carrying either
// `red-test`, `green-test`, or `test-correction` is admitted only after the runtime
// executes the exact authored paths twice and observes a stable verdict. Worker
// receipt.tests[] is empty for these checks and can never carry admission.
// Execution lives here in
// the service layer — inside the receipt-effects critical section — so the
// pure validator stays deterministic and side-effect free. Red-first requires
// fail/fail; explicit green-maintenance requires pass/pass. An independently
// confirmed post-build test correction permits either stable verdict: the
// implementer retry must still fix any remaining production failure.
//
...
// once the build turns the tree green the evidence is gone. Green-maintenance
// uses the same admission point so a self-reported green result cannot replace
// stable, path-scoped runtime evidence.
async function observeAuthoredTest(paths, state, ticket, receipt, config, tree) {
  const green = ticket.required_checks.includes('green-test');
  const correction = ticket.required_checks.includes('test-correction');
  const check = correction ? 'test-correction' : green ? 'green-test' : 'red-test';
  const phase = correction ? 'test correction' : green ? 'green maintenance' : 'red phase';
  // Per-runner routing branch (roadmap: polyglot multi-runner admission).
  ...
  if (Array.isArray(config.runners) && config.runners.length > 0)
    return observeAuthoredTestPerRunner(paths, state, ticket, receipt, config, tree);
  ...
  const testPaths = [];
  if (green || correction || template || shuffleTemplate || !configured) {
    ...
    for (const file of receipt.changed_files.filter((entry) => looksLikeTest(entry, ticket.test_paths))) {
      if (await fileExists(path.join(paths.root, file))) testPaths.push(file);
    }
    testPaths.sort();
  }
  if ((green || correction) && testPaths.length === 0) {
    return {
      ok: false,
      errors: [
        emptyAuthoredTestPathsRefusal(
          state,
          ticket,
          `${check} admission found no runtime-verifiable authored test files changed by this test writer`,
        ),
      ],
    };
  }
  if (template) {
      ...
      return {
        ...
        errors: [
          emptyAuthoredTestPathsRefusal(
            state,
            ticket,
            `${check} admission found no runtime-verifiable authored test files to render into test_commands.targeted_template`,
          ),
        ],
      };
    }
    ...
    if (!invocation) {
      return {
        ok: false,
        errors: [
          `test_commands.targeted_template must contain the {paths} placeholder so the runtime can scope ${phase} to the authored test files`,
        ],
      };
    }
  } else if (!configured) {
        ...
        errors: [
          testPaths.length === 0
            ? emptyAuthoredTestPathsRefusal(
                state,
                ticket,
                `${check} admission found no runtime-verifiable authored test files; configure test_commands.targeted_template or test_commands.targeted`,
              )
            : `${check} admission has no derivable test command; configure test_commands.targeted_template or test_commands.targeted`,
        ],
      };
    }
    if (invocation.scoped !== true) {
      ...
      return {
        ...
        errors: [
          green || correction
            ? `${check} admission cannot scope the detected ${detectedRunner.runner} runner to the authored test paths; a whole-suite verdict is not proof the authored tests were the tests observed — configure test_commands.targeted_template ('{paths}' receives the authored test files)`
  ...
  if (shuffleTemplate) {
      ...
      return {
        ...
        errors: [
          emptyAuthoredTestPathsRefusal(
            state,
            ticket,
            `${check} admission found no runtime-verifiable authored test files to render into test_commands.targeted_shuffle_template`,
          ),
        ],
      };
    }
    ...
    if (!shuffleInvocation) {
      return {
        ok: false,
        errors: [
          `test_commands.targeted_shuffle_template must contain the {paths} placeholder so the runtime can scope ${phase} to the authored test files`,
        ],
      };
    }
  }
  ...
  if (postExecutionTree !== receipt.head_tree_sha) {
    ...
    if (postExecutionTree !== receipt.head_tree_sha) {
      ...
      return {
        ...
        errors: [
          green || correction
            ? `${check} execution modified pre-existing files (${cause.join(', ')}); runner side-effect artifacts must be committed or gitignored before the run — ${correction ? 'test correction' : 'green'} evidence must be tree-stable`
  ...
  if (verificationB === null || noVerdict(verificationB)) {
    return {
      ok: false,
      errors: [`${check} execution did not produce a test verdict; configure test_commands.targeted_template or test_commands.targeted`],
    };
  }
  ...
  for (const run of [verification, verificationB]) {
    ...
    if (run.exit_code === 5) {
      return {
        ok: false,
        errors: [green || correction
          ? `pytest collected no tests (exit 5): the authored tests were never executed — fix test naming/collection; ${phase} was not observed`
          : 'pytest collected no tests (exit 5): the authored tests were never executed — fix test naming/collection; the red phase was not observed'],
      };
    }
    if (run.exit_code === 4) {
      return {
        ok: false,
        errors: [green || correction
          ? `pytest usage error (exit 4): the ${check} command did not run the authored tests — fix test_commands.targeted_template or test_commands.targeted; ${phase} was not observed`
          : 'pytest usage error (exit 4): the red-test command did not run the authored tests — fix test_commands.targeted_template or test_commands.targeted; the red phase was not observed'],
      };
    }
  }
  if (!green && !correction && verification.passed === true && verificationB.passed === true) {
    return {
      ok: false,
  ...
  if (verification.passed !== verificationB.passed) {
    return {
      ...
      errors: [
        correction
          ? `runtime-executed test-correction is nondeterministic: the authored tests passed one invocation and failed the other (exit ${verification.exit_code} then exit ${verificationB.exit_code}); a corrected test must have a stable verdict before the implementer retry`
          : green
          ? `runtime-executed green-test is nondeterministic: the authored tests passed one invocation and failed the other (exit ${verification.exit_code} then exit ${verificationB.exit_code}); unstable green maintenance is not an observed pass/pass result — make the authored tests pass deterministically, then resubmit the receipt`
  ...
  if (green && verification.passed !== true) {
    return {
      ok: false,
      errors: [
        `runtime-executed green-test failed twice (exit ${verification.exit_code} then exit ${verificationB.exit_code}); green-maintenance requires the authored tests to pass deterministically on the incoming behavior`,
      ],
    };
  }
  ...
  return {
    ...
    observation: {
      ...
      ...(invocation
        ...
        : { configured: true, ...(green || correction ? { test_paths: testPaths } : {}) }),
...
// Per-runner runtime-owned test admission (sibling to observeAuthoredTest,
// reached only when
...
// Admission requires EVERY participant to match the requested stable verdict.
// The whole
...
async function observeAuthoredTestPerRunner(paths, state, ticket, receipt, config, tree) {
  const green = ticket.required_checks.includes('green-test');
  const correction = ticket.required_checks.includes('test-correction');
  const check = correction ? 'test-correction' : green ? 'green-test' : 'red-test';
  const phase = correction ? 'test correction' : green ? 'green maintenance' : 'red phase';
    ...
    return {
      ...
      errors: [
        emptyAuthoredTestPathsRefusal(
          state,
          ticket,
          `${check} admission found no runtime-verifiable authored test files; configure a runner profile.targeted_template or test_commands.targeted`,
        ),
      ],
    };
  }
  ...
  for (let index = 0; index < runners.length; index += 1) {
    ...
    if (template) {
      ...
      if (!rendered) {
        errors.push(
          `runner '${runner.id}' targeted_template must contain the {paths} placeholder to scope ${phase} to its authored test files (paths ${subset.join(', ')})`,
        );
        continue;
      }
      invocation = rendered;
      ...
      if (!invocation || invocation.scoped !== true) {
        errors.push(
          green
            ? `runner '${runner.id}' cannot scope the detected ${detected?.runner ?? 'none'} runner to its authored test paths (${subset.join(', ')}); a whole-suite verdict is not proof the authored tests were the tests observed — configure its profile.targeted_template`
  ...
  for (const orphan of orphans) {
    errors.push(
      `${check} admission authored test path '${orphan}' is owned by no configured runner; add an owns glob or a runner that owns it`,
    );
  }
  ...
  if (participants.length === 0) {
    return {
      ok: false,
      errors: [
        `${check} admission found no runtime-verifiable authored test files to route to any configured runner`,
      ],
    };
  }
  ...
  for (const { participant, verification, verificationB } of results) {
    if (verificationB === null || noVerdict(verificationB)) {
      return {
        ok: false,
        errors: [
          `runner '${participant.runner.id}' ${check} execution did not produce a test verdict; configure its profile.targeted_template or test_commands.targeted`,
        ],
      };
    }
    ...
    for (const run of [verification, verificationB]) {
      ...
      if (run.exit_code === 5) {
        return {
          ...
          errors: [
            green || correction
              ? `runner '${participant.runner.id}' pytest collected no tests (exit 5): the authored tests were never executed — fix test naming/collection; ${phase} was not observed`
      ...
      if (run.exit_code === 4) {
        return {
          ...
          errors: [
            green || correction
              ? `runner '${participant.runner.id}' pytest usage error (exit 4): the ${check} command did not run the authored tests — fix its profile.targeted_template or test_commands.targeted; ${phase} was not observed`
    ...
    if (verification.passed !== verificationB.passed) {
      return {
        ...
        errors: [
          correction
            ? `runner '${participant.runner.id}' runtime-executed test-correction is nondeterministic: the authored tests passed one invocation and failed the other (exit ${verification.exit_code} then exit ${verificationB.exit_code}); a corrected test must have a stable verdict before the implementer retry`
            : green
            ? `runner '${participant.runner.id}' runtime-executed green-test is nondeterministic: the authored tests passed one invocation and failed the other (exit ${verification.exit_code} then exit ${verificationB.exit_code}); unstable green maintenance is not an observed pass/pass result — make the authored tests pass deterministically, then resubmit the receipt`
    ...
    if (green && verification.passed !== true) {
      return {
        ok: false,
        errors: [
          `runner '${participant.runner.id}' runtime-executed green-test failed twice (exit ${verification.exit_code} then exit ${verificationB.exit_code}); green-maintenance requires the authored tests to pass deterministically on the incoming behavior`,
        ],
      };
    }
    // This participant matches the requested verdict across both runs.
  ...
  // Every participant is stable — admit. The observation superset is sealed
  // into the runtime-owned evidence key by the caller after validation.
  return {
    ok: true,
    duration_ms: totalDuration,
    observation: {
      observed: true,
      passed: correction ? breakdown.every((participant) => participant.exit_code === 0) : green,
      tree_sha: receipt.head_tree_sha,
...
// prose-bound-exempt: `reason` here is a positional function parameter, not an
// object-literal sink; its sole caller supplies operatorRecoveryReason after a
// boundedGateSummary call, and the return object neutralizes it again below.
function operatorReceiptRecoveryEvidence(state, ticket, dispatch, inputHash, reason, recoveredAt) {
  const sessionIdentity = state.host === 'codex'
    ? (dispatch.bound_session_id ?? dispatch.parent_session_id)
    : dispatch.parent_session_id;
  if (
    typeof dispatch.bound_agent_id !== 'string' || dispatch.bound_agent_id.length === 0 ||
    typeof sessionIdentity !== 'string' || sessionIdentity.length === 0
  ) {
    return null;
  }
  const dispatchIdentity = {
    ...
    agent_id: dispatch.bound_agent_id,
    agent_type: dispatch.agent_type,
    bound_at: dispatch.bound_at ?? null,
    worker_stopped_at: dispatch.agent_stopped_at ?? null,
    physical_worker_dispatches: dispatch.physical_worker_dispatches ?? 1,
  };
  return {
    version: 1,
    ...
    dispatch_identity_hash: sha256(dispatchIdentity),
...
// A prepared receipt transaction can replay after any process boundary between
// its write, the override-class audit append, active-state persistence, and the
// final transaction commit. Suppress only an exact recovery audit already tied
// to this run, ticket, normalized draft hash, and dispatch identity. An
// unreadable/torn log fails open to a duplicate line rather than allowing an
// accepted operator waiver with no audit trail.
async function operatorReceiptRecoveryAudited(file, runId, ticketId, recovery) {
  return receiptAuditContains(file, (entry) =>
      entry?.run_id === runId &&
      entry?.operation === 'recover-receipt' &&
      entry?.ticket_id === ticketId &&
      entry?.receipt_input_hash === recovery.receipt_input_hash &&
      entry?.dispatch_identity_hash === recovery.dispatch_identity_hash);
}
...
// line is strictly safer than a claim growth with none. Scan the complete
// retained log in bounded chunks: a tail cutoff would miss an older audit.
...
function capabilityRecoveryReceipt(receipt) {
  return receipt?.status === 'failed' &&
    receipt?.evidence?.failure_kind === 'capability' &&
    receipt?.evidence?.required_claims &&
    typeof receipt.evidence.required_claims === 'object' &&
    !Array.isArray(receipt.evidence.required_claims);
}
...
function additiveRecoveryScope(ticket, receipt) {
  const required = receipt.evidence.required_claims;
  const sourceTestPaths = ticket.test_paths ?? [];
  const addedTestPaths = Array.isArray(required.test_paths) ? required.test_paths : [];
  const sourceIdentities = new Set(sourceTestPaths.map((entry) => normalizeClaimPath(entry)));
  const addedIdentities = new Set();
  for (const entry of addedTestPaths) {
    const identity = normalizeClaimPath(entry);
    if (sourceIdentities.has(identity) || addedIdentities.has(identity)) return null;
    addedIdentities.add(identity);
  }
  const testPaths = [...sourceTestPaths, ...addedTestPaths];
  const growthVersion = ticket.capability_manifest?.field_bounds?.dynamic_test_paths?.command_max_chars
    ? 2 : 1;
  if (!capabilityTestPathBoundErrors(testPaths, { version: growthVersion }).valid) return null;
  return {
    claimed_paths: [...new Set([
      ...(ticket.claimed_paths ?? []),
      ...(Array.isArray(required.claimed_paths) ? required.claimed_paths : []),
    ])],
    test_paths: testPaths,
  };
}

function authoritativeRecoveryScope(state, ticket, receipt, lanePolicy) {
  const scope = additiveRecoveryScope(ticket, receipt);
  if (!scope) return null;
  const required = receipt.evidence?.required_claims;
  const classification = recoveryClassification(state, scope, lanePolicy ?? {});
  // A recovery that crosses the full-lane production boundary must stop using
  // a role-narrowed claim projection. The full successor inherits the run's
  // complete production authority plus the exact additive request; fast-lane
  // test-only recovery retains the deliberately narrowed ticket scope.
  if (classification.lane === 'full' && (required?.claimed_paths?.length ?? 0) > 0) {
    const baseClaims = required.required_role && required.required_role !== ticket.role
      ? ticket.claimed_paths ?? []
      : state.claimed_paths ?? [];
    return {
      ...scope,
...
function stableRecoveryTicketFields(ticket) {
  const {
    ticket_id: _ticketId,
    ticket_hash: _ticketHash,
    role: _role,
    claimed_paths: _claimedPaths,
    test_paths: _testPaths,
    model_tier: _modelTier,
    model: _model,
    deadline_at: _deadlineAt,
    output_schema: _outputSchema,
    parent_hash: _parentHash,
    writable: _writable,
    issued_at: _issuedAt,
    capability_manifest: _capabilityManifest,
    recovery_lineage: _recoveryLineage,
    recovery_provenance: _recoveryProvenance,
    ...stable
  } = ticket;
  return stable;
}

function recoveryTransactionShapeError(transaction) {
  if (!transaction || typeof transaction !== 'object' || Array.isArray(transaction)) {
    return 'prepared receipt transaction failed schema or binding validation';
  }
  const common = [
    'committed_at',
    'input_hash',
  ...
  const allowed = transaction.version === 2
    ? [...common, 'prepared_effect', 'prepared_effect_hash']
    : common;
  if (
    ![1, 2].includes(transaction.version) ||
    !['prepared', 'committed'].includes(transaction.status) ||
    Object.keys(transaction).some((key) => !allowed.includes(key)) ||
    common.filter((key) => key !== 'committed_at').some((key) => !Object.hasOwn(transaction, key)) ||
    (transaction.status === 'committed' && !Object.hasOwn(transaction, 'committed_at')) ||
    (transaction.version === 2 && transaction.status === 'prepared' &&
      Object.hasOwn(transaction, 'committed_at')) ||
    (transaction.version === 2 && (
      !Object.hasOwn(transaction, 'prepared_effect') ||
      !Object.hasOwn(transaction, 'prepared_effect_hash')
    ))
  ) {
    return 'prepared receipt transaction failed schema or binding validation';
  }
...
function recoveryClassification(state, scope, lanePolicy = state.capability_snapshot?.lane_policy ?? {}) {
  return escalateLane(state.lane, {
    claimed_paths: scope.claimed_paths,
...
async function frozenCapabilityRecoveryContext(paths, state, ticket) {
  const manifest = await readRunContractManifest(
    paths,
    ticket.capability_manifest?.run_contract,
  );
  const catalog = manifest?.capability_catalog;
  const authority = catalog?.frozen_recovery_authority;
  const ticketContract = manifest?.receipt_contract?.ticket_contracts?.find((entry) =>
    entry?.ticket_id === ticket.ticket_id);
  const sourceAuthority = ticketContract?.recovery_authority;
  if (
    manifest?.run_id !== ticket.run_id ||
    manifest?.config_hash !== ticket.capability_manifest?.config_hash ||
    manifest?.objective_hash !== sha256(ticket.objective) ||
    manifest?.capability_catalog_hash !== sha256(catalog) ||
    manifest?.preflight_hash !== (ticket.capability_manifest?.preflight_hash ?? null) ||
    ticketContract?.stage_id !== ticket.stage_id ||
    ticketContract?.role !== ticket.role ||
    ticketContract?.receipt_contract_version !== ticket.receipt_contract_version ||
    ticketContract?.receipt_schema?.hash !== ticket.capability_manifest?.receipt_schema?.hash ||
    !sourceAuthority ||
    !exactObject(sourceAuthority, {
      ...
      ...(ticket.binding_protocol
        ? { binding_protocol: ticket.binding_protocol }
        : {}),
      ...(ticket.test_intent ? { test_intent: ticket.test_intent } : {}),
      ...(ticket.execution_limits ? { execution_limits: ticket.execution_limits } : {}),
    }) ||
    !authority || typeof authority !== 'object' || Array.isArray(authority) ||
    !authority.models || !authority.role_models || !authority.deadlines_ms ||
    !authority.role_policies || !Array.isArray(authority.stages) ||
    !authority.run_scope || !Array.isArray(authority.run_scope.claimed_paths) ||
    !Array.isArray(authority.run_scope.test_paths) ||
    !Array.isArray(authority.run_scope.risk_triggers) ||
    typeof authority.run_scope.host !== 'string' ||
    authority.run_scope.binding_protocol !== ticket.binding_protocol
  ) {
    throw new Error('frozen run contract lacks exact capability-recovery authority');
  }
  const snapshot = {
    version: 1,
    ...(catalog.artifact_limits_version === 2 ? { artifact_limits_version: 2 } : {}),
    manifest_growth_contract_version: catalog.manifest_growth_contract_version,
    manifest_roles: [...(catalog.manifest_roles ?? [])],
    lane_policy: structuredClone(catalog.lane_policy ?? {}),
    config_hash: manifest.config_hash,
    required_capabilities: structuredClone(manifest.required_capabilities ?? []),
    ...
    frozen_recovery_authority: structuredClone(authority),
  };
  return {
    ...
    state: {
      ...state,
      ...(manifest.execution_policy ? { execution_policy: structuredClone(manifest.execution_policy) } : {}),
      run_id: manifest.run_id,
      host: authority.run_scope.host,
      ...(authority.run_scope.binding_protocol
        ? { binding_protocol: authority.run_scope.binding_protocol }
        : {}),
      objective: ticket.objective,
      claimed_paths: structuredClone(authority.run_scope.claimed_paths),
      test_paths: structuredClone(authority.run_scope.test_paths),
      risk_triggers: structuredClone(authority.run_scope.risk_triggers),
      lane: authority.run_scope.lane,
      mode: authority.run_scope.mode,
      behavioral: authority.run_scope.behavioral,
      ...(authority.run_scope.test_intent
        ? { test_intent: authority.run_scope.test_intent }
        : {}),
      policy: {
        design_assurance_required: authority.run_scope.design_assurance_required === true,
      },
      run_contract: structuredClone(ticket.capability_manifest.run_contract),
      capability_snapshot: snapshot,
      preflight: manifest.preflight_hash === null
        ? null
        : { ...(state.preflight ?? {}), artifact_hash: manifest.preflight_hash },
    },
  };
}

function preparedCapabilityRecoveryError(
  transaction,
  state,
  ticket,
  inputHash,
  ...
  const shapeError = recoveryTransactionShapeError(transaction);
  if (shapeError) return shapeError;
  if (transaction.version !== 2) return null;
  const effect = transaction.prepared_effect;
  const receiptValidation = validateReceipt(transaction.receipt);
  const successorValidation = validateTicket(effect?.successor_contract);
  const expectedScope = capabilityRecoveryReceipt(transaction.receipt)
    ? authoritativeRecoveryScope(
        recoveryContext?.state ?? state,
        ticket,
        transaction.receipt,
        recoveryContext?.manifest?.capability_catalog?.lane_policy,
      )
    : null;
  const expectedStateScope = expectedScope ? {
    claimed_paths: [...new Set([
      ...(recoveryContext?.state?.claimed_paths ?? ticket.claimed_paths ?? []),
      ...expectedScope.claimed_paths,
    ])],
    test_paths: structuredClone(expectedScope.test_paths),
  } : null;
  ...
  const successor = effect?.successor_contract;
  const expectedSuccessorRole = transaction.receipt?.evidence?.required_claims?.required_role ??
    ticket.role;
  const successorRolePolicy = recoveryContext?.authority?.role_policies?.[expectedSuccessorRole] ??
    ROLE_POLICIES[expectedSuccessorRole];
  const expectedPolicy = {
    role: successor?.role,
    writable: successor?.writable,
    required_checks: successor?.required_checks,
    model_tier: successor?.model_tier,
    model: successor?.model,
  };
  const lineage = successor?.recovery_lineage;
  const provenance = successor?.recovery_provenance;
  const uuidSuffix = typeof successor?.ticket_id === 'string'
    ? successor.ticket_id.slice(`${state.run_id}:${ticket.stage_id}:`.length)
    : '';
  const generation = effect?.recovery_generation;
  const generationHashValid = generation &&
    generation.hash === hashRecord(generation, ['hash']);
  const authoritativeGeneration = recoveryContext?.authoritative_generation ??
    state.recovery_generation ?? null;
  const expectedPreviousGeneration = authoritativeGeneration
    ? {
        generation: authoritativeGeneration.generation,
        hash: authoritativeGeneration.hash,
      }
    : null;
  const expectedLane = expectedScope
    ...
    : null;
  const expectedValidationSubmissions = attestation
    ? (ticket.recovery_lineage?.validation_submissions ?? 0) +
      attestation.validation.attempt
    : null;
  const expectedPhysicalWorkers = attestation
    ? (ticket.recovery_lineage?.physical_workers ?? 0) +
      attestation.physical_worker_dispatches
    : null;
  const stateAlreadyAdopted = (state.receipts ?? []).some((entry) =>
    entry.ticket_id === ticket.ticket_id && entry.receipt_hash === transaction.receipt?.receipt_hash);
  // Response loss can leave the immutable selector published before the
  // active projection is installed. That exact validated edge is replay
  // authority already; it must not be treated as a new child of itself.
  const published = recoveryContext?.published_recovery;
  const generationAlreadyPublished = published &&
    published.prepared_effect_hash === transaction.prepared_effect_hash &&
    published.transaction.receipt_hash === transaction.receipt?.receipt_hash &&
    exactObject(published.generation, generation);
  if (
    transaction.run_id !== state.run_id ||
    transaction.ticket_id !== ticket.ticket_id ||
    transaction.input_hash !== inputHash ||
    transaction.prepared_effect_hash !== sha256(effect) ||
    !receiptValidation.valid ||
    transaction.receipt.ticket_id !== ticket.ticket_id ||
    transaction.receipt.ticket_hash !== ticket.ticket_hash ||
    !capabilityRecoveryReceipt(transaction.receipt) ||
    !successorValidation.valid ||
    !exactObject(Object.keys(effect ?? {}).sort(), [
      'byte_budgets',
      'capability_manifest',
      'field_bounds',
      'lane',
    ...
    !exactObject(effect?.source, expectedSource) ||
    !exactObject(effect?.scope?.source, {
      claimed_paths: ticket.claimed_paths,
      test_paths: ticket.test_paths,
    }) ||
    !expectedScope ||
    !exactObject(effect?.scope?.successor, expectedScope) ||
    !exactObject(effect?.scope?.state, expectedStateScope) ||
    !exactObject(effect?.policy, expectedPolicy) ||
    effect?.lane !== expectedLane ||
    !exactObject(effect?.risk_triggers, recoveryContext?.state?.risk_triggers ?? ticket.risk_triggers ?? []) ||
    (!stateAlreadyAdopted && !exactObject(effect?.orchestration, state.orchestration)) ||
    !exactObject(effect?.byte_budgets, ticket.capability_manifest?.byte_budgets) ||
    !exactObject(effect?.field_bounds, ticket.capability_manifest?.field_bounds) ||
    !exactObject(effect?.capability_manifest, successor.capability_manifest) ||
    !exactObject(effect?.run_contract, successor.capability_manifest?.run_contract) ||
    effect?.timing?.source_issued_at !== ticket.issued_at ||
    effect?.timing?.source_deadline_at !== ticket.deadline_at ||
    effect?.timing?.successor_issued_at !== successor.issued_at ||
    effect?.timing?.successor_deadline_at !== successor.deadline_at ||
    effect?.timing?.prepared_at !== transaction.prepared_at ||
    !exactObject(effect?.timing, {
      source_issued_at: ticket.issued_at,
      source_deadline_at: ticket.deadline_at,
      successor_issued_at: successor.issued_at,
      successor_deadline_at: successor.deadline_at,
      prepared_at: transaction.prepared_at,
    }) ||
    !exactObject(effect?.recovery, {
      validation_submissions: lineage?.validation_submissions,
      physical_workers: lineage?.physical_workers,
    }) ||
    successor.run_id !== state.run_id ||
    successor.stage_id !== ticket.stage_id ||
    successor.role !== expectedSuccessorRole ||
    successor.writable !== successorRolePolicy?.writable ||
    successor.model_tier !== (
      expectedSuccessorRole === ticket.role ? ticket.model_tier : successorRolePolicy?.model_tier
    ) ||
    successor.parent_hash !== transaction.receipt.receipt_hash ||
    !exactObject(
      stableRecoveryTicketFields(successor),
      stableRecoveryTicketFields(ticket),
    ) ||
    !exactObject(successor.claimed_paths, expectedScope.claimed_paths) ||
    !exactObject(successor.test_paths, expectedScope.test_paths) ||
    lineage?.source_ticket_id !== ticket.ticket_id ||
    lineage?.validation_submissions_per_worker !== receiptLimits(ticket).max_validation_submissions_per_worker ||
    lineage?.max_physical_workers !== receiptLimits(ticket).max_physical_workers_per_ticket ||
    !Number.isSafeInteger(lineage?.validation_submissions) ||
    lineage.validation_submissions < 1 || lineage.validation_submissions >
      receiptLimits(ticket).max_validation_submissions_per_worker * receiptLimits(ticket).max_physical_workers_per_ticket ||
    !Number.isSafeInteger(lineage?.physical_workers) ||
    lineage.physical_workers < 1 ||
    lineage.physical_workers > receiptLimits(ticket).max_physical_workers_per_ticket ||
    provenance?.authority !== 'runtime' ||
    provenance?.source_ticket_id !== ticket.ticket_id ||
    provenance?.source_ticket_hash !== ticket.ticket_hash ||
    provenance?.source_receipt_id !== transaction.receipt.receipt_id ||
    provenance?.source_receipt_hash !== transaction.receipt.receipt_hash ||
    provenance?.receipt_input_hash !== inputHash ||
    provenance?.source_issued_at !== ticket.issued_at ||
    provenance?.source_deadline_at !== ticket.deadline_at ||
    provenance?.derived_at !== transaction.prepared_at ||
    successor.ticket_id !== `${state.run_id}:${ticket.stage_id}:${uuidSuffix}` ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(uuidSuffix) ||
    !generationHashValid ||
    generation.version !== 1 ||
    generation.source_ticket_hash !== ticket.ticket_hash ||
    generation.successor_ticket_id !== successor.ticket_id ||
    generation.successor_ticket_hash !== successor.ticket_hash ||
    (!stateAlreadyAdopted && !generationAlreadyPublished && (
      generation.generation !== (authoritativeGeneration?.generation ?? 0) + 1 ||
      !exactObject(generation.previous, expectedPreviousGeneration)
    )) ||
    (attestation && (
      lineage.validation_submissions !== expectedValidationSubmissions ||
      lineage.physical_workers !== expectedPhysicalWorkers
    ))
  ) {
    return 'prepared capability recovery failed exact schema or binding validation';
  }
  // state.recovery_generation is a compatibility projection, not authority.
  // A valid selector head below repairs stale or substituted active-state
  // bytes from its exact immutable generation.
  return null;
}

async function preparedCapabilityRecoveryRuntimeError(
  paths,
  transaction,
  _state,
  ticket,
  ...
  const required = transaction.receipt.evidence.required_claims;
  const scope = authoritativeRecoveryScope(
    recoveryContext.state,
    ticket,
    transaction.receipt,
    recoveryContext.manifest.capability_catalog.lane_policy,
  );
  if (!scope) return 'prepared capability recovery has a non-canonical or unbounded test path union';
  const role = required.required_role ?? ticket.role;
  const rolePolicy = recoveryContext?.authority?.role_policies?.[role];
  if (!rolePolicy) return 'prepared capability recovery lacks frozen role-policy authority';
  const modelTier = role === ticket.role ? ticket.model_tier : rolePolicy.model_tier;
  const expectedModel = role === ticket.role
    ? ticket.model
    : resolveModel(recoveryContext.authority, recoveryContext.state.host, modelTier, role);
  const deadlineMs = Date.parse(ticket.deadline_at) - Date.parse(ticket.issued_at);
  if (!Number.isFinite(deadlineMs) || deadlineMs <= 0) {
    return 'prepared capability recovery lacks frozen source timing authority';
  }
  const expectedDeadline = new Date(
    Date.parse(successor.issued_at) + deadlineMs,
  ).toISOString();
  const capabilityBase = ticketCapabilityManifest(
    {
      ...recoveryContext.state,
      claimed_paths: scope.claimed_paths,
      test_paths: scope.test_paths,
  ...
  let outputSchema = capabilityBase
    ? receiptOutputSchemaForTicket({
        ...ticket,
        ...
        test_paths: scope.test_paths,
        receipt_contract_version: RECEIPT_CONTRACT_VERSION,
        capability_manifest: capabilityBase,
      })
    : null;
  if (outputSchema && !exactObject(successor.output_schema, outputSchema)) {
    // Prior version-2 envelopes froze this exact underspecialized wire schema.
    // Replaying one must preserve its ticket/hash/run-contract bytes. Accept
    // only the known historical derivation, never an arbitrary stored schema;
    // receipt semantics still come from the complete immutable ticket and all
    // policy, scope, lineage and manifest checks below remain authoritative.
    const historicalSchema = receiptOutputSchemaForTicket({
      ticket_id: successor.ticket_id,
      stage_id: ticket.stage_id,
      role,
      receipt_contract_version: RECEIPT_CONTRACT_VERSION,
    ...
    if (exactObject(successor.output_schema, historicalSchema)) outputSchema = historicalSchema;
  }
  const capabilityManifest = capabilityBase && outputSchema
    ? {
        ...capabilityBase,
        receipt_schema: { ref: 'ticket.output_schema', hash: sha256(outputSchema) },
        field_bounds: runContractFieldBounds({ run: recoveryContext.state,
          manifest_growth_contract_version: recoveryContext.state.capability_snapshot?.manifest_growth_contract_version ?? 1 }),
        byte_budgets: runContractByteBudgets(recoveryContext.state),
      }
    : null;
  if (
    !capabilityManifest ||
    transaction.prepared_effect.lane !== recoveryClassification(
      recoveryContext.state,
      scope,
      recoveryContext.manifest.capability_catalog.lane_policy ?? {},
    ).lane ||
    transaction.prepared_effect.policy.role !== role ||
    transaction.prepared_effect.policy.writable !== rolePolicy.writable ||
    transaction.prepared_effect.policy.model_tier !== modelTier ||
    !exactObject(transaction.prepared_effect.policy.model, expectedModel) ||
    !exactObject(successor.model, expectedModel) ||
    successor.deadline_at !== expectedDeadline ||
    !exactObject(successor.output_schema, outputSchema) ||
    !exactObject(successor.capability_manifest, {
      ...capabilityManifest,
      run_contract: successor.capability_manifest.run_contract,
    })
  ) {
    return 'prepared capability recovery diverged from runtime-derived policy';
  }
  const preparedRunContract = await prepareTicketRunContract(
    paths,
    recoveryContext.state,
    successor,
    capabilityManifest,
  ...
  if (
    !preparedRunContract ||
    !exactObject(preparedRunContract.pointer, successor.capability_manifest.run_contract)
  ) {
    return 'prepared capability recovery diverged from the runtime-derived run contract';
  }
  const lineage = successor.recovery_lineage;
  if (attestation && (
    lineage.validation_submissions !==
      (ticket.recovery_lineage?.validation_submissions ?? 0) + attestation.validation.attempt ||
    lineage.physical_workers !==
      (ticket.recovery_lineage?.physical_workers ?? 0) + attestation.physical_worker_dispatches
  )) {
    return 'prepared capability recovery diverged from runtime-owned worker ceilings';
  }
  return null;
}

async function prepareCapabilityRecovery(
  paths,
  state,
  ticket,
  receipt,
  ...
  const priorLineage = ticket.recovery_lineage ?? null;
  const currentSubmissions = attestation?.validation?.attempt;
  const currentWorkers = attestation?.physical_worker_dispatches ?? 1;
  const validationSubmissions = (priorLineage?.validation_submissions ?? 0) + currentSubmissions;
  const physicalWorkers = (priorLineage?.physical_workers ?? 0) + currentWorkers;
  if (
    !Number.isSafeInteger(currentSubmissions) ||
    currentSubmissions < 1 ||
    currentSubmissions > receiptLimits(ticket).max_validation_submissions_per_worker ||
    !Number.isSafeInteger(currentWorkers) ||
    currentWorkers < 1 ||
    !Number.isSafeInteger(physicalWorkers) || !Number.isSafeInteger(validationSubmissions) ||
    physicalWorkers > receiptLimits(ticket).max_physical_workers_per_ticket ||
  ...
  if (!scope) return null;
  const role = receipt.evidence.required_claims.required_role ?? ticket.role;
  const rolePolicy = recoveryContext?.authority?.role_policies?.[role];
  if (!rolePolicy) return null;
  const modelTier = role === ticket.role ? ticket.model_tier : rolePolicy.model_tier;
  const model = role === ticket.role
    ? structuredClone(ticket.model)
    : resolveModel(recoveryContext.authority, recoveryContext.state.host, modelTier, role);
  const successorId = `${state.run_id}:${ticket.stage_id}:${randomUUID()}`;
  const issuedAt = preparedAt;
  const deadlineMs = Date.parse(ticket.deadline_at) - Date.parse(ticket.issued_at);
  ...
  if (!capabilityBase) return null;
  const outputSchema = receiptOutputSchemaForTicket({
    ...ticket,
  ...
  const recoveryLineage = {
    ...
    validation_submissions: validationSubmissions,
    physical_workers: physicalWorkers,
    validation_submissions_per_worker: receiptLimits(ticket).max_validation_submissions_per_worker,
    max_physical_workers: receiptLimits(ticket).max_physical_workers_per_ticket,
  };
  const recoveryProvenance = {
    ...
    source_issued_at: ticket.issued_at,
    source_deadline_at: ticket.deadline_at,
    derived_at: preparedAt,
  };
  const {
    ticket_id: _sourceTicketId,
    ticket_hash: _sourceTicketHash,
    issued_at: _sourceIssuedAt,
    deadline_at: _sourceDeadlineAt,
    output_schema: _sourceOutputSchema,
    capability_manifest: _sourceCapabilityManifest,
    recovery_lineage: _sourceRecoveryLineage,
    recovery_provenance: _sourceRecoveryProvenance,
    ...sourceTicketFields
  } = structuredClone(ticket);
  const preliminary = finalizeTicket({
    ...sourceTicketFields,
    ticket_id: successorId,
    role,
    claimed_paths: scope.claimed_paths,
  ...
  if (!preparedRunContract) return null;
  const successor = finalizeTicket({
    ...preliminary,
    capability_manifest: {
      ...capabilityManifest,
      run_contract: preparedRunContract.pointer,
    },
  });
  const authoritativeGeneration = recoveryContext.authoritative_generation ?? null;
  const previousGeneration = authoritativeGeneration
    ? {
        generation: authoritativeGeneration.generation,
        hash: authoritativeGeneration.hash,
      }
  ...
  const generationBody = {
    version: 1,
    generation: (authoritativeGeneration?.generation ?? 0) + 1,
  ...
  const effect = {
    ...
    scope: {
      source: {
        claimed_paths: structuredClone(ticket.claimed_paths),
        test_paths: structuredClone(ticket.test_paths),
      },
      successor: structuredClone(scope),
      state: {
        claimed_paths: [...new Set([
          ...(recoveryContext.state.claimed_paths ?? []),
          ...scope.claimed_paths,
        ])],
        test_paths: structuredClone(scope.test_paths),
      },
    },
    policy: {
      role,
      writable: rolePolicy.writable,
      required_checks: structuredClone(preliminary.required_checks),
      model_tier: modelTier,
      model: structuredClone(model),
    },
    lane: recoveryClassification(
      recoveryContext.state,
      scope,
      recoveryContext.manifest.capability_catalog.lane_policy ?? {},
    ).lane,
    risk_triggers: structuredClone(recoveryContext.state.risk_triggers),
    orchestration: structuredClone(state.orchestration),
    timing: {
      source_issued_at: ticket.issued_at,
      source_deadline_at: ticket.deadline_at,
      successor_issued_at: issuedAt,
      successor_deadline_at: deadlineAt,
      prepared_at: preparedAt,
    },
    byte_budgets: structuredClone(ticket.capability_manifest.byte_budgets),
    field_bounds: structuredClone(ticket.capability_manifest.field_bounds),
    capability_manifest: structuredClone(successor.capability_manifest),
    run_contract: structuredClone(preparedRunContract.pointer),
    recovery: {
      validation_submissions: validationSubmissions,
      physical_workers: physicalWorkers,
    },
  ...
  return { effect, effectHash: sha256(effect), capabilityManifest };
}

const RECOVERY_GENERATION_MAX_MEMBERS = 64;
const RECOVERY_MANIFEST_MAX_BYTES = 256 * 1_024;

function rawRecoveryHash(bytes) {
  return createHash('sha256').update(bytes).digest('hex');
}

const RECOVERY_MEMBER_MAX_BYTES = 64 * 1_024 * 1_024;
const RECOVERY_GENERATION_MAX_BYTES = 256 * 1_024 * 1_024;
const RECOVERY_GENERATION_MAX_DIRECTORY_ENTRIES = 256;
const RECOVERY_READ_CHUNK_BYTES = 64 * 1_024;

function recoveryFileIdentity(metadata) {
  return { dev: String(metadata.dev), ino: String(metadata.ino) };
}

function recoveryIdentityKey(identity) {
  return `${identity.dev}:${identity.ino}`;
}

function exactKeys(value, keys) {
  return value && typeof value === 'object' && !Array.isArray(value) &&
    exactObject(Object.keys(value).sort(), [...keys].sort());
}

async function recoveryMemberSnapshot(
  directory,
  relative,
  maxBytes = RECOVERY_MEMBER_MAX_BYTES,
  declaredDescriptor = null,
) {
  const file = path.join(directory, ...relative.split('/'));
  const root = await realpath(directory);
  const resolvedBefore = await realpath(file);
  if (!resolvedBefore.startsWith(`${root}${path.sep}`)) {
    throw new Error(`capability recovery generation member escapes containment: ${relative}`);
  }
  const handle = await open(file, fsConstants.O_RDONLY | (fsConstants.O_NONBLOCK ?? 0) | (fsConstants.O_NOFOLLOW ?? 0));
  let metadata;
  let bytes;
  try {
    metadata = await statFileHandle(handle);
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1) {
      throw new Error(`capability recovery generation member is not a unique regular file: ${relative}`);
    }
    if (
      !Number.isSafeInteger(metadata.size) || metadata.size < 0 ||
      metadata.size > maxBytes
    ) {
      throw new Error(`capability recovery generation member exceeds its byte bound: ${relative}`);
    }
    if (declaredDescriptor && (
      metadata.size !== declaredDescriptor.bytes ||
      !exactObject(recoveryFileIdentity(metadata), declaredDescriptor.identity)
    )) {
      throw new Error(
        `capability recovery generation member actual size or identity disagrees with its declared descriptor: ${relative}`,
      );
    }
    const chunks = [];
    let total = 0;
    while (total < metadata.size) {
      const chunk = Buffer.allocUnsafe(Math.min(
        RECOVERY_READ_CHUNK_BYTES,
        metadata.size - total,
      ));
      const { bytesRead } = await handle.read(chunk, 0, chunk.length, null);
      if (bytesRead === 0) break;
      total += bytesRead;
      if (total > metadata.size || total > maxBytes) {
        throw new Error(`capability recovery generation member changed while reading: ${relative}`);
      }
      chunks.push(chunk.subarray(0, bytesRead));
    }
    const probe = Buffer.allocUnsafe(1);
    const { bytesRead: extraBytes } = await handle.read(probe, 0, 1, null);
    if (total !== metadata.size || extraBytes !== 0) {
      throw new Error(`capability recovery generation member changed while reading: ${relative}`);
    }
  ...
  // Bytes and identity come from one descriptor. Re-resolve the pathname
  // after the read so a same-byte rename/rebind cannot bless evidence for one
  // inode and later project bytes through another.
  const reboundMetadata = await lstat(file);
  const resolvedAfter = await realpath(file);
  const identity = recoveryFileIdentity(metadata);
  if (
    !reboundMetadata.isFile() || reboundMetadata.isSymbolicLink() ||
    reboundMetadata.nlink !== 1 ||
    resolvedAfter !== resolvedBefore ||
    !exactObject(recoveryFileIdentity(reboundMetadata), identity)
  ) {
    throw new Error(`capability recovery generation member path was rebound: ${relative}`);
  }
  return { bytes, descriptor: {
    raw_sha256: rawRecoveryHash(bytes),
    bytes: bytes.length,
    identity,
  } };
}

async function recoveryMemberDescriptor(directory, relative) {
  return (await recoveryMemberSnapshot(directory, relative)).descriptor;
}

async function exactRecoveryMemberNames(directory) {
  const files = [];
  const directories = [];
  let observed = 0;
  const visit = async (relative = '') => {
    ...
    for await (const entry of entries) {
      observed += 1;
      if (observed > RECOVERY_GENERATION_MAX_DIRECTORY_ENTRIES) {
        throw new Error('capability recovery generation exceeds its bounded entry count');
      }
      const child = relative ? `${relative}/${entry.name}` : entry.name;
      if (canonicalProjectRelativePathError(child, { reserveRuntime: false })) {
        throw new Error(`capability recovery generation has a noncanonical member: ${child}`);
      }
      if (entry.isSymbolicLink()) {
        throw new Error(`capability recovery generation contains a symbolic link: ${child}`);
      }
      if (entry.isDirectory()) {
        const root = await realpath(directory);
        const resolved = await realpath(path.join(directory, ...child.split('/')));
        if (!resolved.startsWith(`${root}${path.sep}`)) {
          throw new Error(`capability recovery generation directory escapes containment: ${child}`);
        }
      ...
      if (!entry.isFile()) {
        throw new Error(`capability recovery generation contains a non-file member: ${child}`);
      }
      if (child !== 'manifest.json') files.push(child);
    }
  };
  await visit();
  files.sort();
  if (files.length === 0 || files.length > RECOVERY_GENERATION_MAX_MEMBERS) {
    throw new Error('capability recovery generation member count is outside its exact bound');
  }
  directories.sort();
  return { files, directories };
}

async function buildRecoveryGenerationManifest(directory, generation, preparedEffectHash) {
  const members = {};
  const identities = new Set();
  const layout = await exactRecoveryMemberNames(directory);
  for (const relative of layout.files) {
    const descriptor = await recoveryMemberDescriptor(directory, relative);
    const identity = recoveryIdentityKey(descriptor.identity);
    if (identities.has(identity)) {
      throw new Error(`capability recovery generation members share a filesystem identity: ${relative}`);
    }
    identities.add(identity);
    members[relative] = descriptor;
  }
  const body = {
    version: 2,
    generation: structuredClone(generation),
    prepared_effect_hash: preparedEffectHash,
    members,
  };
  const manifest = { ...body, hash: sha256(body) };
  if (Buffer.byteLength(JSON.stringify(manifest), 'utf8') > RECOVERY_MANIFEST_MAX_BYTES) {
    throw new Error('capability recovery generation manifest exceeds its byte bound');
  }
  return manifest;
}

async function validateRecoveryGeneration(directory, expected) {
  const manifestSnapshot = await recoveryMemberSnapshot(
    directory,
    'manifest.json',
    RECOVERY_MANIFEST_MAX_BYTES,
  );
  if (manifestSnapshot.descriptor.bytes > RECOVERY_MANIFEST_MAX_BYTES) {
    throw new Error('capability recovery generation manifest is not a bounded unique regular file');
  }
  ...
  try {
    ...
    throw new Error('capability recovery generation manifest is not valid JSON');
  }
  if (
    !exactKeys(manifest, ['version', 'generation', 'prepared_effect_hash', 'members', 'hash']) ||
    manifest.version !== 2 ||
    manifest.hash !== hashRecord(manifest, ['hash']) ||
    !exactObject(manifest.generation, expected.generation) ||
    manifest.prepared_effect_hash !== expected.prepared_effect_hash ||
    !manifest.members || typeof manifest.members !== 'object' || Array.isArray(manifest.members) ||
    Object.keys(manifest.members).length === 0 ||
    Object.keys(manifest.members).length > RECOVERY_GENERATION_MAX_MEMBERS
  ) {
    throw new Error('capability recovery generation manifest failed exact schema or binding validation');
  }
  const layout = await exactRecoveryMemberNames(directory);
  const actualMembers = layout.files;
  const declaredMembers = Object.keys(manifest.members).sort();
  const requiredMembers = [...(expected.members ?? [])].sort();
  if (
    (requiredMembers.length > 0 && !exactObject(actualMembers, requiredMembers)) ||
    !exactObject(actualMembers, declaredMembers)
  ) {
    throw new Error('capability recovery generation manifest does not name the exact member set');
  }
  const declaredDirectories = [...new Set(declaredMembers.flatMap((relative) => {
  ...
  if (!exactObject(layout.directories, declaredDirectories)) {
    throw new Error('capability recovery generation contains an undeclared directory');
  }
  const identities = new Set();
  const verifiedMembers = new Map();
  let cumulativeBytes = 0;
  for (const relative of actualMembers) {
    const declared = manifest.members[relative];
    if (
      !exactKeys(declared, ['raw_sha256', 'bytes', 'identity']) ||
      !exactKeys(declared.identity, ['dev', 'ino']) ||
      typeof declared.raw_sha256 !== 'string' ||
      !/^[0-9a-f]{64}$/.test(declared.raw_sha256) ||
      !Number.isInteger(declared.bytes) || declared.bytes < 0 ||
      declared.bytes > RECOVERY_MEMBER_MAX_BYTES ||
      typeof declared.identity.dev !== 'string' ||
      typeof declared.identity.ino !== 'string'
    ) {
      throw new Error(`capability recovery generation manifest has an invalid descriptor: ${relative}`);
    }
    cumulativeBytes += declared.bytes;
    if (!Number.isSafeInteger(cumulativeBytes) || cumulativeBytes > RECOVERY_GENERATION_MAX_BYTES) {
      throw new Error('capability recovery generation exceeds its cumulative byte bound');
    }
  }
  for (const relative of actualMembers) {
    const declared = manifest.members[relative];
    const snapshot = await recoveryMemberSnapshot(
      directory,
      relative,
      RECOVERY_MEMBER_MAX_BYTES,
      declared,
    );
    const actual = snapshot.descriptor;
    if (!exactObject(actual, declared)) {
      throw new Error(`capability recovery generation member identity or bytes changed: ${relative}`);
    }
    const identity = recoveryIdentityKey(actual.identity);
    if (identities.has(identity)) {
      throw new Error(`capability recovery generation has hard-linked members: ${relative}`);
    }
    identities.add(identity);
    verifiedMembers.set(relative, snapshot);
  }
  return { manifest, members: verifiedMembers, manifest_snapshot: manifestSnapshot };
}

async function quarantineRecoveryGeneration(paths, directory) {
  await mkdir(paths.recoveryQuarantine, { recursive: true });
  const destination = path.join(
    paths.recoveryQuarantine,
    `${path.basename(directory)}.${randomUUID()}.invalid`,
  );
  await rename(directory, destination);
}

function recoveryGenerationDirectory(paths, generation) {
  return path.join(
    paths.recoveryGenerations,
    [String(generation.generation).padStart(8, '0'), generation.hash].join('-'),
  );
}

function recoveryTransactionMemberName(ticketId) {
  return `receipt-transactions/${ticketId.replaceAll(':', '_')}.json`;
}

function expectedRecoveryMemberNames(transaction) {
  const effect = transaction.prepared_effect;
  const successor = effect.successor_contract;
  return [
    'active.json',
    `runs/${transaction.run_id}.json`,
    ...
    `receipts/${transaction.receipt.receipt_id}.json`,
    `tickets/${successor.ticket_id.replaceAll(':', '_')}.json`,
    recoveryTransactionMemberName(transaction.ticket_id),
  ].sort();
}

function recoveryDurableStateError(durableState, transaction) {
  const effect = transaction.prepared_effect;
  const successor = effect.successor_contract;
  const matchingReceipts = (durableState?.receipts ?? []).filter((entry) =>
    entry.ticket_id === transaction.ticket_id);
  const matchingTickets = (durableState?.tickets ?? []).filter((entry) =>
    entry.ticket_id === successor.ticket_id);
  const mismatch =
    durableState?.run_id !== transaction.run_id ? 'run id' :
      durableState?.status !== 'running' ? 'status' :
        durableState?.stage !== successor.stage_id ? 'stage' :
          durableState?.lane !== effect.lane ? 'lane' :
            !exactObject(durableState?.claimed_paths, effect.scope.state.claimed_paths) ? 'claimed paths' :
              !exactObject(durableState?.test_paths, effect.scope.state.test_paths) ? 'test paths' :
                !exactObject(durableState?.risk_triggers, effect.risk_triggers) ? 'risk triggers' :
                  !exactObject(durableState?.recovery_generation, effect.recovery_generation) ? 'recovery generation' :
                    !exactObject(durableState?.run_contract, effect.run_contract) ? 'run contract' :
                      matchingReceipts.length !== 1 ? 'receipt cardinality' :
                        !exactObject(matchingReceipts[0], transaction.receipt) ? 'receipt bytes' :
                          matchingTickets.length !== 1 ? 'ticket cardinality' :
                            !exactObject(matchingTickets[0], successor) ? 'ticket bytes' : null;
  if (mismatch) {
    return `capability recovery durable state is not the complete prepared next state (${mismatch} mismatch)`;
  }
  return null;
}

const RECOVERY_SELECTOR_ROOT_MAX_SLOTS = RECOVERY_GENERATION_MAX_MEMBERS * 2;
const RECOVERY_SELECTOR_CHAIN_DIRECTORY = 'chain-v1';

function exactSelectorEdge(edge, slotHash = null) {
  return exactKeys(edge, [
    ...
    edge.version === 1 &&
    (edge.predecessor === null || /^[0-9a-f]{64}$/.test(edge.predecessor)) &&
    (slotHash === null || edge.hash === slotHash) &&
    edge.hash === hashRecord(edge, ['hash']) &&
    exactKeys(edge.transaction, ['ticket_id', 'input_hash', 'receipt_hash']) &&
    exactKeys(edge.owner_source, ['run_id', 'source_ticket_id', 'source_ticket_hash']) &&
    exactKeys(edge.target, ['successor_ticket_id', 'successor_ticket_hash']) &&
    exactKeys(edge.manifest_binding, ['raw_sha256', 'bytes', 'identity']) &&
    exactKeys(edge.manifest_binding?.identity, ['dev', 'ino']) &&
    exactKeys(edge.generation, [
      'version',
      'generation',
      'previous',
      'source_ticket_hash',
      'successor_ticket_id',
      'successor_ticket_hash',
      'hash',
    ]) &&
    edge.generation.version === 1 &&
    Number.isInteger(edge.generation.generation) && edge.generation.generation > 0 &&
    (edge.generation.previous === null || (
      exactKeys(edge.generation.previous, ['generation', 'hash']) &&
      Number.isInteger(edge.generation.previous.generation) &&
      edge.generation.previous.generation > 0 &&
      /^[0-9a-f]{64}$/.test(edge.generation.previous.hash ?? '')
    )) &&
    /^[0-9a-f]{64}$/.test(edge.generation.hash ?? '') &&
    /^[0-9a-f]{64}$/.test(edge.generation.source_ticket_hash ?? '') &&
    /^[0-9a-f]{64}$/.test(edge.generation.successor_ticket_hash ?? '') &&
    typeof edge.generation.successor_ticket_id === 'string' &&
    /^[A-Za-z0-9._:-]+$/.test(edge.generation.successor_ticket_id) &&
    typeof edge.transaction.ticket_id === 'string' &&
    /^[A-Za-z0-9._:-]+$/.test(edge.transaction.ticket_id) &&
    /^[0-9a-f]{64}$/.test(edge.transaction.input_hash ?? '') &&
    /^[0-9a-f]{64}$/.test(edge.transaction.receipt_hash ?? '') &&
    typeof edge.owner_source.run_id === 'string' &&
    typeof edge.owner_source.source_ticket_id === 'string' &&
    /^[0-9a-f]{64}$/.test(edge.owner_source.source_ticket_hash ?? '') &&
    typeof edge.target.successor_ticket_id === 'string' &&
    /^[0-9a-f]{64}$/.test(edge.target.successor_ticket_hash ?? '') &&
    /^[0-9a-f]{64}$/.test(edge.manifest_binding?.raw_sha256 ?? '') &&
    Number.isInteger(edge.manifest_binding?.bytes) && edge.manifest_binding.bytes > 0 &&
    edge.generation?.hash === hashRecord(edge.generation, ['hash']) &&
    /^[0-9a-f]{64}$/.test(edge.manifest_hash ?? '') &&
    /^[0-9a-f]{64}$/.test(edge.prepared_effect_hash ?? '');
}

function deriveSelectorChain(valid) {
  const byHash = new Map(valid.map((edge) => [edge.hash, edge]));
  if (byHash.size !== valid.length) {
    throw new Error('capability recovery selector chain contains duplicate edges');
  }
  const children = new Map();
  for (const edge of valid) {
    const key = edge.predecessor ?? 'genesis';
    const siblings = children.get(key) ?? [];
    siblings.push(edge);
    children.set(key, siblings);
    if (siblings.length > 1) {
      throw new Error('capability recovery selector chain is forked');
    }
    if (edge.predecessor !== null && !byHash.has(edge.predecessor)) {
      throw new Error('capability recovery selector chain has an unbound predecessor');
    }
    if (edge.predecessor !== null) {
      const predecessor = byHash.get(edge.predecessor);
      if (
        edge.generation.generation !== predecessor.generation.generation + 1 ||
        !exactObject(edge.generation.previous, {
          generation: predecessor.generation.generation,
          hash: predecessor.generation.hash,
        })
      ) {
        throw new Error('capability recovery selector generation lineage does not advance its predecessor');
      }
    }
  }
  if (valid.length === 0) return { edges: [], head: null };
  const roots = children.get('genesis') ?? [];
  if (roots.length !== 1) {
    throw new Error('capability recovery selector chain lacks one canonical genesis');
  }
  const visited = new Set();
  let head = roots[0];
  for (;;) {
    if (visited.has(head.hash)) {
      throw new Error('capability recovery selector chain contains a cycle');
    }
    visited.add(head.hash);
    const next = children.get(head.hash) ?? [];
    if (next.length === 0) break;
    head = next[0];
  }
  if (visited.size !== valid.length) {
    throw new Error('capability recovery selector chain contains disconnected authority');
  }
  return { edges: valid, head };
}

async function validateSelectorEdge(paths, edge) {
  const generationDirectory = recoveryGenerationDirectory(paths, edge.generation);
  const transactionRelative = recoveryTransactionMemberName(edge.transaction.ticket_id);
  let transaction;
  try {
    transaction = JSON.parse(
      (await recoveryMemberSnapshot(generationDirectory, transactionRelative)).bytes.toString('utf8'),
    );
  } catch {
    throw new Error('capability recovery selector has no exact prepared transaction');
  }
  const effect = transaction?.prepared_effect;
  const successor = effect?.successor_contract;
  if (
    recoveryTransactionShapeError(transaction) ||
    transaction.version !== 2 || transaction.status !== 'committed' ||
    transaction.prepared_effect_hash !== edge.prepared_effect_hash ||
    transaction.input_hash !== edge.transaction.input_hash ||
    transaction.ticket_id !== edge.transaction.ticket_id ||
    transaction.receipt?.receipt_hash !== edge.transaction.receipt_hash ||
    !exactObject(effect?.recovery_generation, edge.generation) ||
    transaction.run_id !== edge.owner_source.run_id ||
    effect?.source?.ticket_id !== edge.owner_source.source_ticket_id ||
    effect?.source?.ticket_hash !== edge.owner_source.source_ticket_hash ||
    successor?.ticket_id !== edge.target.successor_ticket_id ||
    successor?.ticket_hash !== edge.target.successor_ticket_hash
  ) {
    throw new Error('capability recovery selector transaction, owner, or target binding is invalid');
  }
  let validated;
  try {
    validated = await validateRecoveryGeneration(generationDirectory, {
      generation: edge.generation,
      prepared_effect_hash: edge.prepared_effect_hash,
      members: expectedRecoveryMemberNames(transaction),
    });
  } catch (error) {
    error.code = 'APE_RECOVERY_GENERATION_INVALID';
    throw error;
  }
  if (
    validated.manifest.hash !== edge.manifest_hash ||
    !exactObject(validated.manifest_snapshot.descriptor, edge.manifest_binding)
  ) {
    throw new Error('capability recovery selector manifest binding is invalid');
  }
  const activeBytes = validated.members.get('active.json')?.bytes;
  const runBytes = validated.members.get(`runs/${transaction.run_id}.json`)?.bytes;
  let durableState;
  try {
    durableState = JSON.parse(activeBytes.toString('utf8'));
  } catch {
    throw new Error('capability recovery selector durable state is invalid');
  }
  if (!activeBytes.equals(runBytes) || recoveryDurableStateError(durableState, transaction)) {
    const error = Object.assign(
      new Error('capability recovery selector durable state is not the prepared next state'),
      { code: 'APE_RECOVERY_GENERATION_INVALID' },
    );
    throw error;
  }
  const receipt = JSON.parse(
    validated.members.get(`receipts/${transaction.receipt.receipt_id}.json`).bytes.toString('utf8'),
  );
  const ticket = JSON.parse(
    validated.members.get(`tickets/${successor.ticket_id.replaceAll(':', '_')}.json`).bytes.toString('utf8'),
  );
  const runContract = JSON.parse(
    validated.members.get(`contracts/${successor.capability_manifest.run_contract.hash}.json`).bytes.toString('utf8'),
  );
  const receiptSchema = JSON.parse(
    validated.members.get(`contracts/schemas/${successor.capability_manifest.receipt_schema.hash}.json`).bytes.toString('utf8'),
  );
  if (
    !exactObject(receipt, transaction.receipt) ||
    !exactObject(ticket, successor) ||
    sha256(runContract) !== successor.capability_manifest.run_contract.hash ||
    sha256(receiptSchema) !== successor.capability_manifest.receipt_schema.hash
  ) {
    throw new Error('capability recovery selector materialized authority is invalid');
  }
  return { ...validated, transaction, durableState, generationDirectory };
}

async function quarantineSelectorSlot(paths, slot, receiptLease, expectedSnapshot = null) {
  await assertDirLockLease(paths.receiptLock, receiptLease);
  const expected = expectedSnapshot ?? await recoveryMemberSnapshot(
    path.dirname(slot),
    path.basename(slot),
    RECOVERY_MANIFEST_MAX_BYTES,
  );
  const current = await recoveryMemberSnapshot(
    path.dirname(slot),
    path.basename(slot),
    RECOVERY_MANIFEST_MAX_BYTES,
  );
  if (
    !exactObject(current.descriptor, expected.descriptor) ||
    !current.bytes.equals(expected.bytes)
  ) {
    throw new Error('capability recovery selector slot identity or bytes changed before quarantine');
  }
  let parsed = null;
  try {
    parsed = JSON.parse(expected.bytes.toString('utf8'));
  } catch {
    // Invalid JSON still receives immutable byte/identity evidence. It simply
    // has no claimed selector lineage to retain.
  }
  const lineage = {
    ...(typeof parsed?.hash === 'string' ? { selector_hash: parsed.hash } : {}),
    ...(typeof parsed?.generation?.hash === 'string'
      ? { generation_hash: parsed.generation.hash }
      : {}),
    ...(typeof parsed?.predecessor === 'string'
      ? { predecessor_hash: parsed.predecessor }
      : {}),
  };
  const body = {
    version: 1,
    source_slot: path.basename(slot),
    source: structuredClone(expected.descriptor),
    payload_base64: expected.bytes.toString('base64'),
    lineage,
  };
  const evidence = { ...body, hash: sha256(body) };
  await mkdir(paths.recoverySelectorQuarantine, { recursive: true });
  const destination = path.join(
    paths.recoverySelectorQuarantine,
    `${evidence.hash}.json`,
  );
  await assertDirLockLease(paths.receiptLock, receiptLease);
  const published = await publishImmutableJson(destination, evidence);
  if (!published) {
    let existing = null;
    try {
      existing = await readJson(destination, null);
    } catch {
      throw new Error('capability recovery semantic quarantine record collision');
    }
    if (!exactObject(existing, evidence)) {
      throw new Error('capability recovery semantic quarantine record collision');
    }
  }
  // The live selector pathname is immutable forensic input. Revalidate it
  // after publishing the append-only evidence record, but never rename, link,
  // unlink, delete, replace, restore, or otherwise mutate it.
  const retained = await recoveryMemberSnapshot(
    path.dirname(slot),
    path.basename(slot),
    RECOVERY_MANIFEST_MAX_BYTES,
  );
  if (
    !exactObject(retained.descriptor, expected.descriptor) ||
    !retained.bytes.equals(expected.bytes)
  ) {
    throw new Error('capability recovery selector slot changed while recording quarantine evidence');
  }
}

async function readSelectorChain(paths, receiptLease) {
  await assertDirLockLease(paths.receiptLock, receiptLease);
  await mkdir(paths.recoverySelectors, { recursive: true });
  const valid = [];
  const entries = [];
  let rolloverDirectoryEntry = null;
  const directory = await opendir(paths.recoverySelectors);
  for await (const entry of directory) {
    if (entry.name === RECOVERY_SELECTOR_CHAIN_DIRECTORY) {
      if (rolloverDirectoryEntry !== null || !entry.isDirectory() || entry.isSymbolicLink()) {
        throw new Error('capability recovery selector rollover directory is not canonical');
      }
      rolloverDirectoryEntry = entry;
      continue;
    }
    if (entries.length >= RECOVERY_SELECTOR_ROOT_MAX_SLOTS) {
      throw new Error('capability recovery selector store exceeds its bounded slot count');
    }
    entries.push(entry);
  }
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    const slot = path.join(paths.recoverySelectors, entry.name);
    let edge = null;
    let slotSnapshot = null;
    const match = /^([0-9a-f]{64})\.json$/.exec(entry.name);
    try {
      if (!match || !entry.isFile() || entry.isSymbolicLink()) {
        throw new Error('invalid selector slot');
      }
      slotSnapshot = await recoveryMemberSnapshot(
        paths.recoverySelectors,
        entry.name,
        RECOVERY_MANIFEST_MAX_BYTES,
      );
      edge = JSON.parse(slotSnapshot.bytes.toString('utf8'));
      if (!exactSelectorEdge(edge, match[1])) throw new Error('invalid selector edge');
      await validateSelectorEdge(paths, edge);
    } catch (error) {
      // A valid immutable selector remains useful forensic authority when its
      // referenced generation has been corrupted. Quarantine malformed or
      // forged selector bytes, but preserve a valid edge and fail closed on
      // the damaged materialization.
      if (error?.code === 'APE_RECOVERY_GENERATION_INVALID') throw error;
      await quarantineSelectorSlot(paths, slot, receiptLease, slotSnapshot);
      continue;
    }
    valid.push(edge);
  }
  let chain = deriveSelectorChain(valid);
  const chainDirectory = path.join(paths.recoverySelectors, RECOVERY_SELECTOR_CHAIN_DIRECTORY);
  if (rolloverDirectoryEntry !== null) {
    // Each successor has one deterministic, predecessor-addressed slot. The
    // reader follows that exact linked path and therefore never enumerates an
    // append-only directory whose size can eventually exceed a fixed bound.
    for (;;) {
      const predecessor = chain.head?.hash ?? null;
      const slotName = `${predecessor ?? 'genesis'}.json`;
      let slotSnapshot;
      try {
        slotSnapshot = await recoveryMemberSnapshot(
          chainDirectory,
      ...
      const slot = path.join(chainDirectory, slotName);
      let edge = null;
      try {
        edge = JSON.parse(slotSnapshot.bytes.toString('utf8'));
        if (!exactSelectorEdge(edge) || edge.predecessor !== predecessor) {
          throw new Error('invalid rollover selector edge');
        }
        await validateSelectorEdge(paths, edge);
      } catch (error) {
        if (error?.code === 'APE_RECOVERY_GENERATION_INVALID') throw error;
      ...
      valid.push(edge);
      const next = deriveSelectorChain(valid);
      if (next.head?.hash !== edge.hash) {
        throw new Error('capability recovery rollover selector did not extend the unique head');
      }
      chain = next;
    }
  }
  return {
    ...chain,
    root_slots: entries.length,
    rollover: rolloverDirectoryEntry !== null,
  };
}

async function recoveryManifestBinding(directory) {
  return recoveryMemberDescriptor(directory, 'manifest.json');
}

function selectorEdgeFor(transaction, predecessor, manifest, manifestBinding) {
  const effect = transaction.prepared_effect;
  const successor = effect.successor_contract;
  const body = {
    version: 1,
    predecessor,
    generation: structuredClone(effect.recovery_generation),
    manifest_hash: manifest.hash,
    manifest_binding: structuredClone(manifestBinding),
    prepared_effect_hash: transaction.prepared_effect_hash,
    transaction: {
      ticket_id: transaction.ticket_id,
      input_hash: transaction.input_hash,
      receipt_hash: transaction.receipt.receipt_hash,
    },
    owner_source: {
      run_id: transaction.run_id,
      source_ticket_id: effect.source.ticket_id,
      source_ticket_hash: effect.source.ticket_hash,
    },
    target: {
      successor_ticket_id: successor.ticket_id,
      successor_ticket_hash: successor.ticket_hash,
...
async function ensureSelectorEdge(
  paths,
  _state,
  transaction,
  manifest,
  generationDirectory,
  receiptLease,
  validatedLegacyAnchor = null,
) {
  let chain = await readSelectorChain(paths, receiptLease);
  const manifestBinding = await recoveryManifestBinding(generationDirectory);
  const effect = transaction.prepared_effect;
  const target = chain.edges.find((edge) =>
    edge.prepared_effect_hash === transaction.prepared_effect_hash &&
    edge.transaction.ticket_id === transaction.ticket_id &&
    edge.transaction.input_hash === transaction.input_hash &&
    exactObject(edge.generation, effect.recovery_generation));
  let edge;
  if (target) {
    edge = selectorEdgeFor(transaction, target.predecessor, manifest, manifestBinding);
    if (!exactObject(edge, target)) {
      throw new Error('capability recovery selector edge conflicts with reachable authority');
    }
    return {
      edge: target,
      head: chain.head,
      already_reachable: true,
      non_head: chain.head?.hash !== target.hash,
    };
  }
  if (chain.head) {
    if (!exactObject(effect.recovery_generation.previous, {
      generation: chain.head.generation.generation,
      hash: chain.head.generation.hash,
    })) {
      throw new Error('capability recovery selector predecessor conflicts with the prepared generation');
    }
  } else if (
    effect.recovery_generation.previous !== null &&
    (
      validatedLegacyAnchor?.generation !== effect.recovery_generation.previous.generation ||
      validatedLegacyAnchor?.hash !== effect.recovery_generation.previous.hash
    )
  ) {
    throw new Error('capability recovery selector chain is absent without a validated legacy anchor');
  }
  edge = selectorEdgeFor(
    transaction,
    chain.head?.hash ?? null,
    manifest,
    manifestBinding,
  );
  const useRollover = chain.rollover || chain.root_slots >= RECOVERY_SELECTOR_ROOT_MAX_SLOTS;
  let slot;
  if (useRollover) {
    await assertDirLockLease(paths.receiptLock, receiptLease);
    const chainDirectory = path.join(paths.recoverySelectors, RECOVERY_SELECTOR_CHAIN_DIRECTORY);
    await mkdir(chainDirectory, { recursive: true });
    const directoryIdentity = await lstat(chainDirectory);
    if (!directoryIdentity.isDirectory() || directoryIdentity.isSymbolicLink()) {
      throw new Error('capability recovery selector rollover directory is not a canonical directory');
    }
    await assertDirLockLease(paths.receiptLock, receiptLease);
    slot = path.join(chainDirectory, `${edge.predecessor ?? 'genesis'}.json`);
  } else {
    slot = path.join(paths.recoverySelectors, `${edge.hash}.json`);
  }
  await assertDirLockLease(paths.receiptLock, receiptLease);
  const published = await publishImmutableJson(slot, edge);
  if (!published) {
    const existing = await readJson(slot, null);
    if (!exactObject(existing, edge)) {
      throw new Error('capability recovery selector slot is occupied by conflicting bytes');
    }
  }
  chain = await readSelectorChain(paths, receiptLease);
  if (chain.head?.hash !== edge.hash) {
    throw new Error('capability recovery selector edge did not become the unique head');
  }
  return {
    edge,
    head: chain.head,
...
async function publishCapabilityRecovery(
  paths,
  state,
  ticket,
  transaction,
  ...
  let effect = transaction.prepared_effect;
  const successor = effect.successor_contract;
  const existingReceipt = state.receipts.find((entry) => entry.ticket_id === ticket.ticket_id);
  const existingSuccessor = state.tickets.find((entry) => entry.ticket_id === successor.ticket_id);
  if (existingReceipt && existingReceipt.receipt_hash !== transaction.receipt.receipt_hash) {
    throw new Error('prepared capability recovery conflicts with the adopted source receipt');
  }
  if (existingSuccessor && !exactObject(existingSuccessor, successor)) {
    throw new Error('prepared capability recovery conflicts with the adopted successor ticket');
  }

  const selectorChainAtEntry = await readSelectorChain(paths, receiptLease);
  const reachableTransactionEdge = selectorChainAtEntry.edges.find((edge) =>
    edge.transaction.ticket_id === transaction.ticket_id &&
    edge.transaction.input_hash === transaction.input_hash &&
    edge.transaction.receipt_hash === transaction.receipt.receipt_hash);
  if (
    reachableTransactionEdge &&
    reachableTransactionEdge.prepared_effect_hash !== transaction.prepared_effect_hash
  ) {
    const reachable = await validateSelectorEdge(paths, reachableTransactionEdge);
    if (!exactObject(reachable.transaction.receipt, transaction.receipt)) {
      throw new Error('reachable capability recovery selector conflicts with the source receipt');
    }
    transaction = reachable.transaction;
    effect = transaction.prepared_effect;
  }
  const selectorlessLegacy = Boolean(
    existingReceipt &&
    existingSuccessor &&
    selectorChainAtEntry.edges.length === 0,
  );
  let validatedLegacyAnchor = null;
  if (selectorlessLegacy) {
    const legacyDirectory = recoveryGenerationDirectory(paths, effect.recovery_generation);
    const legacyGeneration = await validateRecoveryGeneration(legacyDirectory, {
      generation: effect.recovery_generation,
      prepared_effect_hash: transaction.prepared_effect_hash,
      members: expectedRecoveryMemberNames(transaction),
    });
    const legacyState = JSON.parse(
      legacyGeneration.members.get('active.json').bytes.toString('utf8'),
    );
    const legacyTransaction = JSON.parse(
      legacyGeneration.members
        .get(recoveryTransactionMemberName(transaction.ticket_id)).bytes.toString('utf8'),
    );
    const legacyStateError = recoveryDurableStateError(legacyState, legacyTransaction);
    if (legacyStateError || !exactObject(legacyTransaction, transaction)) {
      throw new Error(legacyStateError ??
        'selectorless capability recovery generation has a conflicting prepared transaction');
    }
    const previous = {
      generation: effect.recovery_generation.generation,
      hash: effect.recovery_generation.hash,
    };
    validatedLegacyAnchor = previous;
    const generationBody = {
      version: 1,
  ...
  const generationName = path.basename(
    recoveryGenerationDirectory(paths, effect.recovery_generation),
  );
  const generationDirectory = recoveryGenerationDirectory(paths, effect.recovery_generation);
  const committedTransaction = {
    ...transaction,
    status: 'committed',
    // Deterministic across a crash after generation publication but before the
    // canonical transaction projection. The preparation timestamp is already
    // bound into the effect and is therefore the stable commit boundary.
    committed_at: transaction.committed_at ?? transaction.prepared_at,
  };
  const expectedMembers = expectedRecoveryMemberNames(committedTransaction);

  let nextState = state;
  let generationManifest;
  let publishedGeneration = false;
  if ((!existingSuccessor && !reachableTransactionEdge) || selectorlessLegacy) {
    const { run_contract: _runContract, ...capabilityManifest } = successor.capability_manifest;
    const preparedRunContract = existingSuccessor
      ? {
          pointer: structuredClone(successor.capability_manifest.run_contract),
          manifest: await readRunContractManifest(
            paths,
            successor.capability_manifest.run_contract,
          ),
        }
      : await prepareTicketRunContract(
          paths,
          recoveryContext.state,
          successor,
          capabilityManifest,
    ...
    if (
      ...
      throw new Error('published capability recovery run contract changed after prepare');
    }

    nextState = structuredClone(state);
    if (!existingReceipt) nextState.receipts.push(transaction.receipt);
    if (!existingSuccessor) nextState.tickets.push(successor);
    nextState.claimed_paths = structuredClone(effect.scope.state.claimed_paths);
    nextState.test_paths = structuredClone(effect.scope.state.test_paths);
    nextState.lane = effect.lane;
    nextState.risk_triggers = structuredClone(effect.risk_triggers);
    nextState.recovery_generation = structuredClone(effect.recovery_generation);
    nextState.run_contract = structuredClone(successor.capability_manifest.run_contract);
    nextState.host = recoveryContext.state.host;
    nextState.objective = recoveryContext.state.objective;
    nextState.mode = recoveryContext.state.mode;
    nextState.behavioral = recoveryContext.state.behavioral;
    nextState.policy = structuredClone(recoveryContext.state.policy);
    if (recoveryContext?.state?.capability_snapshot) {
      nextState.capability_snapshot = structuredClone(
        recoveryContext.state.capability_snapshot,
      );
    }
    nextState.status = 'running';
    nextState.stage = successor.stage_id;
    delete nextState.block_reason;
    delete nextState.terminal_at;
    if (receiptValidationSummary !== null) {
      nextState.orchestration = recordAcceptedReceipt(nextState.orchestration, {
        ...receiptValidationSummary,
        accepted_at: transaction.receipt.timing?.completed_at ?? transaction.prepared_at,
      });
    }
    // Recovery does not alter the worktree. Keep the predecessor tree evidence
    // and use one bound timestamp instead of introducing a replay-varying byte.
    nextState.updated_at = transaction.prepared_at;

    const schemaHash = successor.capability_manifest.receipt_schema.hash;
    const contractHash = successor.capability_manifest.run_contract.hash;
    const receiptName = `${transaction.receipt.receipt_id}.json`;
    const ticketName = `${successor.ticket_id.replaceAll(':', '_')}.json`;
    const transactionName = path.basename(recoveryTransactionMemberName(ticket.ticket_id));
    const entries = {
      'active.json': nextState,
      [`runs/${state.run_id}.json`]: nextState,
      [`contracts/${contractHash}.json`]: preparedRunContract.manifest,
      [`contracts/schemas/${schemaHash}.json`]: successor.output_schema,
      [`receipts/${receiptName}`]: transaction.receipt,
      [`tickets/${ticketName}`]: successor,
      [`receipt-transactions/${transactionName}`]: committedTransaction,
    };
    await mkdir(paths.recoveryGenerations, { recursive: true });
    const stagingDirectory = path.join(
      paths.recoveryGenerations,
      `${generationName}.acquire.${randomUUID()}`,
    );
    await mkdir(stagingDirectory);
    let published = false;
    try {
      for (const [name, value] of Object.entries(entries)) {
        await atomicWriteJson(path.join(stagingDirectory, name), value);
      }
      generationManifest = await buildRecoveryGenerationManifest(
        stagingDirectory,
        effect.recovery_generation,
        transaction.prepared_effect_hash,
      );
      await atomicWriteJson(path.join(stagingDirectory, 'manifest.json'), generationManifest);
      await validateRecoveryGeneration(stagingDirectory, {
        generation: effect.recovery_generation,
        prepared_effect_hash: transaction.prepared_effect_hash,
        members: expectedMembers,
      });
      published = await publishImmutableDirectory(stagingDirectory, generationDirectory);
  ...
  // A publisher that died after the generation rename, or the loser of a
  // cooperating-writer race, must reuse exactly that immutable generation.
  // Validate the complete manifest and every member before any canonical sink.
  let durableManifest;
  let validatedGeneration;
  try {
    validatedGeneration = await validateRecoveryGeneration(generationDirectory, {
      generation: effect.recovery_generation,
      prepared_effect_hash: transaction.prepared_effect_hash,
      members: expectedMembers,
    });
    durableManifest = validatedGeneration.manifest;
    if (generationManifest && !exactObject(durableManifest, generationManifest)) {
      throw new Error('capability recovery generation manifest changed during publication');
    }
  } catch (error) {
    if (publishedGeneration) {
      await quarantineRecoveryGeneration(paths, generationDirectory).catch(() => {});
    }
    throw error;
  }
  const durableState = JSON.parse(
    validatedGeneration.members.get('active.json').bytes.toString('utf8'),
  );
  assertActiveStateWritable(durableState);
  const durableReceipt = JSON.parse(
    validatedGeneration.members
      .get(`receipts/${transaction.receipt.receipt_id}.json`).bytes.toString('utf8'),
  );
  const durableSuccessor = JSON.parse(
    validatedGeneration.members
      .get(`tickets/${successor.ticket_id.replaceAll(':', '_')}.json`).bytes.toString('utf8'),
  );
  const durableStateError = recoveryDurableStateError(durableState, committedTransaction);
  if (
    durableStateError ||
    !exactObject(durableReceipt, transaction.receipt) ||
    !exactObject(durableSuccessor, successor) ||
    !exactObject(durableState.recovery_generation, effect.recovery_generation) ||
    !durableState.receipts.some((entry) =>
      entry.ticket_id === ticket.ticket_id && entry.receipt_hash === transaction.receipt.receipt_hash) ||
    !durableState.tickets.some((entry) =>
      entry.ticket_id === successor.ticket_id && entry.ticket_hash === successor.ticket_hash)
  ) {
    throw new Error(durableStateError ??
      'capability recovery generation does not bind the prepared successor state');
  }

  // The selector edge is the sole recovery head. It is published only after
  // the destination directory has passed identity, exact-member, byte and
  // hash validation, and it is never replaced on replay.
  const selector = await ensureSelectorEdge(
    paths,
    state,
    transaction,
    durableManifest,
    generationDirectory,
    receiptLease,
    validatedLegacyAnchor,
  );
  if (selector.already_reachable && selector.non_head) {
    return {
      ok: true,
      ...
      capability_recovery: {
        successor_ticket_id: successor.ticket_id,
        generation: selector.head.generation.generation,
      },
    };
  }

  // The generation directory is authoritative. These canonical paths are
  // compatibility projections installed from its already-validated bytes.
  // All land before active.json, so a process death leaves either the old
  // active generation or a new active generation whose projections are whole.
  const installProjection = async (relative, destination, { replace = false } = {}) => {
    const verified = validatedGeneration.members.get(relative);
    if (!verified) {
      throw new Error(`capability recovery projection is not an exact manifest member: ${relative}`);
    }
    const fresh = await recoveryMemberSnapshot(generationDirectory, relative);
    if (
      !exactObject(fresh.descriptor, verified.descriptor) ||
      !fresh.bytes.equals(verified.bytes)
    ) {
      throw new Error(`capability recovery projection source changed at sink: ${relative}`);
    }
    const bytes = verified.bytes;
    let current = null;
    try {
      current = await readBoundedFile(destination, RECOVERY_MEMBER_MAX_BYTES);
    } catch (error) {
      if (error?.code !== 'ENOENT') throw error;
    }
    if (current !== null && current.equals(bytes)) return;
    if (current !== null && !replace) {
      throw new Error(`capability recovery canonical projection conflicts at ${relative}`);
    }
    await withDirLockLeaseMutation(
      paths.receiptLock,
      receiptLease,
      () => atomicReplaceText(destination, bytes),
    );
    const installed = await readBoundedFile(destination, RECOVERY_MEMBER_MAX_BYTES);
    if (!installed.equals(bytes) || rawRecoveryHash(installed) !== verified.descriptor.raw_sha256) {
      throw new Error(`capability recovery projection destination failed exact byte validation: ${relative}`);
    }
  };
  const contractHash = successor.capability_manifest.run_contract.hash;
  const schemaHash = successor.capability_manifest.receipt_schema.hash;
  ...
  await installProjection(
    `contracts/schemas/${schemaHash}.json`,
    path.join(paths.contracts, 'schemas', `${schemaHash}.json`),
  );
  await installProjection(
    `contracts/${contractHash}.json`,
    path.join(paths.contracts, `${contractHash}.json`),
  );
  await installProjection(`receipts/${receiptName}`, path.join(paths.receipts, receiptName));
  await installProjection(`tickets/${ticketName}`, path.join(paths.tickets, ticketName));
  await installProjection(
    `receipt-transactions/${transactionName}`,
    receiptTransactionPath(paths, ticket.ticket_id),
    { replace: true },
  );
  await installProjection(
    `runs/${state.run_id}.json`,
    path.join(paths.runs, `${state.run_id}.json`),
    { replace: true },
  );
  if (!exactObject(state, durableState)) {
    await withDirLockLeaseMutation(
      paths.receiptLock,
      receiptLease,
      () => atomicWriteJson(paths.active, durableState),
    );
  }
  await assertDirLockLease(paths.receiptLock, receiptLease);
  for (const key of Object.keys(state)) delete state[key];
  Object.assign(state, structuredClone(durableState));
  if (dispatchBinding?.record) {
    await withDirLockLeaseMutation(
      ...
      () => completeClaudeReceiptBinding(
        ...
        ticket,
        dispatchBinding,
        transaction.input_hash,
        transaction.receipt,
      ),
    );
  }
  const dispatch = await withDirLockLeaseMutation(
    paths.receiptLock,
    receiptLease,
    () => dispatchForTicket(paths, state, successor, 1, {
  ...
  return {
    ...
    receipt: transaction.receipt,
    run: state,
    actions: existingSuccessor ? [] : [recoveryAction],
    capability_recovery: {
      successor_ticket_id: successor.ticket_id,
      generation: effect.recovery_generation.generation,
    },
    ...(existingSuccessor ? { idempotent: true, recovered: true } : {}),
  };
}

async function parentTreeReceiptRefusal(paths, state) {
  try {
    // This read must be fresh when an observation exists: treeShaSession may
    // predate a parent post event or runtime-owned test execution. The store
    // skips the thunk entirely for runs without unresolved observations.
    const observation = await checkTreeAttribution(paths, state.run_id, () => currentTreeSha(paths.root));
    if (!observation.blocked) return null;
    return {
      ok: false, rejected: true, errors: [treeAttributionRefusal(observation)],
      recovery: receiptRejectionRecovery(state, 'receipt-tree-diverged'),
    };
  } catch {
    return {
      ok: false, rejected: true,
      errors: ['APE receipt denied: parent-tool attribution evidence is unreadable; inspect and recover that evidence before accepting new results'],
      recovery: receiptRejectionRecovery(state, 'receipt-tree-diverged'),
    };
  }
}

export async function recordReceiptLocked(projectDir, raw, normalizedFields = [], options = {}) {
  ...
  const requestedOperatorRecovery = options.operator_recovery ?? null;
  const receiptLease = options.receipt_lease ?? null;
  const operatorRecoveryReason = requestedOperatorRecovery
    ? boundedGateSummary(requestedOperatorRecovery.reason)
    : null;
  if (requestedOperatorRecovery && operatorRecoveryReason === null) {
    throw new Error('recover-receipt requires a nonblank audit reason');
  }
  const paths = runtimePaths(projectDir);
  ...
  let authenticatedRecoveryBinding = null;
  if (
    ticket.receipt_contract_version === RECEIPT_CONTRACT_VERSION &&
    capabilityRecoveryReceipt(raw) &&
    validateReceiptDraft(ticket, raw).valid === true
  ) {
    try {
      recoveryContext = await frozenCapabilityRecoveryContext(paths, state, ticket);
      const recoveryBindingRequired =
        recoveryContext.state.host === 'claude' ||
        (
          recoveryContext.state.host === 'codex' &&
          recoveryContext.state.binding_protocol === 'native-v1'
        );
      if (recoveryBindingRequired) {
        authenticatedRecoveryBinding = await validateClaudeReceiptBinding(
          paths,
          recoveryContext.state,
          ticket,
          raw.receipt_capability,
          inputHash,
          { allow_receipted_recovery: true },
        );
        if (!authenticatedRecoveryBinding.valid) {
          return {
            ok: false,
            rejected: true,
            errors: [
              `${recoveryContext.state.host} receipt identity or one-time capability has no exact active ticket binding`,
            ],
          };
        }
      }
      const selectorChain = await readSelectorChain(paths, receiptLease);
      recoveryContext.authoritative_generation = selectorChain.head?.generation ?? null;
      recoveryContext.published_recovery = selectorChain.edges.find((edge) =>
        edge.transaction.ticket_id === ticket.ticket_id && edge.transaction.input_hash === inputHash) ?? null;
    } catch (error) {
      if (!state.receipts.some((entry) => entry.ticket_id === ticket.ticket_id)) {
        const baseline = await resolveReceiptBaseline({ project_dir: paths.root, state, ticket, tree });
        const files = await tree.diff(baseline, await tree.current());
        if (!sourceOwnsFiles(ticket, files)) {
          return { ok: false, rejected: true, errors: ['receipt role boundary rejects unclaimed tree changes'] };
        }
      }
      return {
        ok: false,
        rejected: true,
        errors: [`frozen run contract authority is missing or invalid: ${error?.message ?? String(error)}`],
        failure_domain: 'orchestration',
      };
    }
  }
  ...
  let transaction = await readJson(transactionFile, null);
  const transactionShapeError = transaction
    ? recoveryTransactionShapeError(transaction)
    : null;
  if (transactionShapeError) {
    return {
      ok: false,
      rejected: true,
      errors: [transactionShapeError],
    };
  }
  ...
  if (transaction?.version === 2) {
    const preparedError = preparedCapabilityRecoveryError(
      transaction,
      state,
      ticket,
      inputHash,
    ...
    if (preparedError) {
      return { ok: false, rejected: true, errors: [preparedError] };
    }
  }
  const transactionRecovery = transaction?.receipt?.evidence?.operator_receipt_recovery ?? null;
  if (
    requestedOperatorRecovery && transactionRecovery &&
    transactionRecovery.reason !== operatorRecoveryReason
  ) {
    return {
      ok: false,
      rejected: true,
      errors: ['receipt recovery replay conflicts with the audit reason sealed in the durable ticket transaction'],
    };
  }
  const existingReceipt = state.receipts.find((entry) => entry.ticket_id === ticket.ticket_id);
  let dispatchBinding = authenticatedRecoveryBinding;
  const receiptAuthorityState = recoveryContext?.state ?? state;
  const bindingRequired =
    receiptAuthorityState.host === 'claude' ||
    (receiptAuthorityState.host === 'codex' && receiptAuthorityState.binding_protocol === 'native-v1');
  if (
    requestedOperatorRecovery &&
    (ticket.receipt_contract_version !== RECEIPT_CONTRACT_VERSION || !bindingRequired)
  ) {
    return {
      ok: false,
      rejected: true,
      errors: ['operator receipt recovery is available only for a native-bound receipt-contract ticket'],
    };
  }
  if (bindingRequired) {
    const bindingState = receiptRetryHold
      ? {
          ...receiptAuthorityState,
    ...
    if (transaction?.version === 2 && existingReceipt) {
      const config = executionConfigForRun(await loadRuntimeConfig(paths.config), state);
      const runtimePreparedError = await preparedCapabilityRecoveryRuntimeError(
        paths,
        transaction,
        state,
        ticket,
      ...
      if (runtimePreparedError) {
        return { ok: false, rejected: true, errors: [runtimePreparedError] };
      }
      return publishCapabilityRecovery(
        paths,
        state,
        ticket,
        transaction,
    ...
    if (!dispatchBinding.valid) {
      return {
        ok: false,
        rejected: true,
        errors: [`${receiptAuthorityState.host} receipt identity or one-time capability has no exact active ticket binding`],
      };
    }
    ...
    if (
      requestedOperatorRecovery &&
      (
        typeof dispatchBinding.record?.agent_stopped_at !== 'string' ||
        dispatchBinding.record.agent_stopped_at.length === 0
      )
    ) {
      return {
        ok: false,
        rejected: true,
        errors: ['operator receipt recovery requires the host-observed stop of the exact bound physical worker'],
        failure_domain: 'operator',
        next_action: { kind: 'wait', failure_domain: 'operator' },
      };
    }
  } else if (
    transaction &&
  ...
  const parentTreeRefusal = await parentTreeReceiptRefusal(paths, state);
  if (parentTreeRefusal) return parentTreeRefusal;
  let receiptValidationSummary = null;
  let receiptContractAttestation = null;
  ...
  if (ticket.receipt_contract_version === RECEIPT_CONTRACT_VERSION) {
    ...
    if (!draftValidation.valid) {
      ...
      const attestation = await readDispatchReceiptAttestation(
        ...
        raw.receipt_capability,
        {
          ...
          receipt_limits: receiptLimits(ticket),
        },
      );
    ...
    if (capabilityManifestGrowthEnabled(receiptAuthorityState) && transaction?.status !== 'prepared') {
      prospectiveHeadTreeSha = await tree.current();
      prospectiveChangedFiles = await tree.diff(await resolveReceiptBaseline({ project_dir: paths.root, state, ticket, tree }), prospectiveHeadTreeSha);
      ...
      if (!draftValidation.valid) {
        const pathGrowthFailure =
          growth.test_path_bounds_valid === false ||
          growth.claimed_path_admission_valid === false;
        const attestation = await readDispatchReceiptAttestation(
          paths,
          ticket.ticket_id,
          inputHash,
    ...
    // Capability recovery is still the exact receipt submitted by the bound
    // physical worker. A schema-valid failure draft and possession of its
    // bearer do not prove that the worker validated these exact bytes. Never
    // manufacture that dispatch observation at the record boundary.
    receiptContractAttestation = attestation;
    if (requestedOperatorRecovery && attestation.valid) {
      return {
        ok: false,
        rejected: true,
        errors: ['receipt draft already has an exact worker attestation; use the ordinary record action'],
        receipt_contract_version: RECEIPT_CONTRACT_VERSION,
        input_hash: inputHash,
    ...
    if (!attestation.valid && !requestedOperatorRecovery) {
      const stoppedLegacyRecovery = await reconcileLegacyInterruptedReceiptLocked(
        paths,
        state,
        ticket,
    ...
    if (!requestedOperatorRecovery) {
      receiptValidationSummary = {
        validation_attempts: receiptContractAttestation.validation?.attempt,
        invalid_attempts: receiptContractAttestation.validation?.invalid_attempts,
        first_validation_valid: receiptContractAttestation.validation?.first_validation_valid,
        validation_summary_recorded:
          receiptContractAttestation.validation?.exhausted === true &&
          Number.isInteger(receiptContractAttestation.validation?.exhaustion_count) &&
          (state.receipt_contract_exhaustions?.[ticket.ticket_id] ?? 0) >=
            receiptContractAttestation.validation.exhaustion_count,
      };
    }
  }
  ...
  const changedFiles = prospectiveChangedFiles ??
    await tree.diff(await resolveReceiptBaseline({ project_dir: paths.root, state, ticket, tree }), headTreeSha);
  ...
  const operatorRecoveryEvidence = transaction?.status === 'prepared'
    ? transactionRecovery
    : requestedOperatorRecovery
      ? operatorReceiptRecoveryEvidence(
          state,
          ticket,
          dispatchBinding?.record ?? {},
          inputHash,
          operatorRecoveryReason,
          completedAt,
        )
      : null;
  if (requestedOperatorRecovery && operatorRecoveryEvidence === null) {
    return {
      ok: false,
      rejected: true,
      errors: ['operator receipt recovery could not preserve the exact bound session and dispatch identity'],
      failure_domain: 'operator',
      next_action: { kind: 'blocked', failure_domain: 'operator' },
    };
  }
  let receipt = transaction?.status === 'prepared'
    ...
    evidence: {
      ...
      ...(operatorRecoveryEvidence
        ? { operator_receipt_recovery: operatorRecoveryEvidence }
        : {}),
    },
    timing: {
  ...
  if (!validation.valid) {
    return {
      ok: false, rejected: true, errors: validation.errors, ...normalizationNote,
      recovery: receiptRejectionRecovery(state, validation.rejection_cause),
    };
  }
  let preflight = null;
  ...
  // a remediation cycle and blocked with the correction unlanded. A
  ...
  // one extra test_writer stage inside the SAME remediation cycle.
  //
  ...
  const config = executionConfigForRun(await loadRuntimeConfig(paths.config), state);
  if (
    capabilityRecoveryReceipt(receipt) &&
    ticket.receipt_contract_version === RECEIPT_CONTRACT_VERSION
  ) {
    const parentTreeRefusal = await parentTreeReceiptRefusal(paths, state);
    if (parentTreeRefusal) return parentTreeRefusal;
    if (transaction?.version === 1) {
      return {
        ok: false,
        rejected: true,
        errors: ['prepared capability recovery requires the complete version 2 bound successor envelope'],
      };
    }
    if (!transaction) {
      const preparedAt = now();
      const prepared = await prepareCapabilityRecovery(
        paths,
        state,
        ticket,
        receipt,
        inputHash,
        receiptContractAttestation,
        config,
        preparedAt,
        recoveryContext,
      );
      if (prepared) {
        transaction = {
          ...
          prepared_at: preparedAt,
          receipt,
          prepared_effect: prepared.effect,
          prepared_effect_hash: prepared.effectHash,
        };
        const preparedError = preparedCapabilityRecoveryError(
          transaction,
          state,
        ...
        await atomicWriteJson(transactionFile, transaction);
      }
    }
    if (transaction?.version === 2) {
      const preparedError = preparedCapabilityRecoveryError(
      ...
      const publicationParentTreeRefusal = await parentTreeReceiptRefusal(paths, state);
      if (publicationParentTreeRefusal) return publicationParentTreeRefusal;
      return publishCapabilityRecovery(
        paths,
        state,
        ticket,
  ...
  // Runtime-owned authored-test execution: before any durable side effect,
  // the runtime executes the exact authored paths twice and requires the
  // ticket-selected stable verdict, then seals the observation into receipt
  // evidence (hash-bound, and
  // durable via the receipt transaction and receipt file). A prepared
  // transaction is crash recovery of an admission that already observed the
  // requested verdict
  ...
  if (
    ...
    ticket.required_checks.some((check) => ['red-test', 'green-test', 'test-correction'].includes(check)) &&
    transaction?.status !== 'prepared'
  ) {
    if (receiptRetryHold) {
    ...
    const authoredTest = await observeAuthoredTest(
      paths,
      state,
      ticket,
      receipt,
    ...
    if (!authoredTest.ok) return { ok: false, rejected: true, errors: authoredTest.errors };
    // Accumulate the runtime-measured authored-test wall clock beside the sealed
    ...
    accumulateTiming(state, 'test_ms', authoredTest.duration_ms);
    const { receipt_hash: _unsealed, ...body } = receipt;
    const {
      red_test: _workerRedTest,
      green_test: _workerGreenTest,
      test_correction: _workerTestCorrection,
      ...workerEvidence
    } = body.evidence;
    const evidenceKey = ticket.required_checks.includes('test-correction')
      ? 'test_correction'
      : ticket.required_checks.includes('green-test') ? 'green_test' : 'red_test';
    receipt = finalizeReceipt({
      ...body,
      evidence: { ...workerEvidence, [evidenceKey]: authoredTest.observation },
    });
  }
  ...
  // Tests and preparatory validation above can outlive another hook process.
  // Re-check retained refusal evidence before publishing a fresh or replayed
  // prepared receipt. Already committed retries returned before these checks.
  const finalParentTreeRefusal = await parentTreeReceiptRefusal(paths, state);
  if (finalParentTreeRefusal) return finalParentTreeRefusal;
  if (!transaction) {
    await atomicWriteJson(transactionFile, {
      version: 1,
      run_id: state.run_id,
  ...
  if (
    requestedOperatorRecovery && operatorRecoveryEvidence &&
    !await operatorReceiptRecoveryAudited(
      paths.overrideLog,
      state.run_id,
      ticket.ticket_id,
      operatorRecoveryEvidence,
    ...
    // This is the only exception to worker-owned exact-draft attestation. It
    // remains an ordinary fully validated receipt admission, but the waiver is
    // durable in three linked places: this override log, the immutable receipt
    // evidence, and the completed dispatch intent. Append after the prepared
    // transaction is durable and before the receipt/state effect; replay uses
    // the exact identity key above to fill a missing line without duplicating
    // one that already landed.
    await appendJsonLine(paths.overrideLog, {
      run_id: state.run_id,
      at: operatorRecoveryEvidence.recovered_at,
      operation: 'recover-receipt',
      ticket_id: ticket.ticket_id,
      ticket_hash: ticket.ticket_hash,
      receipt_id: receipt.receipt_id,
      receipt_hash: receipt.receipt_hash,
      receipt_input_hash: operatorRecoveryEvidence.receipt_input_hash,
      dispatch_identity_hash: operatorRecoveryEvidence.dispatch_identity_hash,
      host: operatorRecoveryEvidence.dispatch.host,
      session_id: operatorRecoveryEvidence.dispatch.session_id,
      agent_id: operatorRecoveryEvidence.dispatch.agent_id,
      physical_worker_dispatches:
        operatorRecoveryEvidence.dispatch.physical_worker_dispatches,
      worker_attestation: 'operator-waived',
      validation: 'runtime-revalidated',
      reason: boundedGateSummary(operatorRecoveryEvidence.reason),
    });
  }
  ...
  // confinement instead of consuming the fast-lane file budget. Count all
  // validated prior production changes too: splitting a change across retries
  // or remediation receipts must not reset the observed file scope.
  const productionChanges = [...new Set([
    ...state.receipts.flatMap((prior) => prior.changed_files ?? []),
  ...
  const expansion = escalateLane(state.lane, await resolveLaneScope(paths.root, {
  ...
  const successorGuidance = successorGuidanceForState(state, config);
  return {
    ok: true,
    receipt,
    run: state,
...
function unauthenticatedReceiptCapabilityRefusal(ticketId) {
  return {
    ok: false,
    rejected: true,
    errors: ['receipt capability does not match the exact bound physical dispatch'],
    ...(typeof ticketId === 'string' && ticketId.length > 0 ? { ticket_id: ticketId } : {}),
  };
}

async function receiptRedactionAuthority(paths, raw, ticketId = receiptTicketIdValue(raw)) {
  const capabilityHash = await readDispatchReceiptCapabilityHash(paths, ticketId);
  const bearer = authenticatedReceiptBearer(raw, capabilityHash);
  return {
    capability_hash: capabilityHash,
    bearer,
    authenticated: capabilityHash !== null && bearer !== null,
  };
}

export async function recordReceipt(projectDir, raw) {
  const paths = runtimePaths(projectDir);
  const ticketId = receiptTicketIdValue(raw);
  const authority = await receiptRedactionAuthority(paths, raw, ticketId);
  if (authority.capability_hash !== null && !authority.authenticated) {
    return unauthenticatedReceiptCapabilityRefusal(ticketId);
  }
  const redactionBearer = authority.bearer;
  try {
    assertSafeReceiptInput(raw);
  } catch (error) {
    if (redactionBearer === null) {
      throw new Error('receipt input failed safety validation before its exact capability was authenticated');
    }
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(redactReceiptBearer(message, redactionBearer));
  }
  try {
    const result = await withReceiptLock(paths, async (receiptLease) => {
      const state = await activeState(paths);
      const ticket = state?.tickets?.find((entry) => entry.ticket_id === raw?.ticket_id);
      const { input, normalized_fields } = normalizeReceiptInput(raw, {
        receipt_contract_version: ticket?.receipt_contract_version,
      });
      return recordReceiptLocked(projectDir, input, normalized_fields, {
        receipt_lease: receiptLease,
      });
    });
    return redactReceiptBearer(result, redactionBearer);
  } catch (error) {
    if (redactionBearer === null) throw error;
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(redactReceiptBearer(message, redactionBearer));
  }
}

export async function recoverReceiptLocked(
  projectDir,
  raw,
  recovery = {},
  receiptLease = null,
) {
  assertSafeReceiptRecoveryInput(raw, recovery);
  if (typeof recovery.reason !== 'string' || recovery.reason.trim() === '') {
    throw new Error('recover-receipt requires a nonblank audit reason');
  }
  if (
    typeof recovery.receipt_input_hash !== 'string' ||
    !/^[0-9a-f]{64}$/iu.test(recovery.receipt_input_hash)
  ) {
    throw new Error('recover-receipt requires the exact 64-character receipt_input_hash');
  }
  const { input, normalized_fields } = normalizeReceiptInput(raw, {
    receipt_contract_version: RECEIPT_CONTRACT_VERSION,
  });
  const actualInputHash = receiptInputHash(input);
  if (actualInputHash !== recovery.receipt_input_hash.toLowerCase()) {
    return {
      ok: false,
      rejected: true,
      errors: ['recover-receipt receipt_input_hash does not match the normalized exact draft'],
      expected_receipt_input_hash: recovery.receipt_input_hash.toLowerCase(),
      actual_receipt_input_hash: actualInputHash,
      failure_domain: 'operator',
      next_action: { kind: 'blocked', failure_domain: 'operator' },
  ...
  return recordReceiptLocked(projectDir, input, normalized_fields, {
    receipt_lease: receiptLease,
    operator_recovery: {
      reason: boundedGateSummary(recovery.reason),
      receipt_input_hash: actualInputHash,
    },
  });
}

export async function recoverReceipt(projectDir, raw, recovery = {}) {
  const paths = runtimePaths(projectDir);
  const ticketId = receiptTicketIdValue(raw);
  const authority = await receiptRedactionAuthority(paths, raw, ticketId);
  if (authority.capability_hash !== null && !authority.authenticated) {
    return {
      ...unauthenticatedReceiptCapabilityRefusal(ticketId),
      failure_domain: 'operator',
      next_action: { kind: 'blocked', failure_domain: 'operator' },
    };
  }
  ...
  try {
    ...
    if (redactionBearer === null) {
      throw new Error('receipt recovery input failed safety validation before its exact capability was authenticated');
    }
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(redactReceiptBearer(message, redactionBearer));
  }
  try {
    const result = await withReceiptLock(
      paths,
      (receiptLease) => recoverReceiptLocked(projectDir, raw, recovery, receiptLease),
      { busyMessage: 'receipt effects are busy; retry the identical recover-receipt action' },
    );
    return redactReceiptBearer(result, redactionBearer);
  } catch (error) {
    if (redactionBearer === null) throw error;
...
async function reconcileReceiptContractExhaustionLocked(paths, state, ticket, observation) {
  ...
  const limits = receiptLimits(ticket);
  if (!Number.isSafeInteger(target) || target < 1 || target > limits.max_physical_workers_per_ticket) {
    return { reconciled: false };
  }
  const recorded = state.receipt_contract_exhaustions ?? {};
  const prior = Number.isSafeInteger(recorded[ticket.ticket_id]) ? recorded[ticket.ticket_id] : 0;
  ...
  const attempts = limits.max_validation_submissions_per_worker;
  const invalid = Number.isSafeInteger(observation.validation.invalid_attempts) &&
    observation.validation.invalid_attempts >= 0 && observation.validation.invalid_attempts <= attempts
    ? observation.validation.invalid_attempts : attempts;
  orchestration = recordReceiptContractExhaustion(orchestration, {
    validation_attempts: attempts * delta,
    invalid_attempts: invalid * delta,
    at: recordedAt,
  });
  orchestration = incrementOrchestration(orchestration, 'protocol_redispatches',
    Math.max(0, Math.min(target, limits.max_physical_workers_per_ticket - 1) - prior));
  if (target >= limits.max_physical_workers_per_ticket) {
...
async function reconcileLegacyInterruptedReceiptLocked(paths, state, ticket, attestation) {
  ...
  const physicalWorkers = Number.isInteger(attestation.physical_worker_dispatches)
    ? attestation.physical_worker_dispatches
    : 1;
  if (physicalWorkers >= receiptLimits(ticket).max_physical_workers_per_ticket) {
    return blockWorkerProtocolLocked(
      paths,
      state,
      ticket,
      `stage ${ticket.stage_id} worker protocol exhausted its configured ${receiptLimits(ticket).max_physical_workers_per_ticket} physical workers; the immutable same-ticket worker policy forbids another dispatch`,
    );
  }
...
function receiptCorrectionFieldValue(input, field) {
  if (Object.hasOwn(input, field)) return { present: true, value: input[field] };
  const tokens = [];
  const tokenPattern = /([^[.\]]+)|\[(\d+)\]/g;
  let match;
  let consumed = '';
  while ((match = tokenPattern.exec(field)) !== null) {
    tokens.push(match[1] ?? Number(match[2]));
    consumed += match[1] ?? `[${match[2]}]`;
    if (tokenPattern.lastIndex < field.length && field[tokenPattern.lastIndex] === '.') {
      consumed += '.';
    }
  }
  if (consumed !== field || tokens.length === 0) return { present: false };
  let value = input;
...
function receiptCorrectionValueContainsBearer(value, bearer, seen = new Set()) {
  if (typeof bearer !== 'string' || bearer.length === 0) return false;
  if (typeof value === 'string') return value.includes(bearer);
  if (value === null || typeof value !== 'object') return false;
  if (seen.has(value)) return false;
  seen.add(value);
  return Object.entries(value).some(([key, nested]) =>
    key.includes(bearer) || receiptCorrectionValueContainsBearer(nested, bearer, seen));
}

function exactReceiptCorrectionDelta(ticket, input, correction) {
  if (
    typeof input.receipt_capability === 'string' &&
    correction.field.includes(input.receipt_capability)
  ) return null;
  const current = receiptCorrectionFieldValue(input, correction.field);
  const reflectsBearer = current.present && receiptCorrectionValueContainsBearer(
    current.value,
    input.receipt_capability,
  );
  const base = {
    field: correction.field,
    ...(current.present && !reflectsBearer ? { current_value: current.value } : {}),
  };
  if (
    correction.correction === `remove ${correction.field}` ||
    correction.correction.startsWith(`remove ${correction.field} `) ||
    correction.correction.startsWith(`remove ${correction.field};`) ||
    (
      correction.field.endsWith('.output_hash') &&
      correction.correction.startsWith('omit output_hash ')
    )
  ) {
    return current.present ? { ...base, operation: 'remove' } : null;
  }
  if (correction.field === 'ticket_id') {
    return {
      ...base,
      required_value: ticket.ticket_id,
      operation: 'replace',
    };
  }
  if (
    ...
    ['success', 'complete', 'completed', 'failure', 'error']
      .includes(input.status.toLowerCase())
  ) {
    return {
      ...base,
      required_value: ['failure', 'error'].includes(input.status.toLowerCase())
        ? 'failed'
        : 'passed',
      operation: 'replace',
    };
  ...
  if (
    correction.field === 'evidence.candidate_plan.version' &&
    Number.isInteger(ticket.plan_contract_version)
  ) {
    return {
      ...base,
      required_value: ticket.plan_contract_version,
      operation: 'replace',
    };
  }
  // The remaining correction prose describes evidence the worker must supply
  // or a choice it must make. Publishing that prose as required_value would
  // falsely claim it is a mechanically applicable JSON replacement.
  return null;
}
...
async function validateReceiptForDispatchLocked(paths, input, normalized_fields, expectedTicketId) {
  ...
  if (capabilityManifestGrowthEnabled(state)) {
    ...
    const growth = prospectiveReceiptCapabilityGrowth(state, ticket, input,
  ...
  // A malicious invalid draft may copy its one-time bearer into arbitrary
  // values or property names that schema correction prose would otherwise
  // reflect. Sanitize before the validation observation is persisted, then
  // sanitize the final wire response again below as defense in depth.
  draftResult = redactReceiptBearer(draftResult, input.receipt_capability);
  ...
  const result = observation.result ?? {
    ...
    corrections: [{
      field: 'receipt',
      issue: `the physical dispatch has exhausted its ${observation.validation.max_attempts} receipt validation attempts`,
      correction: observation.validation.next_action.kind === 'redispatch_same_ticket'
        ? 'return the exhausted result unchanged so the parent can redispatch this same ticket within its remaining worker budget'
  ...
  const candidateCorrectionDeltas = result.corrections
    .slice(0, RECEIPT_DRAFT_CORRECTIONS_MAX)
    .map((correction) => exactReceiptCorrectionDelta(ticket, input, correction))
    .filter(Boolean)
    .filter((entry, index, entries) => entries.findIndex((candidate) =>
      JSON.stringify(candidate) === JSON.stringify(entry)) === index);
  const correctionDeltas = [];
  for (const candidate of candidateCorrectionDeltas) {
    const next = [...correctionDeltas, candidate];
    if (Buffer.byteLength(JSON.stringify(next), 'utf8') > 12_000) break;
    correctionDeltas.push(candidate);
  }
  return {
    ok: true,
    valid: accepted,
    ...
    correction_deltas: correctionDeltas,
...
export async function validateReceiptForDispatch(projectDir, raw, expectedTicketId = raw?.ticket_id) {
  const paths = runtimePaths(projectDir);
  const authority = await receiptRedactionAuthority(paths, raw, expectedTicketId);
  if (authority.capability_hash !== null && !authority.authenticated) {
    return {
      ...unauthenticatedReceiptCapabilityRefusal(expectedTicketId),
      valid: false,
      attested: false,
    };
  }
  ...
  try {
    ...
    if (redactionBearer === null) {
      throw new Error('receipt validation input failed safety validation before its exact capability was authenticated');
    }
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(redactReceiptBearer(message, redactionBearer));
  }
...
export async function settleReceiptValidationSubagentStop(projectDir, ticketId = null, observation = null) {
  const paths = runtimePaths(projectDir);
  return withReceiptLock(paths, () => settleReceiptValidationSubagentStopLocked(paths, ticketId, observation), {
    busyMessage: 'receipt effects are busy; retry SubagentStop settlement',
  });
}

// NEXT already owns the receipt lock, including when invoked by a durable
// task. Share the settlement body without recursively acquiring that lock.
export async function settleReceiptValidationSubagentStopLocked(paths, ticketId = null, observation = null) {
    const state = await activeState(paths);
    if (!state) return { ok: false, reason: 'no active run' };
    // Stop evidence survives abort and other terminal transitions. It cannot
    // authorize new recovery after that transition, or NEXT/RESUME could
    // rewrite an aborted run as a worker-protocol block before refusing it.
    // The legacy execution-budget hold is the one compatibility exception:
    // settle it before NEXT retires the hold and considers ticket deadlines.
    // Check its effective status without mutating an ineligible stored run.
    const eligibleState = { ...state };
    retireLegacyExecutionBudget(eligibleState);
    if (eligibleState.status !== 'running') return { ok: true, settled: false };
    ...
    const physicalLimitReached = legacyInterrupted.find((entry) =>
      entry.physical_worker_dispatches >= receiptLimits(state.tickets.find((ticket) => ticket.ticket_id === entry.ticket_id)).max_physical_workers_per_ticket);
          ...
          .map((entry) => ({
            ...
            observation: {
              ...
              validation: {
                ...
                max_attempts: receiptLimits(state.tickets.find((ticket) => ticket.ticket_id === entry.ticket_id)).max_validation_submissions_per_worker,
              },
            },
          }));
    if (targets.length === 0) {
    ...
    for (const target of targets) {
      ...
      if (exhaustionCount < receiptLimits(ticket).max_physical_workers_per_ticket &&
          (dispatch?.physical_worker_dispatches ?? exhaustionCount) < receiptLimits(ticket).max_physical_workers_per_ticket) {
      ...
      const blocked = await blockWorkerProtocolLocked(
        ...
        state,
        ticket,
        `stage ${ticket.stage_id} worker protocol failed after its configured ${receiptLimits(ticket).max_physical_workers_per_ticket} physical workers exhausted the immutable receipt contract`,
      );
      return { ok: true, settled: true, ...blocked };
    }
    return {
