...
function emptyAuthoredTestPathsRefusal(state, ticket, fallbackMessage) {
  const check = ticket.required_checks?.includes('green-test') ? 'green-test' : 'red-test';
...
// `red-test` or `green-test` is admitted only after the runtime executes the
// exact authored paths twice and observes the requested stable verdict. Worker
...
// fail/fail; explicit green-maintenance requires pass/pass.
//
...
async function observeAuthoredTest(paths, state, ticket, receipt, config, tree) {
  ...
  const check = green ? 'green-test' : 'red-test';
  const phase = green ? 'green maintenance' : 'red phase';
  ...
  const testPaths = [];
  if (green || template || shuffleTemplate || !configured) {
    ...
    return {
      ...
      errors: [
        emptyAuthoredTestPathsRefusal(
          state,
          ticket,
          'green-test admission found no runtime-verifiable authored test files changed by this test writer',
        ),
      ],
    };
  }
  if (template) {
    ...
    if (invocation.scoped !== true) {
      ...
      return {
        ...
        errors: [
          green
            ? `green-test admission cannot scope the detected ${detectedRunner.runner} runner to the authored test paths; a whole-suite verdict is not proof the authored tests were the tests observed — configure test_commands.targeted_template ('{paths}' receives the authored test files)`
  ...
  if (postExecutionTree !== receipt.head_tree_sha) {
    ...
    if (postExecutionTree !== receipt.head_tree_sha) {
      ...
      return {
        ...
        errors: [
          green
            ? `green-test execution modified pre-existing files (${cause.join(', ')}); runner side-effect artifacts must be committed or gitignored before the run — green evidence must be tree-stable`
  ...
  for (const run of [verification, verificationB]) {
    ...
    if (run.exit_code === 5) {
      return {
        ok: false,
        errors: [green
          ? 'pytest collected no tests (exit 5): the authored tests were never executed — fix test naming/collection; green maintenance was not observed'
    ...
    if (run.exit_code === 4) {
      return {
        ok: false,
        errors: [green
          ? 'pytest usage error (exit 4): the green-test command did not run the authored tests — fix test_commands.targeted_template or test_commands.targeted; green maintenance was not observed'
  ...
  if (!green && verification.passed === true && verificationB.passed === true) {
    return {
      ok: false,
  ...
  return {
    ...
    observation: {
      ...
      ...(invocation
        ...
        : { configured: true, ...(green ? { test_paths: testPaths } : {}) }),
      passed: green,
    ...
    for (const run of [verification, verificationB]) {
      ...
      if (run.exit_code === 5) {
        return {
          ...
          errors: [
            green
              ? `runner '${participant.runner.id}' pytest collected no tests (exit 5): the authored tests were never executed — fix test naming/collection; green maintenance was not observed`
      ...
      if (run.exit_code === 4) {
        return {
          ...
          errors: [
            green
              ? `runner '${participant.runner.id}' pytest usage error (exit 4): the green-test command did not run the authored tests — fix its profile.targeted_template or test_commands.targeted; green maintenance was not observed`
...
async function preparedCapabilityRecoveryRuntimeError(
  ...
  const outputSchema = capabilityBase
    ? receiptOutputSchemaForTicket({
...
export async function recordReceiptLocked(projectDir, raw, normalizedFields = [], options = {}) {
  ...
  if (
    ...
    ticket.required_checks.some((check) => check === 'red-test' || check === 'green-test') &&
    transaction?.status !== 'prepared'
  ) {
    if (receiptRetryHold) {
    ...
    const evidenceKey = ticket.required_checks.includes('green-test')
      ? 'green_test'
      : 'red_test';
