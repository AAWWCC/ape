...
import { clampToExecutionDeadline, executionBudgetGuard, pauseForExecutionBudget, recordWorkerDispatch, syncExecutionBudgetClock } from './execution-budget.js';
import {
...
const BUDGET_CONTINUATION_VERSION = 1;
const BUDGET_CONTINUATION_MAX_ACTIONS = 512;
const BUDGET_CONTINUATION_ACTION_TYPES = new Set([
  'acquire_lock',
  'activate_run_branch',
  'apply_override',
  'archive_history',
...
async function issueTicket(paths, state, stage, config, tree, retryOf = null, evidence = {}) {
  ...
  const reviewContractVersion = stage.review_contract_version === REVIEW_CONTRACT_VERSION
    ? REVIEW_CONTRACT_VERSION
    : null;
  const deadlineAt = clampToExecutionDeadline(
    state,
    new Date(Date.parse(issuedAt) + deadlineMs).toISOString(),
    issuedAt,
  );
...
export async function persist(paths, state, tree, { refreshTree = true } = {}) {
  ...
  syncExecutionBudgetClock(state, state.updated_at);
...
function budgetContinuationRecord(state, actions, source = 'action-chain', config = null) {
  if (!Array.isArray(actions) || actions.length === 0 || actions.length > BUDGET_CONTINUATION_MAX_ACTIONS) {
    throw new Error(`execution-budget continuation must contain 1 through ${BUDGET_CONTINUATION_MAX_ACTIONS} actions`);
  }
  const cloned = structuredClone(actions);
  for (const [index, action] of cloned.entries()) {
    if (
      !action || typeof action !== 'object' || Array.isArray(action) ||
      !BUDGET_CONTINUATION_ACTION_TYPES.has(action.type)
    ) {
      throw new Error(`execution-budget continuation action ${index} is not a recognized reducer action`);
    }
  }
  if (!config || typeof config !== 'object' || Array.isArray(config)) {
    throw new Error('execution-budget continuation requires the exact effective config snapshot');
  }
  return {
    version: BUDGET_CONTINUATION_VERSION,
    run_id: state.run_id,
    source,
    queued_at: now(),
    actions_hash: sha256(cloned),
    actions: cloned,
    config_hash: sha256(config),
    config_snapshot: structuredClone(config),
  };
}

function queueBudgetContinuation(state, actions, source, config) {
  // A recursive reducer chain may discover the exhausted clock while an outer
  // chain is still on the stack. The innermost marker is the exact first work
  // that did not start, so never overwrite it with the outer chain's trailing
  // persist action.
  if (state.budget_continuation) return state.budget_continuation;
  state.budget_continuation = budgetContinuationRecord(state, actions, source, config);
  return state.budget_continuation;
}

function checkedBudgetContinuation(state) {
  const continuation = state?.budget_continuation;
  if (!continuation) return null;
  if (
    continuation.version !== BUDGET_CONTINUATION_VERSION ||
    continuation.run_id !== state.run_id ||
    typeof continuation.actions_hash !== 'string'
  ) {
    throw new Error('execution-budget continuation does not match the active run contract');
  }
  if (
    typeof continuation.config_hash !== 'string' ||
    !continuation.config_snapshot ||
    sha256(continuation.config_snapshot) !== continuation.config_hash
  ) {
    throw new Error('execution-budget continuation config hash does not match its durable snapshot');
  }
  const checked = budgetContinuationRecord(
    state,
    continuation.actions,
    typeof continuation.source === 'string' ? continuation.source : 'action-chain',
    continuation.config_snapshot,
  );
  if (checked.actions_hash !== continuation.actions_hash) {
    throw new Error('execution-budget continuation action hash does not match its durable payload');
  }
  return { actions: checked.actions, config: checked.config_snapshot };
}

// Explicit budget extension only re-arms the clock. NEXT calls this helper to
// replay the exact reducer-owned work that was durably queued at the first
// denied execution boundary; receipt/stage attempts are not consumed again.
export async function resumeBudgetContinuation(
  paths,
  state,
  config,
  tree = treeShaSession(paths.root),
) {
  const continuation = checkedBudgetContinuation(state);
  if (!continuation) return null;
  ...
  // deletion after a successful replay; a renewed pause has already restored
  // and persisted a new marker. Terminal reducer chains own their own final
  // persist/release ordering and must not be followed by another write here.
  if (
    !state.budget_continuation &&
    state.input_required?.kind !== 'execution_budget' &&
...
export async function applyActions(paths, state, actions, config, tree = treeShaSession(paths.root)) {
  const emitted = [];
  // Dispatch budget is charged per physical worker, not per delivery of a
  // dispatch response. A prepared/unlaunched intent whose ticket is already in
  // active.json proves that the original charge was persisted but the host had
  // not yet launched a worker. Classify these replays before group admission so
  // an exact-minimum budget can recover a lost START/record response.
  ...
  const budgetDispatchGroupSize = dispatchGroupSize - idempotentDispatchReplays.size;
  const budgetDecision = executionBudgetGuard(state, { dispatches: budgetDispatchGroupSize, at: now() });
  let budgetPaused = false;
  let budgetCountDirty = false;
  const pauseBudget = async (decision = budgetDecision, remainingActions = actions, source = 'action-chain') => {
    if (budgetPaused || decision.allowed) return;
    budgetPaused = true;
    queueBudgetContinuation(state, remainingActions, source, config);
    const pause = pauseForExecutionBudget(state, decision, now());
    if (pause) emitted.push(pause);
    // Persist immediately: not every future reducer chain is guaranteed to
    // carry a trailing persist_state, and a budget pause must survive a host
    // process exit before an explicit monotonic extension.
    await persist(paths, state, tree);
  };
  const recheckActiveBudget = async (remainingActions, source = 'action-chain') => {
    const decision = executionBudgetGuard(state, { dispatches: 0, at: now() });
    if (!decision.allowed) await pauseBudget(decision, remainingActions, source);
    return decision.allowed;
  };
  let deferredTerminalRelease = null;
  const advancesExecution = actions.some((action) => [
    'issue_ticket',
    'dispatch_agent',
    'run_gates',
    'poll_gates',
  ...
  if (!budgetDecision.allowed && advancesExecution) {
    await pauseBudget(budgetDecision, actions, 'action-chain-admission');
    return emitted;
  }
  for (const [actionIndex, action] of actions.entries()) {
    const remainingActions = actions.slice(actionIndex);
    const followingActions = actions.slice(actionIndex + 1);
    switch (action.type) {
      case 'acquire_lock':
      ...
      case 'issue_ticket': {
        if (!budgetDecision.allowed) {
          await pauseBudget(budgetDecision, remainingActions, 'issue-ticket');
          return emitted;
        }
        if (!await recheckActiveBudget(remainingActions, 'issue-ticket')) return emitted;
        const priorStage = state.stage;
        const priorRunContract = state.run_contract
          ? structuredClone(state.run_contract)
          : null;
        const issued = await issueTicket(paths, state, action.stage, config, tree, action.retry_of, {
        ...
        const postIssueDecision = executionBudgetGuard(state, { dispatches: 0, at: now() });
        if (!postIssueDecision.allowed) {
          if (!issued.recovered) {
            // The ticket has not crossed a host dispatch boundary. Roll its
            // just-written artifact back before persisting the pause so the
            // explicit extension replays issue_ticket with a fresh deadline;
            // operator-wait time can never age an undispatched immutable ticket
            // into an already-expired launch.
            state.tickets = state.tickets.filter((entry) => entry.ticket_id !== ticket.ticket_id);
            state.stage = priorStage;
            if (priorRunContract) state.run_contract = priorRunContract;
            else delete state.run_contract;
            await rm(
              path.join(paths.tickets, `${ticket.ticket_id.replaceAll(':', '_')}.json`),
              { force: true },
            );
            await pauseBudget(postIssueDecision, remainingActions, 'issue-ticket-before-dispatch');
          } else {
            // A recovered ticket may already have a durable host intent from a
            // pre-crash launch. Preserve that exact identity and let the normal
            // intent idempotency seam resolve it after extension.
            await pauseBudget(
              postIssueDecision,
              [dispatchContinuation, ...followingActions],
              'dispatch-recovered-ticket',
            );
          }
          return emitted;
        }
        ...
        recordWorkerDispatch(state, dispatchedAt);
        state.orchestration = recordDispatchTokenCoverage(
          state.orchestration,
          action.host_token_attestation ?? null,
        );
        budgetCountDirty = Boolean(state.execution_budget);
        emitted.push({
          type: 'dispatch_agent',
          dispatch,
          ticket,
      ...
      case 'archive_history':
        ...
        break;
      case 'run_gates': {
        if (!await recheckActiveBudget(remainingActions, 'run-gates')) return emitted;
      ...
      case 'poll_gates': {
        if (!await recheckActiveBudget(remainingActions, 'poll-gates')) return emitted;
      ...
      case 'auto_merge': {
        if (!await recheckActiveBudget(remainingActions, 'auto-merge')) return emitted;
      ...
      case 'poll_shipping': {
        if (!await recheckActiveBudget(remainingActions, 'poll-shipping')) return emitted;
      ...
      case 'dispatch_agent': {
        if (!budgetDecision.allowed) {
          await pauseBudget(budgetDecision, remainingActions, 'dispatch-agent');
          return emitted;
        }
        if (!await recheckActiveBudget(remainingActions, 'dispatch-agent')) return emitted;
        const ticket = state.tickets.find((entry) => entry.ticket_id === action.ticket_id);
        if (!ticket) break;
    ...
    // A recursive action chain can be the first place the active clock is
    // observed exhausted. Its exact continuation is already durable; stop the
    // outer chain so no later effect starts after that boundary.
    if (state.input_required?.kind === 'execution_budget') return emitted;
  }
  ...
  if (budgetCountDirty) await persist(paths, state, tree);
  return emitted;
}
...
export async function recordReceiptLocked(projectDir, raw, normalizedFields = []) {
  ...
  if (
    ...
    return {
      ...
      rejected: true,
      errors: [
        `execution-budget recovery requires the identical attested receipt for ticket ${receiptRetryHold.ticket_id}`,
      ],
  ...
  let receiptValidationAttempt = null;
  ...
  if (ticket.receipt_contract_version === RECEIPT_CONTRACT_VERSION) {
    ...
    if (!draftValidation.valid) {
      ...
      const stoppedBudgetRecovery = await reconcileStoppedBudgetInterruptedReceiptLocked(
        paths,
        state,
        ticket,
        attestation,
      ...
      const {
        ...
        ...stoppedBudgetResponse
      } = stoppedBudgetRecovery ?? {};
      return {
        ok: false,
        rejected: true,
    ...
    receiptValidationAttempt = attestation.validation?.attempt;
  }
  ...
  if (
    ...
    // The worker call that produced this receipt was already authorized and
    // may finish after the active-time horizon. The red-test is a new,
    // runtime-owned command, so it must not start after that horizon. Preserve
    // the exact ticket/attestation and ask the operator to extend, then retry
    // this identical receipt; no stage or receipt-correction attempt is spent.
    const redTestBudget = executionBudgetGuard(state, { dispatches: 0, at: now() });
    if (!redTestBudget.allowed) {
      const pause = pauseForExecutionBudget(state, redTestBudget, now());
      if (state.input_required?.kind === 'execution_budget') {
        state.input_required.resume_action = 'retry_receipt';
        state.input_required.ticket_id = ticket.ticket_id;
        state.input_required.input_hash = inputHash;
      }
      await persist(paths, state, tree, { refreshTree: false });
      return {
        ok: false,
        paused: true,
        errors: [
          'execution budget exhausted before APE could authorize the runtime-owned red-test; extend the same run budget, then retry this exact attested receipt',
        ],
        failure_domain: 'orchestration',
    ...
    if (receiptRetryHold) {
      ...
      delete state.input_required;
      syncExecutionBudgetClock(state, now());
    }
  ...
  if (
    ...
    // A still-running physical worker may produce a compliant receipt after
    // the operator extends the clock. Its accepted receipt supersedes the
    // queued same-ticket redispatch and must cancel it durably.
    delete state.budget_continuation;
  }
  const receiptRecordedAt = receipt.timing?.completed_at ?? now();
  if (receiptValidationAttempt !== null) {
    state.orchestration = recordAcceptedReceipt(state.orchestration, {
      validation_attempt: receiptValidationAttempt,
...
function sameTicketReceiptRecoveryAction(ticket) {
  return {
    type: 'dispatch_agent',
    ticket_id: ticket.ticket_id,
...
// A stop-hook validation can discover that the active-time cap has elapsed
// without going through the worker-facing validation service. Once the host
// has also attested SubagentStop, continuing that physical worker is
// impossible. Retire it, durably preserve the one allowed same-ticket
// replacement, and require an explicit budget extension before dispatch.
async function reconcileStoppedBudgetInterruptedReceiptLocked(paths, state, ticket, attestation) {
  ...
  if (physicalWorkers >= RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET) {
    return blockWorkerProtocolLocked(
      ...
      state,
      ticket,
      `stage ${ticket.stage_id} worker protocol could not complete within the execution budget after two physical workers; the immutable same-ticket worker limit forbids another dispatch`,
    );
  }

  markPendingReceiptContractRedispatch(state, ticket.ticket_id);
  const config = await loadRuntimeConfig(paths.config);
  queueBudgetContinuation(
    state,
    [sameTicketReceiptRecoveryAction(ticket)],
    'receipt-protocol-budget-recovery',
    config,
  );
  const at = now();
  const budgetDecision = executionBudgetGuard(state, { dispatches: 0, at });
  const budgetPause = budgetDecision.allowed
    ? null
    : pauseForExecutionBudget(state, budgetDecision, at);
  await persist(paths, state, treeShaSession(paths.root), { refreshTree: false });
  ...
  return budgetDecision.allowed
    ...
    : {
        ...
        recovery_kind: 'receipt_budget_interrupted',
        next_action: { kind: 'extend_budget', failure_domain: 'operator' },
        after_budget_extension: redispatch,
      };
}
...
async function validateReceiptForDispatchLocked(paths, input, normalized_fields, expectedTicketId) {
  ...
  // An invalid validation result normally authorizes this same physical worker
  // to continue correcting its receipt. Check active time at that exact
  // boundary; worker-dispatch exhaustion does not apply because no new worker
  // is being launched. A valid draft may still be recorded as the completion
  // of the already-running host call, but APE will gate every successor effect.
  const continuationBudget = draftResult.valid
    ? { allowed: true }
    : executionBudgetGuard(state, { dispatches: 0, at: now() });
  ...
  if (result.valid !== true && continuationBudget.allowed === false) {
    const exhaustionCount = observation.validation.exhaustion_count ?? 0;
    if (
      observation.validation.exhausted !== true ||
      exhaustionCount < RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET
    ) {
      const config = await loadRuntimeConfig(paths.config);
      queueBudgetContinuation(state, [{
        type: 'dispatch_agent',
        ticket_id: ticket.ticket_id,
        recovery_kind: 'redispatch_same_ticket',
        source_ticket_id: ticket.ticket_id,
        failure_domain: 'orchestration',
      }], 'receipt-protocol-budget-recovery', config);
    }
    budgetPause = pauseForExecutionBudget(state, continuationBudget, now());
    await persist(paths, state, treeShaSession(paths.root), { refreshTree: false });
  }
  ...
  return {
    ...
    ...(budgetPause
      ? {
          ...
          after_budget_extension: receiptRecoveryNextAction,
        }
      : exhausted
      ? {
...
export async function settleReceiptValidationSubagentStop(projectDir, ticketId = null, observation = null) {
  ...
  return withReceiptLock(paths, async () => {
    ...
    // A SubagentStop that APE allowed specifically because the active-time cap
    // forbade another receipt correction is orchestration recovery, not a
    // product/stage failure. Retire that physical intent now; after an explicit
    // budget extension, NEXT will redispatch this same still-pending immutable
    // ticket and the reducer's logical stage-attempt counter stays untouched.
    const budgetInterrupted = statuses.filter((entry) =>
    ...
    const physicalLimitReached = budgetInterrupted.find((entry) =>
    ...
    let budgetRecoveryMarked = false;
    for (const entry of budgetInterrupted) {
      ...
      markPendingReceiptContractRedispatch(state, ticket.ticket_id);
      budgetRecoveryMarked = true;
    }
    if (budgetRecoveryMarked) {
    ...
    if (targets.length === 0) {
      return {
        ok: true,
        settled: budgetInterrupted.length > 0,
        ...(budgetInterrupted.length > 0
          ? {
              next_actions: budgetInterrupted.map((entry) => ({
                kind: 'extend_budget',
                ticket_id: entry.ticket_id,
              })),
            }
