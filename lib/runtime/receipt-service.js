...
import { AUTO_MERGE_HOLD_REASON, CHECKS_REGISTRATION_RETRY_DELAY_MS, GATE_INLINE_GRACE_MS, GATE_NEXT_MAX_WAIT_MS, GATE_NEXT_POLL_FLOOR_MS, GATE_POLL_RETRY_DELAY_MS, RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET, RECEIPT_MAX_SUBMISSIONS_PER_WORKER, RISK_TRIGGERS, ROLE_POLICIES, RUNTIME_VERSION, SCHEMA_VERSION, SCOPE_EXPANSION_REASONS_MAX, SEALED_STATUSES, TERMINAL_STATUSES } from './constants.js';
...
function additiveRecoveryScope(ticket, receipt) {
  ...
  if (!capabilityTestPathBoundErrors(testPaths).valid) return null;
  return {
...
function preparedCapabilityRecoveryError(
  ...
  if (
    ...
    lineage?.validation_submissions_per_worker !== RECEIPT_MAX_SUBMISSIONS_PER_WORKER ||
    lineage?.max_physical_workers !== RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET ||
    !Number.isInteger(lineage?.validation_submissions) ||
    lineage.validation_submissions < 1 || lineage.validation_submissions > 6 ||
    !Number.isInteger(lineage?.physical_workers) ||
    lineage.physical_workers < 1 ||
    lineage.physical_workers > RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET ||
...
async function prepareCapabilityRecovery(
  ...
  if (
    !Number.isInteger(currentSubmissions) ||
    currentSubmissions < 1 ||
    currentSubmissions > RECEIPT_MAX_SUBMISSIONS_PER_WORKER ||
    !Number.isInteger(currentWorkers) ||
    currentWorkers < 1 ||
    physicalWorkers > RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET ||
    validationSubmissions >
      RECEIPT_MAX_SUBMISSIONS_PER_WORKER * RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET
  ) return null;
  ...
  const recoveryLineage = {
    ...
    validation_submissions_per_worker: RECEIPT_MAX_SUBMISSIONS_PER_WORKER,
    max_physical_workers: RECEIPT_MAX_PHYSICAL_WORKERS_PER_TICKET,
  };
...
export async function recoverReceiptLocked(
  ...
  assertSafeInput({ receipt: raw, recovery });
