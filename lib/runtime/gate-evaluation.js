...
import { detectTestRunner, runTestSuite, splitCommand, targetedInvocation, templateInvocation } from './runner.js';
import { currentTreeSha, workingTreeStatus } from './git.js';
...
import { readSuiteCache, writeSuiteCache } from './suite-cache.js';
...
export async function resolveSuiteSelection(projectDir, state, config) {
  ...
  // A deleted path cannot be passed to an impacted command, but its removal
  // still requires coverage. Match the multi-runner fail-safe: if any changed
  // member is missing, verify the full suite rather than dropping that change.
  const changed = [...new Set((state.receipts ?? []).flatMap((receipt) => receipt.changed_files ?? []))];
  const present = [];
  for (const file of changed) {
    if (!await exists(path.join(projectDir, file))) return full;
    present.push(file);
  }
  present.sort();
...
export async function gateSuiteContext(projectDir, paths, state, config) {
  ...
  const cache = await readSuiteCache(cachePath);
  ...
  // treeSha AND the runner id, execution root, and resolved suite, so full↔impacted (mode +
  ...
  const participants = (runnerSet.participants ?? []).map((participant) => {
    ...
    const root = path.resolve(projectDir, participant.root ?? '.');
    const keyR = participant.mode === 'impacted'
      ? `${treeSha}:${sha256({ runner: participant.runner, root, mode: 'impacted', template: participant.template, paths: participant.impacted_paths, invocation: participant.invocation })}`
      : `${treeSha}:${sha256({ runner: participant.runner, root, command,
        // Older detached auto-detection ran at the repository root. Its
        // cached result cannot attest this runner's own detected suite.
        ...(!command ? { auto_detection_version: 2 } : {}),
      })}`;
...
export async function evaluateGatePreflight(projectDir, state, config, ctx) {
  ...
  const requiredProfileIds = (state.preflight?.artifact?.verification_profiles ?? [])
    .filter((profile) => profile.disposition === 'required')
    .map((profile) => profile.id);
  const assignedProfileIds = new Set(
    (state.approved_plan?.plan?.workstreams ?? [])
      .flatMap((workstream) => workstream.verification_profiles ?? []),
  );
  // Fast v2 runs select verification in preflight and proceed directly to tests.
  // Full/escalated runs must still assign every required profile in their plan.
  const requiresPlanAssignment = state.lane !== 'fast' || state.plan_contract_version !== 2 || Boolean(state.approved_plan);
  const snappedProfiles = Array.isArray(state.verification_profiles) ? state.verification_profiles : [];
  const profileById = new Map(snappedProfiles.map((profile) => [profile.id, profile]));
  const rootSnapshotById = new Map(
    (state.verification_profile_roots ?? []).map((snapshot) => [snapshot.id, snapshot]),
  );
  const verificationProfileResults = [];
  const invocationResults = new Map();
  for (const id of requiredProfileIds) {
    const profile = profileById.get(id);
    if (!profile || (requiresPlanAssignment && !assignedProfileIds.has(id))) {
      // prose-bound-exempt: both reason branches are fixed runtime literals.
      verificationProfileResults.push({ id, passed: false, reason: !profile ? 'profile missing from start snapshot' : 'required profile unassigned by approved plan' });
      continue;
    }
    const root = profile.root ?? '.';
    let result;
    try {
      const argv = splitCommand(profile.command);
      ...
      const metadata = await stat(declaredRoot);
      const rootSnapshot = rootSnapshotById.get(id);
      if (
        !metadata.isDirectory() ||
        resolvedRoot !== declaredRoot ||
        (resolvedRoot !== canonicalProject && !resolvedRoot.startsWith(`${canonicalProject}${path.sep}`)) ||
        !rootSnapshot ||
        rootSnapshot.realpath !== resolvedRoot ||
        rootSnapshot.dev !== String(metadata.dev) ||
        rootSnapshot.ino !== String(metadata.ino)
      ) {
        throw new Error('verification profile root must retain its snapshotted contained identity');
      }
      // The deadline is part of the executable contract. A slower passing
      // profile cannot attest another profile's stricter time budget. Root
      // identity is checked above for every profile, including a cache hit.
      const key = JSON.stringify([resolvedRoot, argv, profile.timeout_ms]);
      result = invocationResults.get(key);
      if (!result) {
        const verification = await runTestSuite(resolvedRoot, {
        ...
        invocationResults.set(key, result);
      }
    } catch {
      result = {
        passed: false,
        command: profile.command,
        root,
        reason: 'verification profile root is missing, symlinked, non-directory, or outside the project',
      };
    }
    verificationProfileResults.push({ id, ...result });
  }
  const verificationProfiles = {
    passed: verificationProfileResults.every((entry) => entry.passed === true),
    results: verificationProfileResults,
  };
  ...
  const targetedTemplate = !isMulti && typeof config.test_commands?.targeted_template === 'string'
    && config.test_commands.targeted_template.trim()
    ? config.test_commands.targeted_template : null;
  let targeted = null;
  if (targetedCommand) {
    ...
    if (isMulti) {
      ...
      if (targetedTemplate && present.length > 0) {
        try {
          invocation = templateInvocation(targetedTemplate, present);
          if (!invocation) templateFailure = 'test_commands.targeted_template must contain the {paths} placeholder';
        } catch {
          templateFailure = 'test_commands.targeted_template is malformed';
        }
      } else if (!targetedTemplate) {
        const runner = await detectTestRunner(projectDir);
        invocation = targetedInvocation(runner, present);
      }
      if (!invocation) {
        targeted = {
          executed: false,
          reason: templateFailure ?? (present.length === 0
  ...
  return {
    ...
    passed: evaluated.passed && verificationProfiles.passed,
...
function gatePreflightKey(state, config, ctx) {
  return sha256({
    ...
    targeted_template: config.test_commands?.targeted_template ?? null,
...
export function ownsGlobToRegExp(glob) {
  ...
  // `[^/]*[^/]*`.
  const translated = escaped.replace(/\*\*\/|\*\*|\*/g, (match) =>
    match === '**/' ? '(?:.*/)?' : match === '**' ? '.*' : '[^/]*');
...
export async function resolveRunnerSet(projectDir, state, config) {
  ...
  // Ownership follows the complete attested diff, including deletions. Missing
  // files cannot be passed to an impacted command, so their owners run full.
  const changed = [...new Set((state.receipts ?? []).flatMap((receipt) => receipt.changed_files ?? []))].sort();
  const present = new Set();
  for (const file of changed) {
    if (await exists(path.join(projectDir, file))) present.add(file);
  }

  // (1) UNION owns-matching: each changed file belongs to EVERY runner whose any
  // `owns` glob matches; a file matched by NO runner is an ORPHAN. `changed` is
  ...
  // no changed files remain on disk or an orphan slipped through under the
  ...
  if (present.size === 0 || orphans.length > 0) {
    return {
      strategy: 'multi',
  ...
  // Only runners that own a changed file participate from here on.
  ...
  const participants = participating.map((runner) => {
    ...
    const template = typeof templateRaw === 'string' && templateRaw.trim() ? templateRaw : null;
    if (template && remoteChecksEnabled && changedSubset.length > 0 && changedSubset.every((file) => present.has(file))) {
      let invocation = null;
      try {
        // The ownership ledger stays repository-relative, while the command
        // executes inside this runner's root, like its targeted sibling.
        const runnerRoot = normalizeClaimPath(runner.root ?? '.');
        const operands = changedSubset.map((file) => path.posix.relative(runnerRoot, normalizeClaimPath(file)));
        invocation = templateInvocation(template, operands);
      } catch {
        // Malformed template (e.g. an unterminated quote): fail-safe to full.
        invocation = null;
      }
...
export async function runMergeGates(projectDir, paths, state, config) {
  ...
  if (ctx.strategy === 'multi') {
    ...
    for (const participant of ctx.participants) {
      let result = participant.cachedEntry?.passed === true ? participant.cachedEntry : null;
      const cached = result !== null;
      if (!result) {
        const verification = await runTestSuite(path.join(projectDir, participant.root ?? '.'), {
          ...(participant.mode === 'impacted' && participant.invocation
            ? { override: participant.invocation }
            : { command: participant.command ?? undefined }),
          timeout_ms: config.deadlines_ms?.[state.lane],
        });
        result = {
          passed: verification.passed === true,
    ...
    return evaluateGates(projectDir, paths, state, config, { ...ctx, runnerResults });
  }
...
export async function evaluateGates(projectDir, _paths, state, config, ctx) {
  ...
  if (preflight.passed !== true) {
    ...
    if (postGateTreeSha === treeSha) {
      ...
      if (dirty) await writeSuiteCache(cachePath, cache);
    }
  } else if (!cached && full?.skipped !== true && postGateTreeSha === treeSha) {
    cache.results[cacheKey] = full;
    await writeSuiteCache(cachePath, cache);
  }
  const treeBinding = {
    passed: lastReceipt !== null
      && lastReceipt.head_tree_sha === treeSha
  ...
  return {
    ...
    checks: {
      ...evaluated.checks,
      ...(preflight.verificationProfiles
        ? { verification_profiles: preflight.verificationProfiles }
        : {}),
    },
  };
}
