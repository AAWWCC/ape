...
import {
  ...
  LEGACY_PLAN_CONTRACT_MAX_BYTES,
...
export function runContractFieldBounds({
  run = {},
  include_dynamic_test_paths = true,
  manifest_growth_contract_version = 2,
} = {}) {
  const limits = pipelineLimits(run);
  return {
    validation_attempts_per_worker: limits.max_validation_submissions_per_worker,
    max_physical_workers_per_ticket: limits.max_physical_workers_per_ticket,
    corrections_per_validation: 20,
    ...(include_dynamic_test_paths
      ? { dynamic_test_paths: capabilityDynamicTestPathBounds({ version: manifest_growth_contract_version }) }
      : {}),
  };
}

export function runContractByteBudgets(state = undefined) {
  const legacy = state !== undefined && state?.capability_snapshot?.artifact_limits_version !== 2;
  return {
    candidate_plan_utf8_bytes: legacy ? LEGACY_PLAN_CONTRACT_MAX_BYTES : PLAN_CONTRACT_MAX_BYTES,
...
function capabilityCatalog(state) {
  ...
  return {
    ...(snapshot.artifact_limits_version === 2 ? { artifact_limits_version: 2 } : {}),
    ...([1, 2].includes(snapshot.manifest_growth_contract_version)
      ? {
          manifest_growth_contract_version: snapshot.manifest_growth_contract_version,
    ...
    ...(snapshot.frozen_recovery_authority
      ? { frozen_recovery_authority: structuredClone(snapshot.frozen_recovery_authority) }
      : {}),
  };
}
...
export async function initializeRunContractManifest(paths, state, at = state.created_at) {
  ...
  const manifest = {
    ...
    ...(state.execution_policy ? { execution_policy: structuredClone(state.execution_policy) } : {}),
...
function ticketRoleView(_ticket, capabilityManifest) {
  return {
...
function ticketRecoveryAuthority(ticket) {
  return {
    ...(ticket.execution_limits ? { execution_limits: structuredClone(ticket.execution_limits) } : {}),
    claimed_paths: structuredClone(ticket.claimed_paths ?? []),
    test_paths: structuredClone(ticket.test_paths ?? []),
    risk_triggers: structuredClone(ticket.risk_triggers ?? []),
...
export async function appendTicketRunContract(
  ...
  const prepared = await prepareTicketRunContract(
    paths,
    state,
    ticket,
    capabilityManifest,
  ...
  await writeContentAddressedReceiptSchema(
    paths,
    ticket.output_schema,
    capabilityManifest.receipt_schema.hash,
  );
  const pointer = await writeContentAddressedManifest(paths, prepared.manifest);
  if (pointer.hash !== prepared.pointer.hash || pointer.ref !== prepared.pointer.ref) {
    throw new Error('prepared run-contract pointer changed during publication');
  }
  state.run_contract = pointer;
  return pointer;
}
...
/**
 * Derive the next content-addressed ticket contract without writing it. This
 * lets a receipt transaction bind the complete successor generation before
 * any receipt, ticket, schema, manifest, audit, or active-state sink occurs.
 */
export async function prepareTicketRunContract(
  paths,
  state,
  ticket,
  capabilityManifest,
  ...
  const receiptSchemaHash = sha256(ticket.output_schema);
  if (receiptSchemaHash !== capabilityManifest.receipt_schema.hash) {
    throw new Error('ticket receipt schema conflicts with its immutable schema hash');
  }
  const receiptSchema = {
    ref: receiptSchemaRef(receiptSchemaHash),
    hash: receiptSchemaHash,
  };
  const ticketContract = {
    ...
    role_view: roleView,
    recovery_authority: ticketRecoveryAuthority(ticket),
  };
  ...
  return { pointer: pointerFor(manifest), manifest };
}
