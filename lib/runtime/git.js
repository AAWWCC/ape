...
// The optional cache must never block tree observation on a FIFO/device or
// allocate without a bound. Larger indexes still work through the cold path;
// this limit controls only the in-memory warm-cache optimization.
const TREE_INDEX_CACHE_MAX_BYTES = 64 * 1024 * 1024;

async function readCachedIndex(file) {
  const handle = await open(file, constants.O_RDONLY | (constants.O_NONBLOCK ?? 0) |
    (constants.O_NOFOLLOW ?? 0)).catch((error) => {
  ...
  try {
    ...
    if (!metadata.isFile() || !Number.isSafeInteger(metadata.size) ||
        metadata.size < 0 || metadata.size > TREE_INDEX_CACHE_MAX_BYTES) {
      throw new Error('unsafe optional tree-index cache');
    }
    ...
    if (used !== metadata.size || !after.isFile() ||
        ['dev', 'ino', 'size', 'mtimeMs', 'ctimeMs'].some((key) => metadata[key] !== after[key])) {
      throw new Error('optional tree-index cache changed during read');
    }
    return { content: buffer.subarray(0, used), metadata };
  } finally {
    await handle.close();
  }
}
...
export async function currentTreeSha(projectDir) {
  ...
  if (warmable) {
    ...
    // serializes individual subcommands, not this sequence, so a
    ...
    try {
      const cached = await readCachedIndex(paths.treeIndex);
      if (cached) {
        await writeFile(scratch, cached.content);
        await utimes(scratch, cached.metadata.atime, cached.metadata.mtime);
      }
      ...
      // a git error. Excluding .ape keeps live runtime mutations out of the
      // observed tree while preserving any reserved paths already in HEAD.
      // Those baseline bytes must remain: shipping compares the complete
      // staged/committed tree exactly, including tracked project configuration.
      // A staged .ape edit/addition/deletion remains absent from this evidence
      // and is therefore rejected by the prospective shipping-index guard.
  ...
  try {
    ...
    // An explicit excluded .ape pathspec can still fail when Git ignores the
    // entire directory. Stage normally, then restore only the reserved paths
    // from HEAD in this private index. This retains tracked baseline bytes
    // without letting live runtime state or a reserved deletion enter evidence.
    await runGit(projectDir, ['add', '-A'], { env });
    await runGit(projectDir, ['reset', '-q', 'HEAD', '--', '.ape'], { env });
