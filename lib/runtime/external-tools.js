...
const APE_CONTROL = /(?:^|__)ape_(run|status|config|history)$/;
...
// MCP server without a reviewed adapter, a sealed ticket may opt into an exact
// operation with `<server>:tool:<operation>:<access>` where access is one of
// read, write, or execute. When multiple claims match the same operation, the
// highest access level wins (execute > write > read). Wildcard and prefix
// claims never upgrade an unknown operation. The conservative drift marker
// keeps post-call tree reconciliation enabled on every resolved effect.
// Downstream policy (externalToolPolicy) continues to enforce writable ticket
// for writes and high_risk for privileged execution.
export function resolveClaimedPluginEffect(classification, claims) {
  if (
    !classification ||
  ...
  for (const access of ['execute', 'write', 'read']) {
    const authorized = parsed.some((claim) =>
    ...
    if (authorized) {
      ...
      if (access === 'read') result.claimed_plugin_read = true;
      return result;
    }
  }
  return classification;
...
export { resolveClaimedPluginEffect as resolveClaimedPluginRead };
