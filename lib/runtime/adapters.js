...
export function receiptContractContext(ticket, { reference_schema = false } = {}) {
  if (!TICKET_ID_PATTERN.test(ticket?.ticket_id ?? '')) {
    throw new Error('APE dispatch ticket carries an invalid ticket id');
  }
  const excerpt = receiptSchemaExcerpt(ticket);
  const schemaReference = {
    projection: 'immutable-ticket-schema-reference-v1',
    ticket_id: ticket.ticket_id,
    path: `.ape/runtime/tickets/${ticket.ticket_id.replaceAll(':', '_')}.json`,
    json_pointer: '/output_schema',
    schema_hash: sha256(ticket.output_schema),
    required: true,
    instruction: 'Before constructing the receipt, read the complete output_schema and capability_manifest from this exact immutable ticket using the permitted native file reader. This reference preserves every field and exact command enum; it does not replace or relax the schema.',
  };
  const render = (schema) => [
    'APE hook-enforced receipt construction (authoritative)',
    [
      'The SubagentStop hook refuses termination for an invalid receipt while correction allowance remains; exhaustion returns control to runtime recovery. The final receipt must match this immutable ticket output_schema.',
      ...
      'Never claim an unexecuted test. For runtime-owned red-test, green-test, or test-correction, leave tests empty; the runtime executes and seals those checks. A passed targeted-tests receipt requires actually executed passing test evidence.',
    ].join(' '),
    ...
    'Before stopping, call ape_validate_receipt with explicit project_dir, this immutable ticket_id, and this exact completed object as draft. On Codex, reuse the exact project_dir from the successful ape_bind call; on Claude, use the host-provided project root. Do not rely on the MCP server working directory. If invalid, correct only within the returned corrections_remaining budget; on exhausted, stop for runtime recovery. Once valid, return it byte-for-byte unchanged without another validation.',
  ].join('\n\n');
  let context = render(reference_schema ? schemaReference : excerpt);
  if (Buffer.byteLength(context, 'utf8') > MAX_RECEIPT_CONSTRUCTION_CONTEXT_BYTES) {
    context = render(schemaReference);
  }
  if (Buffer.byteLength(context, 'utf8') > MAX_RECEIPT_CONSTRUCTION_CONTEXT_BYTES) {
...
export function codexInjectedDispatchContext(ticket) {
  ...
  let context = render(receiptContractContext(ticket));
  if (Buffer.byteLength(context, 'utf8') > MAX_CODEX_INJECTED_CONTEXT_BYTES) {
    context = render(receiptContractContext(ticket, { reference_schema: true }));
  }
  if (Buffer.byteLength(context, 'utf8') > MAX_CODEX_INJECTED_CONTEXT_BYTES) {
...
export function nativeDispatch(host, ticket, dispatchIntent = null, _context = {}) {
