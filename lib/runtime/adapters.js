...
export function receiptContractContext(ticket, { reference_schema = false } = {}) {
  ...
  const render = (schema) => [
    ...
    'Before stopping, call ape_validate_receipt with this exact completed object. If invalid, correct only within the returned corrections_remaining budget; on exhausted, stop for runtime recovery. Once valid, return it byte-for-byte unchanged without another validation.',
  ].join('\n\n');
