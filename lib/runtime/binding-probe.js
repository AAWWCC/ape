...
export async function launchBindingProbe(paths, input) {
  ...
  return withProbeLock(paths, async () => {
    ...
    if (record.status === 'launched') {
      const bindingAgentType = suppliedAgentType ?? CODEX_V2_DEFAULT_AGENT_TYPE;
      const same =
      ...
      if (same && !record.binding_agent_type) {
        await atomicWriteJson(paths.bindingProbe, { ...record, binding_agent_type: bindingAgentType });
      }
      return same
