...
export async function launchBindingProbe(paths, input) {
  ...
  // agent_type field. The random task name is already the probe's launch
  // capability, so Codex may omit the redundant type and recover the one fixed
  // probe type only after that capability uniquely matches the current record.
  // A host that does supply a type still has to supply a bounded, exact match.
  if (
...
export async function bindBindingProbe(paths, input) {
  ...
  if (
    !bounded(sessionId) ||
    !bounded(agentId) ||
    (agentType !== undefined && agentType !== null && !bounded(agentType))
  ) {
  ...
  return withProbeLock(paths, async () => {
    ...
    if (
      ...
      (agentType != null && record.agent_type !== agentType)
    ) {
