import { timestampAtLeast } from './timestamps.js';
import { isCanonicalTimestamp as validTimestamp } from './timestamps.js';
...
import { SCHEMA_VERSION, NATIVE_MODEL_MAX_CHARS, CODEX_REASONING_EFFORT_MAX_CHARS } from './constants.js';
...
import { BOOTSTRAP_TOOL_PATTERN, codexProbeReservationOrientation, recordCodexBootstrapCandidate, resolveCodexBootstrapCandidate } from './codex-bootstrap.js';

// A valid live reservation deliberately denies unmatched pre-run children.
// Keep that policy outcome distinct from malformed identity or unreadable state.
export const BINDING_PROBE_RESERVATION_DENIED = 'APE_BINDING_PROBE_RESERVATION_DENIED';
function probeReservationDenial(message) {
  return Object.assign(new Error(message), { code: BINDING_PROBE_RESERVATION_DENIED });
}
...
function validLegacyProbeRecord(record) {
  if (
    ...
    !bounded(record.model.model, NATIVE_MODEL_MAX_CHARS[record.host]) ||
    (
      record.model.reasoning_effort !== undefined &&
      !bounded(record.model.reasoning_effort, CODEX_REASONING_EFFORT_MAX_CHARS)
    ) ||
...
export function projectBindingProbe(record, at = Date.now()) {
  ...
  const projectedModel = plainObject(record.model)
    ? {
        ...(bounded(record.model.model, NATIVE_MODEL_MAX_CHARS[record.host]) ? { model: record.model.model } : {}),
        ...(bounded(record.model.reasoning_effort, CODEX_REASONING_EFFORT_MAX_CHARS)
  ...
  return {
    ...
    ...(status === 'expired'
      ? { reason: record.status === 'completed'
        ? 'completed native binding proof expired before consumption'
        : 'native binding diagnostic expired before acknowledgement' }
      : launchExpired
...
export async function prepareBindingProbe(paths, { host, model, bootstrap_protocol = 1 }) {
  ...
  if (!model || !bounded(model.model, NATIVE_MODEL_MAX_CHARS[host])) throw new Error('native binding probe requires a resolved model');
...
export async function bindBindingProbe(paths, input) {
  ...
  return withProbeLock(paths, async () => {
    ...
    if (rawRecord?.version >= 3 || rawRecord?.bootstrap_protocol !== undefined) {
      ...
      const orientationStatus = rawRecord.status === 'launched' && expired(rawRecord.launch_expires_at)
        ? 'launch_expired' : rawRecord.status;
      return {
        matched: true,
        valid: observed.recorded === true,
        bootstrap_required: true,
        ...(observed.recorded === true ? { additional_context: codexProbeReservationOrientation(orientationStatus) } : {}),
    ...
    const bindingDecidedAt = timestampAtLeast(Date.now(), record.launched_at, record.bound_at);
...
export async function resolvesBindingProbeIdentity(paths, input) {
  ...
  if (validLegacyProbeRecord(rawRecord)) {
    ...
    if (
      ...
      throw probeReservationDenial('legacy native binding probe owns the pre-run child identity window');
    }
    return false;
  }
  ...
  if (PROBE_RESERVATION_STATUSES.has(record.status)) {
    throw probeReservationDenial('native binding probe owns the pre-run child identity window');
  }
  return false;
}
...
export async function observeBindingProbeStop(paths, input) {
  ...
  return withProbeLock(paths, async () => {
    ...
    if (currentIdentityPairMatches(record, sessionId, agentId)) {
      ...
      await atomicWriteJson(paths.bindingProbe, {
        ...record,
        canary_stopped_at: iso(timestampAtLeast(Date.now(), record.bound_at)),
      });
      return { matched: true };
    }
...
export async function acknowledgeBindingProbe(paths, input = {}) {
  ...
  return withProbeLock(paths, async () => {
    ...
    const completedAt = timestampAtLeast(Date.now(), record.bound_at);
...
export async function consumeBindingProbe(paths, host) {
  return withProbeLock(paths, async () => {
    ...
    const consumedAt = timestampAtLeast(Date.now(), record.completed_at);
