...
import { MAX_DIRECTED_REPLANS } from './pipeline-limits.js';
...
const StageTicketSchema = z.object({
  ...
  plan_recovery: z.object({
    version: z.literal(1),
    attempt: z.number().int().min(1).max(MAX_DIRECTED_REPLANS),
    source_ticket_id: nonEmpty,
...
export const AnswerPreflightInputSchema = z.object({
  ...
  reason: z.string().min(1).max(4_000).regex(/\S/),
  preflight_hash: digest,
  answers: z.array(z.object({
    id: z.string().min(1).max(160),
    answer: z.string().min(1).max(16_384).regex(/\S/),
  }).strict()).max(64),
  claimed_paths: z.array(z.string().min(1).max(512)).max(64).optional(),
  test_paths: z.array(z.string().min(1).max(512)).max(64).optional(),
  risk_triggers: z.array(z.enum(RISK_TRIGGERS)).max(RISK_TRIGGERS.length).optional(),
}).strict();
