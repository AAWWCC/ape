...
import { ApprovedPlanSchema, CandidatePlanSchema, LEGACY_PLAN_CONTRACT_MAX_BYTES, PLAN_CONTRACT_MAX_BYTES, PLAN_CONTRACT_VERSION } from './plan-contract.js';
import { markExactTestScope, isCanonicalVerificationRoot, VERIFICATION_PROFILE_ID_PATTERN } from './path-scope.js';
import { RECEIPT_INPUT_SCHEMA } from './receipt-input.js';
import { EXECUTION_POLICY_DEFAULTS, EXECUTION_POSITIVE_COUNTS, assertExecutionPolicy, pipelineLimits, receiptLimits, stageRecoveryLimits } from './pipeline-limits.js';
import { GENERAL_INPUT_MAX_BYTES, INPUT_LIMITS, RECEIPT_INPUT_MAX_BYTES, TASK_REQUEST_MAX_BYTES } from './input-guard.js';
...
const positiveSafeCount = z.number().int().min(1).max(Number.MAX_SAFE_INTEGER);
export const ExecutionLimitsSchema = z.object(Object.fromEntries(
  Object.keys(EXECUTION_POLICY_DEFAULTS).map((key) => [key,
    z.number().int().min(EXECUTION_POSITIVE_COUNTS.has(key) ? 1 : 0).max(Number.MAX_SAFE_INTEGER)]),
)).strict().superRefine((limits, context) => {
  try { assertExecutionPolicy(limits); }
...
export function structuredReviewFindingSchemaForTicket(ticket) {
  const limits = receiptLimitsForTicket(ticket);
  return StructuredReviewFindingSchema.safeExtend({
    line: z.number().int().positive().max(limits.finding_line),
    title: structuredReviewString(limits.finding_title),
    detail: structuredReviewString(limits.finding_detail),
    remediation: ReviewRemediationSchema.safeExtend({
      test_paths: z.array(structuredReviewString(REVIEW_FILE_MAX))
        .min(1).max(limits.paths).optional(),
    }).optional(),
  });
}
...
export function validateStructuredReviewReceipt(ticket, receipt) {
  ...
  const parsed = z.array(structuredReviewFindingSchemaForTicket(ticket))
    .max(receiptLimitsForTicket(ticket).findings)
...
// Preserve the original ordinary-update capacity, including existing journals
// at its ceiling. Two additional generations are reserved for a cancellation
// request and the terminal result, so reaching the update budget cannot strand
// a working task.
export const TASK_MAX_UPDATE_GENERATION = 1_024;
export const TASK_MAX_GENERATION = TASK_MAX_UPDATE_GENERATION + 2;
...
export const TaskGenerationSchema = z.object({
  ...
  generation: z.number().int().min(0).max(TASK_MAX_GENERATION),
  ...
  request: boundedJson(TASK_REQUEST_MAX_BYTES, 'task request'),
  ...
  record.generation <= TASK_MAX_UPDATE_GENERATION ||
  TASK_TERMINAL_STATUSES.includes(record.status) ||
  (record.generation === TASK_MAX_UPDATE_GENERATION + 1 && record.cancellation !== null),
{ message: 'task generations beyond the update budget are reserved for cancellation and termination' });

export const TaskGenerationPatchSchema = z.object({
  expectedGeneration: z.number().int().min(0).max(TASK_MAX_GENERATION).optional(),
...
export const TaskGcRecordSchema = z.object({
  ...
  operationId: TaskOperationIdSchema,
  status: z.enum(TASK_STATUSES),
  generations: z.number().int().min(1).max(TASK_MAX_GENERATION + 1),
  createdAt: isoTimestamp,
...
export const CapabilityManifestSchema = z.object({
  ...
  field_bounds: z.object({
    validation_attempts_per_worker: positiveSafeCount,
    max_physical_workers_per_ticket: positiveSafeCount,
    ...
    dynamic_test_paths: z.union([
      z.object({
        max_items: z.literal(64),
        max_serialized_utf8_bytes: z.literal(4_096),
      }).strict(),
      z.object({
        max_items: z.literal(INPUT_LIMITS.maxArrayLength),
        command_max_chars: z.literal(8_192),
        manifest_max_serialized_utf8_bytes: z.literal(CAPABILITY_MANIFEST_MAX_UTF8_BYTES),
      }).strict(),
    ]).optional(),
  }).strict(),
  byte_budgets: z.object({
    candidate_plan_utf8_bytes: z.union([z.literal(16_384), z.literal(65_536)]),
...
const StageTicketSchema = z.object({
  ...
  recovery_lineage: z.object({
    ...
    validation_submissions: positiveSafeCount,
    physical_workers: positiveSafeCount,
    validation_submissions_per_worker: positiveSafeCount,
    max_physical_workers: positiveSafeCount,
  }).strict().optional(),
  ...
  verification_profiles: z.array(z.object({
    id: z.string().regex(VERIFICATION_PROFILE_ID_PATTERN),
    description: z.string().min(1).max(GENERAL_INPUT_MAX_BYTES)
    ...
    root: z.string().min(1).max(512).refine(isCanonicalVerificationRoot, {
      message: 'verification profile root must be a canonical contained project-relative path',
    }).optional(),
    timeout_ms: z.number().int().min(1).max(MAX_TIMER_DELAY_MS),
  }).strict()).max(CAPABILITY_MANIFEST_MAX_VERIFICATION_PROFILES).superRefine((profiles, context) => {
    const ids = new Set();
  ...
  preflight: z.object({
    ...
    operator_evidence: z.object({
      ...
      answers: z.array(z.object({
        ...
        answer: z.string().min(1).max(GENERAL_INPUT_MAX_BYTES),
      }).strict()).max(INPUT_LIMITS.maxArrayLength),
    }).strict().optional(),
  }).strict().optional(),
  ...
  plan_recovery: z.object({
    ...
    missing_assurances: z.array(z.object({
      ...
      source_stage: nonEmpty,
      summary: z.string().min(1).max(RECEIPT_INPUT_MAX_BYTES),
      evidence_anchor: z.string().min(1).max(RECEIPT_INPUT_MAX_BYTES).optional(),
      requirement_id: z.string().min(1).max(128).optional(),
      risk_trigger: z.enum(RISK_TRIGGERS).optional(),
    }).strict()).min(1).max(INPUT_LIMITS.maxArrayLength),
  }).strict().optional(),
  test_reconciliation: z.object({
    ...
    report: z.string().min(1).max(RECEIPT_INPUT_MAX_BYTES),
    test_paths: z.array(z.string().min(1).max(REVIEW_FILE_MAX)).max(INPUT_LIMITS.maxArrayLength),
  }).strict().optional(),
  ticket_hash: digest,
  ...
  let receipts;
  try {
    limits = pipelineLimits(ticket);
    receipts = receiptLimits(ticket);
  } catch (error) {
    context.addIssue({ code: 'custom', path: ['execution_limits'], message: error.message });
    return;
  }
  if (ticket.attempt > stageRecoveryLimits(ticket.stage_id, ticket).max_stage_attempts) {
    context.addIssue({ code: 'custom', path: ['attempt'], message: 'exceeds immutable stage attempt policy' });
  }
  if ((ticket.plan_recovery?.attempt ?? 0) > limits.max_directed_replans) {
    context.addIssue({ code: 'custom', path: ['plan_recovery', 'attempt'], message: 'exceeds immutable directed replan policy' });
  }
  if (ticket.plan_recovery) {
    const receiptBounds = receiptLimitsForTicket(ticket);
    if (ticket.plan_recovery.missing_assurances.length > receiptBounds.assurances ||
        ticket.plan_recovery.missing_assurances.some((entry) =>
          entry.summary.length > receiptBounds.assurance_text ||
          (entry.evidence_anchor?.length ?? 0) > receiptBounds.assurance_anchor)) {
      context.addIssue({ code: 'custom', path: ['plan_recovery'], message: 'exceeds immutable assurance feedback limits' });
    }
  }
  const fields = ticket.capability_manifest?.field_bounds;
  if (ticket.execution_limits && fields && (
    fields.validation_attempts_per_worker !== receipts.max_validation_submissions_per_worker ||
    fields.max_physical_workers_per_ticket !== receipts.max_physical_workers_per_ticket
  )) context.addIssue({ code: 'custom', path: ['capability_manifest', 'field_bounds'], message: 'does not match immutable execution limits' });
  const lineage = ticket.recovery_lineage;
  if (lineage && (
    lineage.validation_submissions_per_worker !== receipts.max_validation_submissions_per_worker ||
    lineage.max_physical_workers !== receipts.max_physical_workers_per_ticket ||
    lineage.physical_workers > receipts.max_physical_workers_per_ticket ||
    lineage.validation_submissions > receipts.max_physical_workers_per_ticket * receipts.max_validation_submissions_per_worker
  )) context.addIssue({ code: 'custom', path: ['recovery_lineage'], message: 'exceeds or changes immutable receipt recovery limits' });
});

export const LegacyRoadmapFollowupSchema = z.object({
  id: z.string().min(1).max(128),
...
export const RoadmapFollowupSchema = z.object({
  ...
  depends_on: z.array(z.string().min(1).max(128)).max(INPUT_LIMITS.maxArrayLength).default([]),
}).strict();

export const ReceiptEvidenceSchema = z.object({
  roadmap_followups: z.array(RoadmapFollowupSchema).max(INPUT_LIMITS.maxArrayLength).optional(),
...
export const RunCommandProfileSchema = z.object({
  ...
  reason: z.string().min(1).max(GENERAL_INPUT_MAX_BYTES).refine((value) => value.trim().length > 0, {
    message: 'reason must contain non-whitespace audit text',
  }),
}).strict();

// Shared by the public tool's conditional fields and the pre-mutation service
// validator. Dynamic question identity and lifecycle checks remain in service.
export const AnswerPreflightInputSchema = z.object({
  run_id: z.string().min(1).max(160).optional(),
  reason: z.string().min(1).max(GENERAL_INPUT_MAX_BYTES).regex(/\S/),
  preflight_hash: digest,
  answers: z.array(z.object({
    id: z.string().min(1).max(160),
    answer: z.string().min(1).max(GENERAL_INPUT_MAX_BYTES).regex(/\S/),
  }).strict()).max(INPUT_LIMITS.maxArrayLength),
  claimed_paths: z.array(z.string().min(1).max(512)).max(INPUT_LIMITS.maxArrayLength).optional(),
  test_paths: z.array(z.string().min(1).max(512)).max(INPUT_LIMITS.maxArrayLength).optional(),
  risk_triggers: z.array(z.enum(RISK_TRIGGERS)).max(RISK_TRIGGERS.length).optional(),
}).strict();
export const ANSWER_PREFLIGHT_INPUT_JSON_SCHEMA = z.toJSONSchema(AnswerPreflightInputSchema);

export const RunStartInputSchema = z.object({
  objective: nonEmpty,
  // New adapters bind START to a read-only preview of these exact inputs.
  // Omission is retained for host-neutral legacy service simulations only.
  admission_contract_version: z.literal(1).optional(),
  expected_admission_digest: digest.optional(),
...
export const ReceiptValidationResultSchema = z.object({
  ...
  budgets: z.object({
    candidate_plan_utf8_bytes: z.object({
      ...
      max_bytes: z.union([z.literal(LEGACY_PLAN_CONTRACT_MAX_BYTES), z.literal(PLAN_CONTRACT_MAX_BYTES)]),
      remaining_bytes: z.number().int().nonnegative().max(PLAN_CONTRACT_MAX_BYTES),
    }).strict().refine((budget) => budget.remaining_bytes <= budget.max_bytes, {
      path: ['remaining_bytes'], message: 'remaining bytes cannot exceed the ticket budget',
    }),
  }).strict().optional(),
}).strict();
