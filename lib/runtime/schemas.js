...
const REMEDIATION_HISTORY_ITEM_LIMIT = 64;
const REMEDIATION_HISTORY_BYTES_MAX = 65_536;

// Optional, default-free run-state evidence for bounded review remediation.
// Legacy runs remain byte-stable because no reader materializes this field.
// Each cycle is already a deduplicated identity set, so accepting a duplicate
// or non-canonical ordering would make replay/history comparisons ambiguous.
const remediationFindingIdentity = z.string().regex(/^[0-9a-f]{64}$/);
const remediationFindingAlias = z.object({
  descriptor: remediationFindingIdentity,
  canonical: remediationFindingIdentity,
}).strict();
const remediationFindingProvenance = z.object({
  receipt_hashes: z.array(remediationFindingIdentity).min(1).max(REVIEW_FINDING_LIMIT),
  descriptor_hashes: z.array(remediationFindingIdentity).min(1).max(REVIEW_FINDING_LIMIT),
  legacy_descriptor_hashes: z.array(remediationFindingIdentity)
    .min(1).max(REVIEW_FINDING_LIMIT).optional(),
}).strict().superRefine((provenance, context) => {
  for (const key of ['receipt_hashes', 'descriptor_hashes', 'legacy_descriptor_hashes']) {
    ...
    for (let index = 0; index < values.length; index += 1) {
      if (index > 0 && values[index - 1] >= values[index]) {
        context.addIssue({
          code: 'custom',
          path: [key, index],
          message: `${key} must be sorted and unique`,
        });
      }
    }
  }
});
const remediationFindingIdentityEpoch = z.object({
  version: z.literal(1),
  legacy_cycle_count: z.number().int().min(1).max(10),
  legacy_history_hash: remediationFindingIdentity,
  normalized_history_hash: remediationFindingIdentity,
  normalized_provenance: z.array(remediationFindingProvenance).min(1).max(10),
}).strict();
export const RemediationFindingHistorySchema = z.object({
  version: z.literal(1),
  cycles: z.array(
    z.array(remediationFindingIdentity).min(1).max(REVIEW_FINDING_LIMIT)
      .superRefine((identities, context) => {
        for (let index = 0; index < identities.length; index += 1) {
          if (index > 0 && identities[index - 1] >= identities[index]) {
            context.addIssue({
              code: 'custom',
              path: [index],
              message: 'remediation finding identities must be sorted and unique',
            });
          }
        }
      }),
  ).min(1).max(10),
  // Optional and default-free so retained version-1 state remains readable.
  // New cycles always persist both fields; a continuation that sees neither
  // is legacy state and fails closed in remediationFindingHistoryIsConsistent.
  aliases: z.array(remediationFindingAlias).min(1).max(REVIEW_FINDING_LIMIT).optional(),
  provenance: z.array(remediationFindingProvenance).min(1).max(10).optional(),
  identity_epoch: remediationFindingIdentityEpoch.optional(),
}).strict().superRefine((history, context) => {
  if ((history.aliases === undefined) !== (history.provenance === undefined)) {
    context.addIssue({
      code: 'custom',
      message: 'remediation alias history and provenance must be present together',
    });
    return;
  }
  if (!history.aliases || !history.provenance) return;
  if (history.provenance.length !== history.cycles.length) {
    context.addIssue({
      code: 'custom',
      path: ['provenance'],
      message: 'remediation provenance must have one entry per cycle',
    });
  }
  if (history.identity_epoch) {
    const epoch = history.identity_epoch;
    if (
      epoch.legacy_cycle_count > history.cycles.length ||
      epoch.normalized_provenance.length !== epoch.legacy_cycle_count
    ) {
      context.addIssue({
        code: 'custom',
        path: ['identity_epoch'],
        message: 'remediation identity epoch must cover exactly its legacy cycle prefix',
      });
    } else {
      const legacyProvenance = history.provenance.slice(0, epoch.legacy_cycle_count);
      const legacyDescriptors = new Set(
        legacyProvenance.flatMap((entry) => entry.descriptor_hashes),
      );
      const legacyAliases = history.aliases.filter(
        (alias) => legacyDescriptors.has(alias.descriptor),
      );
      const expectedLegacyHash = sha256({
        cycles: history.cycles.slice(0, epoch.legacy_cycle_count),
        aliases: legacyAliases,
        provenance: legacyProvenance,
      });
      const normalizedAliases = history.aliases.filter((alias) =>
        legacyDescriptors.has(alias.descriptor) ||
        epoch.normalized_provenance.some((entry) =>
          entry.descriptor_hashes.includes(alias.descriptor)));
      const expectedNormalizedHash = sha256({
        aliases: normalizedAliases,
        provenance: epoch.normalized_provenance,
      });
      if (
        expectedLegacyHash !== epoch.legacy_history_hash ||
        expectedNormalizedHash !== epoch.normalized_history_hash
      ) {
        context.addIssue({
          code: 'custom',
          path: ['identity_epoch'],
          message: 'remediation identity epoch hashes must bind the legacy and normalized prefix',
        });
      }
    }
  }
  const canonical = new Set(history.cycles.flat());
  const aliasMap = new Map();
  for (let index = 0; index < history.aliases.length; index += 1) {
    const alias = history.aliases[index];
    const previous = history.aliases[index - 1];
    if (previous && (
      previous.descriptor > alias.descriptor ||
      (previous.descriptor === alias.descriptor && previous.canonical >= alias.canonical)
    )) {
      context.addIssue({
        code: 'custom',
        path: ['aliases', index],
        message: 'remediation descriptor aliases must be sorted and unique',
      });
    }
    if (aliasMap.has(alias.descriptor) && aliasMap.get(alias.descriptor) !== alias.canonical) {
      context.addIssue({
        code: 'custom',
        path: ['aliases', index],
        message: 'one remediation descriptor cannot name multiple canonical identities',
      });
    }
    aliasMap.set(alias.descriptor, alias.canonical);
    if (!canonical.has(alias.canonical)) {
      context.addIssue({
        code: 'custom',
        path: ['aliases', index, 'canonical'],
        message: 'remediation alias canonical identity must occur in history',
      });
    }
  }
  for (const [descriptor, target] of aliasMap) {
    const next = aliasMap.get(target);
    if (descriptor !== target && next !== undefined && next !== target) {
      context.addIssue({
        code: 'custom',
        path: ['aliases'],
        message: 'remediation descriptor aliases must be flattened and acyclic',
      });
      break;
    }
  }
  for (const identity of canonical) {
    if (aliasMap.get(identity) !== identity) {
      context.addIssue({
        code: 'custom',
        path: ['aliases'],
        message: 'every remediation canonical identity must be a self-rooted proven descriptor',
      });
      break;
    }
  }
  const provenDescriptors = new Set([
    ...history.provenance.flatMap((entry) => entry.descriptor_hashes),
    ...(history.identity_epoch
      ? history.identity_epoch.normalized_provenance.flatMap(
        (entry) => entry.descriptor_hashes,
      )
      : []),
  ]);
  if (
    provenDescriptors.size !== aliasMap.size ||
    [...provenDescriptors].some((descriptor) => !aliasMap.has(descriptor))
  ) {
    context.addIssue({
      code: 'custom',
      path: ['provenance'],
      message: 'remediation aliases must exactly match provenance descriptors',
    });
  }
  for (let index = 0; index < history.cycles.length; index += 1) {
    const cycle = history.cycles[index];
    const provenance = history.provenance[index];
    if (!provenance) continue;
    const roots = new Set(
      provenance.descriptor_hashes.map((descriptor) => aliasMap.get(descriptor)),
    );
    if (
      roots.has(undefined) || roots.size !== cycle.length ||
      cycle.some((identity) => !roots.has(identity))
    ) {
      context.addIssue({
        code: 'custom',
        path: ['provenance', index, 'descriptor_hashes'],
        message: 'remediation cycle provenance must resolve exactly to its cycle identities',
      });
    }
  }
  if (history.identity_epoch) {
    for (let index = 0; index < history.identity_epoch.legacy_cycle_count; index += 1) {
      const roots = new Set(
        history.identity_epoch.normalized_provenance[index].descriptor_hashes
          .map((descriptor) => aliasMap.get(descriptor)),
      );
      const cycle = history.cycles[index];
      if (
        roots.has(undefined) || roots.size !== cycle.length ||
        ...
        context.addIssue({
          code: 'custom',
          path: ['identity_epoch', 'normalized_provenance', index, 'descriptor_hashes'],
          message: 'normalized migration provenance must resolve to the retained legacy roots',
        });
      }
    }
  }
  const aggregateCounts = {
    cycle_identities: history.cycles.reduce((total, cycle) => total + cycle.length, 0),
    receipt_hashes: history.provenance.reduce(
      (total, entry) => total + entry.receipt_hashes.length,
      0,
    ),
    descriptor_hashes: history.provenance.reduce(
      (total, entry) => total + entry.descriptor_hashes.length +
        (entry.legacy_descriptor_hashes?.length ?? 0),
      0,
    ),
    normalized_descriptor_hashes: history.identity_epoch
      ? history.identity_epoch.normalized_provenance.reduce(
        (total, entry) => total + entry.descriptor_hashes.length +
          (entry.legacy_descriptor_hashes?.length ?? 0),
        0,
      )
  ...
  for (const [field, count] of Object.entries(aggregateCounts)) {
    if (count > REMEDIATION_HISTORY_ITEM_LIMIT) {
      context.addIssue({
        code: 'custom',
        path: ['provenance'],
        message: `aggregate remediation history ${field} exceed ${REMEDIATION_HISTORY_ITEM_LIMIT}`,
      });
    }
  }
  if (Buffer.byteLength(canonicalJson(history), 'utf8') > REMEDIATION_HISTORY_BYTES_MAX) {
    context.addIssue({
      code: 'custom',
      message: `remediation finding history exceeds ${REMEDIATION_HISTORY_BYTES_MAX} serialized UTF-8 bytes`,
    });
  }
});

export function remediationFindingHistoryIsConsistent(state) {
  const parsed = RemediationFindingHistorySchema.safeParse(state?.remediation_finding_history);
  if (!parsed.success) return false;
  if (!parsed.data.aliases || !parsed.data.provenance) return false;
  if (!Number.isInteger(state?.remediation_cycles) || state.remediation_cycles < 1) return false;
  if (parsed.data.cycles.length !== state.remediation_cycles) return false;
  const globallySeen = new Set(parsed.data.cycles[0]);
  for (let index = 1; index < parsed.data.cycles.length; index += 1) {
    const previous = new Set(parsed.data.cycles[index - 1]);
    const current = new Set(parsed.data.cycles[index]);
    if (![...previous].some((identity) => !current.has(identity))) return false;
    for (const identity of current) {
      if (!previous.has(identity) && globallySeen.has(identity)) return false;
      globallySeen.add(identity);
    }
  }
  const latest = parsed.data.cycles.at(-1);
  if (!Array.isArray(state?.remediation_finding_fingerprints)) return false;
  if (state.remediation_finding_fingerprints.length !== latest.length) return false;
  if (!latest.every((identity, index) => state.remediation_finding_fingerprints[index] === identity)) {
    return false;
  }
  return state.remediation_finding_count === latest.length;
}
...
export function finalizeTicket(ticket) {
  const compatibilityCapabilityManifest = ticket.capability_manifest === undefined &&
    ticket.receipt_contract_version === undefined
    ? {
        version: 1,
        config_hash: sha256({ contract: 'service-ticket-v1', run_id: ticket.run_id }),
        required_capabilities: [],
        allowed_evidence_commands: [],
        command_profiles: [],
        verification_profiles: [],
        objective_hash: sha256(ticket.objective),
        preflight_hash: ticket.preflight?.artifact_hash ?? null,
        risk_triggers: [...(ticket.risk_triggers ?? [])],
        design_assurance_required: false,
        receipt_schema: { ref: 'ticket.output_schema', hash: sha256(ticket.output_schema) },
        field_bounds: {
    ...
    : null;
  const materialized = StageTicketSchema.parse({
    ...ticket,
    ...(compatibilityCapabilityManifest
      ? { capability_manifest: compatibilityCapabilityManifest }
      : {}),
    ticket_hash: HASH_PLACEHOLDER,
  });
