...
const capabilityRequirement = z.discriminatedUnion('kind', [
    ...
    z.object({ kind: z.literal('tool_claim'), id: toolClaim }).strict(),
  ]);
...
export const RunStartInputSchema = z.object({
  ...
  // The public adapter opts new-contract starts into a bounded run by setting
  // execution_budget_required. Direct and persisted legacy callers may omit
  // both fields without acquiring a retroactive admission requirement.
  execution_budget_required: z.literal(true).optional(),
  execution_budget: z.object({
    max_worker_dispatches: z.number().int().min(1).max(10_000),
    max_active_seconds: z.number().int().min(1).max(2_592_000),
  }).strict().optional(),
