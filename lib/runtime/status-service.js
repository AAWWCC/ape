...
import { AUTO_MERGE_HOLD_REASON, SEALED_STATUSES, TERMINAL_STATUSES } from './constants.js';
import { runtimePaths } from './paths.js';
import { appendJsonLine } from './storage.js';
import { inspectRunLock, withDirLock } from './lock.js';
import { assertRunnersValue, DEFAULT_CONFIG, loadRuntimeConfig, proposeTestCommands, setRuntimeConfig, setRuntimeConfigBatch } from './config.js';
import { calculateProjectMetrics, explainRun, logicalLineageForRun, queryHistory, queryHistoryPage, selectEffectiveRecord } from './history.js';
...
import { attestRequirements, deriveRoadmap, registerEntries, supersedeEntries } from './roadmap.js';
import {
...
export async function statusRun(projectDir) {
  ...
  try {
    ...
    if (error?.code === 'APE_ACTIVE_STATE_BUSY') {
      return {
        ok: false,
        code: 'APE_ACTIVE_STATE_BUSY',
        retryable: true,
        reason: 'active run state is being updated; retry ape_status to read a stable snapshot',
        next_action: { kind: 'retry', automatic_successor: false },
      };
    }
...
export function deriveRunFacts(state, options = {}) {
  ...
  const latestAttempt = Number.isSafeInteger(rawLatestAttempt) && rawLatestAttempt > 0
    ? rawLatestAttempt
    : 1;
  ...
  const remediation = {
    ...
    cycle: Number.isSafeInteger(state.remediation_route?.cycle ?? state.remediation_cycles) &&
      (state.remediation_route?.cycle ?? state.remediation_cycles) >= 0
      ? (state.remediation_route?.cycle ?? state.remediation_cycles)
      : 0,
...
export async function compactStatus(projectDir) {
  ...
  if (status.code === 'APE_ACTIVE_STATE_BUSY') return { ...status, next_safe_action: 'retry ape_status' };
  if (!status.ok) {
  ...
  const nextAction = !state
    ...
    : corrupt || dispatchEvidenceUnreadable
      ...
      : state.status === 'completed'
        ? null
        : ['execution_budget_input_required', 'continuation_pending'].includes(diagnostic.reason_code)
...
function manualRetentionRequest(input) {
  ...
  if (!Number.isSafeInteger(keepRecentRuns) || keepRecentRuns < 0) {
    throw new Error('keep_recent_runs must be a non-negative safe integer');
  }
...
export async function historyAction(projectDir, action, input = {}) {
  ...
  const paths = runtimePaths(projectDir);
  if (action === 'query') {
    const page = await queryHistoryPage(paths, input);
    const response = { ok: true, records: page.records, pagination: page.pagination };
    // Internal continuation witnesses let the wire projection shorten a page
    // without skipping rows. They are not serialized into the public result.
...
export async function configAction(projectDir, action, input = {}) {
  ...
  if (action === 'init') {
    ...
    // Publish one validated aggregate in stable slot order. The shared config
    // transaction preserves sparse overrides/provenance and merges additive
    // evidence scripts under its lock, before any bytes are published.
    const updates = slots.map((slot) => [`test_commands.${slot}`, effective[slot]]);
    const applied = updates.map(([key]) => key);
    if (effectiveRunners.length > 0) {
      updates.push(['runners', effectiveRunners]);
      applied.push('runners');
    }
    if (effectiveEvidenceScripts.length > 0) {
      applied.push('policy.evidence_scripts');
    }
    const config = await setRuntimeConfigBatch(paths.config, updates, {
      appendEvidenceScripts: effectiveEvidenceScripts,
    });
    return {
      ok: true,
      init: {
