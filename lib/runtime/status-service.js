...
export async function dispatchLiveness(paths, state) {
  ...
  const dispatches = state && ['claude', 'codex', 'gemini'].includes(state.host)
...
export async function statusRun(projectDir) {
  ...
  return {
    ...
    ...(state && ['claude', 'codex', 'gemini'].includes(state.host)
...
export function deriveRunFacts(state, options = {}) {
  ...
  const latestAttempt =
    activeTicket?.attempt ??
    (typeof currentStageAttempt === 'number' ? currentStageAttempt : 1);
  ...
  const remediation = {
    ...
    cycle: state.remediation_route?.cycle ?? state.remediation_cycles ?? 0,
  ...
  return {
    ...
    run: state
      ? {
          ...
          stage: state.stage,
          ...(state.remediation_route
            ? { remediation_route: state.remediation_route }
            : {}),
        }
      : null,
    ...
    ...(state ? { facts: deriveRunFacts(state, { dispatchState }) } : {}),
  };
}
