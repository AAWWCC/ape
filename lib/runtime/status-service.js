...
import { archiveRun as archiveRunRecord, calculateProjectMetrics, explainRun, queryHistory, selectEffectiveRecord } from './history.js';
...
import { executionBudgetElapsedMs, executionBudgetRemainingMs } from './execution-budget.js';
...
export function deriveRunFacts(state, options = {}) {
  ...
  const hasInputHold = Boolean(inputReq);
  ...
  // Execution-budget facts are absent for legacy runs. New bounded runs expose
  // both declared ceilings and the runtime counters used to pause dispatch.
  const executionBudget = state.execution_budget;
  const budgetObservedAt = now();
  const budgetElapsedMs = executionBudget ? executionBudgetElapsedMs(state, budgetObservedAt) : null;
  const budgetRemainingMs = executionBudget
    ? executionBudgetRemainingMs(state, budgetObservedAt)
    : null;
  const budget = executionBudget
    ? {
        max_worker_dispatches: executionBudget.max_worker_dispatches,
        worker_dispatches_used: executionBudget.worker_dispatches_used ?? 0,
        worker_dispatches_remaining: Math.max(
          0,
          executionBudget.max_worker_dispatches - (executionBudget.worker_dispatches_used ?? 0),
        ),
        max_active_seconds: executionBudget.max_active_seconds,
        active_deadline_at: executionBudget.active_deadline_at ?? null,
        // Include the currently active slice without mutating authoritative
        // state. Otherwise a worker that ran past the cap would project
        // remaining=0 while still claiming elapsed/overrun=0 until the next
        // writer happened to persist the run.
        active_elapsed_ms: budgetElapsedMs ?? 0,
        overrun_ms: Math.max(
          0,
          (budgetElapsedMs ?? 0) - executionBudget.max_active_seconds * 1_000,
        ),
        active_seconds_remaining: budgetRemainingMs === null
          ? 0
          : Math.max(0, Math.floor(budgetRemainingMs / 1_000)),
        paused: state.input_required?.kind === 'execution_budget',
        pause_code: state.input_required?.kind === 'execution_budget'
          ? boundedGateSummary(state.input_required.code, 64)
          : null,
      }
    : null;
      ...
      : state.status === 'completed'
        ...
        : state.status === 'input_required'
          ? state.input_required?.kind === 'receipt_retry'
            ...
            : {
                kind: state.input_required?.kind === 'execution_budget'
                  ? 'extend_budget'
                  : 'answer_preflight',
              }
