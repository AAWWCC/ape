...
// generation. POSIX and Windows both make the directory rename the single
// visibility boundary; a racing publisher that finds the canonical name
// already present must validate/reuse it rather than overwrite it.
...
// Content-addressed selector edges are append-only files. Hard-linking a
// completely written private temporary into its canonical slot supplies the
// missing no-replace primitive on both POSIX and Windows: EEXIST means another
// publisher won and its bytes must be validated, never overwritten.
