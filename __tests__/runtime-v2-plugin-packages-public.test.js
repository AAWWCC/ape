...
describe('2.17 public plugin packages', () => {
  ...
  it('ships companion-file local stdio MCP declarations with explicit host pins', async () => {
    ...
    expect(codexManifest).toMatchObject({ version: '2.18.3', mcpServers: './.mcp.json' });
    expect(claudeManifest).toMatchObject({ version: '2.18.3', hooks: './hooks/claude-hooks.json' });
    expect(geminiManifest).toMatchObject({ version: '2.18.3' });
