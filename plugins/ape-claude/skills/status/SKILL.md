---
name: status
description: "Show the active APE run, pending tickets, lane, and gate state."
...
`next_action.kind`, and `failure_domain` compactly. When present, show execution-budget usage,
pause/overrun state, and exact token-counter coverage as observed versus unobserved; never turn
missing host counters into an estimate. This distinction is required when explaining whether time
and tokens produced product progress or orchestration repair. When roadmap data is present, show counts only; direct users who need entries to the
