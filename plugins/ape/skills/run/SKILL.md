...
- `plan_contract_version: 1` for every newly started `phase` run whose requested lane is `auto` or
  `full`; the runtime uses it only when planning exists. Omit it for explicit `fast` or `mechanical`
  lanes, non-phase modes, and every resume.
